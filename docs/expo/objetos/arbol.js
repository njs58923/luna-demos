...
// El árbol.
//
// Con modo "malla" (el de siempre) es un árbol de verdad, de Obra.arbolMalla:
// el tronco se abre en ramas, las ramas en otras, y en las puntas miles de
// hojas; decenas de miles de triángulos en dos o tres mallas, armadas acá, en
// el isolate del árbol, con su propio cupo. Con modo "simple" es el de antes:
// una docena de primitivas (lo que usa la calle para sus veredas).
//
// En los dos el tronco queda quieto y la copa va en un grupo aparte, con el
// origen en la horqueta, que se mece. El viento son dos senos de periodos que
// no se repiten entre sí más una ráfaga lenta, en X y en Z por separado: una
// copa que va y viene en un solo eje parece un metrónomo.
//
// Tocarlo pasa a la especie siguiente, con la misma semilla, salvo que venga
// con tocable: false (en la arboleda, donde un árbol es paisaje).
const P = Obj.props({ especie: "copa", alto: 4, semilla: 1, hoja: "", tronco: "", viento: 0.5, modo: "malla", detalle: 1, tocable: true });
const MALLA = Object.keys(Obra.ESPECIES_MALLA);
const SIMPLE = Obra.ESPECIES;
const obra = Obj.$("obra");
const armarSimple = Obra.montarConMallas(obra, Obj.crear);
const toque = Obj.$("toque");
const conMalla = () => P.modo !== "simple" && typeof MeshResource !== "undefined";
const orden = () => (conMalla() ? MALLA : SIMPLE);
let especie = orden().includes(P.especie) ? P.especie : "copa";
let copa = null;
let hechos = [], recursos = [];
...
function dibujarMalla(H) {
  const propio = especie === P.especie;
  const t = Obra.arbolMalla({ especie, alto: H, semilla: P.semilla, detalle: Obra.num(P.detalle, 1, 0.1, 2), hoja: propio ? P.hoja : "", tronco: propio ? P.tronco : "" });
  soltar();
  armarSimple([]);
  const troncoG = Obj.crear("group", {}, obra);
  const copaG = Obj.crear("group", { id: "copa", y: t.yCopa }, obra);
  // La copa tiene los vértices en coordenadas del árbol: un grupo adentro la
  // baja a su lugar, y el de afuera gira sobre la horqueta.
  const dentro = Obj.crear("group", { y: -t.yCopa }, copaG);
  hechos = [troncoG, copaG];
  const nuevas = [];
  for (const [L, padre] of [[t.tronco, troncoG], [t.copa, dentro]]) {
    for (const b of Obra.mallasDe(L)) {
      try {
        ...
        console.error("[arbol] MeshResource: " + (e && e.message || e));
      }
    }
  }
  for (const m of recursos) { try { m.dispose(); } catch (e) { /* ya no estaba */ } }
...
function dibujar() {
  const H = Obra.num(P.alto, 4, 0.5, 30);
  if (!orden().includes(especie)) especie = "copa";
  if (conMalla()) dibujarMalla(H);
  else {
    soltar();
    for (const m of recursos) { try { m.dispose(); } catch (e) { /* ya no estaba */ } }
    recursos = [];
    const propio = especie === P.especie;
    armarSimple(Obra.arbol({ especie, alto: H, semilla: P.semilla, hoja: propio ? P.hoja : "", tronco: propio ? P.tronco : "", id: "copa" }));
  }
  copa = Obj.$("copa");
  const ancho = especie === "cipres" ? H * 0.3 : H * 0.6;
  toque.position = { x: 0, y: H / 2, z: 0 };
  toque.scale = { x: ancho, y: H, z: ancho };
  toque.setAttribute("touchable", P.tocable === false ? "false" : "true");
  anim.iniciar();
}

const fase = Obra.azar(Number(P.semilla) || 1)() * 10;
const anim = Obj.animador((t) => {
  const v = Obra.num(P.viento, 0.5, 0, 3);
  if (!copa || v <= 0) return false;
  const k = v * (especie === "palmera" ? 0.05 : especie === "pino" || especie === "cipres" ? 0.02 : 0.035);
  const rafaga = 0.6 + 0.4 * Math.sin(t * 0.23 + fase);
  const rx = k * rafaga * (Math.sin(t * 1.1 + fase) + 0.4 * Math.sin(t * 2.7 + fase * 2));
  const rz = k * rafaga * (Math.sin(t * 0.83 + fase * 3) + 0.35 * Math.sin(t * 2.1 + fase));
  copa.rotation = { x: rx, y: 0, z: rz };
  return true;
});

Obj.boton(toque, () => {
  if (P.tocable === false) return;
  especie = Obra.siguiente(orden(), especie);
  dibujar();
  Obj.emitir("cambio", { especie });
});
Obj.alCambiar((p, antes, cambios) => {
  if (cambios.includes("especie") && orden().includes(p.especie)) especie = p.especie;
  if (cambios.every((c) => c === "viento" || c === "tocable")) {
    toque.setAttribute("touchable", P.tocable === false ? "false" : "true");
    return void anim.iniciar();
  }
  dibujar();
});
dibujar();
