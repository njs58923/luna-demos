...
;
// El árbol: el tronco quieto y la copa en un grupo aparte, con el origen en la
// horqueta, que se mece. El viento son dos senos de periodos que no se
// repiten entre sí más una ráfaga lenta, en X y en Z por separado: una copa
// que va y viene en un solo eje parece un metrónomo.
//
// Tocarlo pasa a la especie siguiente, con la misma semilla.
const P = Obj.props({ especie: "copa", alto: 4, semilla: 1, hoja: "", tronco: "", viento: 0.5 });
const ORDEN = Obra.ESPECIES;
...
function dibujar() {
  ...
  armar(Obra.arbol({ especie, alto: H, semilla: P.semilla, hoja: propio ? P.hoja : "", tronco: propio ? P.tronco : "", id: "copa" }));
  copa = Obj.$("copa");
...
Obj.alCambiar((p, antes, cambios) => {
  ...
  if (cambios.length === 1 && cambios[0] === "viento") return void anim.iniciar();
  dibujar();
});
dibujar();
