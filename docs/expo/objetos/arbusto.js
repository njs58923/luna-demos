...
;
// El arbusto: bollos de hojas y, si tiene color de flores, flores. Tocarlo lo
// hace florecer (o le saca las flores).
const P = Obj.props({ ancho: 1, alto: 0.8, color: "", flores: "", semilla: 5 });
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let flores = Obra.color(P.flores, "");

function dibujar() {
  const W = Obra.num(P.ancho, 1, 0.2, 5), H = Obra.num(P.alto, 0.8, 0.2, 4);
  armar(Obra.arbusto({ ancho: W, alto: H, color: P.color, flores, semilla: P.semilla }));
  toque.position = { x: 0, y: H / 2, z: 0 };
  toque.scale = { x: W, y: H, z: W };
}

Obj.boton(toque, () => {
  flores = flores ? "" : Obra.color(P.flores, "#F4A7C0");
  dibujar();
  Obj.emitir("cambio", { flores });
});
Obj.alCambiar((p, antes, cambios) => {
  if (cambios.includes("flores")) flores = Obra.color(p.flores, "");
  dibujar();
});
dibujar();
