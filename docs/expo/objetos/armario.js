...
;
// El armario: dos puertas con bisagra, ropa colgada y un estante.
//
// Cada puerta es un grupo cuyo origen es la bisagra: girarlo en Y la abre
// como una puerta de verdad, hacia afuera. El toque va en la propia puerta,
// así que una puerta abierta se cierra tocándola donde quedó.
const P = Obj.props({ madera: "#A47148", tirador: "#2A2A2A", clave: "" });

const W = 1.0, H = 1.9, D = 0.56, T = 0.022;
const memo = Obj.memoria("puertas");
const mueble = Obj.$("mueble");
const adentro = Obj.$("adentro");
const madera = () => Obj.color(P.madera, "#A47148");
const pintables = [];
...
// La carcasa.
tabla({ x: -W / 2 + T / 2, y: 0.07 + (H - 0.07) / 2, sx: T, sy: H - 0.07, sz: D }, mueble);
tabla({ x: W / 2 - T / 2, y: 0.07 + (H - 0.07) / 2, sx: T, sy: H - 0.07, sz: D }, mueble);
tabla({ y: H - T / 2, sx: W + 0.03, sy: T * 1.5, sz: D + 0.03 }, mueble, 0.06);
tabla({ y: 0.07 + T / 2, sx: W, sy: T, sz: D }, mueble);
tabla({ y: 0.035, sx: W - 0.04, sy: 0.07, sz: D - 0.04 }, mueble, -0.3);
tabla({ y: H / 2, z: -D / 2 + 0.005, sx: W, sy: H, sz: 0.01 }, mueble, -0.4);
tabla({ y: 1.55, z: 0, sx: W - 2 * T, sy: 0.018, sz: D - 0.03 }, mueble, -0.1);
// Adentro: la barra, perchas con ropa y ropa doblada en el estante.
Obj.crear("cylinder", { y: 1.45, rz: Math.PI / 2, sx: 0.02, sy: W - 2 * T, sz: 0.02, color: "#B0B0B0" }, adentro);
const ROPA = ["#264653", "#E76F51", "#2A9D8F", "#E9C46A", "#6D597A", "#F4A261"];
for (let i = 0; i < 6; i++) {
  const x = -0.36 + i * 0.145;
  Obj.crear("box", { x, y: 1.42, sx: 0.02, sy: 0.04, sz: 0.3, rx: 0, color: "#6B4A30" }, adentro);
  const largo = i % 2 ? 0.75 : 0.95;
  Obj.crear("box", { x, y: 1.4 - largo / 2, sx: 0.035, sy: largo, sz: 0.38, color: ROPA[i] }, adentro);
}
for (let i = 0; i < 3; i++) {
  Obj.crear("box", { x: -0.25 + i * 0.25, y: 1.6 + 0.02, sx: 0.2, sy: 0.04, sz: 0.3, color: ROPA[(i + 3) % 6] }, adentro);
  Obj.crear("box", { x: -0.25 + i * 0.25, y: 1.66, sx: 0.2, sy: 0.04, sz: 0.3, color: ROPA[(i + 1) % 6] }, adentro);
}

// Las puertas: cada una cuelga de su bisagra y se extiende hacia el medio.
const tiradores = [];
function puerta(lado) {
  const g = Obj.$(lado < 0 ? "bisagra_izq" : "bisagra_der");
  const ancho = W / 2 - 0.012;
  const dx = -lado * ancho / 2;   // del eje de la bisagra al centro de la puerta
  tabla({ x: dx, sx: ancho - 0.004, sy: H - 0.12, sz: T }, g, 0.05);
  // Molduras: dos rectángulos en relieve.
  for (const y of [0.42, -0.28]) tabla({ x: dx, y, z: T / 2 + 0.004, sx: ancho - 0.12, sy: y > 0 ? 0.62 : 0.9, sz: 0.008 }, g, 0.12);
  tiradores.push(Obj.crear("cylinder", { x: -lado * (ancho - 0.06), y: 0.05, z: 0.035, sx: 0.018, sy: 0.14, sz: 0.018, color: Obj.color(P.tirador, "#2A2A2A") }, g));
  const blanco = Obj.crear("plane", { x: dx, z: 0.03, sx: ancho, sy: H - 0.12, color: "transparent", "material-alpha": "blend" }, g);
  blanco.setAttribute("manos", "false");   // con la mano se toma la hoja entera
  return { g, blanco, lado, ancho, abierta: false, k: 0, mano: null };
}
const puertas = { izquierda: puerta(-1), derecha: puerta(1) };

const guardado = memo.leer({});
for (const [nombre, p] of Object.entries(puertas)) {
  if (guardado[nombre]) { p.abierta = true; p.k = 1; }
}
function dibujar() {
  // La izquierda gira en -Y y la derecha en +Y: las dos se abren hacia +Z.
  for (const p of Object.values(puertas)) p.g.rotation = { x: 0, y: p.mano !== null ? p.mano : p.lado * 1.75 * Obj.suave(p.k), z: 0 };
}
const anim = Obj.animador((t, dt) => {
  ...
  for (const p of Object.values(puertas)) {
    ...
    if (p.mano === null && p.k !== meta) {
      p.k = meta > p.k ? Math.min(1, p.k + dt / 0.6) : Math.max(0, p.k - dt / 0.5);
      if (p.k !== meta) seguir = true;
    }
  }
  dibujar();
  return seguir;
});
function poner(nombre, abierta) {
  const p = puertas[nombre];
  if (!p || p.abierta === abierta) return;
  p.abierta = abierta;
  memo.guardar({ izquierda: puertas.izquierda.abierta, derecha: puertas.derecha.abierta });
  anim.iniciar();
  Obj.emitir("cambio", { puerta: nombre, abierta });
}
for (const [nombre, p] of Object.entries(puertas)) Obj.boton(p.blanco, () => poner(nombre, !p.abierta));

// En VR: cada hoja se toma de cualquier punto y gira sobre su bisagra (la
// puerta de las manos, _vr.js); al soltarla termina de abrirse o se cierra.
const bisagras = { izquierda: { x: -0.49, y: 0.97, z: 0.27 }, derecha: { x: 0.49, y: 0.97, z: 0.27 } };
for (const [nombre, p] of Object.entries(puertas)) {
  Obj.manos().puerta({
    pivote: bisagras[nombre], eje: { x: 0, y: 1, z: 0 }, brazo: { x: -p.lado, y: 0, z: 0 },
    largo: p.ancho, alto: H - 0.12, max: p.lado * 1.75,
    angulo: () => (p.mano !== null ? p.mano : p.lado * 1.75 * Obj.suave(p.k)),
    mover(a) { p.mano = a; dibujar(); },
    soltar(abierta, a) {
      p.mano = null;
      p.k = Obj.suaveInversa(Math.min(1, Math.abs(a) / 1.75));
      if (abierta !== p.abierta) poner(nombre, abierta); else anim.iniciar();
    },
  });
}
Obj.mensaje("abrir", (d) => poner(d && d.puerta, true));
Obj.mensaje("cerrar", (d) => poner(d && d.puerta, false));
Obj.alCambiar(() => {
  for (const [el, k] of pintables) el.setAttribute("color", Obj.tono(madera(), k));
  for (const el of tiradores) el.setAttribute("color", Obj.color(P.tirador, "#2A2A2A"));
});
dibujar();
