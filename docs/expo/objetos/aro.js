...
;
// El aro: tiros con parábola, rebotes contra el tablero y el aro, y la red.
//
// El aro se trata como un anillo de radio RA con un tubo fino: si el centro de
// la pelota queda más cerca que (radio + tubo) del punto más cercano del
// anillo, rebota en la dirección de ese punto hacia la pelota. Entra cuando
// el centro cruza hacia abajo el plano del aro, adentro del anillo.
const P = Obj.props({ altura: 2.1, punteria: 0.65, clave: "" });

const RB = 0.12, RA = 0.2, TUBO = 0.01, G = 9.8;
const memo = Obj.memoria("record");
const donde = Obj.marco();
const pelota = Obj.$("pelota");
const toque = Obj.$("toque");
const aroG = Obj.$("aro");
const redG = Obj.$("red");
const INICIO = { x: 0, y: RB, z: 1.6 };

function altura() { return Math.max(1.2, Math.min(3.05, Number(P.altura) || 2.1)); }
const aro = () => ({ x: 0, y: altura(), z: 0.2 });

// El aro, en dovelas; la red, en cordones que cuelgan y se angostan.
const aroPiezas = [];
for (let i = 0; i < 20; i++) {
  const a = (i / 20) * Math.PI * 2;
  aroPiezas.push(Obj.crear("box", { x: Math.sin(a) * RA, z: Math.cos(a) * RA, ry: a, sx: 0.075, sy: 0.018, sz: 0.018, color: "#FF5A1F" }, aroG));
}
const cordones = [];
for (let i = 0; i < 12; i++) {
  const a = (i / 12) * Math.PI * 2;
  const c = Obj.crear("group", { x: Math.sin(a) * RA, z: Math.cos(a) * RA }, redG);
  Obj.crear("box", { y: -0.18, rx: Math.cos(a) * 0.35, rz: -Math.sin(a) * 0.35, sx: 0.006, sy: 0.38, sz: 0.006, color: "#F4F4F4" }, c);
  cordones.push({ g: c, a });
}
function ubicarAro() {
  aroG.position = aro();
  redG.position = aro();
  Obj.$("tablero").position = { x: 0, y: altura() + 0.2, z: -0.04 };
}

let p = { ...INICIO }, v = { x: 0, y: 0, z: 0 };
let volando = false, enMano = false, cruzo = false, quieta = 0, sacudida = 0;
let seguidas = 0, record = Math.max(0, Number(memo.leer(0)) || 0);

function marcador(txt) { Obj.$("marcador").setAttribute("value", txt || ("seguidas " + seguidas + " · récord " + record)); }
function dibujar() {
  pelota.position = p;
  toque.position = p;
}
...
const anim = Obj.animador((t, dt) => {
  if (sacudida > 0) {
    ...
    for (const c of cordones) c.g.rotation = { x: Math.cos(c.a) * 0.3 * sacudida * Math.sin(t * 30), y: 0, z: 0 };
  }
  if (enMano) { dibujar(); return true; }
  if (!volando) return sacudida > 0;
  const A = aro();
  const PASOS = 6, h = dt / PASOS;
  for (let i = 0; i < PASOS; i++) {
    const yAntes = p.y;
    p.x += v.x * h; p.y += v.y * h - 0.5 * G * h * h; p.z += v.z * h;
    v.y -= G * h;
    // El tablero: una caja de 0.9 × 0.6 con la cara en z = -0.025.
    const tb = { y: altura() + 0.2 };
    if (p.z - RB < -0.025 && p.z > -0.1 && Math.abs(p.x) < 0.45 + RB && Math.abs(p.y - tb.y) < 0.3 + RB && v.z < 0) {
      p.z = -0.025 + RB; v.z = -v.z * 0.6;
    }
    // El aro.
    const dx = p.x - A.x, dz = p.z - A.z;
    const rh = Math.hypot(dx, dz) || 1e-6;
    const cx = A.x + (dx / rh) * RA, cz = A.z + (dz / rh) * RA;
    const nx = p.x - cx, ny = p.y - A.y, nz = p.z - cz;
    const d = Math.hypot(nx, ny, nz);
    if (d < RB + TUBO) {
      const k = 1 / (d || 1e-6);
      const vn = v.x * nx * k + v.y * ny * k + v.z * nz * k;
      if (vn < 0) { v.x -= 1.6 * vn * nx * k; v.y -= 1.6 * vn * ny * k; v.z -= 1.6 * vn * nz * k; }
      p.x = cx + nx * k * (RB + TUBO); p.y = A.y + ny * k * (RB + TUBO); p.z = cz + nz * k * (RB + TUBO);
    }
    // ¿Entró? Cruza el plano del aro hacia abajo, por adentro.
    if (!cruzo && yAntes >= A.y && p.y < A.y && rh < RA - RB * 0.3) {
      cruzo = true;
      sacudida = 0.8;
      seguidas++;
      if (seguidas > record) { record = seguidas; memo.guardar(record); }
      marcador("¡adentro! " + seguidas);
      Obj.emitir("tiro", { entro: true, seguidas });
    }
    // El piso.
    if (p.y < RB) { p.y = RB; if (v.y < 0) v.y = -v.y * 0.65; v.x *= 0.9; v.z *= 0.9; }
  }
  dibujar();
  // Cuando se queda (o se va lejos), vuelve a su lugar.
  const lenta = Math.hypot(v.x, v.y, v.z) < 0.3 && p.y < RB + 0.01;
  quieta = lenta ? quieta + dt : 0;
  if (quieta > 0.6 || Math.hypot(p.x, p.z - 1) > 4 || t - lanzado > 7) {
    volando = false;
    if (!cruzo) { seguidas = 0; Obj.emitir("tiro", { entro: false, seguidas }); }
    setTimeout(() => { p = { ...INICIO }; v = { x: 0, y: 0, z: 0 }; dibujar(); marcador(); }, 500);
    return sacudida > 0;
  }
  return true;
});
...
function lanzar(desde, vel) {
  p = { ...desde }; v = { ...vel };
  volando = true; cruzo = false; quieta = 0;
  lanzado = performance.now() / 1000;
  anim.iniciar();
}

/** Un tiro con el mouse: la velocidad que llevaría la pelota al centro del
 *  aro en ~0.9 s, con un error según la puntería. */
function tiroAutomatico() {
  if (volando || enMano) return;
  const A = aro();
  const T = 0.95;
  const dx = A.x - p.x, dy = A.y + 0.05 - p.y, dz = A.z - p.z;
  const err = (1 - Math.max(0, Math.min(1, Number(P.punteria) || 0.65))) * 0.6;
  lanzar(p, {
    x: dx / T + (Math.random() - 0.5) * err,
    y: dy / T + 0.5 * G * T + (Math.random() - 0.5) * err,
    z: dz / T + (Math.random() - 0.5) * err,
  });
}

Obj.boton(toque, tiroAutomatico);
Obj.mensaje("tirar", tiroAutomatico);
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca: (m) => !volando && Math.hypot(m.x - p.x, m.y - p.y, m.z - p.z) < RB + 0.12,
  tomar: (m) => { enMano = true; p = { ...m }; anim.iniciar(); },
  mover: (m) => { p = { x: m.x, y: Math.max(RB, m.y), z: m.z }; dibujar(); },
  soltar: (m, vel) => { enMano = false; lanzar({ x: m.x, y: Math.max(RB, m.y), z: m.z }, vel); },
});
Obj.alCambiar(() => { ubicarAro(); marcador(); });
ubicarAro();
dibujar();
marcador();
