...
;
// La balanza: una cruz que busca el ángulo que le dicen los pesos.
//
// La diferencia de peso entre los platillos pide un ángulo (más inclinado
// cuanto más desparejo, con tope) y la cruz va hacia él como un resorte
// amortiguado, así se hamaca un poco antes de quedarse. Los platillos cuelgan
// de las puntas de la cruz y no giran con ella: se ubican cada cuadro, y las
// pesas que tienen encima con ellos.
const P = Obj.props({ semilla: 0 });

const sinte = Obj.sinte({ volumen: 0.7 });
const donde = Obj.marco();
const cruz = Obj.$("cruz");
const PIVOTE = { x: 0, y: 0.36, z: 0 };
const BRAZO = 0.2, CUELGA = 0.16;
const platos = { izq: Obj.$("plato_izq"), der: Obj.$("plato_der") };
const caja = Obj.$("caja");
let azar = Obj.azar(Number(P.semilla) || Math.floor(Math.random() * 1e9));

// ── Hilos: tres por platillo, de la punta de la cruz al borde ──────────────
const hilos = { izq: [], der: [] };
for (const lado of ["izq", "der"]) for (let k = 0; k < 3; k++) hilos[lado].push(Obj.crear("cylinder", { sx: 0.0015, sy: 0.1, sz: 0.0015, color: "#8C8C8C" }, Obj.$("hilos")));

// ── Pesas ──────────────────────────────────────────────────────────────────
const VALORES = [1, 2, 2, 5, 10];
const pesas = VALORES.map((v, i) => {
  const d = 0.021 * Math.cbrt(v), h = d * 0.9;
  const g = Obj.crear("group", {}, Obj.$("pesas"));
  Obj.crear("cylinder", { y: h / 2, sx: d, sy: h, sz: d, color: "#B8860B" }, g);
  Obj.crear("cylinder", { y: h + 0.004, sx: d * 0.35, sy: 0.008, sz: d * 0.35, color: "#B8860B" }, g);
  Obj.crear("sphere", { y: h + 0.009, sx: d * 0.5, sy: d * 0.4, sz: d * 0.5, color: "#B8860B" }, g);
  Obj.crear("text", { y: h / 2, z: d / 2 + 0.001, value: String(v), size: Math.max(0.009, d * 0.4), color: "#3B2A1A" }, g);
  const blanco = Obj.crear("box", { y: h / 2, sx: d + 0.02, sy: h + 0.03, sz: d + 0.02, color: "transparent", "material-alpha": "blend" }, g);
  const p = { v, i, d, g, lugar: "bandeja", tomada: false, pos: null };
  Obj.boton(blanco, () => tocarPesa(p));
  return p;
});
const BANDEJA = (i) => ({ x: -0.15 + i * 0.075, y: 0.03, z: 0.07 });
const HUECOS = [[0, 0], [-0.03, 0.02], [0.03, 0.02], [-0.03, -0.025], [0.03, -0.025]];

// ── La caja secreta ────────────────────────────────────────────────────────
let secreto = 0;
function nuevaCaja() {
  secreto = 1 + Math.floor(azar() * 20);
  const s = 0.035 + 0.03 * Math.cbrt(secreto / 20);
  Obj.$("caja_cuerpo").position = { x: 0, y: s / 2, z: 0 };
  Obj.$("caja_cuerpo").scale = { x: s, y: s, z: s };
  Obj.$("caja_texto").position = { x: 0, y: s / 2, z: s / 2 + 0.001 };
  Obj.$("caja_texto").setAttribute("value", "?");
  const colores = ["#7B2FF7", "#D62828", "#0A84FF", "#30D158", "#FF9F0A"];
  Obj.$("caja_cuerpo").setAttribute("color", colores[Math.floor(azar() * colores.length)]);
  resuelto = false;
  Obj.$("cartel").setAttribute("value", "");
  for (const p of pesas) p.lugar = "bandeja";
  pesar();
}

// ── La cruz ────────────────────────────────────────────────────────────────
let ang = 0, vel = 0, resuelto = false;
function suma(lado) { return pesas.filter((p) => p.lugar === lado && !p.tomada).reduce((a, p) => a + p.v, 0) + (lado === "der" ? secreto : 0); }
function meta() {
  const dif = suma("izq") - suma("der");
  // Más peso a la izquierda: esa punta baja, que es girar en +Z.
  return Math.max(-0.3, Math.min(0.3, dif * 0.03 + Math.sign(dif) * 0.05));
}
function punta(lado, a) {
  const x = lado === "izq" ? -BRAZO : BRAZO;
  return { x: PIVOTE.x + x * Math.cos(a), y: PIVOTE.y + x * Math.sin(a), z: 0 };
}
const centros = { izq: null, der: null };
function colocar() {
  cruz.rotation = { x: 0, y: 0, z: ang };
  for (const lado of ["izq", "der"]) {
    const t = punta(lado, ang);
    const c = { x: t.x, y: t.y - CUELGA, z: 0 };
    centros[lado] = c;
    platos[lado].position = c;
    hilos[lado].forEach((h, k) => {
      const a = (k / 3) * Math.PI * 2 + 0.5;
      const borde = { x: c.x + Math.cos(a) * 0.058, y: c.y + 0.003, z: Math.sin(a) * 0.058 };
      const l = Obj.tender(h, t, borde);
      h.scale = { x: 0.0015, y: l, z: 0.0015 };
    });
    if (lado === "der") caja.position = { x: c.x, y: c.y + 0.003, z: 0 };
    let n = 0;
    for (const p of pesas) {
      if (p.tomada) continue;
      if (p.lugar === lado) {
        const [hx, hz] = HUECOS[(lado === "der" ? 1 : 0) + n++] || [0, 0];
        p.pos = { x: c.x + hx, y: c.y + 0.003, z: hz };
      } else if (p.lugar === "bandeja") p.pos = BANDEJA(p.i);
      p.g.position = p.pos;
    }
  }
}
const anim = Obj.animador((t, dt) => {
  const m = meta();
  vel += ((m - ang) * 60 - vel * 7) * dt;
  ang += vel * dt;
  colocar();
  if (Math.abs(m - ang) < 0.0005 && Math.abs(vel) < 0.001) {
    ang = m; vel = 0; colocar();
    revisar();
    return false;
  }
...
function revisar() {
  if (resuelto || suma("izq") !== suma("der") || pesas.some((p) => p.tomada)) return;
  resuelto = true;
  Obj.$("caja_texto").setAttribute("value", String(secreto));
  Obj.$("cartel").setAttribute("value", "¡Pesa " + secreto + "!");
  for (let i = 0; i < 3; i++) sinte.nota(Obj.frecuencia(3 + [0, 4, 7][i]), 0.6, { tipo: "campana", vol: 0.2, retraso: i * 0.12 });
  Obj.emitir("equilibrio", { peso: secreto });
  setTimeout(nuevaCaja, 3500);
}
function pesar() {
  Obj.emitir("pesada", { izquierda: suma("izq"), derecha: suma("der") - secreto });
  anim.iniciar();
}

function tocarPesa(p) {
  if (resuelto) return;
  p.lugar = p.lugar === "bandeja" ? "izq" : "bandeja";
  sinte.nota(900 + p.v * 20, 0.08, { tipo: "madera", vol: 0.4 });
  pesar();
}

// ── En VR: tomar una pesa y soltarla sobre un platillo ─────────────────────
let enMano = null;
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) {
    let mejor = null, dm = 0.06;
    for (const k of pesas) {
      const d = Math.hypot(p.x - k.pos.x, p.y - (k.pos.y + k.d / 2), p.z - k.pos.z);
      if (d < dm) { dm = d; mejor = k; }
    }
    enMano = mejor;
    return !!mejor && !resuelto;
  },
  tomar() { enMano.tomada = true; pesar(); },
  mover(p) { enMano.g.position = { x: p.x, y: p.y - enMano.d / 2, z: p.z }; },
  soltar(p) {
    const k = enMano;
    enMano = null;
    k.tomada = false;
    k.lugar = "bandeja";
    for (const lado of ["izq", "der"]) {
      const c = centros[lado];
      if (Math.hypot(p.x - c.x, p.z - c.z) < 0.08 && p.y > c.y - 0.03 && p.y < c.y + 0.2) k.lugar = lado;
    }
    sinte.nota(900 + k.v * 20, 0.08, { tipo: "madera", vol: 0.4 });
    pesar();
  },
});

Obj.boton(Obj.$("caja_cuerpo"), () => { if (!resuelto) nuevaCaja(); });
Obj.mensaje("nueva", nuevaCaja);
Obj.alCambiar((p, a, cambios) => { if (cambios.includes("semilla")) { azar = Obj.azar(Number(P.semilla) || 1); nuevaCaja(); } });
nuevaCaja();
colocar();
