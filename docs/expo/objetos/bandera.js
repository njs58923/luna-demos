...
;
// La bandera: columnas que ondulan, franjas que se reparten, y un mástil.
//
// La tela se corta en columnas verticales; cada columna es un grupo con sus
// pedazos de franja, y lo que se anima es la columna entera: un
// desplazamiento en Z y un giro que siguen una onda que viaja del mástil a la
// punta, más fuerte cuanto más lejos del mástil. Doce grupos por cuadro, no
// cuarenta pedazos.
const P = Obj.props({ franjas: ["#74ACDF", "#FFFFFF", "#74ACDF"], emblema: "#F6B40E", viento: 0.6, clave: "" });

const L = 1.1, A = 0.7, COLS = 12;
const memo = Obj.memoria("izada");
const bandera = Obj.$("bandera");
let columnas = [];

function franjas() {
  const f = Array.isArray(P.franjas) ? P.franjas.map((c) => Obj.color(c, null)).filter(Boolean) : [];
  return f.length ? f.slice(0, 6) : ["#74ACDF", "#FFFFFF", "#74ACDF"];
}
function armar() {
  for (const c of columnas) c.g.remove();
  columnas = [];
  const fs = franjas();
  const alto = A / fs.length;
  const ancho = L / COLS;
  const emblema = Obj.color(P.emblema, null);
  for (let i = 0; i < COLS; i++) {
    const g = Obj.crear("group", { x: ancho * (i + 0.5) }, bandera);
    fs.forEach((c, k) => {
      Obj.crear("plane", { y: -alto * (k + 0.5), sx: ancho + 0.002, sy: alto + 0.001, color: c }, g);
    });
    // El emblema: un disco en el medio, hecho de las columnas que lo cruzan.
    if (emblema) {
      const cx = L / 2, r = A * 0.16;
      const x = ancho * (i + 0.5) - cx;
      if (Math.abs(x) < r) {
        const h = 2 * Math.sqrt(r * r - x * x);
        Obj.crear("plane", { y: -A / 2, z: 0.002, sx: ancho + 0.002, sy: h, color: emblema }, g);
        Obj.crear("plane", { y: -A / 2, z: -0.002, sx: ancho + 0.002, sy: h, color: emblema }, g);
      }
    }
    columnas.push({ g, x: ancho * (i + 0.5) });
  }
}

let izada = memo.leer(true) !== false;
let altura = izada ? 1 : 0;   // 0 abajo, 1 arriba
function subirBajar(dt) {
  const meta = izada ? 1 : 0;
  if (altura !== meta) altura = meta > altura ? Math.min(1, altura + dt / 3) : Math.max(0, altura - dt / 3);
  bandera.position = { x: 0.025, y: 0.9 + 2.0 * altura, z: 0 };
}

const anim = Obj.animador((t, dt) => {
  subirBajar(dt);
  const v = Math.max(0, Math.min(1, Number(P.viento) || 0));
  for (const c of columnas) {
    const k = c.x / L;                       // 0 en el mástil, 1 en la punta
    const onda = Math.sin(t * (2 + 3 * v) - k * 5);
    const z = onda * 0.08 * k * (0.3 + v);
    c.g.position = { x: c.x, y: -0.03 * k * k * (1 - v), z };
    c.g.rotation = { x: 0, y: Math.cos(t * (2 + 3 * v) - k * 5) * 0.35 * k * (0.3 + v), z: 0 };
  }
  return true;
});
...
function poner(v) {
  ...
  izada = v;
  memo.guardar(izada);
  Obj.emitir("izada", { izada });
}
Obj.boton(Obj.$("toque"), () => poner(!izada));
Obj.mensaje("izar", () => poner(true));
Obj.mensaje("arriar", () => poner(false));
Obj.alCambiar((p, a, cambios) => { if (cambios.includes("franjas") || cambios.includes("emblema")) armar(); });
armar();
subirBajar(0);
anim.iniciar();
