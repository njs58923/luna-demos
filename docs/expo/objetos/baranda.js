...
;
// La baranda: balaustres, barrotes, vidrio o cables. Tocarla pasa al tipo
// siguiente.
const P = Obj.props({ largo: 1.6, alto: 0.95, tipo: "balaustres", color: "" });
const ORDEN = ["balaustres", "barrotes", "vidrio", "cables"];
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let tipo = ORDEN.includes(P.tipo) ? P.tipo : "balaustres";

function dibujar() {
  const L = Obra.num(P.largo, 1.6, 0.3, 40), H = Obra.num(P.alto, 0.95, 0.4, 2);
  const propio = tipo === P.tipo;
  armar(Obra.baranda({ largo: L, alto: H, tipo, color: propio ? P.color : "" }));
  toque.position = { x: 0, y: H / 2, z: 0 };
  toque.scale = { x: L + 0.1, y: H + 0.05, z: 0.2 };
}
