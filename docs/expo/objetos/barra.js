...
;
// La barra: un relleno que se desliza hacia la fracción pedida.
//
// Continua, es una caja que se estira desde la izquierda. En bloques, son N
// cajitas que se prenden de a una. El número va siempre con la meta; lo que
// se anima es el relleno.
const P = Obj.props({ min: 0, max: 1, segmentos: 0, color: "#2EC4B6", titulo: "", lleno: "#30D158" });

const rellenoG = Obj.$("relleno");
const ANCHO = 0.214, ALTO = 0.022;
const min = () => Number(P.min) || 0;
const max = () => (Number.isFinite(Number(P.max)) && Number(P.max) !== min() ? Number(P.max) : min() + 1);
const N = () => Math.max(0, Math.min(40, Math.round(Number(P.segmentos) || 0)));

let meta = 0, visto = 0;
let continuo = null, bloques = [];
function armar() {
  if (continuo) continuo.remove();
  for (const b of bloques) b.remove();
  continuo = null; bloques = [];
  const n = N();
  if (!n) continuo = Obj.crear("box", { sx: 0.001, sy: ALTO, sz: 0.002, color: "#2EC4B6" }, rellenoG);
  else {
    const w = ANCHO / n;
    for (let i = 0; i < n; i++) bloques.push(Obj.crear("box", { x: -ANCHO / 2 + w * (i + 0.5), sx: w * 0.82, sy: ALTO, sz: 0.002, color: "#2B2F3A" }, rellenoG));
  }
  pintado = "";
  dibujar();
}
let pintado = "";
function dibujar() {
  const color = meta >= 0.999 ? Obj.color(P.lleno, "#30D158") : Obj.color(P.color, "#2EC4B6");
  if (continuo) {
    continuo.position = { x: -ANCHO / 2 + (visto * ANCHO) / 2, y: 0, z: 0 };
    continuo.scale = { x: Math.max(0.001, visto * ANCHO), y: ALTO, z: 0.002 };
    if (pintado !== color) { pintado = color; continuo.setAttribute("color", color); }
  } else {
    const prendidos = Math.round(visto * bloques.length);
    const firma = prendidos + color;
    if (firma !== pintado) {
      pintado = firma;
      bloques.forEach((b, i) => b.setAttribute("color", i < prendidos ? color : "#2B2F3A"));
    }
  }
}
const anim = Obj.animador((t, dt) => {
  visto += (meta - visto) * (1 - Math.exp(-dt * 8));
  if (Math.abs(meta - visto) < 5e-4) visto = meta;
  dibujar();
  return visto !== meta;
});
function poner(f) {
  meta = Math.max(0, Math.min(1, f));
  Obj.$("porcentaje").setAttribute("value", Math.round(meta * 100) + "%");
  anim.iniciar();
}
Obj.mensaje("valor", (d) => {
  if (d == null) return;
  if (typeof d === "number") return poner(d);
  if (d.fraccion != null && Number.isFinite(Number(d.fraccion))) poner(Number(d.fraccion));
  else if (Number.isFinite(Number(d.valor))) poner((Number(d.valor) - min()) / (max() - min()));
});
Obj.boton(Obj.$("toque"), () => poner(meta >= 1 ? 0 : Math.min(1, meta + 0.25)));
Obj.alCambiar(() => { Obj.$("titulo").setAttribute("value", String(P.titulo || "").slice(0, 24)); armar(); });
Obj.$("titulo").setAttribute("value", String(P.titulo || "").slice(0, 24));
armar();
