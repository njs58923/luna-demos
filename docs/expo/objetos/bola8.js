...
;
// La bola 8: sacudirla, esperar, y que suba la respuesta.
//
// La respuesta aparece como en la de verdad: el dado azul emerge desde el
// fondo de la ventanita (sube en Z y se agranda) con un giro que se asienta.
// Mientras se sacude, la ventana queda oscura.
const P = Obj.props({ respuestas: null });

const CLASICAS = [
  "Es cierto", "Sin duda", "Definitivamente", "Podés contar con eso", "Como yo lo veo, sí",
  "Lo más probable", "Pinta bien", "Sí", "Las señales dicen que sí", "Todo indica que sí",
  "Respuesta confusa, probá otra vez", "Preguntá después", "Mejor no decirte ahora",
  "No se puede predecir ahora", "Concentrate y preguntá otra vez",
  "No cuentes con eso", "Mi respuesta es no", "Mis fuentes dicen que no", "No pinta bien", "Muy dudoso",
];
const bola = Obj.$("bola");
const dado = Obj.$("dado");
// Tres renglones, un nodo cada uno: un <text> con saltos de línea crece en
// alto y no queda claro cómo lo escala el motor.
const renglones = ["r0", "r1", "r2"].map((id) => Obj.$(id));
const texto = {
  setAttribute(_k, v) {
    const r = String(v).split(String.fromCharCode(10));
    // Centrados en vertical: con un renglón, va en el del medio.
    const desde = r.length === 1 ? 1 : r.length === 2 ? 0 : 0;
    renglones.forEach((el, i) => el.setAttribute("value", r[i - desde] || ""));
  },
};

function respuestas() {
  const r = Array.isArray(P.respuestas) ? P.respuestas.map(String).filter(Boolean) : [];
  return r.length ? r : CLASICAS;
}
/** Parte la respuesta en renglones cortos para el triangulito del dado. */
function acomodar(t) {
  const palabras = String(t).split(" ");
  const out = [];
  let r = "";
  for (const p of palabras) { if ((r + " " + p).trim().length > 11) { out.push(r.trim()); r = p; } else r += " " + p; }
  if (r.trim()) out.push(r.trim());
  return out.slice(0, 3).join(String.fromCharCode(10));
}

let estado = "quieta", inicio = 0, actual = "";
const anim = Obj.animador((t) => {
  const k = (performance.now() - inicio) / 1000;
  if (estado === "sacudiendo") {
    bola.rotation = { x: 0.25 * Math.sin(k * 30), y: 0, z: 0.3 * Math.sin(k * 23) };
    bola.position = { x: 0.01 * Math.sin(k * 27), y: 0.1 + 0.01 * Math.abs(Math.sin(k * 31)), z: 0 };
    if (k > 0.9) {
      estado = "subiendo";
      inicio = performance.now();
      bola.rotation = { x: 0, y: 0, z: 0 };
      bola.position = { x: 0, y: 0.1, z: 0 };
      texto.setAttribute("value", acomodar(actual));
    }
    return true;
  }
  if (estado === "subiendo") {
    const e = Obj.salida(Math.min(1, k / 1.4));
    dado.position = { x: 0, y: 0, z: 0.05 + 0.021 * e };
    dado.scale = { x: 0.4 + 0.6 * e, y: 0.4 + 0.6 * e, z: 1 };
    dado.rotation = { x: 0, y: 0, z: (1 - e) * 1.2 };
    if (k >= 1.4) { estado = "quieta"; Obj.emitir("respuesta", { texto: actual }); return false; }
    return true;
  }
  return false;
});
...
function preguntar() {
  ...
  const r = respuestas();
  actual = r[Math.floor(Math.random() * r.length)];
  estado = "sacudiendo";
  inicio = performance.now();
  texto.setAttribute("value", "");
  dado.position = { x: 0, y: 0, z: 0.05 };
  dado.scale = { x: 0.4, y: 0.4, z: 1 };
  anim.iniciar();
}
Obj.boton(Obj.$("toque"), preguntar);
Obj.mensaje("preguntar", preguntar);
dado.position = { x: 0, y: 0, z: 0.05 };
dado.scale = { x: 0.4, y: 0.4, z: 1 };
