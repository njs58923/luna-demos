...
;
// La bola de espejos: facetas, giro y puntos de luz barriendo el piso.
//
// Las facetas son cajitas chatas sobre la esfera, por anillos de latitud, cada
// una girada para que su cara mire hacia afuera. Los puntos del piso no se
// calculan reflejando nada: son discos translúcidos repartidos en un anillo
// que gira a la misma velocidad que la bola, que es lo que el ojo espera.
const P = Obj.props({ alcance: 2.5, velocidad: 0.25, colores: true, encendida: true });

const bola = Obj.$("bola");
const puntos = Obj.$("puntos");
const R = 0.15;
const BRILLOS = ["#FFFFFF", "#E8F4FF", "#C8D8E8", "#F0F0F0"];
const facetas = [];

// Anillos de latitud; en cada uno, tantas facetas como entren.
for (let lat = -5; lat <= 5; lat++) {
  const phi = (lat / 6) * (Math.PI / 2);
  const r = Math.cos(phi) * R;
  const n = Math.max(4, Math.round((2 * Math.PI * r) / 0.028));
  for (let k = 0; k < n; k++) {
    const th = (k / n) * Math.PI * 2 + (lat % 2) * (Math.PI / n);
    // Girada en Y hacia su meridiano y en X hacia su paralelo.
    facetas.push(Obj.crear("box", {
      x: Math.sin(th) * r, y: Math.sin(phi) * R, z: Math.cos(th) * r,
      ry: th, rx: -phi, sx: 0.024, sy: 0.024, sz: 0.004,
      color: BRILLOS[(k + lat * 3 + 12) % BRILLOS.length],
    }, bola));
  }
}

const COLORES = ["#FF3B30", "#FFD60A", "#30D158", "#0A84FF", "#BF5AF2", "#FF9F0A", "#40E0D0", "#FFFFFF"];
const manchas = [];
function armarPuntos() {
  for (const m of manchas) m.remove();
  manchas.length = 0;
  const A = Math.max(0.8, Math.min(6, Number(P.alcance) || 2.5));
  const azar = Obj.azar(7);
  for (let i = 0; i < 28; i++) {
    const a = azar() * Math.PI * 2, r = 0.5 + azar() * (A - 0.5);
    const c = P.colores === false ? "#FFFFFF" : COLORES[i % COLORES.length];
    const d = 0.08 + azar() * 0.08;
    manchas.push(Obj.crear("plane", {
      x: Math.sin(a) * r, z: Math.cos(a) * r, rx: -Math.PI / 2, sx: d, sy: d, "border-radius": 0.5,
      color: c + "70", "material-alpha": "blend", "material-unlit": "true",
    }, puntos));
  }
}
...
const anim = Obj.animador((t, dt) => {
  const v = Math.max(0, Math.min(2, Number(P.velocidad) || 0.25));
  angulo += v * Math.PI * 2 * dt;
  bola.rotation = { x: 0, y: angulo, z: 0 };
  puntos.rotation = { x: 0, y: angulo, z: 0 };
  return encendida;
});
...
function poner(v) {
  ...
  encendida = v;
  puntos.setAttribute("visible", encendida ? "inherit" : "false");
  if (encendida) anim.iniciar();
...
Obj.alCambiar((p, antes, cambios) => { if (cambios.includes("alcance") || cambios.includes("colores")) armarPuntos(); });
