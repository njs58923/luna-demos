...
;
// La bola de nieve: copos en un fluido espeso, dentro de una esfera.
//
// Cada copo tiene una velocidad que el líquido frena rápido y una caída
// terminal muy lenta, así que bajan flotando. Agitar les da una velocidad al
// azar más un remolino alrededor del eje vertical; la pared de vidrio los
// rebota hacia adentro y el piso de nieve los junta, donde se quedan.
const P = Obj.props({ base: "#6B2737", texto: "", copos: 50, casa: "#C0392B" });

const donde = Obj.marco();
const grupo = Obj.$("nieve");
const C = { x: 0, y: 0.16, z: 0 };   // centro del vidrio
const RADIO = 0.112;
const PISO = 0.083;
const N = Math.max(20, Math.min(90, Math.round(Number(P.copos) || 50)));

const copos = [];
for (let i = 0; i < N; i++) {
  const el = Obj.crear("sphere", { sx: 0.006, sy: 0.006, sz: 0.006, color: "#FFFFFF", "material-unlit": "true" }, grupo);
  // Arrancan asentados en el piso de la bola.
  const a = Math.random() * Math.PI * 2, r = Math.sqrt(Math.random()) * 0.07;
  const p = { x: Math.cos(a) * r, y: PISO, z: Math.sin(a) * r };
  el.position = p;
  copos.push({ el, p, v: { x: 0, y: 0, z: 0 } });
}

let remolino = 0;
const anim = Obj.animador((t, dt) => {
  let seguir = false;
  remolino *= Math.exp(-dt * 0.6);
  for (const c of copos) {
    const p = c.p, v = c.v;
    if (p.y <= PISO + 1e-4 && Math.abs(v.x) + Math.abs(v.y) + Math.abs(v.z) < 1e-3) continue;
    seguir = true;
    // Remolino: velocidad tangencial alrededor del eje Y.
    const rx = p.x - C.x, rz = p.z - C.z;
    const fx = -rz * remolino * 6, fz = rx * remolino * 6;
    // El líquido: frena fuerte hacia (remolino, caída lenta).
    const k = 2.2;
    v.x += (fx - v.x) * k * dt;
    v.z += (fz - v.z) * k * dt;
    v.y += (-0.02 - v.y) * k * dt;
    ...
    if (d > RADIO) {
      ...
      p.x = C.x + nx * RADIO; p.y = C.y + ny * RADIO; p.z = C.z + nz * RADIO;
      const vn = v.x * nx + v.y * ny + v.z * nz;
      if (vn > 0) { v.x -= 1.5 * vn * nx; v.y -= 1.5 * vn * ny; v.z -= 1.5 * vn * nz; }
    }
    // El piso: ahí se quedan, dentro del disco de nieve.
    if (p.y < PISO) {
      p.y = PISO; v.x = v.y = v.z = 0;
      const r = Math.hypot(p.x - C.x, p.z - C.z);
      if (r > 0.072) { p.x = C.x + (p.x - C.x) * 0.072 / r; p.z = C.z + (p.z - C.z) * 0.072 / r; }
    }
    c.el.position = p;
  }
  return seguir || Math.abs(remolino) > 0.01;
});

function agitar(fuerza) {
  const f = Math.max(0.3, Math.min(1.5, fuerza || 1));
  for (const c of copos) {
    c.v.x += (Math.random() - 0.5) * 0.5 * f;
    c.v.y += (0.15 + Math.random() * 0.4) * f;
    c.v.z += (Math.random() - 0.5) * 0.5 * f;
    // Un empujón para despegarlos del piso.
    if (c.p.y <= PISO) c.p.y = PISO + 0.002;
  }
  remolino = Math.max(-3, Math.min(3, remolino + (Math.random() < 0.5 ? -1 : 1) * 1.5 * f));
  Obj.emitir("agitada", {});
  anim.iniciar();
}

Obj.boton(Obj.$("toque"), () => agitar(1));
Obj.mensaje("agitar", () => agitar(1));

// En VR: sacudir la mano cerca (ir y venir rápido) la agita. Se mide cuántas
// veces cambia de sentido la velocidad en el último medio segundo.
const zona = Obj.$("zona");
const manos = { left: null, right: null };
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    if (Math.hypot(l.x - C.x, l.y - C.y, l.z - C.z) > 0.25) { manos[mano] = null; return; }
    const ahora = performance.now();
    const m = manos[mano] || (manos[mano] = { p: l, t: ahora, v: null, vueltas: [] });
    const dt = (ahora - m.t) / 1000;
    if (dt < 0.02) return;
    const v = { x: (l.x - m.p.x) / dt, y: (l.y - m.p.y) / dt, z: (l.z - m.p.z) / dt };
    const rapidez = Math.hypot(v.x, v.y, v.z);
    if (m.v && rapidez > 0.6 && v.x * m.v.x + v.y * m.v.y + v.z * m.v.z < 0) m.vueltas.push(ahora);
    m.vueltas = m.vueltas.filter((x) => ahora - x < 600);
    if (m.vueltas.length >= 2) { m.vueltas = []; agitar(Math.min(1.5, rapidez / 1.5)); }
    if (rapidez > 0.3) m.v = v;
    m.p = l; m.t = ahora;
  });
}

function pintar() {
  Obj.$("pie").setAttribute("color", Obj.color(P.base, "#6B2737"));
  Obj.$("casa").setAttribute("color", Obj.color(P.casa, "#C0392B"));
  Obj.$("texto").setAttribute("value", String(P.texto || "").slice(0, 16));
}
