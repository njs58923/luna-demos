...
;
// Los bolos: una bocha que rueda, palos que caen y se empujan entre sí.
//
// Todo pasa en el plano de la pista (x, z). La bocha empuja a los palos que
// toca, y un palo que cae empuja a los que alcanza al caer —así se hacen las
// chuzas—. Un palo caído gira hasta quedar acostado en la dirección en que lo
// empujaron y se desliza un poco.
const P = Obj.props({ madera: "#D4A373", bocha: "#1D3557" });

const RB = 0.07, RP = 0.035, ALTO = 0.22;
const INICIO = { x: 0, z: 1.0 };
const FONDO = -1.3;
const donde = Obj.marco();
const bocha = Obj.$("bocha");
const toque = Obj.$("toque");
const palosG = Obj.$("palos");
const cuenta = Obj.$("cuenta");

// Seis palos en triángulo: 1, 2, 3.
const LUGARES = [[0, -0.95], [-0.06, -1.05], [0.06, -1.05], [-0.12, -1.15], [0, -1.15], [0.12, -1.15]];
const palos = LUGARES.map(([x, z]) => {
  const g = Obj.crear("group", { x, z }, palosG);
  // El palo parado sobre su base; el pivote de la caída es la base.
  Obj.crear("cylinder", { y: 0.07, sx: 0.07, sy: 0.14, sz: 0.07, color: "#F4F4F4" }, g);
  Obj.crear("cylinder", { y: 0.15, sx: 0.04, sy: 0.04, sz: 0.04, color: "#F4F4F4" }, g);
  Obj.crear("cylinder", { y: 0.13, sx: 0.05, sy: 0.012, sz: 0.05, color: "#C8102E" }, g);
  Obj.crear("sphere", { y: 0.19, sx: 0.05, sy: 0.06, sz: 0.05, color: "#F4F4F4" }, g);
  return { g, x0: x, z0: z, x, z, caido: false, k: 0, dir: 0, v: { x: 0, z: 0 } };
});

let b = { x: INICIO.x, y: 0, z: INICIO.z }, vb = { x: 0, z: 0 };
let rodando = false, enMano = false, giro = 0, fin = 0;

function dibujarBocha() {
  bocha.position = { x: b.x, y: 0.04 + b.y, z: b.z };
  bocha.rotation = { x: giro, y: 0, z: 0 };
  toque.position = { x: b.x, y: 0.11 + b.y, z: b.z };
}
function caidos() { return palos.filter((p) => p.caido).length; }

function voltear(p, dx, dz, fuerza) {
  if (p.caido) return;
  p.caido = true;
  p.dir = Math.atan2(dx, dz);
  p.v = { x: dx * fuerza * 0.5, z: dz * fuerza * 0.5 };
}

const anim = Obj.animador((t, dt) => {
  let seguir = false;
  if (enMano) { dibujarBocha(); return true; }
  if (rodando) {
    seguir = true;
    b.x += vb.x * dt; b.z += vb.z * dt;
    giro += (-vb.z * dt) / RB;
    // Las canaletas: la bocha no se va de la pista.
    if (Math.abs(b.x) > 0.26) { b.x = Math.sign(b.x) * 0.26; vb.x = -vb.x * 0.3; }
    for (const p of palos) {
      if (p.caido) continue;
      const dx = p.x - b.x, dz = p.z - b.z, d = Math.hypot(dx, dz);
      if (d < RB + RP) voltear(p, dx / d, dz / d, Math.hypot(vb.x, vb.z));
    }
    if (b.z < FONDO - 0.05) { rodando = false; fin = t; }
  }
  // Palos cayendo: giran alrededor de la base y empujan a los vecinos.
  for (const p of palos) {
    if (!p.caido || p.k >= 1) continue;
    seguir = true;
    p.k = Math.min(1, p.k + dt / 0.35);
    p.x += p.v.x * dt; p.z += p.v.z * dt;
    p.v.x *= 0.96; p.v.z *= 0.96;
    // La cabeza del palo barre el piso: si pasa por otro palo, lo voltea.
    const cabeza = { x: p.x + Math.sin(p.dir) * ALTO * p.k, z: p.z + Math.cos(p.dir) * ALTO * p.k };
    for (const o of palos) {
      if (o === p || o.caido) continue;
      const dx = o.x - cabeza.x, dz = o.z - cabeza.z, d = Math.hypot(dx, dz);
      if (d < RP * 2) voltear(o, (dx / (d || 1)) * 0.6 + Math.sin(p.dir) * 0.4, (dz / (d || 1)) * 0.6 + Math.cos(p.dir) * 0.4, 1.2);
    }
    // Se acuesta hacia su dirección: giro de 90° alrededor del eje horizontal
    // perpendicular a esa dirección.
    const q = Obj.qMul(Obj.qEje({ x: Math.cos(p.dir), y: 0, z: -Math.sin(p.dir) }, (Math.PI / 2) * Obj.salida(p.k)), { x: 0, y: 0, z: 0, w: 1 });
    p.g.rotation = Obj.euler(q);
    p.g.position = { x: p.x, y: 0, z: p.z };
  }
  dibujarBocha();
  // Terminada la tirada, se cuenta y se vuelve a armar.
  if (!rodando && fin && t - fin > 1.2 && palos.every((p) => !p.caido || p.k >= 1)) {
    const n = caidos();
    cuenta.setAttribute("value", n === 6 ? "¡CHUZA!" : n + " de 6");
    Obj.emitir("tirada", { caidos: n });
    fin = 0;
    setTimeout(armar, 2500);
    return false;
  }
  return seguir || (!!fin);
});
...
function armar() {
  for (const p of palos) {
    p.caido = false; p.k = 0; p.x = p.x0; p.z = p.z0;
    p.g.position = { x: p.x, y: 0, z: p.z };
    p.g.rotation = { x: 0, y: 0, z: 0 };
  }
  b = { x: INICIO.x, y: 0, z: INICIO.z };
  vb = { x: 0, z: 0 };
  rodando = false;
  cuenta.setAttribute("value", "");
  dibujarBocha();
}

function tirar(vx, vz) {
  if (rodando) return;
  vb = { x: vx, z: Math.min(-0.5, vz) };
  rodando = true; fin = 0;
  anim.iniciar();
}
Obj.boton(toque, () => tirar((Math.random() - 0.5) * 0.25, -2.6 - Math.random() * 0.6));
Obj.mensaje("tirar", () => tirar((Math.random() - 0.5) * 0.25, -2.8));
Obj.mensaje("armar", armar);
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca: (m) => !rodando && Math.hypot(m.x - b.x, m.y - (0.11 + b.y), m.z - b.z) < 0.18,
  tomar: (m) => { enMano = true; b = { x: m.x, y: Math.max(0, m.y - 0.11), z: m.z }; anim.iniciar(); },
  mover: (m) => { b = { x: Math.max(-0.26, Math.min(0.26, m.x)), y: Math.max(0, m.y - 0.11), z: m.z }; dibujarBocha(); },
  soltar: (m, v) => { enMano = false; b.y = 0; tirar(v.x * 0.5, v.z); },
});
function pintar() {
  Obj.$("pista").setAttribute("color", Obj.color(P.madera, "#D4A373"));
  Obj.$("bocha_cuerpo").setAttribute("color", Obj.color(P.bocha, "#1D3557"));
}
