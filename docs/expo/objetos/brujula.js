...
;
// La brújula: la aguja busca una dirección del mundo, no del objeto.
//
// Para saber hacia dónde queda el norte del mundo visto desde el objeto, se
// usa Obj.marco(): pasa la dirección "norte" del mundo a coordenadas del
// objeto. La aguja es un péndulo de torsión con amortiguación: si el objeto
// gira (alguien lo lleva con el manipulador), la aguja se atrasa, oscila y se
// asienta, como una de verdad.
const P = Obj.props({ norte: 0, caja: "#8B6B3E" });

const donde = Obj.marco();
const aguja = Obj.$("aguja");
const rosa = Obj.$("rosa");

// La rosa de los vientos: marcas cada 30° y las cuatro letras.
for (let i = 0; i < 12; i++) {
  const a = (i / 12) * Math.PI * 2;
  Obj.crear("box", { x: Math.sin(a) * 0.05, z: -Math.cos(a) * 0.05, ry: -a, sx: 0.002, sy: 0.001, sz: i % 3 ? 0.008 : 0.014, color: "#5A4A32" }, rosa);
}
for (const [letra, a] of [["N", 0], ["E", 90], ["S", 180], ["O", 270]]) {
  const r = (a * Math.PI) / 180;
  Obj.crear("text", { x: Math.sin(r) * 0.036, y: 0.001, z: -Math.cos(r) * 0.036, rx: -Math.PI / 2, value: letra, size: 0.014, color: letra === "N" ? "#C8102E" : "#3A3228" }, rosa);
}
// La aguja: mitad roja al norte, mitad blanca al sur.
Obj.crear("box", { z: -0.022, rx: 0, sx: 0.008, sy: 0.003, sz: 0.044, color: "#C8102E" }, aguja);
Obj.crear("box", { z: 0.022, sx: 0.008, sy: 0.003, sz: 0.044, color: "#F4F4F4" }, aguja);

/** El ángulo (alrededor de Y, en el objeto) hacia el que queda el norte del mundo. */
function norteLocal() {
  const g = ((Number(P.norte) || 0) * Math.PI) / 180;
  const n = { x: -Math.sin(g), y: 0, z: -Math.cos(g) };   // el norte del mundo
  const l = donde.dirLocal(n);
  // Ángulo de giro en Y que lleva el -Z de la aguja a esa dirección.
  return Math.atan2(-l.x, -l.z);
}

let ang = 0, vel = 0, quieta = 0, asentada = false;
const anim = Obj.animador((t, dt) => {
  const meta = norteLocal();
  let d = meta - ang;
  while (d > Math.PI) d -= Math.PI * 2;
  while (d < -Math.PI) d += Math.PI * 2;
  vel += (d * 18 - vel * 2.2) * dt;
  ang += vel * dt;
  aguja.rotation = { x: 0, y: ang, z: 0 };
  if (Math.abs(d) < 0.003 && Math.abs(vel) < 0.01) {
    quieta += dt;
    if (!asentada && quieta > 0.3) {
      asentada = true;
      const grados = Math.round((((-ang * 180) / Math.PI) % 360 + 360) % 360);
      Obj.emitir("rumbo", { grados });
    }
  } else { quieta = 0; asentada = false; }
  return !(asentada && quieta > 0.5);
});
// La aguja no puede saber que el objeto giró si no mira: se revisa siempre,
// pero una brújula quieta y asentada sólo pide cuadros de a ratos.
setInterval(() => anim.iniciar(), 500);

Obj.boton(Obj.$("toque"), () => { vel += (Math.random() < 0.5 ? -1 : 1) * 6; asentada = false; anim.iniciar(); });
Obj.alCambiar(() => { Obj.$("caja").setAttribute("color", Obj.color(P.caja, "#8B6B3E")); asentada = false; anim.iniciar(); });
Obj.$("caja").setAttribute("color", Obj.color(P.caja, "#8B6B3E"));
anim.iniciar();
