...
;
// Los cables: enchufes que se llevan a tomas, con el cable tendido detrás.
//
// Cada cable es un cilindro tendido entre su borne (fijo, a la izquierda) y
// el enchufe, que está en su lugar, en la mano o en una toma. El enchufe
// sólo se queda en la toma de su color: en cualquier otra rebota y vuelve.
// Todo pasa en el plano del panel, a una altura fija sobre él.
const P = Obj.props({ cantidad: 4, colores: ["#E63946", "#FFD60A", "#0A84FF", "#FF4FD8", "#2EC4B6"], mezclar: true });

const sinte = Obj.sinte({ volumen: 0.6 });
const donde = Obj.marco();
const cablesG = Obj.$("cables"), tomasG = Obj.$("tomas");
const X_BORNE = -0.155, X_TOMA = 0.155, Z = 0.018;

function colores() {
  const c = Array.isArray(P.colores) ? P.colores.map((x) => Obj.color(x, null)).filter(Boolean) : [];
  return c.length >= 2 ? c : ["#E63946", "#FFD60A", "#0A84FF", "#FF4FD8", "#2EC4B6"];
}
const N = () => Math.max(2, Math.min(5, Math.round(Number(P.cantidad) || 4), colores().length));
const filaY = (i, n) => (n === 1 ? 0 : 0.09 - (i * 0.18) / (n - 1));

let cables = [], tomas = [], nodos = [];
let elegido = null;       // escritorio: el cable elegido con un clic
let completo = false;

function armar() {
  for (const n of nodos) n.remove();
  nodos = []; cables = []; tomas = [];
  elegido = null; completo = false;
  const n = N(), cs = colores().slice(0, n);
  const orden = cs.map((_, i) => i);
  if (P.mezclar !== false) {
    for (let i = orden.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [orden[i], orden[j]] = [orden[j], orden[i]]; }
    // Que no quede ninguna igual que a la izquierda si se puede (si no, es muy fácil).
    if (n > 1 && orden.every((o, i) => o === i)) orden.push(orden.shift());
  }
  orden.forEach((ci, i) => {
    const y = filaY(i, n);
    const t = { i, color: cs[ci], x: X_TOMA, y, ocupada: null };
    const n1 = Obj.crear("box", { x: X_TOMA, y, z: 0.006, sx: 0.034, sy: 0.026, sz: 0.012, color: "#101216" }, tomasG);
    const n2 = Obj.crear("box", { x: X_TOMA + 0.013, y, z: 0.0125, sx: 0.006, sy: 0.02, sz: 0.001, color: cs[ci], "material-unlit": "true" }, tomasG);
    const b = Obj.crear("box", { x: X_TOMA, y, z: 0.02, sx: 0.05, sy: 0.04, sz: 0.03, color: "transparent", "material-alpha": "blend" }, tomasG);
    Obj.boton(b, () => tomaTocada(t));
    nodos.push(n1, n2, b);
    tomas.push(t);
  });
  cs.forEach((color, i) => {
    const y = filaY(i, n);
    const borne = Obj.crear("box", { x: X_BORNE, y, z: 0.008, sx: 0.02, sy: 0.02, sz: 0.012, color: "#101216" }, cablesG);
    const hilo = Obj.crear("cylinder", { sx: 0.008, sy: 0.01, sz: 0.008, color }, cablesG);
    const enchufe = Obj.crear("group", {}, cablesG);
    Obj.crear("box", { sx: 0.03, sy: 0.018, sz: 0.014, color, "border-radius": 0.004 }, enchufe);
    Obj.crear("box", { x: 0.019, sx: 0.01, sy: 0.006, sz: 0.004, color: "#C0C0C0" }, enchufe);
    const b = Obj.crear("box", { sx: 0.05, sy: 0.04, sz: 0.03, color: "transparent", "material-alpha": "blend" }, enchufe);
    const c = { i, color, casa: { x: X_BORNE + 0.035, y }, p: { x: X_BORNE + 0.035, y }, toma: null, hilo, enchufe };
    Obj.boton(b, () => cableTocado(c));
    nodos.push(borne, hilo, enchufe);
    cables.push(c);
  });
  cables.forEach(dibujar);
  pintarLuz();
}
function dibujar(c) {
  c.enchufe.position = { x: c.p.x, y: c.p.y, z: Z };
  const a = { x: X_BORNE, y: c.casa.y, z: Z }, b = { x: c.p.x - 0.015, y: c.p.y, z: Z };
  const l = Obj.tender(c.hilo, a, b);
  c.hilo.scale = { x: 0.008, y: Math.max(0.001, l), z: 0.008 };
  c.enchufe.scale = elegido === c ? { x: 1.25, y: 1.25, z: 1.25 } : { x: 1, y: 1, z: 1 };
}
function pintarLuz() { Obj.$("luz").setAttribute("color", completo ? "#30D158" : "#3A3F4B"); }
const conectados = () => cables.filter((c) => c.toma).length;

/** El enchufe llegó a la toma t: se queda si es de su color. */
function enchufar(c, t) {
  const bien = t.color === c.color && !t.ocupada;
  Obj.emitir("conectado", { color: c.color, bien });
  if (!bien) {
    sinte.nota(140, 0.25, { tipo: "cuadrada", vol: 0.1, caida: 6 });
    volver(c);
    return;
  }
  c.toma = t; t.ocupada = c;
  c.p = { x: t.x - 0.02, y: t.y };
  dibujar(c);
  sinte.nota(900, 0.05, { tipo: "madera", vol: 0.45 });
  const n = conectados();
  Obj.emitir("valor", { valor: n, fraccion: +(n / cables.length).toFixed(3) });
  if (n === cables.length && !completo) {
    completo = true;
    pintarLuz();
    for (let i = 0; i < 4; i++) sinte.nota(Obj.frecuencia(i * 4), 0.35, { tipo: "campana", vol: 0.18, retraso: 0.1 + i * 0.1 });
    Obj.emitir("completo", {});
  }
}
function volver(c) { animar(c, c.casa); }

// Un viaje animado del enchufe (escritorio y rebote).
let viajes = [];
const anim = Obj.animador((t, dt) => {
  viajes = viajes.filter((v) => {
    const k = Math.min(1, (performance.now() - v.t0) / 280);
    const s = Obj.suave(k);
    v.c.p = { x: v.a.x + (v.b.x - v.a.x) * s, y: v.a.y + (v.b.y - v.a.y) * s };
    dibujar(v.c);
    if (k >= 1) { if (v.luego) v.luego(); return false; }
    return true;
  });
  return viajes.length > 0;
});
function animar(c, destino, luego) {
  viajes = viajes.filter((v) => v.c !== c);
  viajes.push({ c, a: { ...c.p }, b: { ...destino }, t0: performance.now(), luego });
  anim.iniciar();
}

// Escritorio: clic en el enchufe lo elige; clic en una toma lo lleva.
function cableTocado(c) {
  if (c.toma) return;
  const antes = elegido;
  elegido = elegido === c ? null : c;
  if (antes) dibujar(antes);
  dibujar(c);
  sinte.nota(1300, 0.02, { tipo: "pulso", vol: 0.25 });
}
function tomaTocada(t) {
  const c = elegido;
  if (!c) return;
  elegido = null;
  dibujar(c);
  animar(c, { x: t.x - 0.02, y: t.y }, () => enchufar(c, t));
}

// VR: el enchufe en la mano; soltarlo cerca de una toma lo enchufa.
let enMano = null;
Obj.agarre({
  zona: Obj.$("zona"), donde, gatillo: true,
  cerca(p) {
    enMano = null;
    let dm = 0.045;
    for (const c of cables) {
      if (c.toma) continue;
      const d = Math.hypot(p.x - c.p.x, p.y - c.p.y, p.z - Z - 0.02);
      if (d < dm) { dm = d; enMano = c; }
    }
    return !!enMano && p.z < 0.12;
  },
  mover(p) {
    enMano.p = { x: Math.max(-0.16, Math.min(0.17, p.x)), y: Math.max(-0.12, Math.min(0.12, p.y)) };
    dibujar(enMano);
  },
  soltar(p) {
    const c = enMano;
    enMano = null;
    if (!c) return;
    const t = tomas.find((x) => Math.hypot(c.p.x - (x.x - 0.02), c.p.y - x.y) < 0.03);
    if (t) enchufar(c, t);
    else volver(c);
  },
});

Obj.mensaje("reiniciar", () => { armar(); Obj.emitir("valor", { valor: 0, fraccion: 0 }); });
Obj.alCambiar(armar);
armar();
