...
;
// La cafetera: un hilo de café, una taza que se llena y vapor.
//
// El café en la taza es un disco que sube y se ensancha un poco; el vapor son
// tres bocanadas que suben, crecen y se desvanecen en bucle mientras la taza
// esté llena. El gorgoteo es ruido grave con burbujas: un flujo del
// sintetizador que dura lo que dura el café.
const P = Obj.props({ color: "#212529", taza: "#F8F9FA", segundos: 5 });

const sinte = Obj.sinte({ volumen: 0.6 });
const chorro = Obj.$("chorro");
const cafe = Obj.$("cafe");
const luz = Obj.$("luz");
const vaporG = Obj.$("vapor");
const bocanadas = [0, 1, 2].map((i) => ({ el: Obj.crear("sphere", { sx: 0.03, sy: 0.03, sz: 0.03, color: "#FFFFFF60", "material-alpha": "blend", visible: "false" }, vaporG), fase: i / 3 }));

let estado = "vacia";   // vacia | cargando | llena
let inicio = 0;
function segundos() { return Math.max(2, Math.min(60, Number(P.segundos) || 5)); }

function gorgoteo() {
  let s = 99, lp = 0;
  return (t) => {
    s = (s * 1103515245 + 12345) & 0x7fffffff;
    lp += ((s / 0x3fffffff - 1) - lp) * 0.05;
    const burbuja = Math.random() < 0.002 ? 1 : 0;
    return lp * 0.4 + burbuja * 0.15 * Math.sin(t * 900);
  };
}
...
const anim = Obj.animador((t) => {
  if (estado === "cargando") {
    ...
    // El nivel: de 0 a 7 cm dentro de la taza.
    const nivel = 0.005 + 0.07 * k;
    cafe.position = { x: 0, y: nivel, z: 0 };
    cafe.scale = { x: 0.066 + 0.006 * k, y: 0.002, z: 0.066 + 0.006 * k };
    chorro.scale = { x: 0.006, y: 0.26 - nivel - 0.03, z: 0.006 };
    chorro.position = { x: 0, y: 0.03 + nivel + (0.26 - nivel - 0.03) / 2, z: 0.05 };
    if (k >= 1) {
      estado = "llena";
      chorro.setAttribute("visible", "false");
      luz.setAttribute("color", "#3A1A1A");
      sinte.flujo(null);
      sinte.nota(1046.5, 0.8, { tipo: "campana", vol: 0.25 });
      Obj.emitir("listo", {});
    }
  }
  const humea = estado === "llena" || estado === "cargando";
  for (const b of bocanadas) {
    b.el.setAttribute("visible", humea ? "inherit" : "false");
    if (!humea) continue;
    const k = ((t * 0.5 + b.fase) % 1);
    const s = 0.02 + 0.05 * k;
    b.el.position = { x: 0.015 * Math.sin(t * 2 + b.fase * 6), y: k * 0.18, z: 0 };
...
function preparar() {
  if (estado !== "vacia") return;
  estado = "cargando";
  inicio = performance.now();
  cafe.setAttribute("visible", "inherit");
  chorro.setAttribute("visible", "inherit");
  luz.setAttribute("color", "#FF6B6B");
  sinte.flujo(gorgoteo());
  anim.iniciar();
}
function tomar() {
  if (estado !== "llena") return;
  estado = "vacia";
  cafe.setAttribute("visible", "false");
  Obj.emitir("tomado", {});
}

Obj.boton(Obj.$("boton"), preparar, { color: "#C92A2A" });
Obj.boton(Obj.$("toque_taza"), tomar);
Obj.mensaje("preparar", preparar);
function pintar() {
  for (const id of ["base", "torre", "cabezal"]) Obj.$(id).setAttribute("color", Obj.color(P.color, "#212529"));
  for (const id of ["taza_cuerpo", "taza_asa"]) Obj.$(id).setAttribute("color", Obj.color(P.taza, "#F8F9FA"));
}
