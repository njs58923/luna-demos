...
;
// La caja fuerte: un dial que acumula ángulo y una máquina de tres pasos.
//
// Los números están impresos en el dial en el sentido del reloj, cada 9°.
// Girar el dial en el sentido del reloj un ángulo θ trae arriba (a la marca
// roja) el número que estaba a -θ: el número de arriba *baja*, como en una
// caja de verdad. Los dos primeros números se marcan al cambiar de sentido
// (derecha hasta el primero y vuelta; izquierda hasta el segundo y vuelta);
// el tercero es el que está arriba al tirar de la manija.
const P = Obj.props({ combinacion: [12, 30, 5], color: "#C0C0C0" });

const sinte = Obj.sinte({ volumen: 0.6 });
const donde = Obj.marco();
const dialG = Obj.$("dial"), manija = Obj.$("manija");
const C = { x: 0, y: 0.025 };
const R = 0.085, PASO = (9 * Math.PI) / 180, N = 40;

// ── El dial ────────────────────────────────────────────────────────────────
Obj.crear("cylinder", { z: 0.008, rx: Math.PI / 2, sx: R * 2, sy: 0.016, sz: R * 2, color: "#1C1F27" }, dialG);
const cara = Obj.crear("cylinder", { z: 0.0165, rx: Math.PI / 2, sx: R * 2 - 0.01, sy: 0.001, sz: R * 2 - 0.01, color: "#C0C0C0" }, dialG);
for (let n = 0; n < N; n++) {
  const a = n * PASO;
  const larga = n % 5 === 0;
  const l = larga ? 0.012 : 0.006;
  Obj.crear("box", { x: Math.sin(a) * (R - 0.008 - l / 2), y: Math.cos(a) * (R - 0.008 - l / 2), z: 0.0172, rz: -a, sx: 0.0018, sy: l, sz: 0.001, color: "#1C1C1C" }, dialG);
  if (n % 5 === 0) Obj.crear("text", { x: Math.sin(a) * (R - 0.03), y: Math.cos(a) * (R - 0.03), z: 0.0175, rz: -a, value: String(n), size: 0.012, color: "#1C1C1C" }, dialG);
}
Obj.crear("cylinder", { z: 0.024, rx: Math.PI / 2, sx: 0.05, sy: 0.016, sz: 0.05, color: "#2B2F3A" }, dialG);
for (let i = 0; i < 12; i++) {
  const a = (i / 12) * Math.PI * 2;
  Obj.crear("box", { x: Math.sin(a) * 0.025, y: Math.cos(a) * 0.025, z: 0.024, rz: -a, sx: 0.004, sy: 0.004, sz: 0.014, color: "#1C1F27" }, dialG);
}

// ── Estado ─────────────────────────────────────────────────────────────────
let theta = 0;              // lo que giró el dial (acumulado; + = sentido del reloj)
let visto = 0;              // lo que se ve (en escritorio se anima hacia theta)
let entradas = [];          // los números marcados
let sentido = 0, ancla = 0, extremo = 0;   // para detectar los cambios de sentido
let abierta = false;
let ultimoNumero = null;

function combinacion() {
  const c = Array.isArray(P.combinacion) ? P.combinacion.map((x) => ((Math.round(Number(x)) % N) + N) % N) : [];
  return c.length === 3 && c.every(Number.isFinite) ? c : [12, 30, 5];
}
const arriba = (t) => ((Math.round(-t / PASO) % N) + N) % N;
/** El sentido que corresponde al paso: derecha (+1), izquierda (-1), derecha. */
const esperado = () => (entradas.length % 2 === 0 ? 1 : -1);

function mostrarPasos() {
  const txt = [0, 1, 2].map((i) => (entradas[i] != null ? String(entradas[i]) : "_")).join("  ");
  Obj.$("pasos").setAttribute("value", txt);
}
function marcar(n) {
  if (entradas.length >= 2) return;
  const bien = n === combinacion()[entradas.length];
  entradas.push(n);
  sinte.nota(1400, 0.03, { tipo: "pulso", vol: 0.3 });
  Obj.emitir("numero", { n, paso: entradas.length, bien });
  const correctos = entradas.filter((x, i) => x === combinacion()[i]).length;
  Obj.emitir("valor", { valor: correctos, fraccion: +(correctos / 3).toFixed(3) });
  mostrarPasos();
}
function reiniciar() {
  entradas = []; sentido = 0; ancla = extremo = theta;
  mostrarPasos();
}

/** El dial giró d (en radianes). Clics al pasar números y cambios de sentido. */
function girar(d) {
  if (!d || abierta) return;
  theta += d;
  const n = arriba(theta);
  if (n !== ultimoNumero) { ultimoNumero = n; sinte.nota(2600, 0.006, { tipo: "pulso", vol: 0.08 }); }
  // Un cambio de sentido cuenta recién cuando vuelve medio número: el temblor
  // del mando no marca nada. Se marca el número del punto donde dio la vuelta.
  if (sentido === 0) { sentido = Math.sign(d); ancla = theta - d; extremo = theta; return; }
  if (Math.sign(theta - extremo) === sentido) extremo = theta;
  else if (Math.abs(theta - extremo) > PASO / 2) {
    if (Math.abs(extremo - ancla) >= PASO && sentido === esperado()) marcar(arriba(extremo));
    ancla = extremo;
    sentido = -sentido;
    extremo = theta;
  }
...
function tirar() {
  ...
  const n = arriba(theta);
  const intento = entradas.length === 2 ? [...entradas, n] : entradas;
  const c = combinacion();
  if (intento.length === 3 && intento.every((x, i) => x === c[i])) {
    entradas = intento;
    mostrarPasos();
    abierta = true;
    Obj.$("luz").setAttribute("color", "#30D158");
    manija.rotation = { x: 0, y: 0, z: -Math.PI / 2 };
    sinte.nota(180, 0.3, { tipo: "madera", vol: 0.6 });
    for (let i = 0; i < 3; i++) sinte.nota(Obj.frecuencia(3 + i * 4), 0.35, { tipo: "campana", vol: 0.2, retraso: 0.2 + i * 0.1 });
    Obj.emitir("valor", { valor: 3, fraccion: 1 });
    Obj.emitir("abierto", {});
  } else {
    // Trabada: la manija tiembla y todo vuelve a empezar.
    sinte.nota(110, 0.3, { tipo: "cuadrada", vol: 0.1, caida: 6 });
    let k = 0;
    const t = setInterval(() => { manija.rotation = { x: 0, y: 0, z: k++ % 2 ? -0.08 : 0.04 }; if (k > 6) { clearInterval(t); manija.rotation = { x: 0, y: 0, z: 0 }; } }, 40);
    reiniciar();
    Obj.emitir("valor", { valor: 0, fraccion: 0 });
  }
}
function cerrar() {
  abierta = false;
  Obj.$("luz").setAttribute("color", "#5A0B12");
  manija.rotation = { x: 0, y: 0, z: 0 };
  sinte.nota(220, 0.15, { tipo: "madera", vol: 0.5 });
  reiniciar();
  Obj.emitir("valor", { valor: 0, fraccion: 0 });
  Obj.emitir("cerrado", {});
}

// Lo que se ve sigue a theta (en escritorio con una animación).
let meta = null;
const anim = Obj.animador((t, dt) => {
  if (meta !== null) {
    const falta = meta - theta;
    const paso = Math.sign(falta) * Math.min(Math.abs(falta), dt * 6);
    girar(paso);
    if (Math.abs(meta - theta) < 1e-6) meta = null;
  }
  visto = theta;
  dialG.rotation = { x: 0, y: 0, z: -visto };
  return meta !== null;
});

// Escritorio: clic en un número lo trae arriba por el lado que corresponde y lo marca.
Obj.boton(Obj.$("toque"), (e) => {
  if (abierta) return;
  let dx = 0, dy = 0;
  if (Number.isFinite(e.x)) { const l = donde.aLocal({ x: e.x, y: e.y, z: e.z }); dx = l.x - C.x; dy = l.y - C.y; }
  if (Math.hypot(dx, dy) < 0.03) { reiniciar(); sinte.nota(600, 0.05, { tipo: "madera", vol: 0.3 }); return; }
  // El número impreso en el punto tocado: está a ψ en la cara, y el dial giró theta.
  const psi = Math.atan2(dx, dy);
  const n = ((Math.round((psi - theta) / PASO) % N) + N) % N;
  // Traerlo arriba: girar hasta que arriba(theta) = n, en el sentido esperado.
  const dir = entradas.length >= 2 ? 1 : esperado();
  let d = (-n * PASO - theta) % (Math.PI * 2);
  if (dir > 0) { while (d <= 1e-9) d += Math.PI * 2; }
  else { while (d >= -1e-9) d -= Math.PI * 2; }
  const base = theta;
  meta = base + d;
  anim.iniciar();
  // Al llegar, se marca (como si hubiera cambiado de sentido ahí).
  setTimeout(() => { if (entradas.length < 2 && arriba(theta) === n) { marcar(n); sentido = 0; ancla = extremo = theta; } }, (Math.abs(d) / 6) * 1000 + 80);
});
Obj.boton(Obj.$("toque_manija"), tirar);

// VR, con las piezas de las manos (_vr.js): el dial es una perilla (se gira
// la muñeca, o la mano alrededor) y la manija se tira con grip o gatillo.
const M = Obj.manos();
M.perilla({
  centro: { x: C.x, y: C.y, z: 0.03 }, eje: { x: 0, y: 0, z: 1 }, radio: R,
  alTomar() { meta = null; },
  alGirar(d) { girar(d); dialG.rotation = { x: 0, y: 0, z: -theta }; },
});
M.agarrable({
  cerca: (p) => Math.abs(p.x - 0.02) < 0.06 && Math.abs(p.y + 0.1) < 0.04 && p.z < 0.1 && p.z > -0.03,
  tomar() { tirar(); },
});

Obj.mensaje("cerrar", () => { if (abierta) cerrar(); });
Obj.alCambiar(() => { cara.setAttribute("color", Obj.color(P.color, "#C0C0C0")); reiniciar(); });
cara.setAttribute("color", Obj.color(P.color, "#C0C0C0"));
mostrarPasos();
