...
;
// La caja de música: tapa, bailarina y una melodía de peine metálico.
//
// Las melodías son listas de notas por nombre ("C5", "G4", "-" es silencio).
// Se agendan en el sintetizador de a medio segundo por delante, así la música
// no depende de que cada cuadro llegue a tiempo, y al cerrar la tapa se deja
// de agendar: lo ya agendado suena hasta el final de su nota, como una caja de
// verdad que se frena.
const P = Obj.props({ melodia: "estrellita", tempo: 150, caja: "#8E3B46", forro: "#F2C6D0" });

const MELODIAS = {
  // Dominio público: "Estrellita" (Twinkle Twinkle) y "Cumpleaños feliz".
  estrellita: "C5 C5 G5 G5 A5 A5 G5 - F5 F5 E5 E5 D5 D5 C5 - G5 G5 F5 F5 E5 E5 D5 - G5 G5 F5 F5 E5 E5 D5 - C5 C5 G5 G5 A5 A5 G5 - F5 F5 E5 E5 D5 D5 C5 - - -",
  cumpleanos: "G4 G4 A4 G4 C5 B4 - - G4 G4 A4 G4 D5 C5 - - G4 G4 G5 E5 C5 B4 A4 - F5 F5 E5 C5 D5 C5 - - -",
  escala: "C5 D5 E5 F5 G5 A5 B5 C6 B5 A5 G5 F5 E5 D5 C5 - -",
};
...
function frecuencia(nota) {
  ...
  return Obj.frecuencia(s);
}
function notas() {
  const txt = MELODIAS[P.melodia] || (typeof P.melodia === "string" ? P.melodia : MELODIAS.estrellita);
  const lista = txt.split(/\s+/).filter(Boolean).slice(0, 400);
  return lista.length ? lista : MELODIAS.estrellita.split(" ");
}

const sinte = Obj.sinte({ volumen: 0.8 });
const bisagra = Obj.$("bisagra");
const bailarina = Obj.$("bailarina");
const gira = Obj.$("gira");

let abierta = false;
let apertura = 0;       // 0 cerrada .. 1 abierta
let giro = 0;
let paso = 0;           // qué nota va
let siguiente = 0;      // performance.now() de la próxima nota
let agenda = null;

function agendar() {
  const lista = notas();
  const dur = 60000 / Math.max(40, Math.min(400, Number(P.tempo) || 150));
  const horizonte = performance.now() + 500;
  while (siguiente < horizonte) {
    const n = lista[paso % lista.length];
    const f = frecuencia(n);
    const retraso = Math.max(0, (siguiente - performance.now()) / 1000);
    // El peine de una caja de música: seno con un parcial alto que se apaga
    // enseguida, una octava arriba de lo escrito.
    if (f) sinte.nota(f * 2, 1.2, { tipo: "campana", vol: 0.2, caida: 3.5, retraso });
    paso++;
    siguiente += dur;
  }
}

const ABRE = 1.83;          // la tapa abierta: 105°
let enMano = null;         // el ángulo de la tapa mientras una mano la sostiene
const anim = Obj.animador((t, dt) => {
  const meta = abierta ? 1 : 0;
  if (enMano === null) {
    if (meta > apertura) apertura = Math.min(1, apertura + dt / 0.6);
    else if (meta < apertura) apertura = Math.max(0, apertura - dt / 0.4);
  }
  // La tapa abre hasta 105°: bisagra atrás, gira hacia -X... hacia atrás.
  bisagra.rotation = { x: -(enMano !== null ? enMano : ABRE * Obj.suave(apertura)), y: 0, z: 0 };
  bailarina.setAttribute("visible", apertura > 0.3 ? "inherit" : "false");
  if (abierta) {
    giro += dt * 1.8;
    gira.rotation = { x: 0, y: giro, z: 0 };
  }
  return abierta || apertura > 0 || enMano !== null;
});
...
function abrir() {
  ...
  abierta = true;
  paso = 0;
  siguiente = performance.now() + 450;
  agenda = setInterval(agendar, 200);
  anim.iniciar();
  Obj.emitir("cambio", { abierta });
}
function cerrar() {
  if (!abierta) return;
  abierta = false;
...
Obj.boton(Obj.$("toque"), () => (abierta ? cerrar() : abrir()));

// En VR: la tapa se levanta con la mano, sobre su bisagra (la palanca de las
// manos, _vr.js). Pasada la mitad empieza la música; al soltarla termina de
// abrirse o se cierra, según dónde quedó y cómo venía.
const aperturaDe = (a) => {
  let lo = 0, hi = 1;
  for (let i = 0; i < 24; i++) { const m = (lo + hi) / 2; if (ABRE * Obj.suave(m) < a) lo = m; else hi = m; }
  return (lo + hi) / 2;
};
Obj.manos().palanca({
  pivote: { x: 0, y: 0.09, z: -0.07 }, eje: { x: -1, y: 0, z: 0 }, brazo: { x: 0, y: 0, z: 1 },
  largo: 0.14, min: 0, max: ABRE, todo: true,
  angulo: () => (enMano !== null ? enMano : ABRE * Obj.suave(apertura)),
  alTomar() { enMano = ABRE * Obj.suave(apertura); anim.iniciar(); },
  alMover(a) {
    enMano = a;
    apertura = aperturaDe(a);
    bisagra.rotation = { x: -a, y: 0, z: 0 };
    bailarina.setAttribute("visible", apertura > 0.3 ? "inherit" : "false");
    if (a > ABRE * 0.5 && !abierta) abrir();
    else if (a < 0.2 && abierta) cerrar();
  },
  alSoltar(a, v) {
    enMano = null;
    if (a > ABRE * 0.6 || v > 2) abrir(); else cerrar();
    anim.iniciar();
  },
});
Obj.mensaje("abrir", abrir);
Obj.mensaje("cerrar", cerrar);
function pintar() {
  const c = Obj.color(P.caja, "#8E3B46");
  Obj.$("caja").setAttribute("color", c);
  Obj.$("tapa").setAttribute("color", c);
  Obj.$("forro").setAttribute("color", Obj.color(P.forro, "#F2C6D0"));
}
