...
;
// La caja sorpresa: una melodía que avanza con la manivela y un resorte.
//
// Cada octavo de vuelta de la manivela suena la nota siguiente, como en una
// caja de música de verdad: la melodía va al ritmo del que gira. Al llegar a
// la nota del ¡pop! la tapa se abre de golpe y el payaso sube con un resorte
// amortiguado (la respuesta de un resorte a un salto: h = alto·(1 − e^(−4t)·cos 18t),
// que se pasa, rebota y se asienta), que se ve en los aros separándose y
// juntándose.
const P = Obj.props({ colores: ["#E63946", "#457B9D", "#FFD166", "#06D6A0"] });

const sinte = Obj.sinte({ volumen: 0.7 });
const tapa = Obj.$("tapa"), cabeza = Obj.$("cabeza"), manivela = Obj.$("manivela");
const PIVOTE = { x: 0.09, y: 0.09, z: 0 };

// "Pop Goes the Weasel" (tradicional), en semitonos desde el La 440; null = el pop.
const MELODIA = [
  3, 3, 5, 5, 7, 10, 7, 3,
  3, 3, 5, 5, 7, 3, -2,
  3, 3, 5, 5, 7, 10, 7, 3,
...
// El resorte: aros que se separan con el alto.
const aros = [];
for (let k = 0; k < 9; k++) aros.push(Obj.crear("cylinder", { sx: 0.036, sy: 0.004, sz: 0.036, color: "#C0C0C0" }, Obj.$("resorte")));
function ponerResorte(h) {
  aros.forEach((a, k) => { a.position = { x: 0, y: 0.012 + (k / (aros.length - 1)) * h, z: 0 }; });
  cabeza.position = { x: 0, y: 0.012 + h + 0.006, z: 0 };
}

let nota = 0;             // la próxima nota
let abierta = false, popEn = 0;
let angulo = 0, avance = 0;   // de la manivela
let solo = false;         // girando sola (por un toque)
let tapaAng = 0;

const BAJO = 0.0, ALTO = 0.2;
function tocarNota() {
  if (abierta) return;
  const s = MELODIA[nota];
  if (s === null) { pop(); return; }
  sinte.nota(Obj.frecuencia(s + 12), 0.5, { tipo: "campana", vol: 0.22, caida: 4 });
  Obj.emitir("nota", { i: nota });
  nota = (nota + 1) % MELODIA.length;
}
function pop() {
  abierta = true;
  popEn = performance.now();
  solo = false;
  nota = 0;
  sinte.nota(Obj.frecuencia(15), 0.4, { tipo: "campana", vol: 0.35, caida: 5 });
  sinte.nota(90, 0.5, { tipo: "seno", vol: 0.3, caida: 4 });
  // El "boing" del resorte: una nota que baja.
  for (let i = 0; i < 6; i++) sinte.nota(420 - i * 40, 0.08, { tipo: "seno", vol: 0.15, caida: 18, retraso: 0.05 + i * 0.05 });
  Obj.emitir("pop", {});
  anim.iniciar();
}
function cerrar() {
  ...
  abierta = false;
  popEn = 0;
  sinte.nota(300, 0.1, { tipo: "madera", vol: 0.4, retraso: 0.25 });
  Obj.emitir("cerrada", {});
  anim.iniciar();
}
...
const anim = Obj.animador((t, dt) => {
  let seguir = false;
  const ahora = performance.now();
  // La manivela girando sola: una nota cada 0.28 s.
  if (solo && !abierta) {
    angulo += dt * (Math.PI / 4) / 0.28;
    avance += dt * (Math.PI / 4) / 0.28;
    while (avance >= Math.PI / 4) { avance -= Math.PI / 4; tocarNota(); }
    seguir = true;
  }
  manivela.rotation = { x: angulo, y: 0, z: 0 };
  // La tapa y el payaso.
  if (abierta) {
    const s = (ahora - popEn) / 1000;
    tapaAng += (-1.95 - tapaAng) * Math.min(1, dt * 25);
    const h = ALTO * (1 - Math.exp(-4 * s) * Math.cos(18 * s));
    ponerResorte(Math.max(0, h));
    cabeza.rotation = { x: 0, y: 0, z: 0.25 * Math.exp(-1.5 * s) * Math.sin(s * 9) };
    seguir = s < 4 || Math.abs(tapaAng + 1.95) > 0.01;
  } else {
    // Cerrando: el payaso se mete y la tapa baja.
    tapaAng += (0 - tapaAng) * Math.min(1, dt * 10);
    ponerResorte(BAJO);
    cabeza.rotation = { x: 0, y: 0, z: 0 };
    if (Math.abs(tapaAng) > 0.002) seguir = true; else tapaAng = 0;
  }
  tapa.rotation = { x: tapaAng, y: 0, z: 0 };
  return seguir;
});

Obj.boton(Obj.$("toque_manivela"), () => { if (!abierta) { solo = !solo; anim.iniciar(); } });
Obj.boton(Obj.$("toque_payaso"), cerrar);
Obj.mensaje("girar", () => { if (!abierta) { solo = true; anim.iniciar(); } });
Obj.mensaje("cerrar", cerrar);

// En VR: la manivela de las manos (_vr.js). Gira alrededor del eje X; mirada
// desde la derecha de la caja (desde -X), el sentido del reloj es el que
// levanta la manija hacia adelante, que es el que hace sonar la melodía.
Obj.manos().manivela({
  centro: PIVOTE, eje: { x: -1, y: 0, z: 0 }, arriba: { x: 0, y: 1, z: 0 },
  radio: 0.048, salida: -0.03, aro: true,
  angulo: () => angulo,
  alTomar() { solo = false; },
  alGirar(d) {
    angulo += d;
...
function pintar() {
  ...
  ["cara0", "cara1", "cara2", "cara3"].forEach((k, i) => {
    const col = c[i] || ["#E63946", "#457B9D", "#FFD166", "#06D6A0"][i];
