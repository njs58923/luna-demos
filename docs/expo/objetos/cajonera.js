...
;
// La cajonera: cajones que se abren de verdad.
//
// El mueble es una cáscara —laterales, tapa, fondo, piso y separadores— y
// cada cajón es una caja abierta arriba (frente, laterales, fondo y piso) que
// se desliza hacia +Z. Así, al abrirlo, se ve lo que tiene adentro, y el
// hueco que deja en el mueble es oscuro como un hueco de verdad.
const P = Obj.props({ madera: "#8B5A3C", tirador: "#C9A45C", cajones: 3, clave: "" });

const W = 0.8, H = 0.92, D = 0.5, T = 0.02;   // ancho, alto, fondo, espesor
const PATA = 0.06;
const SALIDA = 0.36;
const N = Math.max(2, Math.min(5, Math.round(Number(P.cajones) || 3)));
const memo = Obj.memoria("abiertos");

const mueble = Obj.$("mueble");
const grupo = Obj.$("cajones");
const madera = () => Obj.color(P.madera, "#8B5A3C");
const pintables = [];   // [el, k]: color = tono(madera, k)
function tabla(attrs, padre, k) {
  const el = Obj.crear("box", Object.assign({ color: Obj.tono(madera(), k || 0) }, attrs), padre);
  pintables.push([el, k || 0]);
  return el;
}

// La cáscara.
const altoUtil = H - PATA - 2 * T;
tabla({ x: -W / 2 + T / 2, y: PATA + (H - PATA) / 2, sx: T, sy: H - PATA, sz: D }, mueble);
tabla({ x: W / 2 - T / 2, y: PATA + (H - PATA) / 2, sx: T, sy: H - PATA, sz: D }, mueble);
tabla({ y: H - T / 2, sx: W + 0.02, sy: T, sz: D + 0.02 }, mueble, 0.08);
tabla({ y: PATA + T / 2, sx: W, sy: T, sz: D }, mueble);
tabla({ y: PATA + (H - PATA) / 2, z: -D / 2 + T / 4, sx: W, sy: H - PATA, sz: T / 2 }, mueble, -0.35);
for (const x of [-W / 2 + 0.05, W / 2 - 0.05]) {
  for (const z of [-D / 2 + 0.05, D / 2 - 0.05]) {
    tabla({ x, y: PATA / 2, z, sx: 0.05, sy: PATA, sz: 0.05 }, mueble, -0.3);
  }
}
const alto = altoUtil / N;
for (let i = 1; i < N; i++) {
  tabla({ y: PATA + T + i * alto, z: 0.01, sx: W - 2 * T, sy: 0.012, sz: D - 0.02 }, mueble, -0.1);
}

// Lo que hay adentro de cada cajón: cosas chicas y de colores.
const CONTENIDO = [
  [["#E63946", 0.12, 0.05, 0.08], ["#457B9D", 0.1, 0.04, 0.14], ["#F1FAEE", 0.08, 0.08, 0.08]],
  [["#2A9D8F", 0.22, 0.03, 0.16], ["#E9C46A", 0.06, 0.1, 0.06]],
  [["#264653", 0.18, 0.06, 0.12], ["#F4A261", 0.09, 0.05, 0.09], ["#E76F51", 0.07, 0.07, 0.07]],
  [["#8AC926", 0.14, 0.04, 0.1], ["#6A4C93", 0.1, 0.06, 0.1]],
  [["#FFCA3A", 0.2, 0.03, 0.2]],
];
...
const cajones = [];
for (let i = 0; i < N; i++) {
  const y0 = PATA + T + i * alto;             // piso del hueco
  const hFrente = alto - 0.016;
  const g = Obj.crear("group", { y: y0 }, grupo);
  const dz = D - 0.05;                         // largo del cajón
  // Frente, que tapa el hueco; y la caja abierta detrás.
  const frente = tabla({ y: hFrente / 2 + 0.004, z: D / 2 - T / 2, sx: W - 2 * T - 0.008, sy: hFrente, sz: T, "border-radius": 0.004 }, g, 0.04);
  tabla({ x: -(W / 2 - T - 0.02), y: (hFrente - 0.03) / 2 + 0.01, z: D / 2 - T - dz / 2, sx: 0.012, sy: hFrente - 0.03, sz: dz }, g, -0.15);
  tabla({ x: W / 2 - T - 0.02, y: (hFrente - 0.03) / 2 + 0.01, z: D / 2 - T - dz / 2, sx: 0.012, sy: hFrente - 0.03, sz: dz }, g, -0.15);
  tabla({ y: (hFrente - 0.03) / 2 + 0.01, z: D / 2 - T - dz, sx: W - 2 * T - 0.04, sy: hFrente - 0.03, sz: 0.012 }, g, -0.15);
  tabla({ y: 0.01, z: D / 2 - T - dz / 2, sx: W - 2 * T - 0.04, sy: 0.01, sz: dz }, g, -0.25);
  // El tirador: una barra con dos soportes.
  const tir = [
    Obj.crear("cylinder", { y: hFrente / 2 + 0.004, z: D / 2 + 0.025, rz: Math.PI / 2, sx: 0.014, sy: 0.16, sz: 0.014, color: Obj.color(P.tirador, "#C9A45C") }, g),
    Obj.crear("box", { x: -0.065, y: hFrente / 2 + 0.004, z: D / 2 + 0.012, sx: 0.01, sy: 0.01, sz: 0.025, color: Obj.color(P.tirador, "#C9A45C") }, g),
    Obj.crear("box", { x: 0.065, y: hFrente / 2 + 0.004, z: D / 2 + 0.012, sx: 0.01, sy: 0.01, sz: 0.025, color: Obj.color(P.tirador, "#C9A45C") }, g),
  ];
  tiradores.push(...tir);
  // El contenido.
  let x = -W / 2 + T + 0.08;
  for (const [c, sx, sy, sz] of CONTENIDO[i % CONTENIDO.length]) {
    Obj.crear("box", { x: x + sx / 2, y: 0.015 + sy / 2, z: D / 2 - T - dz / 2 + (x > 0 ? 0.04 : -0.05), ry: (x * 7) % 0.5, sx, sy, sz, color: c }, g);
    x += sx + 0.06;
  }
  // El blanco del toque: el frente entero, un pelo por delante.
  const blanco = Obj.crear("plane", { y: hFrente / 2 + 0.004, z: D / 2 + 0.035, sx: W - 2 * T, sy: hFrente, color: "transparent", "material-alpha": "blend" }, g);
  const c = { g, y0, frente, abierto: false, k: 0, meta: 0, mano: null };
  cajones.push(c);
  Obj.boton(blanco, () => poner(i, !c.abierto));
  blanco.setAttribute("manos", "false");
  // En VR: el tirador en la mano y el cajón corre por su riel (la corredera
  // de las manos, _vr.js). Al soltarlo sigue hasta abierto o cerrado según
  // dónde quedó y cómo venía.
  const yT = y0 + hFrente / 2 + 0.004, zT = D / 2 + 0.025;
  Obj.manos().corredera({
    desde: { x: 0, y: yT, z: zT }, hasta: { x: 0, y: yT, z: zT + SALIDA }, alcance: 0.09,
    t: () => (c.mano !== null ? c.mano : Obj.suave(c.k)),
    alMover(t) { c.mano = t; c.g.position = { x: 0, y: c.y0, z: SALIDA * t }; },
    alSoltar(t, v) {
      c.mano = null;
      c.k = Obj.suaveInversa(t);
      const abierto = v > 1.2 ? true : v < -1.2 ? false : t > 0.5;
      if (abierto !== c.abierto) poner(i, abierto);
      else { c.meta = abierto ? 1 : 0; anim.iniciar(); }
    },
  });
}
...
const anim = Obj.animador((t, dt) => {
  ...
  for (const c of cajones) {
    if (c.mano !== null || c.k === c.meta) continue;
    const paso = dt / 0.35;
    c.k = c.meta > c.k ? Math.min(c.meta, c.k + paso) : Math.max(c.meta, c.k - paso);
    c.g.position = { x: 0, y: c.y0, z: SALIDA * Obj.suave(c.k) };
    if (c.k !== c.meta) seguir = true;
  }
  return seguir || cajones.some((c) => c.k !== c.meta);
});

function poner(i, abierto, avisar) {
  const c = cajones[i];
  if (!c || c.abierto === abierto) return;
  c.abierto = abierto;
  c.meta = abierto ? 1 : 0;
  memo.guardar(cajones.map((x) => x.abierto));
  anim.iniciar();
  if (avisar !== false) Obj.emitir("cambio", { cajon: i, abierto });
}

// Lo guardado se aplica sin animación: la cajonera aparece como quedó.
const guardado = memo.leer([]);
cajones.forEach((c, i) => {
  if (guardado[i]) {
    c.abierto = true; c.k = c.meta = 1;
    c.g.position = { x: 0, y: c.y0, z: SALIDA };
  }
});

Obj.mensaje("abrir", (d) => poner(d && d.cajon, true));
Obj.mensaje("cerrar", (d) => poner(d && d.cajon, false));
Obj.mensaje("cerrar_todo", () => cajones.forEach((_, i) => poner(i, false)));
Obj.alCambiar(() => {
  for (const [el, k] of pintables) el.setAttribute("color", Obj.tono(madera(), k));
  for (const el of tiradores) el.setAttribute("color", Obj.color(P.tirador, "#C9A45C"));
});
