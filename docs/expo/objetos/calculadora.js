...
;
// La calculadora: la lógica de una de escritorio, sin sorpresas.
//
// Ejecución inmediata, como las de bolsillo: 2 + 3 × 4 = da 20, no 14. El
// visor tiene 12 lugares; lo que no entra pasa a notación científica, y una
// división por cero muestra "Error" hasta la próxima C.
const P = Obj.props({ color: "#2B2D42", sonido: true });

const sinte = Obj.sinte({ volumen: 0.5 });
const visor = Obj.$("visor");
const grupo = Obj.$("teclas");
...
const COLOR_TECLA = (t) => (t === "=" ? "#FF9F0A" : "÷×−+".includes(t) ? "#4A5A7A" : t === "C" ? "#C8102E" : "#E8E8E8");

// Estado de una calculadora de ejecución inmediata.
let pantalla = "0";   // lo que se está escribiendo
let acumulado = null; // el número a la izquierda de la operación
let operacion = null; // "+", "−", "×", "÷"
let nuevo = true;     // el próximo dígito empieza un número nuevo
let error = false;

function formato(v) {
  if (!Number.isFinite(v)) return "Error";
  // Doce lugares: si no entra redondeado, notación científica.
  let s = String(Math.round(v * 1e10) / 1e10);
  if (s.length > 12) s = v.toPrecision(7).replace(/\.?0+e/, "e");
  return s.length > 12 ? v.toExponential(5) : s;
}
...
function mostrar() {
  visor.setAttribute("value", pantalla);
  // El visor alinea a la derecha: el texto se centra, así que se corre.
  const w = typeof TextLayout !== "undefined" ? (() => { try { return TextLayout.create(pantalla, 0.03).w; } catch (e) { return pantalla.length * 0.017; } })() : pantalla.length * 0.017;
  visor.position = { x: 0.075 - w / 2, y: 0.016, z: -0.085 };
  Obj.$("memoria").setAttribute("value", operacion && acumulado != null ? formato(acumulado) + " " + operacion : "");
}

function tecla(t) {
  if (error && t !== "C") return;
  if (/^[0-9]$/.test(t)) {
    if (nuevo) { pantalla = t; nuevo = false; }
    else if (pantalla.replace(/[-.]/g, "").length < 12) pantalla = pantalla === "0" ? t : pantalla + t;
  } else if (t === ".") {
    if (nuevo) { pantalla = "0."; nuevo = false; }
    else if (!pantalla.includes(".")) pantalla += ".";
  } else if (t === "DEL") {
    if (!nuevo) pantalla = pantalla.length > 1 && !(pantalla.length === 2 && pantalla[0] === "-") ? pantalla.slice(0, -1) : "0";
  } else if (t === "C") {
    pantalla = "0"; acumulado = null; operacion = null; nuevo = true; error = false;
  } else if (t === "±") {
    if (pantalla !== "0") pantalla = pantalla.startsWith("-") ? pantalla.slice(1) : "-" + pantalla;
  } else if (t === "%") {
    const v = Number(pantalla);
    pantalla = formato(acumulado != null && (operacion === "+" || operacion === "−") ? acumulado * v / 100 : v / 100);
    nuevo = true;
  } else if ("÷×−+".includes(t)) {
    const v = Number(pantalla);
    if (acumulado != null && operacion && !nuevo) {
      acumulado = operar(acumulado, operacion, v);
      pantalla = formato(acumulado);
      if (pantalla === "Error") error = true;
    } else acumulado = v;
    operacion = t;
    nuevo = true;
  } else if (t === "=") {
    if (operacion && acumulado != null) {
      const r = operar(acumulado, operacion, Number(pantalla));
      pantalla = formato(r);
      if (pantalla === "Error") error = true;
      else Obj.emitir("resultado", { valor: r });
      acumulado = null; operacion = null; nuevo = true;
    }
  }
  mostrar();
}

// Las teclas: un botón con su texto encima, en la cara de arriba del cuerpo.
const botones = [];
FILAS.forEach((fila, f) => {
  fila.forEach((t, c) => {
    const x = -0.066 + c * 0.044, z = -0.03 + f * 0.036;
    const col = COLOR_TECLA(t);
    const el = Obj.crear("box", { x, z, sx: 0.036, sy: 0.008, sz: 0.028, color: col, "border-radius": 0.004 }, grupo);
    Obj.crear("text", { x, y: 0.0045, z, rx: -Math.PI / 2, value: t, size: 0.016, color: col === "#E8E8E8" ? "#1C1C1C" : "#FFFFFF" }, grupo);
    Obj.boton(el, () => {
      if (P.sonido !== false) sinte.nota(2600, 0.015, { tipo: "pulso", vol: 0.15 });
      el.position = { x, y: -0.002, z };
      setTimeout(() => { el.position = { x, y: 0, z }; }, 90);
      tecla(t);
    }, { color: col });
    botones.push(el);
  });
...
// Desde afuera: "12+30=" aprieta esas teclas. Acepta * / - como sinónimos.
Obj.mensaje("teclas", (d) => {
  const s = String((d && d.teclas) || "").slice(0, 64);
  for (const ch of s) {
    const t = { "*": "×", "/": "÷", "-": "−", "c": "C", "C": "C" }[ch] || ch;
    if (FILAS.some((f) => f.includes(t))) tecla(t);
  }
});
Obj.alCambiar(() => Obj.$("carcasa").setAttribute("color", Obj.color(P.color, "#2B2D42")));
Obj.$("carcasa").setAttribute("color", Obj.color(P.color, "#2B2D42"));
mostrar();
