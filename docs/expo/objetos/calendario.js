...
;
// El calendario: el mes en una grilla, hoy en rojo, y días que se marcan.
//
// La grilla son 42 casilleros fijos —seis semanas de siete días, que es lo
// más que puede ocupar un mes— creados una sola vez; cambiar de mes sólo les
// cambia el número y el color. Cada casillero tiene su blanco táctil.
const P = Obj.props({ color: "#1D3557", utc: null, clave: "" });

const memo = Obj.memoria("marcas");
let marcas = new Set(memo.leer([]));
const MESES = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
const grilla = Obj.$("grilla");
const mesTxt = Obj.$("mes");

for (let i = 0; i < 12; i++) {
  Obj.crear("cylinder", { x: -0.2 + i * (0.4 / 11), rx: Math.PI / 2, sx: 0.012, sy: 0.02, sz: 0.012, color: "#6A6A6A" }, Obj.$("espiral"));
}

const CX = 0.056, CY = 0.056, X0 = -3 * CX, Y0 = 0.105;
["L", "M", "M", "J", "V", "S", "D"].forEach((d, i) => {
  Obj.crear("text", { x: X0 + i * CX, y: Y0 + 0.045, value: d, size: 0.022, color: i >= 5 ? "#C8102E" : "#6A6258" }, grilla);
});

const celdas = [];
for (let i = 0; i < 42; i++) {
  const col = i % 7, fila = Math.floor(i / 7);
  const x = X0 + col * CX, y = Y0 - fila * CY;
  const marca = Obj.crear("plane", { x, y, z: 0.001, sx: 0.046, sy: 0.046, "border-radius": 0.5, color: "#FFD60A", visible: "false" }, grilla);
  const num = Obj.crear("text", { x, y, z: 0.003, value: "", size: 0.024, color: "#1C1C1C" }, grilla);
  const toque = Obj.crear("plane", { x, y, z: 0.006, sx: 0.052, sy: 0.052, color: "transparent", "material-alpha": "blend" }, grilla);
  const c = { marca, num, toque, fecha: null };
  Obj.boton(toque, () => { if (c.fecha) alternar(c.fecha); });
  celdas.push(c);
}
...
function hoy() {
  ...
  if (utc == null) return { a: d.getFullYear(), m: d.getMonth(), d: d.getDate() };
  const u = new Date(d.getTime() + utc * 3600000);
  return { a: u.getUTCFullYear(), m: u.getUTCMonth(), d: u.getUTCDate() };
}
let vista = (() => { const h = hoy(); return { a: h.a, m: h.m }; })();
const clave = (a, m, d) => a + "-" + String(m + 1).padStart(2, "0") + "-" + String(d).padStart(2, "0");

function dibujar() {
  const h = hoy();
  mesTxt.setAttribute("value", MESES[vista.m] + " " + vista.a);
  // Qué día de la semana cae el 1, con la semana empezando en lunes.
  const primero = (new Date(Date.UTC(vista.a, vista.m, 1)).getUTCDay() + 6) % 7;
  const dias = new Date(Date.UTC(vista.a, vista.m + 1, 0)).getUTCDate();
  celdas.forEach((c, i) => {
    const d = i - primero + 1;
    if (d < 1 || d > dias) {
      c.fecha = null;
      c.num.setAttribute("value", "");
      c.marca.setAttribute("visible", "false");
      return;
    }
    c.fecha = clave(vista.a, vista.m, d);
    const esHoy = h.a === vista.a && h.m === vista.m && h.d === d;
    const finde = i % 7 >= 5;
    c.num.setAttribute("value", String(d));
    c.num.setAttribute("color", esHoy ? "#FFFFFF" : finde ? "#C8102E" : "#1C1C1C");
    const marcada = marcas.has(c.fecha);
    c.marca.setAttribute("visible", esHoy || marcada ? "inherit" : "false");
    c.marca.setAttribute("color", esHoy ? "#C8102E" : "#FFD60A");
  });
}

function alternar(fecha) {
  const marcado = !marcas.has(fecha);
  if (marcado) marcas.add(fecha); else marcas.delete(fecha);
  memo.guardar([...marcas].slice(-400));
  Obj.emitir("marca", { fecha, marcado });
  dibujar();
}
function mover(k) {
  vista.m += k;
  if (vista.m < 0) { vista.m = 11; vista.a--; }
  if (vista.m > 11) { vista.m = 0; vista.a++; }
  dibujar();
}
Obj.boton(Obj.$("anterior"), () => mover(-1), { color: "#1D3557" });
Obj.boton(Obj.$("siguiente"), () => mover(1), { color: "#1D3557" });
Obj.mensaje("hoy", () => { const h = hoy(); vista = { a: h.a, m: h.m }; dibujar(); });

function pintar() {
  const c = Obj.color(P.color, "#1D3557");
  for (const id of ["cabecera", "anterior", "siguiente"]) Obj.$(id).setAttribute("color", c);
}
Obj.alCambiar(() => { pintar(); dibujar(); });
pintar();
dibujar();
// A medianoche cambia el día: se revisa cada minuto.
