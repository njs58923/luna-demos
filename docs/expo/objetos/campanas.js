...
;
// Las campanas de viento: siete tubos en pentatónica que se mecen.
//
// Cada tubo es un péndulo con su propio período (los largos, más lentos) y
// una amplitud que decae; el viento le da empujones chicos al azar. Un tubo
// suena cuando su amplitud supera un umbral después de un empujón: así la
// cascada al tocarlas no es simultánea, cada tubo tarda lo que tarda en ir.
const P = Obj.props({ metal: "#B8C4CE", viento: 0.3 });
...
for (let i = 0; i < N; i++) {
  ...
  const largo = 0.5 - i * 0.045;
  const g = Obj.crear("group", { x: Math.sin(a) * 0.09, y: -0.01, z: Math.cos(a) * 0.09 }, aro);
  Obj.crear("cylinder", { y: -0.03, sx: 0.002, sy: 0.06, sz: 0.002, color: "#8A8A8A" }, g);
  const tubo = Obj.crear("cylinder", { y: -0.06 - largo / 2, sx: 0.022, sy: largo, sz: 0.022, color: "#B8C4CE" }, g);
  tubos.push({
    g, tubo, largo, a,
    // Una campana de verdad: más largo, más grave.
    f: Obj.frecuencia(3 + PENTA[N - 1 - i]),
    amp: 0, fase: Math.random() * 6, omega: Math.sqrt(9.8 / (largo * 0.7)), ultimo: 0,
  });
}
// El badajo del medio, que cuelga más bajo.
const badajo = Obj.crear("group", { y: -0.01 }, aro);
Obj.crear("cylinder", { y: -0.2, sx: 0.002, sy: 0.4, sz: 0.002, color: "#8A8A8A" }, badajo);
Obj.crear("cylinder", { y: -0.3, sx: 0.07, sy: 0.012, sz: 0.07, color: "#6B4A30" }, badajo);
Obj.crear("box", { y: -0.47, sx: 0.08, sy: 0.12, sz: 0.004, color: "#6B4A30" }, badajo);

function empujar(i, fuerza) {
  const t = tubos[i];
  t.amp = Math.min(0.5, t.amp + fuerza);
  anim.iniciar();
}
function sonar(i, fuerza) {
  const t = tubos[i];
  const ahora = performance.now();
  if (ahora - t.ultimo < 120) return;
  t.ultimo = ahora;
  sinte.nota(t.f * 2, 3.5, { tipo: "campana", vol: 0.12 + 0.2 * Math.min(1, fuerza), caida: 1.1 });
  Obj.emitir("suena", { tubo: i });
}

// Ráfagas: de vez en cuando un tubo al azar recibe un empujoncito. Van por
// temporizador y no por cuadro: entre ráfaga y ráfaga, con los tubos quietos,
// el objeto no pide cuadros.
function rafaga() {
  const viento = Math.max(0, Math.min(1, Number(P.viento) || 0));
  if (viento > 0) empujar(Math.floor(Math.random() * N), 0.04 + viento * 0.06);
  setTimeout(rafaga, (2.5 - viento * 2) * 1000 * (0.7 + Math.random() * 0.6));
}
setTimeout(rafaga, 1500);

const anim = Obj.animador((t, dt) => {
  let seguir = false;
  tubos.forEach((tb, i) => {
    if (tb.amp < 0.002) { tb.amp = 0; return; }
    seguir = true;
    tb.fase += tb.omega * dt;
    tb.amp *= Math.exp(-0.5 * dt);
    const ang = tb.amp * Math.sin(tb.fase);
    tb.g.rotation = { x: ang * Math.cos(tb.a), y: 0, z: -ang * Math.sin(tb.a) };
    // Suena al pasar por el centro con fuerza, donde choca con el badajo.
    const pasa = Math.sin(tb.fase) * Math.sin(tb.fase - tb.omega * dt) <= 0;
    if (pasa && tb.amp > 0.06) sonar(i, tb.amp * 2);
  });
  badajo.rotation = { x: 0.05 * Math.sin(t * 1.3) * (tubos.reduce((s, x) => s + x.amp, 0) > 0.05 ? 1 : 0), y: 0, z: 0 };
  return seguir;
});

function cascada() {
  tubos.forEach((_, i) => setTimeout(() => empujar(i, 0.18 + Math.random() * 0.08), i * 90));
}
Obj.boton(Obj.$("toque"), cascada);
Obj.mensaje("sonar", cascada);

// En VR: la mano que pasa entre los tubos los empuja.
const zona = Obj.$("zona");
const manos = { left: null, right: null };
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    manos[mano] = l;
    if (!antes) return;
    const v = Math.hypot(l.x - antes.x, l.z - antes.z);
    if (v < 0.01) return;
    tubos.forEach((tb, i) => {
      const cx = Math.sin(tb.a) * 0.09, cz = 0.18 + Math.cos(tb.a) * 0.09;
      const abajo = -0.09 - 0.06 - tb.largo;
      if (l.y < -0.1 && l.y > abajo && Math.hypot(l.x - cx, l.z - cz) < 0.05) empujar(i, Math.min(0.25, v * 4));
    });
  });
}

function pintar() { for (const tb of tubos) tb.tubo.setAttribute("color", Obj.color(P.metal, "#B8C4CE")); }
Obj.alCambiar(() => { pintar(); anim.iniciar(); });
pintar();
anim.iniciar();
