...
;
// El canario: un pájaro con tres lugares (el palito, el comedero y el
// bebedero) y un canto hecho de sílabas.
//
// El toque de la jaula llega en la superficie de afuera (el cilindro que la
// envuelve), así que para saber si tocaron al pájaro se mira si el punto,
// visto de frente, cae sobre él. El canto es un flujo del sintetizador: una
// lista de sílabas (trinos que suben, silbidos que bajan, una nota larga con
// vibrato) armada al azar y leída muestra por muestra.
const P = Obj.props({ color: "#FFD60A", jaula: "#C9A227", canta: true });

const sinte = Obj.sinte({ volumen: 0.7 });
const donde = Obj.marco();
const pajaro = Obj.$("pajaro"), cabeza = Obj.$("cabeza_p");
const alas = [Obj.$("ala_i"), Obj.$("ala_d")];
const RATE = 22050;

// ── La jaula ───────────────────────────────────────────────────────────────
const R = 0.135, ALTO = 0.34, CUMBRE = 0.46;
const barrotes = [];
for (let i = 0; i < 16; i++) {
  const a = (i / 16) * Math.PI * 2;
  const abajo = { x: Math.cos(a) * R, y: 0.03, z: Math.sin(a) * R };
  const arriba = { x: abajo.x, y: ALTO, z: abajo.z };
  const cima = { x: Math.cos(a) * 0.015, y: CUMBRE, z: Math.sin(a) * 0.015 };
  for (const [p, q] of [[abajo, arriba], [arriba, cima]]) {
    const b = Obj.crear("cylinder", { sx: 0.004, sy: 0.1, sz: 0.004, color: "#C9A227" }, Obj.$("barrotes"));
    const l = Obj.tender(b, p, q);
    b.scale = { x: 0.004, y: l, z: 0.004 };
    barrotes.push(b);
  }
}
for (const y of [0.12, ALTO]) barrotes.push(Obj.crear("cylinder", { y, sx: R * 2 + 0.006, sy: 0.006, sz: R * 2 + 0.006, color: "#C9A227" }, Obj.$("barrotes")));
barrotes.push(Obj.crear("cylinder", { y: CUMBRE + 0.02, rx: Math.PI / 2, sx: 0.04, sy: 0.005, sz: 0.04, color: "#C9A227" }, Obj.$("barrotes")));

// ── Dónde puede estar ──────────────────────────────────────────────────────
const PALITO = 0.155;
const LUGARES = {
  comedero: { x: -0.06, y: 0.032, z: 0.025 },
  bebedero: { x: 0.06, y: 0.032, z: 0.025 },
};
let pos = { x: 0, y: PALITO, z: 0 }, giro = 0;
let salto = null;            // { desde, hasta, t0, dura, giro0, giro1 }
let cantandoHasta = 0, aleteoHasta = 0;
let proximo = performance.now() + 2000, proximoCanto = performance.now() + 6000;
let mirada = 0, proximaMirada = 0;

function saltarA(dest, giroFinal, dura) {
  salto = { desde: { ...pos }, hasta: dest, t0: performance.now(), dura: dura || 250, giro0: giro, giro1: giroFinal };
  if (Math.abs(dest.y - pos.y) > 0.03) aleteoHasta = performance.now() + (dura || 250);
}
function puntoDelPalito(x) { return { x: Math.max(-0.085, Math.min(0.085, x)), y: PALITO, z: 0 }; }

function decidir() {
  const r = Math.random();
  if (pos.y < 0.1) { saltarA(puntoDelPalito((Math.random() - 0.5) * 0.12), 0, 450); return; }
  if (r < 0.2) { const l = Math.random() < 0.5 ? LUGARES.comedero : LUGARES.bebedero; saltarA(l, 0, 450); return; }
  if (r < 0.7) {
    const x = pos.x + (Math.random() < 0.5 ? -1 : 1) * (0.03 + Math.random() * 0.04);
    const d = puntoDelPalito(x);
    saltarA(d, d.x > pos.x ? Math.PI / 2 : -Math.PI / 2, 200);
    // Después de saltar de costado vuelve a mirar al frente.
    setTimeout(() => { if (!salto) giro = 0; }, 900);
  }
}
...
function paso() {
  ...
  const t = ahora / 1000;
  if (salto) {
    const k = Math.min(1, (ahora - salto.t0) / salto.dura);
    const h = Math.sin(Math.PI * k) * (0.02 + Math.abs(salto.hasta.y - salto.desde.y) * 0.4);
    pos = {
      x: salto.desde.x + (salto.hasta.x - salto.desde.x) * k,
      y: salto.desde.y + (salto.hasta.y - salto.desde.y) * k + h,
      z: salto.desde.z + (salto.hasta.z - salto.desde.z) * k,
    };
    giro = salto.giro0 + (salto.giro1 - salto.giro0) * Math.min(1, k * 2);
    if (k >= 1) { pos = { ...salto.hasta }; salto = null; }
  } else if (ahora > proximo) {
    proximo = ahora + 1200 + Math.random() * 2500;
    if (!(ahora < cantandoHasta) && !manoQuieta) decidir();
  }
  pajaro.position = pos;
  pajaro.rotation = { x: 0, y: giro, z: 0 };

  // La cabeza: mira a los costados a saltitos, picotea o se infla al cantar.
  if (ahora > proximaMirada) { proximaMirada = ahora + 400 + Math.random() * 1200; mirada = (Math.random() - 0.5) * 1.6; }
  const enComedero = !salto && pos.y < 0.1;
  const pica = enComedero && Math.sin(t * 14) > 0.3;
  if (ahora < cantandoHasta) cabeza.rotation = { x: -0.35 + 0.08 * Math.sin(t * 30), y: mirada * 0.3, z: 0 };
  else cabeza.rotation = { x: pica ? 0.9 : 0, y: pica ? 0 : mirada, z: 0 };
  cabeza.position = { x: 0, y: pica ? 0.05 : 0.07, z: pica ? 0.03 : 0.012 };

  // Alas: aletea al volar, al asustarse o un poquito al cantar.
  const aletea = ahora < aleteoHasta;
  const a = aletea ? 0.9 * (0.5 + 0.5 * Math.sin(t * 60)) : ahora < cantandoHasta ? 0.12 * (0.5 + 0.5 * Math.sin(t * 20)) : 0;
  alas[0].rotation = { x: 0, y: 0, z: a };
  alas[1].rotation = { x: 0, y: 0, z: -a };

  if (P.canta && ahora > proximoCanto && !salto) { cantar(false); }
}
setInterval(paso, 50);

// ── El canto ───────────────────────────────────────────────────────────────
/** Una sílaba: dura (s), frecuencia al empezar y al terminar, vibrato. */
function frase(largo) {
  const s = [];
  const n = largo ? 4 : 2;
  for (let i = 0; i < n; i++) {
    const tipo = Math.floor(Math.random() * 3);
    if (tipo === 0) {   // trino: sílabas cortas que suben, repetidas
      const f0 = 2400 + Math.random() * 800, f1 = f0 + 1200 + Math.random() * 800, rep = 5 + Math.floor(Math.random() * 7);
      for (let k = 0; k < rep; k++) s.push({ d: 0.035, f0, f1, v: 0 }, { d: 0.02, silencio: true });
    } else if (tipo === 1) {   // silbidos que bajan
      for (let k = 0; k < 3; k++) s.push({ d: 0.07, f0: 5200 - k * 200, f1: 3000, v: 0 }, { d: 0.04, silencio: true });
    } else {   // una nota larga con vibrato
      s.push({ d: 0.25 + Math.random() * 0.2, f0: 2800 + Math.random() * 600, f1: 2900 + Math.random() * 600, v: 90 }, { d: 0.06, silencio: true });
    }
  }
  // Tiempos acumulados, para buscar rápido en qué sílaba cae t.
  let t = 0;
  for (const x of s) { x.t = t; t += x.d; }
  return { silabas: s, dura: t };
}
function generador(f) {
  let fase = 0, i = 0;
  return (t) => {
    if (t >= f.dura) return 0;
    while (i < f.silabas.length - 1 && t >= f.silabas[i].t + f.silabas[i].d) i++;
    const x = f.silabas[i];
    if (x.silencio) return 0;
    const k = (t - x.t) / x.d;
    const frec = x.f0 + (x.f1 - x.f0) * k + (x.v ? 120 * Math.sin(2 * Math.PI * x.v * t) : 0);
    fase += (2 * Math.PI * frec) / RATE;
    const env = Math.min(1, k * 8) * Math.min(1, (1 - k) * 6);
    return Math.sin(fase) * env * 0.22;
  };
}
function cantar(largo) {
  const f = frase(largo);
  sinte.flujo(generador(f));
  cantandoHasta = performance.now() + f.dura * 1000;
  proximoCanto = cantandoHasta + 8000 + Math.random() * 12000;
  setTimeout(() => { if (performance.now() >= cantandoHasta - 5) sinte.flujo(null); }, f.dura * 1000 + 50);
  Obj.emitir("canto", { largo: !!largo });
}

function asustar(desdeX) {
  aleteoHasta = performance.now() + 700;
  proximo = performance.now() + 2500;   // asustado, se queda quieto un rato
  const lejos = puntoDelPalito(desdeX > pos.x ? -0.08 : 0.08);
  saltarA(lejos, 0, 300);
  sinte.nota(4200, 0.05, { tipo: "seno", vol: 0.2, caida: 30 });
  sinte.nota(3600, 0.05, { tipo: "seno", vol: 0.2, caida: 30, retraso: 0.07 });
  Obj.emitir("susto", {});
}

// Tocar: si el punto cae sobre el pájaro (visto de frente), canta; si no, se asusta.
Obj.boton(Obj.$("toque"), (e) => {
  let sobre = false, x = 0;
  if (Number.isFinite(e.x)) {
    const l = donde.aLocal({ x: e.x, y: e.y, z: e.z });
    x = l.x;
    sobre = Math.abs(l.x - pos.x) < 0.05 && Math.abs(l.y - (pos.y + 0.04)) < 0.07;
  }
  if (sobre) { aleteoHasta = performance.now() + 150; cantar(true); }
  else asustar(x);
});

// En VR: una mano que se acerca rápido lo asusta; quieta un rato, lo atrae.
let manoQuieta = false, quietaDesde = 0, manoAntes = null, ultimoSusto = 0;
const zona = Obj.$("zona");
if (zona) {
  zona.addEventListener("posemove", (e) => {
    const l = donde.aLocal({ x: e.px, y: e.py, z: e.pz });
    const ahora = performance.now();
    const cerca = Math.hypot(l.x, l.z) < 0.3 && l.y > 0 && l.y < 0.55;
    const antes = manoAntes && manoAntes.mano === e.hand ? manoAntes : null;
    manoAntes = { mano: e.hand, p: l, t: ahora };
    if (!cerca || !antes) { if (!cerca && antes) { manoQuieta = false; quietaDesde = 0; } return; }
    const v = Math.hypot(l.x - antes.p.x, l.y - antes.p.y, l.z - antes.p.z) / Math.max(0.001, (ahora - antes.t) / 1000);
    if (v > 0.8) {
      quietaDesde = 0; manoQuieta = false;
      if (ahora - ultimoSusto > 1500) { ultimoSusto = ahora; asustar(l.x); }
    } else if (v < 0.08) {
      if (!quietaDesde) quietaDesde = ahora;
      if (!manoQuieta && ahora - quietaDesde > 2000) {
        manoQuieta = true;
        const d = puntoDelPalito(l.x);
        saltarA(d, 0, 300);
        setTimeout(() => cantar(true), 400);
      }
    }
  });
}

Obj.mensaje("cantar", () => cantar(true));
function pintar() {
  const c = Obj.color(P.color, "#FFD60A");
  for (const el of Obj.root.getElementsByClass("pluma")) el.setAttribute("color", c);
  for (const el of Obj.root.getElementsByClass("pluma2")) el.setAttribute("color", Obj.tono(c, -0.1));
  const j = Obj.color(P.jaula, "#C9A227");
  for (const b of barrotes) b.setAttribute("color", j);
  Obj.$("base").setAttribute("color", j);
}
