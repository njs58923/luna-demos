...
;
// El cantero: borde de ladrillo, tierra y flores al azar de la semilla.
// Tocarlo lo vuelve a plantar.
const P = Obj.props({ ancho: 1.2, largo: 0.6, flores: "#E63946,#FFD60A,#F4A7C0,#9B5DE5", borde: "", semilla: 11 });
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let semilla = Number(P.semilla) || 11;

function dibujar() {
  const W = Obra.num(P.ancho, 1.2, 0.3, 20), D = Obra.num(P.largo, 0.6, 0.3, 20);
  armar(Obra.cantero({ ancho: W, largo: D, flores: P.flores, borde: P.borde, semilla }));
  toque.position = { x: 0, y: 0.3, z: 0 };
  toque.scale = { x: W, y: 0.6, z: D };
}

Obj.boton(toque, () => {
  semilla += 1;
  dibujar();
  Obj.emitir("cambio", { semilla });
});
Obj.alCambiar((p, antes, cambios) => {
  if (cambios.includes("semilla")) semilla = Number(p.semilla) || 11;
  dibujar();
});
dibujar();
