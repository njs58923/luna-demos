...
//
// El tamaño de la letra se ajusta solo: se mide el renglón más largo y se
// achica hasta que entra en el tablero. Tres estilos: madera tallada (texto
// claro sobre madera), pizarra (tiza sobre verde oscuro) y neón (tubos de
// color sin iluminación que titilan un poco).
const P = Obj.props({ texto: "ABIERTO", estilo: "madera", color: "#F4E3C1", editable: true, clave: "" });

const SALTO = String.fromCharCode(10);
const ANCHO = 1.1, ALTO = 0.62, RENGLONES = 4;
const memo = Obj.memoria("texto");
const grupo = Obj.$("renglones");
const ayuda = Obj.$("ayuda");
const lineas = [0, 1, 2, 3].map(() => Obj.crear("text", { value: "", size: 0.12, color: "#F4E3C1" }, grupo));

const ESTILOS = {
  madera: { marco: "#6B4A30", fondo: "#8B5E3C", poste: "#6B4A30", unlit: false },
  pizarra: { marco: "#7A5230", fondo: "#23372E", poste: "#5A3E2A", unlit: false, texto: "#F2F2EE" },
  neon: { marco: "#1C1C26", fondo: "#0B0B12", poste: "#2A2A38", unlit: true, texto: "#FF4FD8" },
};
...
let texto = String(memo.leer(null) ?? P.texto ?? "").split(SALTO).slice(0, RENGLONES).join(SALTO);

function dibujar(cursor) {
  const e = ESTILOS[P.estilo] || ESTILOS.madera;
  Obj.$("marco").setAttribute("color", e.marco);
  Obj.$("fondo").setAttribute("color", e.fondo);
  for (const p of Obj.root.getElementsByClass("poste")) p.setAttribute("color", e.poste);
  const color = e.texto || Obj.color(P.color, "#F4E3C1");
  const rs = texto.split(SALTO).slice(0, RENGLONES);
  if (cursor) rs[rs.length - 1] += "|";
  // Un solo tamaño para todos los renglones: el que hace entrar al más largo.
  let size = Math.min(0.16, ALTO / Math.max(1, rs.length) * 0.7);
  for (const r of rs) {
    const w = mide(r, size);
    if (w > ANCHO) size *= ANCHO / w;
  }
  const paso = size * 1.3;
  lineas.forEach((l, i) => {
    const r = rs[i] || "";
    l.setAttribute("value", r);
    l.setAttribute("size", String(Math.max(0.03, size)));
    l.setAttribute("color", color);
    l.setAttribute("material-unlit", e.unlit ? "true" : "false");
    l.position = { x: 0, y: ((rs.length - 1) / 2 - i) * paso, z: 0 };
  });
}

// El neón titila apenas, de vez en cuando: un tubo que parpadea.
setInterval(() => {
  if (P.estilo !== "neon" || editor.enfocado) return;
  const e = ESTILOS.neon;
  const i = Math.floor(Math.random() * RENGLONES);
  lineas[i].setAttribute("color", Obj.tono(e.texto, -0.5));
  setTimeout(() => lineas[i].setAttribute("color", e.texto), 90);
}, 2600);

let parpadeo = null, cursor = false;
const editor = Obj.teclado(Obj.$("fondo"), {
  texto, max: 160,
  cambio: (t) => {
    // Más de cuatro renglones no: el Enter de más no hace nada.
    const rs = t.split(SALTO);
    if (rs.length > RENGLONES) { editor.texto = rs.slice(0, RENGLONES).join(SALTO); }
    texto = editor.texto;
    dibujar(true);
  },
  foco: (si) => {
    ...
    if (si) {
      ayuda.setAttribute("value", "escribiendo · Enter: renglón · Esc: listo");
      parpadeo = setInterval(() => { cursor = !cursor; dibujar(cursor); }, 530);
      dibujar(true);
    } else {
      ayuda.setAttribute("value", "");
      memo.guardar(texto);
      Obj.emitir("cambio", { texto });
      dibujar(false);
    }
  },
});
// Escape termina; Enter pasa de renglón. El teclado base ya hace eso con
// `unaLinea: false`.
Obj.boton(Obj.$("toque"), () => { if (P.editable !== false) editor.tomar(); });
Obj.mensaje("texto", (d) => {
  if (!d || typeof d.texto !== "string") return;
  texto = d.texto.split(SALTO).slice(0, RENGLONES).join(SALTO).slice(0, 160);
  editor.texto = texto;
  memo.guardar(texto);
  dibujar(false);
});
Obj.alCambiar((p, a, cambios) => {
  if (cambios.includes("texto") && memo.leer(null) == null) { texto = String(p.texto || ""); editor.texto = texto; }
  dibujar(false);
});
dibujar(false);
