...
;
// El cerco: estacas, tablas, reja o alambrado. Tocarlo pasa al tipo siguiente.
const P = Obj.props({ largo: 2, alto: 1, tipo: "estacas", color: "", puntas: "" });
const ORDEN = ["estacas", "tablas", "reja", "alambrado"];
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let tipo = ORDEN.includes(P.tipo) ? P.tipo : "estacas";

function dibujar() {
  const L = Obra.num(P.largo, 2, 0.3, 60), H = Obra.num(P.alto, 1, 0.3, 3);
  const propio = tipo === P.tipo;
  armar(Obra.cerco({ largo: L, alto: H, tipo, color: propio ? P.color : "", puntas: P.puntas }));
  toque.position = { x: 0, y: (H + 0.15) / 2, z: 0 };
  toque.scale = { x: L + 0.1, y: H + 0.15, z: 0.2 };
}
