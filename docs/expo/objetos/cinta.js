...
;
// La cinta métrica: una cinta recta de la boca de la caja a la chapita.
//
// La cinta es un grupo con su propio marco: +Y a lo largo, +X hacia "arriba"
// de la cara impresa (la vertical del mundo, corregida para que quede
// perpendicular) y +Z de costado. Adentro van la tira, las rayitas cada 5 cm
// y los números cada 10: todos creados una vez y mostrados hasta donde llega.
const P = Obj.props({ largo: 3, color: "#F2C200" });

const sinte = Obj.sinte({ volumen: 0.7 });
const donde = Obj.marco();
const cintaG = Obj.$("cinta"), gancho = Obj.$("gancho");
const BOCA = { x: 0.048, y: 0.012, z: 0 };
const maximo = () => Math.max(1, Math.min(5, Number(P.largo) || 3));

const tira = Obj.crear("box", { y: 0.5, sx: 0.001, sy: 1, sz: 0.025, color: "#F5D300" }, cintaG);
const marcas = [];
for (let k = 1; k <= 100; k++) {
  const y = k * 0.05;
  const metro = k % 20 === 0, diez = k % 2 === 0;
  const raya = Obj.crear("box", { x: 0.0006, y, z: 0.0125 - (diez ? 0.006 : 0.004), sx: 0.0004, sy: metro ? 0.003 : 0.0015, sz: diez ? 0.012 : 0.008, color: metro ? "#D62828" : "#111111", visible: "false" }, cintaG);
  let num = null;
  if (diez) {
    // El texto mira a +X (la cara impresa) y se lee a lo largo de +Y.
    num = Obj.crear("text", { x: 0.0008, y: y - 0.012, z: -0.004, rx: Math.PI / 2, ry: Math.PI / 2, value: metro ? k / 20 + "m" : String((k * 5) % 100), size: 0.008, color: metro ? "#D62828" : "#111111", visible: "false" }, cintaG);
  }
  marcas.push({ y, raya, num, visto: false });
}

let largo = 0;           // lo estirado, en m
let dir = { x: 1, y: 0, z: 0 };
let tomada = false;
...
function colocar() {
  ...
  const a = dir;
  let n = { x: -a.y * a.x, y: 1 - a.y * a.y, z: -a.y * a.z };   // la vertical, sin la parte de a
  let m = Math.hypot(n.x, n.y, n.z);
  if (m < 1e-3) { n = { x: 0, y: 0, z: -1 }; m = 1; }
  n = { x: n.x / m, y: n.y / m, z: n.z / m };
  const z = { x: n.y * a.z - n.z * a.y, y: n.z * a.x - n.x * a.z, z: n.x * a.y - n.y * a.x };
  cintaG.position = BOCA;
  cintaG.rotation = Obj.deEjes(n, a, z);
  const L = Math.max(0.0005, largo);
  tira.position = { x: 0, y: L / 2, z: 0 };
  tira.scale = { x: 0.001, y: L, z: 0.025 };
  tira.setAttribute("visible", largo > 0.002 ? "inherit" : "false");
  for (const mk of marcas) {
    const ver = mk.y <= largo;
    if (ver === mk.visto) continue;
    mk.visto = ver;
    mk.raya.setAttribute("visible", ver ? "inherit" : "false");
    if (mk.num) mk.num.setAttribute("visible", ver ? "inherit" : "false");
  }
  // La chapita, en la punta, mirando hacia afuera.
  gancho.position = punta();
  gancho.rotation = Obj.deEjes(a, n, { x: a.y * n.z - a.z * n.y, y: a.z * n.x - a.x * n.z, z: a.x * n.y - a.y * n.x });
  poner("visor", (largo * 100).toFixed(1) + " cm");
  poner("visor2", (largo / 0.0254).toFixed(2) + " in");
}
const puesto = {};
function poner(id, v) {
  if (puesto[id] === v) return;
  puesto[id] = v;
  Obj.$(id).setAttribute("value", v);
}
const punta = () => ({ x: BOCA.x + dir.x * largo, y: BOCA.y + dir.y * largo, z: BOCA.z + dir.z * largo });

function avisar() { Obj.emitir("medida", { metros: +largo.toFixed(4) }); }

// Recoger: la cinta vuelve a 2 m/s, con su zumbido.
let recogiendo = false;
const anim = Obj.animador((t, dt) => {
  if (!recogiendo) return false;
  largo = Math.max(0, largo - dt * 2);
  colocar();
  if (largo <= 0) {
    recogiendo = false;
    dir = { x: 1, y: 0, z: 0 };
    colocar();
    sinte.nota(900, 0.06, { tipo: "madera", vol: 0.4 });
    avisar();
    return false;
  }
  return true;
...
function recoger() {
  if (tomada || largo <= 0) return;
  recogiendo = true;
  sinte.nota(220, Math.min(2.5, largo / 2), { tipo: "ruido", vol: 0.05, caida: 0.5 });
  anim.iniciar();
}
function estirar(m) {
  recogiendo = false;
  largo = Math.max(0, Math.min(maximo(), m));
  colocar();
  sinte.nota(1400, 0.03, { tipo: "pulso", vol: 0.2 });
  avisar();
}

Obj.boton(Obj.$("boton"), recoger, { color: "#D62828" });
Obj.boton(Obj.$("agarrar"), () => { if (!tomada) estirar(largo + 0.2); });
Obj.mensaje("recoger", recoger);
Obj.mensaje("estirar", (d) => estirar(Number(d && d.metros) || 0));

// En VR: tomar la chapita y llevarla.
let ultimoCm = 0, clicT = 0;
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) {
    const g = punta();
    return Math.hypot(p.x - g.x, p.y - g.y, p.z - g.z) < 0.08;
  },
  tomar() { tomada = true; recogiendo = false; },
  mover(p) {
    const d = { x: p.x - BOCA.x, y: p.y - BOCA.y, z: p.z - BOCA.z };
    const l = Math.hypot(d.x, d.y, d.z);
    if (l > 0.01) dir = { x: d.x / l, y: d.y / l, z: d.z / l };
    largo = Math.min(maximo(), l);
    colocar();
    // Un clic cada centímetro que sale, como el fleje de verdad.
    const ahora = performance.now();
    const cm = Math.floor(largo * 100);
    if (cm !== ultimoCm && ahora - clicT > 25) {
      clicT = ahora;
      sinte.nota(2200, 0.01, { tipo: "pulso", vol: 0.05 });
    }
    ultimoCm = cm;
  },
  soltar() { tomada = false; avisar(); },
});

Obj.alCambiar(() => {
  Obj.$("caja").setAttribute("color", Obj.color(P.color, "#F2C200"));
  if (largo > maximo()) estirar(maximo());
});
Obj.$("caja").setAttribute("color", Obj.color(P.color, "#F2C200"));
colocar();
