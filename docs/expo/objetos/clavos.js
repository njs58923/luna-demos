...
;
// El tablón con clavos: el martillo y cuánto entra cada clavo.
//
// El martillo se arma en su propio marco: +Y es el mango (de la mano hacia
// la cabeza) y +X la cara que golpea. En la mesa descansa acostado; al tocar
// un clavo hace un golpe animado; en VR el mango sigue hacia dónde apunta el
// mando y la cara mira hacia "abajo" del mando, que es como se agarra un
// martillo. Un clavo entra cuando la cara lo alcanza bajando rápido.
const P = Obj.props({ clavos: 5, madera: "#C19A6B" });

const sinte = Obj.sinte({ volumen: 0.8 });
const donde = Obj.marco();
const martillo = Obj.$("martillo");
const TOPE = 0.05;            // la tabla
const ASOMA = 0.045;          // cuánto sobresale un clavo nuevo
const CARA = { y: 0.28, x: 0.05 };   // la cara, en el marco del martillo
const N = Math.max(1, Math.min(7, Math.round(Number(P.clavos) || 5)));
const ZC = 0.035;

const clavos = [];
for (let i = 0; i < N; i++) {
  const x = N === 1 ? 0 : -0.18 + (0.36 * i) / (N - 1);
  const g = Obj.crear("group", { x, z: ZC }, Obj.$("clavos"));
  Obj.crear("cylinder", { y: -0.03, sx: 0.005, sy: 0.06, sz: 0.005, color: "#9EA3A8" }, g);
  Obj.crear("cylinder", { y: 0.0015, sx: 0.014, sy: 0.003, sz: 0.014, color: "#B8BDC2" }, g);
  const blanco = Obj.crear("box", { y: -0.01, sx: 0.05, sy: 0.08, sz: 0.05, color: "transparent", "material-alpha": "blend" }, g);
  const c = { i, x, g, hundido: 0 };
  Obj.boton(blanco, () => golpeAnimado(c));
  clavos.push(c);
}
const alto = (c) => TOPE + ASOMA * (1 - c.hundido);   // la cabeza
function ponerClavo(c) { c.g.position = { x: c.x, y: alto(c), z: ZC }; }
...
/** Poner el martillo con el mango en `mano`, apuntando a f, la cara hacia c. */
function pose(mano, f, c) {
  f = norm(f);
  const d = c.x * f.x + c.y * f.y + c.z * f.z;
  const x = norm({ x: c.x - d * f.x, y: c.y - d * f.y, z: c.z - d * f.z });
  martillo.position = mano;
  martillo.rotation = Obj.deEjes(x, f, cruz(x, f));
  return { x: mano.x + f.x * CARA.y + x.x * CARA.x, y: mano.y + f.y * CARA.y + x.y * CARA.x, z: mano.z + f.z * CARA.y + x.z * CARA.x };
}
const REPOSO = () => pose({ x: -0.2, y: 0.068, z: -0.07 }, { x: 1, y: 0, z: 0 }, { x: 0, y: 0, z: 1 });

function sonar(fuerte) {
  sinte.nota(1900, 0.25, { tipo: "campana", vol: 0.25 * fuerte, caida: 14 });
  sinte.nota(160, 0.12, { tipo: "madera", vol: 0.5 * fuerte });
}

function clavar(c, cuanto) {
  if (c.hundido >= 1) { sonar(0.4); return; }
  c.hundido = Math.min(1, c.hundido + cuanto);
  if (c.hundido > 0.97) c.hundido = 1;
  ponerClavo(c);
  sonar(1);
  Obj.emitir("golpe", { clavo: c.i, hundido: +c.hundido.toFixed(3) });
  if (clavos.every((k) => k.hundido >= 1)) {
    Obj.emitir("listo", {});
    for (let i = 0; i < 4; i++) sinte.nota(Obj.frecuencia(3 + i * 4), 0.4, { tipo: "campana", vol: 0.15, retraso: 0.3 + i * 0.1 });
    setTimeout(reponer, 2500);
  }
}
function reponer() {
  for (const c of clavos) { c.hundido = 0; ponerClavo(c); }
}

// Un golpe animado: el martillo se levanta sobre el clavo y baja.
let animando = null;
const anim = Obj.animador(() => {
  if (!animando) return false;
  const { c, t0 } = animando;
  const t = (performance.now() - t0) / 1000;
  const subir = 0.14, bajar = 0.09;
  let th;
  if (t < subir) th = 0.9 * Obj.salida(t / subir);
  else if (t < subir + bajar) th = 0.9 * (1 - Math.pow((t - subir) / bajar, 2));
  else th = 0;
  // Con th = 0 la cara apoya justo en la cabeza del clavo.
  const f = { x: 0, y: Math.sin(th), z: -Math.cos(th) };
  const cara = { x: 0, y: -Math.cos(th), z: -Math.sin(th) };
  const y = alto(c);
  pose({ x: c.x, y: y + CARA.x, z: ZC + CARA.y }, f, cara);
  if (t >= subir + bajar) {
    clavar(c, 1 / 3 + 0.001);
    animando = null;
    setTimeout(() => { if (!animando && !tomado.tomado) REPOSO(); }, 350);
    return false;
  }
  return true;
});
function golpeAnimado(c) {
  if (animando || tomado.tomado) return;
  animando = { c, t0: performance.now() };
  anim.iniciar();
}

// ── En VR ──────────────────────────────────────────────────────────────────
let caraAntes = null, tAntes = 0, volver = 0;
const tomado = Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) {
    // Cerca del mango (de la mano de reposo a la cabeza).
    const a = { x: -0.2, y: 0.068, z: -0.07 };
    const t = Math.max(0, Math.min(0.3, p.x - a.x));
    return Math.hypot(p.x - (a.x + t), p.y - a.y, p.z - a.z) < 0.09;
  },
  tomar(p, mano, e) { animando = null; clearTimeout(volver); caraAntes = null; seguir(p, e); },
  mover(p, mano, e) { seguir(p, e); },
  soltar() { caraAntes = null; volver = setTimeout(REPOSO, 800); },
});
function seguir(p, e) {
  ...
  // Si el mando apunta derecho abajo, "abajo del mando" se confunde con f: se
  // usa entonces el frente del objeto.
  const c = Math.abs(abajo.x * f.x + abajo.y * f.y + abajo.z * f.z) > 0.95 ? { x: 0, y: 0, z: -1 } : abajo;
  const cara = pose(p, f, c);
  const ahora = performance.now();
  if (caraAntes) {
    const dt = Math.max(0.001, (ahora - tAntes) / 1000);
    const vy = (cara.y - caraAntes.y) / dt;
    for (const k of clavos) {
      const top = alto(k);
      // La cara cruzó la cabeza del clavo bajando, sobre su eje.
      if (caraAntes.y > top && cara.y <= top + 0.004 && Math.hypot(cara.x - k.x, cara.z - ZC) < 0.03 && vy < -0.6) {
        clavar(k, Math.min(0.6, 0.08 + (-vy) * 0.09));
      }
    }
  }
  caraAntes = cara; tAntes = ahora;
}

Obj.mensaje("reponer", reponer);
Obj.alCambiar(() => Obj.$("tabla").setAttribute("color", Obj.color(P.madera, "#C19A6B")));
Obj.$("tabla").setAttribute("color", Obj.color(P.madera, "#C19A6B"));
clavos.forEach(ponerClavo);
REPOSO();
