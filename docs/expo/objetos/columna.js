...
;
// La columna: clásica, salomónica, cuadrada o moderna. Tocarla pasa al tipo
// siguiente.
const P = Obj.props({ alto: 2.6, diametro: 0.34, tipo: "clasica", color: "" });
const ORDEN = ["clasica", "salomonica", "cuadrada", "moderna"];
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let tipo = ORDEN.includes(P.tipo) ? P.tipo : "clasica";

function dibujar() {
  const H = Obra.num(P.alto, 2.6, 0.5, 20), Dm = Obra.num(P.diametro, 0.34, 0.08, 3);
  armar(Obra.columna({ alto: H, diametro: Dm, tipo, color: P.color }));
  toque.position = { x: 0, y: H / 2, z: 0 };
  toque.scale = { x: Dm * 1.6, y: H, z: Dm * 1.6 };
}
