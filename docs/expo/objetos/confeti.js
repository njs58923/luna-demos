...
;
// El cañón de confeti: papelitos con rozamiento de aire y giro propio.
//
// Un papelito cae despacio porque el aire lo frena mucho: su velocidad tiende
// a una terminal chica, y además deriva de costado mientras gira. Eso es lo
// que hace que se vea como confeti y no como piedritas. Todos salen de la boca
// del tubo con un cono de direcciones.
const P = Obj.props({ colores: ["#FF3B30", "#FFD60A", "#30D158", "#0A84FF", "#BF5AF2", "#FF9F0A", "#40E0D0"], cantidad: 60, color: "#7B2FF7" });

const sinte = Obj.sinte({ volumen: 0.8 });
const grupo = Obj.$("papelitos");
const N = Math.max(20, Math.min(120, Math.round(Number(P.cantidad) || 60)));
const BOCA = { x: 0, y: 0.06 + 0.41 * Math.cos(0.25), z: 0.41 * Math.sin(0.25) };
function colores() {
  const c = Array.isArray(P.colores) ? P.colores.map((x) => Obj.color(x, null)).filter(Boolean) : [];
  return c.length ? c : ["#FF3B30", "#FFD60A", "#30D158", "#0A84FF"];
}
const papelitos = [];
for (let i = 0; i < N; i++) {
  const el = Obj.crear("box", { sx: 0.03, sy: 0.02, sz: 0.002, color: "#FFFFFF", visible: "false" }, grupo);
  papelitos.push({ el, p: null, v: null, giro: null, fase: 0, vivo: false });
}

let hasta = 0;
const anim = Obj.animador((t, dt) => {
  let volando = false;
  for (const c of papelitos) {
    if (!c.vivo) continue;
    if (c.p.y > 0.004) {
      volando = true;
      // Aire: la velocidad tiende a caer a 0.5 m/s, con deriva lateral.
      c.v.x += (-c.v.x * 2.5 + Math.sin(t * 3 + c.fase) * 0.4) * dt;
      c.v.z += (-c.v.z * 2.5 + Math.cos(t * 2.4 + c.fase) * 0.4) * dt;
      c.v.y += (-9.8 - c.v.y * 3) * dt;
      c.v.y = Math.max(c.v.y, -0.6);
      c.p.x += c.v.x * dt; c.p.y += c.v.y * dt; c.p.z += c.v.z * dt;
      c.giro.x += dt * 7; c.giro.y += dt * 5;
      if (c.p.y <= 0.004) { c.p.y = 0.003; c.giro = { x: -Math.PI / 2, y: c.giro.y, z: 0 }; }
      c.el.position = c.p;
      c.el.rotation = c.giro;
    }
  }
  // En el piso duran un rato y se van.
  if (!volando && performance.now() > hasta) {
    for (const c of papelitos) { c.vivo = false; c.el.setAttribute("visible", "false"); }
    return false;
  }
  return true;
});
...
function disparar() {
  const cs = colores();
  papelitos.forEach((c, i) => {
    c.vivo = true;
    c.p = { ...BOCA };
    // Un cono de ±25° alrededor de la dirección del tubo.
    const a = Math.random() * Math.PI * 2, r = Math.random() * 0.45;
    const rapidez = 5 + Math.random() * 3;
    const dir = { x: Math.sin(r) * Math.cos(a), y: Math.cos(r) * Math.cos(0.25), z: Math.sin(0.25) + Math.sin(r) * Math.sin(a) };
    c.v = { x: dir.x * rapidez, y: dir.y * rapidez, z: dir.z * rapidez };
    c.giro = { x: Math.random() * 6, y: Math.random() * 6, z: 0 };
    c.fase = Math.random() * 6;
    c.el.setAttribute("color", cs[i % cs.length]);
    c.el.setAttribute("visible", "inherit");
  });
  hasta = performance.now() + 8000;
  sinte.nota(120, 0.3, { tipo: "ruido", vol: 0.5, caida: 12 });
  sinte.nota(80, 0.25, { tipo: "seno", vol: 0.4, caida: 10 });
  Obj.emitir("disparo", {});
  anim.iniciar();
}

Obj.boton(Obj.$("toque"), disparar);
Obj.mensaje("disparar", disparar);
Obj.alCambiar(() => Obj.$("cuerpo").setAttribute("color", Obj.color(P.color, "#7B2FF7")));
Obj.$("cuerpo").setAttribute("color", Obj.color(P.color, "#7B2FF7"));
