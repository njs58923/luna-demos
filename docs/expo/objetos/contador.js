...
;
// El contador de mano: cuatro ruedas de números y un clic.
//
// Cada rueda es un número en un recuadro blanco; al pasar de 9 a 0 la rueda
// de al lado avanza, como en uno mecánico. El número que cambia hace un salto
// chico hacia arriba, que es lo que se ve cuando una rueda gira.
const P = Obj.props({ metal: "#C0C0C0", clave: "" });

const memo = Obj.memoria("cuenta");
const sinte = Obj.sinte({ volumen: 0.6 });
const ruedas = Obj.$("ruedas");
const boton = Obj.$("boton");
let valor = Math.max(0, Math.min(9999, Math.floor(Number(memo.leer(0)) || 0)));

const digitos = [0, 1, 2, 3].map((i) => {
  const x = -0.021 + i * 0.014;
  Obj.crear("box", { x, sx: 0.012, sy: 0.018, sz: 0.001, color: "#F4F4F4" }, ruedas);
  return { t: Obj.crear("text", { x, z: 0.001, value: "0", size: 0.014, color: "#101010" }, ruedas), x, hasta: 0 };
});

function dibujar(cambiadas) {
  const s = String(valor).padStart(4, "0");
  digitos.forEach((d, i) => {
    d.t.setAttribute("value", s[i]);
    if (cambiadas && cambiadas[i]) d.hasta = performance.now() + 120;
  });
  anim.iniciar();
}
const anim = Obj.animador(() => {
  ...
  for (const d of digitos) {
    const k = Math.max(0, (d.hasta - ahora) / 120);
    d.t.position = { x: d.x, y: 0.004 * k, z: 0.001 };
    if (k > 0) seguir = true;
  }
  return seguir;
});
...
function sumar(n) {
  const antes = String(valor).padStart(4, "0");
  valor = (((valor + n) % 10000) + 10000) % 10000;
  const despues = String(valor).padStart(4, "0");
  memo.guardar(valor);
  dibujar([0, 1, 2, 3].map((i) => antes[i] !== despues[i]));
  Obj.emitir("cuenta", { valor });
}

Obj.boton(boton, () => {
  sinte.nota(3200, 0.02, { tipo: "pulso", vol: 0.25 });
  boton.position = { x: 0, y: 0.046, z: 0 };
  setTimeout(() => { boton.position = { x: 0, y: 0.052, z: 0 }; }, 80);
  sumar(1);
}, { color: "#B0B0B0" });
Obj.boton(Obj.$("cero"), () => {
  sinte.nota(900, 0.05, { tipo: "madera", vol: 0.2 });
  if (valor === 0) return;
  valor = 0;
  memo.guardar(0);
  dibujar([true, true, true, true]);
  Obj.emitir("cuenta", { valor });
}, { color: "#8A8A8A" });
Obj.mensaje("sumar", (d) => { if (d && Number.isFinite(d.n)) sumar(Math.round(d.n)); });
Obj.mensaje("cero", () => { valor = 0; memo.guardar(0); dibujar([true, true, true, true]); Obj.emitir("cuenta", { valor }); });
Obj.alCambiar(() => Obj.$("cuerpo").setAttribute("color", Obj.color(P.metal, "#C0C0C0")));
Obj.$("cuerpo").setAttribute("color", Obj.color(P.metal, "#C0C0C0"));
dibujar();
