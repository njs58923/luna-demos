...
;
// El tablero de corcho: tarjetas, chinches e hilos.
//
// Hay un pozo fijo de tarjetas (nodos creados una vez) y de hilos. Una
// tarjeta viva tiene un id que no cambia, su lugar y su texto; los hilos unen
// ids. Cada tarjeta tiene su propio editor de teclado. Un hilo es un
// cilindro tendido entre las dos chinches: se retiende cuando una tarjeta se
// mueve.
const P = Obj.props({ clave: "", madera: "#8B5A2B" });

const donde = Obj.marco();
const guardado = Obj.memoria("tablero");
const MAX_CARTAS = 12, MAX_HILOS = 24;
const ANCHO = 0.8, ALTO = 0.6, FRENTE = 0.018;
const COLORES = ["#FFF3B0", "#CDEAC0", "#BDE0FE", "#FFC8DD", "#FFFFFF"];
const COLOR_CHINCHE = ["#D62828", "#1D4ED8", "#2A9D8F", "#F77F00", "#7B2FF7"];

let cartas = [];      // { id, x, y, texto, color, slot }
let hilos = [];       // [idA, idB]
let proximoId = 1;
let elegida = null;   // id de la chinche tocada primero

// ── El pozo de tarjetas ────────────────────────────────────────────────────
const slots = [];
for (let i = 0; i < MAX_CARTAS; i++) {
  const g = Obj.crear("group", { visible: "false" }, Obj.$("cartas"));
  const borde = Obj.crear("box", { z: -0.001, sx: 0.118, sy: 0.088, sz: 0.001, color: "#0A84FF", visible: "false" }, g);
  const papel = Obj.crear("box", { sx: 0.11, sy: 0.08, sz: 0.002, color: "#FFF3B0" }, g);
  const lineas = [0, 1, 2].map((k) => Obj.crear("text", { y: 0.012 - k * 0.016, z: 0.0015, value: "", size: 0.0095, color: "#1C1C1C" }, g));
  const pin = Obj.crear("sphere", { y: 0.03, z: 0.006, sx: 0.014, sy: 0.014, sz: 0.014, color: "#D62828" }, g);
  const blancoCarta = Obj.crear("box", { y: -0.006, z: 0.003, sx: 0.11, sy: 0.066, sz: 0.004, color: "transparent", "material-alpha": "blend" }, g);
  const blancoPin = Obj.crear("box", { y: 0.03, z: 0.008, sx: 0.03, sy: 0.022, sz: 0.012, color: "transparent", "material-alpha": "blend" }, g);
  const slot = { i, g, borde, papel, lineas, pin, carta: null };
  slot.editor = Obj.teclado(blancoCarta, {
    max: 60, unaLinea: true,
    cambio: (t) => { if (slot.carta) { slot.carta.texto = t; pintarCarta(slot.carta); guardar(); } },
    foco: (si) => {
      borde.setAttribute("visible", si ? "inherit" : "false");
      if (!si && slot.carta && !slot.carta.texto.trim()) borrar(slot.carta.id);
    },
  });
  Obj.boton(blancoCarta, () => { if (slot.carta) editar(slot.carta); });
  Obj.boton(blancoPin, () => { if (slot.carta) chinche(slot.carta.id); });
  slots.push(slot);
}
const cartaDe = (id) => cartas.find((c) => c.id === id);

/** Partir en tres renglones de hasta 17 letras, por palabras. */
function partir(t) {
  const out = [];
  let actual = "";
  for (const p of String(t).split(/\s+/)) {
    if (!p) continue;
    if ((actual + " " + p).trim().length <= 17) actual = (actual + " " + p).trim();
    else { if (actual) out.push(actual); actual = p.slice(0, 17); }
  }
  if (actual) out.push(actual);
  if (out.length > 3) { out.length = 3; out[2] = out[2].slice(0, 15) + " …"; }
  return out;
}
function pintarCarta(c) {
  const s = slots[c.slot];
  s.g.setAttribute("visible", "inherit");
  s.g.position = { x: c.x, y: c.y, z: FRENTE };
  s.papel.setAttribute("color", COLORES[c.color % COLORES.length]);
  s.pin.setAttribute("color", COLOR_CHINCHE[c.color % COLOR_CHINCHE.length]);
  const r = partir(c.texto);
  s.lineas.forEach((l, k) => l.setAttribute("value", r[k] || ""));
  const sel = elegida === c.id;
  s.pin.scale = sel ? { x: 0.02, y: 0.02, z: 0.02 } : { x: 0.014, y: 0.014, z: 0.014 };
}

// ── Los hilos ──────────────────────────────────────────────────────────────
const nodosHilo = [];
for (let i = 0; i < MAX_HILOS; i++) nodosHilo.push(Obj.crear("cylinder", { sx: 0.0022, sy: 0.1, sz: 0.0022, color: "#C1121F", visible: "false" }, Obj.$("hilos")));
const chincheDe = (c) => ({ x: c.x, y: c.y + 0.03, z: FRENTE + 0.008 });
function pintarHilos() {
  nodosHilo.forEach((n, i) => {
    const h = hilos[i];
    const a = h && cartaDe(h[0]), b = h && cartaDe(h[1]);
    if (!a || !b) { n.setAttribute("visible", "false"); return; }
    const l = Obj.tender(n, chincheDe(a), chincheDe(b));
    n.scale = { x: 0.0022, y: Math.max(0.001, l), z: 0.0022 };
    n.setAttribute("visible", "inherit");
  });
}

// ── Lo que se puede hacer ──────────────────────────────────────────────────
function guardar() {
  guardado.guardar({ cartas: cartas.map(({ id, x, y, texto, color }) => ({ id, x, y, texto, color })), hilos, proximoId });
  Obj.emitir("cambio", { cartas: cartas.length, hilos: hilos.length });
}
function acomodar(x, y) {
  return { x: Math.max(-ANCHO / 2 + 0.06, Math.min(ANCHO / 2 - 0.06, x)), y: Math.max(-ALTO / 2 + 0.045, Math.min(ALTO / 2 - 0.045, y)) };
}
function nueva(x, y, texto) {
  const libre = slots.find((s) => !s.carta);
  if (!libre) return null;
  const p = acomodar(x, y);
  const c = { id: proximoId++, x: p.x, y: p.y, texto: texto || "", color: (proximoId * 3) % COLORES.length, slot: libre.i };
  libre.carta = c;
  cartas.push(c);
  pintarCarta(c);
  return c;
...
function borrar(id) {
  ...
  const s = slots[c.slot];
  s.carta = null;
  s.g.setAttribute("visible", "false");
  cartas = cartas.filter((x) => x.id !== id);
  hilos = hilos.filter(([a, b]) => a !== id && b !== id);
  if (elegida === id) elegida = null;
  pintarHilos();
  guardar();
}
function chinche(id) {
  if (elegida === null) { elegida = id; pintarCarta(cartaDe(id)); return; }
  const antes = elegida;
  elegida = null;
  if (cartaDe(antes)) pintarCarta(cartaDe(antes));
  if (antes === id) return;
  const i = hilos.findIndex(([a, b]) => (a === antes && b === id) || (a === id && b === antes));
  if (i >= 0) hilos.splice(i, 1);
  else if (hilos.length < MAX_HILOS) hilos.push([antes, id]);
  pintarHilos();
  guardar();
}
function limpiar() {
  for (const c of [...cartas]) { slots[c.slot].carta = null; slots[c.slot].g.setAttribute("visible", "false"); }
  cartas = []; hilos = []; elegida = null;
  pintarHilos();
  guardar();
}

Obj.boton(Obj.$("corcho"), (e) => {
  if (!Number.isFinite(e.x)) return;
  const l = donde.aLocal({ x: e.x, y: e.y, z: e.z });
  const c = nueva(l.x, l.y - 0.01, "");
  if (c) editar(c);
});
Obj.mensaje("limpiar", limpiar);

// ── En VR: mover tarjetas ──────────────────────────────────────────────────
let enMano = null;
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) {
    enMano = null;
    if (p.z > 0.15) return false;
    let dm = 0.07;
    for (const c of cartas) {
      const d = Math.hypot(p.x - c.x, p.y - c.y);
      if (d < dm) { dm = d; enMano = c; }
    }
    return !!enMano;
  },
  mover(p) {
    const q = acomodar(p.x, p.y);
    enMano.x = q.x; enMano.y = q.y;
    pintarCarta(enMano);
    pintarHilos();
  },
  soltar() { if (enMano) guardar(); enMano = null; },
});

// ── Arranque ───────────────────────────────────────────────────────────────
const previo = guardado.leer(null);
if (previo && Array.isArray(previo.cartas)) {
  for (const c of previo.cartas.slice(0, MAX_CARTAS)) {
    const n = nueva(Number(c.x) || 0, Number(c.y) || 0, String(c.texto || ""));
    if (n) { n.id = c.id; n.color = Number(c.color) || 0; pintarCarta(n); }
  }
  proximoId = Math.max(Number(previo.proximoId) || 1, ...cartas.map((c) => c.id + 1));
  hilos = Array.isArray(previo.hilos) ? previo.hilos.filter((h) => Array.isArray(h) && cartaDe(h[0]) && cartaDe(h[1])) : [];
} else {
  // Un ejemplo, para que se entienda qué es.
  const a = nueva(-0.24, 0.14, "una idea");
  const b = nueva(0.05, -0.08, "cómo hacerla");
  const c = nueva(0.26, 0.15, "¡a hacerla!");
  hilos = [[a.id, b.id], [b.id, c.id]];
}
pintarHilos();
Obj.alCambiar(() => Obj.$("marco").setAttribute("color", Obj.color(P.madera, "#8B5A2B")));
Obj.$("marco").setAttribute("color", Obj.color(P.madera, "#8B5A2B"));
