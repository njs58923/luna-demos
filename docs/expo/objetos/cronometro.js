...
;
// El cronómetro: corona para arrancar y parar, botón para volver a cero.
//
// Mientras corre pide cuadros para que la aguja barra suave, pero la lectura
// en números se reescribe sólo cuando cambia la centésima que se muestra: un
// setAttribute por cuadro de más es un mensaje al motor por nada.
const P = Obj.props({ caja: "#C9A45C", esfera: "#FAF7F0" });

const aguja = Obj.$("aguja");
const lectura = Obj.$("lectura");
const marcas = Obj.$("marcas");
const corona = Obj.$("corona");

for (let i = 0; i < 60; i++) {
  const a = (i / 60) * Math.PI * 2, largo = i % 5 === 0 ? 0.012 : 0.005;
  Obj.crear("box", { x: Math.sin(a) * (0.058 - largo / 2), y: Math.cos(a) * (0.058 - largo / 2), rz: -a, sx: i % 5 === 0 ? 0.0025 : 0.0012, sy: largo, sz: 0.001, color: "#2A2A2A" }, marcas);
}
Obj.crear("box", { y: 0.024, sx: 0.002, sy: 0.058, sz: 0.001, color: "#C8102E" }, aguja);
Obj.crear("box", { y: -0.01, sx: 0.004, sy: 0.014, sz: 0.001, color: "#C8102E" }, aguja);

let acumulado = 0;    // ms de las corridas anteriores
let desde = null;     // performance.now() de cuando arrancó esta corrida
let mostrado = "";

const dos = (n) => String(Math.floor(n)).padStart(2, "0");
function transcurrido() { return acumulado + (desde == null ? 0 : performance.now() - desde); }
function dibujar() {
  const ms = transcurrido();
  aguja.rotation = { x: 0, y: 0, z: -((ms / 60000) % 1) * Math.PI * 2 };
  const txt = dos(ms / 60000) + ":" + dos((ms / 1000) % 60) + "." + dos((ms / 10) % 100);
  if (txt !== mostrado) { mostrado = txt; lectura.setAttribute("value", txt); }
}

const anim = Obj.animador(() => { dibujar(); return desde != null; });

// La corona baja un poco al apretarla.
function apretar(el, y) {
  el.position = { x: 0, y: y - 0.004, z: 0 };
  setTimeout(() => { el.position = { x: 0, y, z: 0 }; }, 120);
}
function arrancar() {
...
function parar() {
  ...
  desde = null;
  dibujar();
  Obj.emitir("parado", { ms: Math.round(acumulado) });
}
function reiniciar() {
  acumulado = 0;
  if (desde != null) desde = performance.now();
  dibujar();
}

Obj.boton(corona, () => { apretar(corona, 0.095); desde == null ? arrancar() : parar(); }, { color: "#B8923E" });
Obj.boton(Obj.$("reinicio"), reiniciar, { color: "#B8923E" });
Obj.mensaje("arrancar", arrancar);
Obj.mensaje("parar", parar);
Obj.mensaje("reiniciar", reiniciar);

function pintar() {
  Obj.$("caja").setAttribute("color", Obj.color(P.caja, "#C9A45C"));
  Obj.$("corona_tallo").setAttribute("color", Obj.color(P.caja, "#C9A45C"));
  Obj.$("esfera").setAttribute("color", Obj.color(P.esfera, "#FAF7F0"));
}
