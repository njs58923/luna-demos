...
;
// Cuatro en línea: columnas, fichas que caen y cuatro en fila.
//
// Las fichas no se esconden en huecos —no hay agujeros en una caja— sino que
// se apoyan en la cara del tablero sobre un círculo oscuro que hace de hueco.
// La caída es una aceleración de verdad, con un rebotecito al llegar.
const P = Obj.props({ maquina: false, tablero: "#1565C0", clave: "" });

const COLS = 7, FILAS = 6, PASO = 0.062;
const X0 = -((COLS - 1) * PASO) / 2, Y0 = -((FILAS - 1) * PASO) / 2;
const ROJO = "#E63946", AMARILLO = "#FFD60A";
const memo = Obj.memoria("partida");
const fichasG = Obj.$("fichas");
const estado = Obj.$("estado");
const xy = (c, f) => ({ x: X0 + c * PASO, y: Y0 + f * PASO });

for (let c = 0; c < COLS; c++) {
  for (let f = 0; f < FILAS; f++) {
    const { x, y } = xy(c, f);
    Obj.crear("cylinder", { x, y, z: 0.011, rx: Math.PI / 2, sx: 0.05, sy: 0.002, sz: 0.05, color: "#0B2545" }, Obj.$("huecos"));
  }
}

// grilla[c][f]: "R", "A" o null, con f = 0 abajo.
let grilla = [...Array(COLS)].map(() => Array(FILAS).fill(null));
let turno = "R";
let terminada = false;
const nodos = [...Array(COLS)].map(() => Array(FILAS).fill(null));
const g = memo.leer(null);
if (g && Array.isArray(g.grilla) && g.grilla.length === COLS) {
  grilla = g.grilla.map((col) => (Array.isArray(col) ? col.slice(0, FILAS).map((v) => (v === "R" || v === "A" ? v : null)) : Array(FILAS).fill(null)));
  while (grilla.some((col) => col.length < FILAS)) for (const col of grilla) while (col.length < FILAS) col.push(null);
  turno = g.turno === "A" ? "A" : "R";
}

function ficha(c, f, cual) {
  const { x, y } = xy(c, f);
  return Obj.crear("cylinder", { x, y, z: 0.018, rx: Math.PI / 2, sx: 0.052, sy: 0.012, sz: 0.052, color: cual === "R" ? ROJO : AMARILLO }, fichasG);
}

/** Las cuatro (o más) en fila que contienen (c, f), o null. */
function linea(t, c, f) {
  const quien = t[c][f];
  if (!quien) return null;
  for (const [dc, df] of [[1, 0], [0, 1], [1, 1], [1, -1]]) {
    const celdas = [[c, f]];
    for (const s of [1, -1]) {
      let k = 1;
      for (;;) {
        const cc = c + dc * k * s, ff = f + df * k * s;
        if (cc < 0 || cc >= COLS || ff < 0 || ff >= FILAS || t[cc][ff] !== quien) break;
        celdas.push([cc, ff]);
        k++;
      }
    }
...
function ganadorGeneral(t) {
  for (let c = 0; c < COLS; c++) for (let f = 0; f < FILAS; f++) { const l = linea(t, c, f); if (l) return { quien: t[c][f], celdas: l }; }
  return t.every((col) => col.every(Boolean)) ? { quien: "empate", celdas: [] } : null;
}

function texto() {
  const w = ganadorGeneral(grilla);
  if (!w) return "juegan las " + (turno === "R" ? "rojas" : "amarillas");
  return w.quien === "empate" ? "empate" : "¡ganaron las " + (w.quien === "R" ? "rojas" : "amarillas") + "!";
}
function resaltar() {
  const w = ganadorGeneral(grilla);
  terminada = !!w;
  if (w) for (const [c, f] of w.celdas) if (nodos[c][f]) nodos[c][f].setAttribute("material-unlit", "true");
  estado.setAttribute("value", texto());
}
function dibujarTodo() {
  for (let c = 0; c < COLS; c++) for (let f = 0; f < FILAS; f++) {
    if (nodos[c][f]) { nodos[c][f].remove(); nodos[c][f] = null; }
    if (grilla[c][f]) nodos[c][f] = ficha(c, f, grilla[c][f]);
  }
  resaltar();
}

// ── Caídas ──────────────────────────────────────────────────────────────────
const cayendo = [];   // { el, c, f, y, v }
const anim = Obj.animador((t, dt) => {
  for (let i = cayendo.length - 1; i >= 0; i--) {
    const k = cayendo[i];
    const piso = xy(k.c, k.f).y;
    k.v -= 3.5 * dt;
    k.y += k.v * dt;
    if (k.y <= piso) {
      k.y = piso;
      // Un rebote chico y ya.
      if (Math.abs(k.v) > 0.25 && !k.reboto) { k.v = -k.v * 0.25; k.reboto = true; }
      else { k.v = 0; cayendo.splice(i, 1); }
    }
    k.el.position = { x: xy(k.c, k.f).x, y: k.y, z: 0.018 };
  }
  if (!cayendo.length) resaltar();
  return cayendo.length > 0;
});

function jugar(c, cual) {
  if (terminada || cual !== turno || c < 0 || c >= COLS) return false;
  const f = grilla[c].indexOf(null);
  if (f < 0) return false;
  grilla[c][f] = cual;
  turno = cual === "R" ? "A" : "R";
  memo.guardar({ grilla, turno });
  const el = ficha(c, f, cual);
  nodos[c][f] = el;
  const arriba = Y0 + FILAS * PASO + 0.03;
  el.position = { x: xy(c, f).x, y: arriba, z: 0.018 };
  cayendo.push({ el, c, f, y: arriba, v: 0 });
  anim.iniciar();
  Obj.emitir("jugada", { columna: c, ficha: cual === "R" ? "rojo" : "amarillo" });
  const w = ganadorGeneral(grilla);
  if (w) { terminada = true; Obj.emitir("fin", { ganador: w.quien === "empate" ? "empate" : w.quien === "R" ? "rojo" : "amarillo" }); }
  estado.setAttribute("value", texto());
  return true;
}

// ── La máquina: gana si puede, tapa si tiene que, y si no, cerca del centro ──
function jugadaMaquina() {
  const libres = [...Array(COLS).keys()].filter((c) => grilla[c].includes(null));
  const prueba = (c, quien) => {
    const f = grilla[c].indexOf(null);
    grilla[c][f] = quien;
    const gana = !!linea(grilla, c, f);
    grilla[c][f] = null;
    return gana;
  };
  let c = libres.find((x) => prueba(x, "A"));
  if (c == null) c = libres.find((x) => prueba(x, "R"));
  if (c == null) {
    // No regalar: evitar la columna que le deja al rival ganar arriba.
    const seguras = libres.filter((x) => {
      const f = grilla[x].indexOf(null);
      if (f + 1 >= FILAS) return true;
      grilla[x][f] = "A";
      const regala = prueba(x, "R");
      grilla[x][f] = null;
      return !regala;
    });
    const opciones = seguras.length ? seguras : libres;
    opciones.sort((a, b) => Math.abs(a - 3) - Math.abs(b - 3) + (Math.random() - 0.5) * 1.5);
    c = opciones[0];
  }
  if (c != null) jugar(c, "A");
}
...
for (let c = 0; c < COLS; c++) {
  const b = Obj.crear("plane", { x: X0 + c * PASO, z: 0.03, sx: PASO, sy: 0.4, color: "transparent", "material-alpha": "blend" }, Obj.$("toques"));
  Obj.boton(b, () => {
    if (P.maquina && turno !== "R") return;
    if (jugar(c, P.maquina ? "R" : turno) && P.maquina && !terminada) setTimeout(jugadaMaquina, 700);
  });
}

function nueva() {
  grilla = [...Array(COLS)].map(() => Array(FILAS).fill(null));
  turno = "R";
  cayendo.length = 0;
  memo.guardar({ grilla, turno });
  dibujarTodo();
}
Obj.boton(Obj.$("nueva"), nueva, { color: "#FFD60A" });
Obj.mensaje("nueva", nueva);
Obj.alCambiar(() => Obj.$("tablero").setAttribute("color", Obj.color(P.tablero, "#1565C0")));
Obj.$("tablero").setAttribute("color", Obj.color(P.tablero, "#1565C0"));
dibujarTodo();
if (P.maquina && turno === "A" && !terminada) setTimeout(jugadaMaquina, 800);
