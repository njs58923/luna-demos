...
;
// El cubo mágico: 26 cubitos con posición entera y orientación.
//
// Cada cubito sabe dónde está (x, y, z en -1..1) y cómo está girado (un
// cuaternión). Girar una cara es tomar los cubitos de esa capa y rotarlos un
// cuarto de vuelta alrededor de la normal de la cara: sus posiciones se
// redondean de nuevo a enteros y sus giros se componen. Mientras dura la
// animación se dibujan en un ángulo intermedio. La historia de giros sirve
// para resolverlo (deshaciéndola) y para guardarlo: el estado es "resuelto +
// estos giros".
const P = Obj.props({ clave: "", lado: 0.15 });

const cuboG = Obj.$("cubo");
const LADO = Math.max(0.08, Math.min(0.4, Number(P.lado) || 0.15));
const S = LADO / 3;
cuboG.position = { x: 0, y: 0.05 + LADO / 2, z: 0 };
const guardado = Obj.memoria("giros");

const CARAS = {
  U: { n: { x: 0, y: 1, z: 0 }, eje: "y", capa: 1, color: "#FFFFFF" },
  D: { n: { x: 0, y: -1, z: 0 }, eje: "y", capa: -1, color: "#FFD500" },
  R: { n: { x: 1, y: 0, z: 0 }, eje: "x", capa: 1, color: "#C41E3A" },
  L: { n: { x: -1, y: 0, z: 0 }, eje: "x", capa: -1, color: "#FF5800" },
  F: { n: { x: 0, y: 0, z: 1 }, eje: "z", capa: 1, color: "#009E60" },
  B: { n: { x: 0, y: 0, z: -1 }, eje: "z", capa: -1, color: "#0051BA" },
};
...
for (let x = -1; x <= 1; x++) for (let y = -1; y <= 1; y++) for (let z = -1; z <= 1; z++) {
  if (!x && !y && !z) continue;
  const g = Obj.crear("group", {}, cuboG);
  Obj.crear("box", { sx: S * 0.97, sy: S * 0.97, sz: S * 0.97, color: "#151515", "border-radius": S * 0.12 }, g);
  for (const c of Object.values(CARAS)) {
    const k = c.eje === "x" ? x : c.eje === "y" ? y : z;
    if (k !== c.capa) continue;
    const o = S * 0.49;
    const t = S * 0.8, d = S * 0.03;
    Obj.crear("box", {
      x: c.n.x * o, y: c.n.y * o, z: c.n.z * o,
      sx: c.eje === "x" ? d : t, sy: c.eje === "y" ? d : t, sz: c.eje === "z" ? d : t,
      color: c.color, "border-radius": S * 0.08,
    }, g);
  }
  cubitos.push({ g, casa: { x, y, z }, p: { x, y, z }, q: { x: 0, y: 0, z: 0, w: 1 } });
}
function dibujar(c, eje, ang) {
  let p = { x: c.p.x * S, y: c.p.y * S, z: c.p.z * S }, q = c.q;
  if (eje) {
    const r = Obj.qEje(eje, ang);
    p = Obj.rotarV(r, p);
    q = Obj.qMul(r, q);
  }
  c.g.position = p;
  c.g.rotation = Obj.euler(q);
}

// ── Girar ──────────────────────────────────────────────────────────────────
let historia = [];           // ["U", "R'", ...]
let cola = [];               // giros por hacer: { cara, dir, rapido, historia }
let animando = null;         // { cara, dir, t0, dura, capa }
let movidas = 0;

const enCapa = (c, cara) => (cara.eje === "x" ? c.p.x : cara.eje === "y" ? c.p.y : c.p.z) === cara.capa;
/** El giro de un cuarto en el sentido del reloj, mirando la cara: -90° sobre su normal. */
const anguloDe = (dir) => (-Math.PI / 2) * dir;

function aplicar(caraId, dir) {
  const cara = CARAS[caraId];
  const r = Obj.qEje(cara.n, anguloDe(dir));
  for (const c of cubitos) {
    if (!enCapa(c, cara)) continue;
    const p = Obj.rotarV(r, c.p);
    c.p = { x: Math.round(p.x), y: Math.round(p.y), z: Math.round(p.z) };
    const q = Obj.qMul(r, c.q);
    const m = Math.hypot(q.x, q.y, q.z, q.w);
    c.q = { x: q.x / m, y: q.y / m, z: q.z / m, w: q.w / m };
    dibujar(c);
  }
}
const nombre = (cara, dir) => cara + (dir < 0 ? "'" : "");

function resuelto() {
  return cubitos.every((c) => {
    const centro = Math.abs(c.casa.x) + Math.abs(c.casa.y) + Math.abs(c.casa.z) === 1;
    if (centro) return true;
    return c.p.x === c.casa.x && c.p.y === c.casa.y && c.p.z === c.casa.z && Math.abs(c.q.w) > 0.9999;
  });
}
...
const anim = Obj.animador(() => {
  ...
  if (!animando) {
    ...
    if (!sig) return false;
    animando = { ...sig, t0: ahora, dura: sig.rapido ? 70 : 180, capa: cubitos.filter((c) => enCapa(c, CARAS[sig.cara])) };
  }
  const k = Math.min(1, (ahora - animando.t0) / animando.dura);
  const cara = CARAS[animando.cara];
  for (const c of animando.capa) dibujar(c, cara.n, anguloDe(animando.dir) * Obj.suave(k));
  if (k >= 1) {
    const a = animando;
    animando = null;
    aplicar(a.cara, a.dir);
    terminarGiro(a);
  }
  return true;
});

function terminarGiro(a) {
  if (a.historia) {
    // Un giro que deshace el anterior lo borra de la historia.
    const ultimo = historia[historia.length - 1];
    if (ultimo === nombre(a.cara, -a.dir)) historia.pop();
    else historia.push(nombre(a.cara, a.dir));
    // Las vueltas de mezclar y de resolver no cuentan como movidas del que juega.
    if (!a.rapido) movidas++;
    Obj.emitir("giro", { cara: nombre(a.cara, a.dir), movidas });
  }
  if (!cola.length) {
    if (resuelto()) {
      if (historia.length || movidas || a.rapido) {
        const m = movidas;
        historia = [];
        movidas = 0;
        Obj.$("cuenta").setAttribute("value", "¡Resuelto!");
        Obj.emitir("resuelto", { movidas: m });
      }
    } else Obj.$("cuenta").setAttribute("value", movidas ? movidas + " movidas" : "");
    guardado.guardar(historia.join(" "));
  }
}

function girar(cara, dir, rapido, conHistoria = true) {
  if (!CARAS[cara]) return;
  cola.push({ cara, dir: dir < 0 ? -1 : 1, rapido: !!rapido, historia: conHistoria });
  anim.iniciar();
}

function mezclar() {
  if (cola.length || animando) return;
  const letras = Object.keys(CARAS);
  let antes = "";
  for (let i = 0; i < 20; i++) {
    let c;
    do c = letras[Math.floor(Math.random() * 6)]; while (c === antes);
    antes = c;
    girar(c, Math.random() < 0.5 ? 1 : -1, true);
  }
}
function resolver() {
  if (cola.length || animando || !historia.length) return;
  // Deshacer de atrás para adelante; la historia se va vaciando sola.
  for (const m of [...historia].reverse()) girar(m[0], m.endsWith("'") ? 1 : -1, true);
}

// ── Tocar ──────────────────────────────────────────────────────────────────
// Un blanco que envuelve el cubo: la cara es el eje dominante del punto tocado.
const donde = Obj.marco();
const blanco = Obj.crear("box", { sx: LADO + 0.006, sy: LADO + 0.006, sz: LADO + 0.006, color: "transparent", "material-alpha": "blend" }, cuboG);
Obj.boton(blanco, (e) => {
  if (!Number.isFinite(e.x)) return girar("F", 1);
  const l = donde.aLocal({ x: e.x, y: e.y, z: e.z });
  const d = { x: l.x, y: l.y - (0.05 + LADO / 2), z: l.z };
  const ax = Math.abs(d.x), ay = Math.abs(d.y), az = Math.abs(d.z);
  const cara = ax >= ay && ax >= az ? (d.x > 0 ? "R" : "L") : ay >= az ? (d.y > 0 ? "U" : "D") : (d.z > 0 ? "F" : "B");
  girar(cara, 1);
});
Obj.boton(Obj.$("mezclar"), mezclar, { color: "#0A84FF" });
Obj.boton(Obj.$("resolver"), resolver, { color: "#30D158" });
Obj.mensaje("mezclar", mezclar);
Obj.mensaje("resolver", resolver);
Obj.mensaje("girar", (d) => {
  if (!d || !CARAS[d.cara]) return;
  const v = Math.round(Number(d.vueltas) || 1);
  for (let i = 0; i < Math.abs(v); i++) girar(d.cara, Math.sign(v));
});

// ── Arranque: lo que quedó guardado ────────────────────────────────────────
for (const c of cubitos) dibujar(c);
const previo = String(guardado.leer("") || "").trim();
if (previo) {
  for (const m of previo.split(/\s+/)) {
    if (!CARAS[m[0]]) continue;
    aplicar(m[0], m.endsWith("'") ? -1 : 1);
    historia.push(m);
  }
  movidas = historia.length;
  Obj.$("cuenta").setAttribute("value", movidas + " movidas");
}
