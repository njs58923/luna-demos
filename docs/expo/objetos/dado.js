...
;
// El dado: salta, da vueltas y cae con una cara arriba.
//
// La caída se resuelve al revés: primero se elige el número y la orientación
// final (esa cara hacia +Y, con un giro al azar alrededor de la vertical), y
// después se anima desde donde estaba sumando vueltas que se van apagando.
// Así el dado nunca termina torcido ni apoyado en una arista.
const P = Obj.props({ color: "#F4F1EA", puntos: "#1C1C1C", tam: 0.1 });

const salto = Obj.$("salto");
const cuerpo = Obj.$("cuerpo");
const resultado = Obj.$("resultado");
const L = Math.max(0.03, Math.min(0.4, Number(P.tam) || 0.1));

// Las caras: normal local y el número que muestra. Opuestas suman siete.
const CARAS = [
  { n: 1, eje: Obj.v3(0, 1, 0) }, { n: 6, eje: Obj.v3(0, -1, 0) },
  { n: 3, eje: Obj.v3(1, 0, 0) }, { n: 4, eje: Obj.v3(-1, 0, 0) },
  { n: 2, eje: Obj.v3(0, 0, 1) }, { n: 5, eje: Obj.v3(0, 0, -1) },
];
// Dónde van los puntos en cada cara, en una grilla de -1..1.
const PATRON = {
...
const caja = Obj.crear("box", { sx: L, sy: L, sz: L, "border-radius": L * 0.14, color: Obj.color(P.color, "#F4F1EA") }, cuerpo);
const pips = [];
for (const c of CARAS) {
  const e = c.eje;
  // Dos ejes sobre la cara, para repartir los puntos.
  const u = Math.abs(e.y) > 0.5 ? Obj.v3(1, 0, 0) : Obj.v3(0, 1, 0);
  const w = Obj.v3(e.y * u.z - e.z * u.y, e.z * u.x - e.x * u.z, e.x * u.y - e.y * u.x);
  const paso = L * 0.26, d = L * 0.18;
  for (const [a, b] of PATRON[c.n]) {
    const p = {
      x: e.x * (L / 2) + u.x * a * paso + w.x * b * paso,
      y: e.y * (L / 2) + u.y * a * paso + w.y * b * paso,
      z: e.z * (L / 2) + u.z * a * paso + w.z * b * paso,
    };
    // Un botón chato: una esfera aplastada en la dirección de la normal.
    const aplast = { x: Math.abs(e.x) > 0.5 ? d * 0.3 : d, y: Math.abs(e.y) > 0.5 ? d * 0.3 : d, z: Math.abs(e.z) > 0.5 ? d * 0.3 : d };
    pips.push(Obj.crear("sphere", { x: p.x, y: p.y, z: p.z, sx: aplast.x, sy: aplast.y, sz: aplast.z, color: Obj.color(P.puntos, "#1C1C1C") }, cuerpo));
  }
}

/** La rotación que deja la cara `c` hacia arriba. */
function haciaArriba(c) {
  const e = c.eje;
  if (e.y > 0.5) return { x: 0, y: 0, z: 0, w: 1 };
  if (e.y < -0.5) return Obj.qEje(Obj.X, Math.PI);
  if (e.x > 0.5) return Obj.qEje(Obj.Z, Math.PI / 2);
  if (e.x < -0.5) return Obj.qEje(Obj.Z, -Math.PI / 2);
  if (e.z > 0.5) return Obj.qEje(Obj.X, -Math.PI / 2);
  return Obj.qEje(Obj.X, Math.PI / 2);
}

let q = { x: 0, y: 0, z: 0, w: 1 };
let tiro = null;   // { t0, q0, q1, vueltas: [ejes], valor }
salto.position = { x: 0, y: L / 2, z: 0 };

const anim = Obj.animador((t) => {
  if (!tiro) return false;
  if (!tiro.t0) tiro.t0 = t;
  const DUR = 1.1;
  const k = Math.min(1, (t - tiro.t0) / DUR);
  // Salto: una parábola con un rebote chico al final.
  const alto = k < 0.75 ? Math.sin((k / 0.75) * Math.PI) * L * 3.2 : Math.sin(((k - 0.75) / 0.25) * Math.PI) * L * 0.4;
  salto.position = { x: 0, y: L / 2 + alto, z: 0 };
  // Giro: el final más unas vueltas que se apagan.
  const e = Obj.salida(k);
  let qq = Obj.qSlerp(tiro.q0, tiro.q1, e);
  const resto = (1 - e);
  for (const v of tiro.vueltas) qq = Obj.qMul(Obj.qEje(v.eje, v.ang * resto), qq);
  cuerpo.rotation = Obj.euler(qq);
  if (k >= 1) {
    q = tiro.q1;
    cuerpo.rotation = Obj.euler(q);
    resultado.setAttribute("value", String(tiro.valor));
    resultado.setAttribute("visible", "inherit");
    Obj.emitir("tirada", { valor: tiro.valor });
    tiro = null;
    setTimeout(() => { if (!tiro) resultado.setAttribute("visible", "false"); }, 2500);
    return false;
  }
  return true;
});
...
function tirar() {
  if (tiro) return;
  const c = CARAS[Math.floor(Math.random() * 6)];
  const giro = Obj.qEje(Obj.Y, Math.random() * Math.PI * 2);
  const q1 = Obj.qMul(giro, haciaArriba(c));
  const rnd = () => { const v = Obj.v3(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5); const m = Math.hypot(v.x, v.y, v.z) || 1; return Obj.v3(v.x / m, v.y / m, v.z / m); };
  tiro = { t0: 0, q0: q, q1, valor: c.n, vueltas: [{ eje: rnd(), ang: Math.PI * (4 + Math.random() * 3) }] };
  resultado.setAttribute("visible", "false");
  anim.iniciar();
}

Obj.boton(Obj.$("toque"), tirar);
Obj.mensaje("tirar", tirar);
Obj.alCambiar((p) => {
  caja.setAttribute("color", Obj.color(p.color, "#F4F1EA"));
  for (const el of pips) el.setAttribute("color", Obj.color(p.puntos, "#1C1C1C"));
});
