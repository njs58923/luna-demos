...
;
// El tablero de dardos: sectores de verdad, y dardos que vuelan.
//
// El tablero se dibuja con cuñas hechas de tiras radiales (como la ruleta),
// alternando negro y crema, y los anillos de doble y triple en rojo y verde.
// El puntaje sale de la geometría del impacto: el ángulo da el sector (con el
// 20 arriba, como siempre) y la distancia al centro da el anillo.
const P = Obj.props({ pulso: 0.35, clave: "" });

const R = 0.225;   // radio del área de juego
const ORDEN = [20, 1, 18, 4, 13, 6, 10, 15, 2, 17, 3, 19, 7, 16, 8, 11, 14, 9, 12, 5];
// Anillos (radios relativos a R): centro, 25, simple, triple, simple, doble.
const ANILLOS = { bull: 0.037, veinticinco: 0.094, tripleIn: 0.582, tripleOut: 0.629, dobleIn: 0.953, dobleOut: 1 };
const memo = Obj.memoria("record");
const donde = Obj.marco();
const tablero = Obj.$("tablero");
const clavadosG = Obj.$("clavados");

// ── El tablero ──────────────────────────────────────────────────────────────
const anchoSector = (Math.PI * 2) / 20;
function cuna(r0, r1, a, color, z) {
  // Una cuña de r0 a r1 centrada en el ángulo a (desde +Y, horario), con
  // tiras radiales.
  const g = Obj.crear("group", { rz: -a, z }, tablero);
  const TIRAS = Math.max(1, Math.ceil((r1 - r0) / 0.03));
  for (let k = 0; k < TIRAS; k++) {
    const a0 = r0 + ((r1 - r0) * k) / TIRAS, a1 = r0 + ((r1 - r0) * (k + 1)) / TIRAS;
    const w = 2 * Math.tan(anchoSector / 2) * a1 + 0.002;
    Obj.crear("box", { y: (a0 + a1) / 2, sx: w, sy: a1 - a0 + 0.001, sz: 0.002, color }, g);
  }
}
ORDEN.forEach((n, i) => {
  const a = i * anchoSector;
  const oscuro = i % 2 === 0;
  const base = oscuro ? "#1C1C1C" : "#F2E6C9";
  const color = oscuro ? "#C8102E" : "#1E8E3E";
  cuna(ANILLOS.veinticinco * R, ANILLOS.tripleIn * R, a, base, 0);
  cuna(ANILLOS.tripleIn * R, ANILLOS.tripleOut * R, a, color, 0.0005);
  cuna(ANILLOS.tripleOut * R, ANILLOS.dobleIn * R, a, base, 0);
  cuna(ANILLOS.dobleIn * R, R, a, color, 0.0005);
  const rn = R * 1.08;
  Obj.crear("text", { x: Math.sin(a) * rn, y: Math.cos(a) * rn, value: String(n), size: 0.022, color: "#F2F2EE" }, Obj.$("numeros"));
});
Obj.crear("cylinder", { z: 0.001, rx: Math.PI / 2, sx: ANILLOS.veinticinco * R * 2, sy: 0.002, sz: ANILLOS.veinticinco * R * 2, color: "#1E8E3E" }, tablero);
Obj.crear("cylinder", { z: 0.0015, rx: Math.PI / 2, sx: ANILLOS.bull * R * 2, sy: 0.002, sz: ANILLOS.bull * R * 2, color: "#C8102E" }, tablero);

/** El puntaje de un punto del tablero (x, y en metros, centro en 0). */
function puntaje(x, y) {
  const r = Math.hypot(x, y) / R;
  if (r > 1) return { puntos: 0, sector: 0, anillo: "afuera" };
  if (r <= ANILLOS.bull) return { puntos: 50, sector: 25, anillo: "centro" };
  if (r <= ANILLOS.veinticinco) return { puntos: 25, sector: 25, anillo: "veinticinco" };
  // Ángulo desde +Y en sentido horario, con el 20 centrado arriba.
  let a = Math.atan2(x, y);
  if (a < 0) a += Math.PI * 2;
  const i = Math.floor((a + anchoSector / 2) / anchoSector) % 20;
  const s = ORDEN[i];
  if (r >= ANILLOS.dobleIn) return { puntos: s * 2, sector: s, anillo: "doble" };
  if (r >= ANILLOS.tripleIn && r <= ANILLOS.tripleOut) return { puntos: s * 3, sector: s, anillo: "triple" };
  return { puntos: s, sector: s, anillo: "simple" };
}

// ── Los dardos ──────────────────────────────────────────────────────────────
const COLORES = ["#FF3B30", "#0A84FF", "#FFD60A"];
function crearDardo(color) {
  const g = Obj.crear("group", {}, clavadosG);
  Obj.crear("cylinder", { z: 0.02, rx: Math.PI / 2, sx: 0.004, sy: 0.04, sz: 0.004, color: "#C0C0C0" }, g);
  Obj.crear("cylinder", { z: 0.065, rx: Math.PI / 2, sx: 0.01, sy: 0.05, sz: 0.01, color: "#2A2A2A" }, g);
  for (const r of [0, Math.PI / 2]) Obj.crear("box", { z: 0.1, rz: r, sx: 0.03, sy: 0.002, sz: 0.03, color }, g);
  return g;
}
const dardos = COLORES.map((c) => ({ el: crearDardo(c), estado: "listo", p: null, v: null }));
const ronda = { tiros: 0, total: 0 };
let record = Math.max(0, Number(memo.leer(0)) || 0);

function portadardos(i) { return { x: -0.08 + i * 0.08, y: -0.3, z: 0.05 }; }
/** Un dardo parado en su agujero del portadardos, con la punta abajo. */
function aSuLugar(d) {
  const p = portadardos(dardos.indexOf(d));
  d.estado = "listo";
  d.el.position = { x: p.x, y: p.y - 0.025, z: p.z - 0.025 };
  d.el.rotation = { x: -Math.PI / 2, y: 0, z: 0 };
}
function aLaRepisa() { for (const d of dardos) aSuLugar(d); }
function dibujarPuntaje(ultimo) {
  Obj.$("puntaje").setAttribute("value", ultimo || ("ronda: " + ronda.total));
  Obj.$("record").setAttribute("value", "récord " + record + " · dardo " + Math.min(3, ronda.tiros + (ronda.tiros < 3 ? 1 : 0)) + "/3");
}

function clavar(d, x, y) {
  d.estado = "clavado";
  d.el.position = { x, y, z: 0 };
  d.el.rotation = { x: 0, y: 0, z: Math.random() * 6 };
  const pz = puntaje(x, y);
  ronda.tiros++;
  ronda.total += pz.puntos;
  Obj.emitir("dardo", pz);
  dibujarPuntaje(pz.puntos ? (pz.anillo === "doble" ? "doble " : pz.anillo === "triple" ? "triple " : "") + pz.sector + " = " + pz.puntos : "afuera");
  if (ronda.tiros >= 3) {
    if (ronda.total > record) { record = ronda.total; memo.guardar(record); }
    Obj.emitir("ronda", { total: ronda.total, record });
    setTimeout(() => { Obj.$("puntaje").setAttribute("value", "total: " + ronda.total); }, 1200);
    setTimeout(nueva, 3500);
  }
}
function nueva() {
  ronda.tiros = 0; ronda.total = 0;
  aLaRepisa();
  dibujarPuntaje("tirá: 3 dardos");
}

// ── Vuelo ───────────────────────────────────────────────────────────────────
const anim = Obj.animador((t, dt) => {
  let alguno = false;
  for (const d of dardos) {
    if (d.estado !== "vuela") continue;
    alguno = true;
    const PASOS = 3, h = dt / PASOS;
    for (let k = 0; k < PASOS && d.estado === "vuela"; k++) {
      // Caída exacta en el paso (no Euler): con Euler, un tiro de 0.3 s
      // llega 8 mm más abajo, y eso es la mitad del anillo de triples.
      const z0 = d.p.z;
      d.p.x += d.v.x * h; d.p.y += d.v.y * h - 4.9 * h * h; d.p.z += d.v.z * h;
      d.v.y -= 9.8 * h;
      if (z0 > 0 && d.p.z <= 0) {
        // Cruza el plano del tablero: se interpola dónde.
        const k2 = z0 / (z0 - d.p.z);
        const x = d.p.x - d.v.x * h * (1 - k2), y = d.p.y - d.v.y * h * (1 - k2);
        if (Math.hypot(x, y) <= R * 1.12) clavar(d, x, y);
        else { d.estado = "caido"; ronda.tiros++; dibujarPuntaje("afuera"); if (ronda.tiros >= 3) setTimeout(nueva, 2500); }
      }
      if (d.p.y < -1.6 || d.p.z > 4) { d.estado = "caido"; }
    }
    if (d.estado === "vuela") {
      d.el.position = d.p;
      // Apunta hacia donde va.
      const m = Math.hypot(d.v.x, d.v.y, d.v.z) || 1;
      d.el.rotation = { x: Math.asin(Math.max(-1, Math.min(1, d.v.y / m))), y: -Math.atan2(d.v.x, -d.v.z), z: 0 };
    } else if (d.estado === "caido") d.el.position = { x: 0, y: -10, z: 0 };
  }
  return alguno;
});

function tirar(d, p, v) {
  d.estado = "vuela";
  d.p = { x: p.x, y: p.y, z: p.z };
  d.v = { x: v.x, y: v.y, z: v.z };
  anim.iniciar();
}

// Con el mouse: el próximo dardo sale de 1.8 m adelante hacia el punto
// tocado, con el temblor del pulso.
Obj.boton(Obj.$("toque"), (e) => {
  const d = dardos.find((x) => x.estado === "listo");
  if (!d || ronda.tiros >= 3) return;
  let tx = 0, ty = 0;
  const tl = Obj.toqueLocal(e);
  if (tl) { tx = tl[0] * 0.5; ty = tl[1] * 0.5; }
  const pulso = Math.max(0, Math.min(1, Number(P.pulso) || 0));
  tx += (Math.random() - 0.5) * 0.08 * pulso;
  ty += (Math.random() - 0.5) * 0.08 * pulso;
  // Un tiro de 0.3 s: se compensa la caída.
  const T = 0.3, z0 = 1.8;
  tirar(d, { x: tx, y: ty, z: z0 }, { x: 0, y: 0.5 * 9.8 * T, z: -z0 / T });
});

// En VR: agarrar un dardo del portadardos y arrojarlo.
let enMano = null;
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca: (m) => dardos.some((d, i) => d.estado === "listo" && Math.hypot(m.x - portadardos(i).x, m.y - portadardos(i).y, m.z - portadardos(i).z) < 0.12),
  tomar: (m) => {
    enMano = dardos.find((d, i) => d.estado === "listo" && Math.hypot(m.x - portadardos(i).x, m.y - portadardos(i).y, m.z - portadardos(i).z) < 0.12);
    if (enMano) enMano.estado = "mano";
  },
  mover: (m) => { if (enMano) { enMano.el.position = m; enMano.el.rotation = { x: 0, y: 0, z: 0 }; } },
  soltar: (m, v) => {
    if (!enMano) return;
    const d = enMano;
    enMano = null;
    // Soltado sin tirar: vuelve a su lugar, sin tocar los que ya se clavaron.
    if (Math.hypot(v.x, v.y, v.z) < 0.8) { aSuLugar(d); return; }
    tirar(d, m, v);
  },
});
