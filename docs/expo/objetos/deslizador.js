...
;
// El deslizador: una posición a lo largo de un riel.
//
// Todo se arma horizontal dentro de un grupo; vertical es el mismo grupo
// girado un cuarto de vuelta (el mínimo queda abajo). Las cuentas se hacen en
// la coordenada del riel: la mano o el clic se pasan al marco del grupo
// girándolos al revés.
const P = Obj.props({ orientacion: "horizontal", min: 0, max: 1, pasos: 0, inicial: null, color: "#2EC4B6", etiqueta: "", clave: "" });

const sinte = Obj.sinte({ volumen: 0.5 });
const donde = Obj.marco();
const guardado = Obj.memoria("fraccion");
const perilla = Obj.$("perilla"), relleno = Obj.$("relleno"), todo = Obj.$("todo");
const LARGO = 0.24, Y = 0.008;
...
function ajustar(f) { f = Math.max(0, Math.min(1, f)); const n = pasos(); return n ? Math.round(f * n) / n : f; }
const valorDe = (f) => min() + (max() - min()) * f;
/** Del objeto al riel: la coordenada a lo largo (de -LARGO/2 a +LARGO/2). */
const aRiel = (l) => (vertical() ? l.y : l.x);
const texto = (v) => (pasos() && Number.isInteger((max() - min()) / pasos()) ? v.toFixed(0) : v.toFixed(2));
...
let visto = fraccion, tomado = false;

const marcas = [];
function armar() {
  todo.rotation = { x: 0, y: 0, z: vertical() ? Math.PI / 2 : 0 };
  // Vertical, los textos van a la derecha del riel, parados.
  Obj.$("lectura").position = vertical() ? { x: 0.07, y: 0.1, z: 0.021 } : { x: 0.09, y: -0.035, z: 0.021 };
  Obj.$("etiqueta").position = vertical() ? { x: 0.07, y: -0.12, z: 0.021 } : { x: -0.06, y: -0.035, z: 0.021 };
  for (const m of marcas) m.remove();
  marcas.length = 0;
  const n = pasos() || 10;
  for (let i = 0; i <= n; i++) marcas.push(Obj.crear("box", { x: -LARGO / 2 + (i / n) * LARGO, y: -0.016, sx: 0.0022, sy: i % 5 === 0 || i === n ? 0.01 : 0.006, sz: 0.002, color: "#8E9CB4" }, Obj.$("marcas")));
}
function dibujar() {
  const x = -LARGO / 2 + visto * LARGO;
  perilla.position = { x, y: Y, z: 0.024 };
  relleno.position = { x: -LARGO / 2 + (visto * LARGO) / 2, y: Y, z: 0.0245 };
  relleno.scale = { x: Math.max(0.001, visto * LARGO), y: 0.006, z: 0.002 };
}
function poner(f, avisar) {
  const nf = ajustar(f);
  if (Math.abs(nf - fraccion) < 1e-9) return;
  if (pasos()) sinte.nota(2000, 0.012, { tipo: "pulso", vol: 0.2 });
  fraccion = nf;
...
const anim = Obj.animador((t, dt) => {
  visto += (fraccion - visto) * (1 - Math.exp(-dt * (tomado ? 40 : 14)));
  if (Math.abs(fraccion - visto) < 1e-4) visto = fraccion;
  dibujar();
  return visto !== fraccion || tomado;
});

Obj.boton(Obj.$("toque"), (e) => {
  if (!Number.isFinite(e.x)) return;
  const l = donde.aLocal({ x: e.x, y: e.y, z: e.z });
  poner((aRiel(l) + LARGO / 2) / LARGO);
  anim.iniciar();
});

// VR: la corredera de las manos (_vr.js). El riel sigue a la orientación.
Obj.manos().corredera({
  desde: () => (vertical() ? { x: -Y, y: -LARGO / 2, z: 0.02 } : { x: -LARGO / 2, y: Y, z: 0.02 }),
  hasta: () => (vertical() ? { x: -Y, y: LARGO / 2, z: 0.02 } : { x: LARGO / 2, y: Y, z: 0.02 }),
  t: () => fraccion, alcance: 0.06,
  alTomar() { tomado = true; anim.iniciar(); },
  alMover(t) { poner(t); },
  alSoltar() { tomado = false; anim.iniciar(); },
});
...
function pintar() {
  const c = Obj.color(P.color, "#2EC4B6");
  relleno.setAttribute("color", c);
  Obj.$("etiqueta").setAttribute("value", String(P.etiqueta || "").slice(0, 14));
  Obj.$("lectura").setAttribute("value", texto(valorDe(fraccion)));
}
Obj.alCambiar(() => { armar(); pintar(); dibujar(); });
armar();
pintar();
dibujar();
