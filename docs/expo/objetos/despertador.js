...
;
// El despertador: cuatro dígitos de siete segmentos y una alarma.
//
// Los segmentos son cajas que se prenden y se apagan cambiando su color entre
// el encendido y un gris casi negro —el "fantasma" que tienen las pantallas de
// verdad—, en vez de esconderse: así se lee como un display y no como números
// que aparecen y desaparecen. Sólo se escribe lo que cambió.
const P = Obj.props({ color: "#E63946", digitos: "#FF3B30", alarma: "07:00", activa: false, utc: null, clave: "" });

const memo = Obj.memoria("alarma");
const sinte = Obj.sinte({ volumen: 0.7 });
const numeros = Obj.$("numeros");
const led = Obj.$("led");
const leyenda = Obj.$("leyenda");
...
//  d
const SEG = {
  a: [0, 1, 1], b: [1, 0.5, 0], c: [1, -0.5, 0], d: [0, -1, 1], e: [-1, -0.5, 0], f: [-1, 0.5, 0], g: [0, 0, 1],
};
const DIGITO = {
  0: "abcdef", 1: "bc", 2: "abged", 3: "abgcd", 4: "fgbc", 5: "afgcd", 6: "afgedc", 7: "abc", 8: "abcdefg", 9: "abcdfg", "-": "g", " ": "",
};
const W = 0.016, H = 0.022, T = 0.0045;   // medio ancho, medio alto, espesor
const X = [-0.068, -0.03, 0.03, 0.068];
const digitos = X.map((x) => {
  const g = Obj.crear("group", { x }, numeros);
  const segs = {};
  for (const [k, [sx, sy, horiz]] of Object.entries(SEG)) {
    segs[k] = Obj.crear("box", {
      x: sx * W, y: sy * H, sx: horiz ? W * 1.7 : T, sy: horiz ? T : H * 0.85, sz: 0.002,
      color: "#1E2226", "material-unlit": "true",
    }, g);
    segs[k]._prendido = null;
  }
  return segs;
});
const dosPuntos = [0.009, -0.009].map((y) => Obj.crear("box", { y, sx: 0.005, sy: 0.005, sz: 0.002, color: "#1E2226", "material-unlit": "true" }, numeros));

const APAGADO = "#1E2226";
function pintarDigito(i, c, color) {
  const on = DIGITO[c] || "";
  for (const [k, el] of Object.entries(digitos[i])) {
    const quiere = on.includes(k) ? color : APAGADO;
    if (el._prendido !== quiere) { el._prendido = quiere; el.setAttribute("color", quiere); }
  }
}
let puntosPrendidos = null;
function pintarPuntos(on, color) {
  const quiere = on ? color : APAGADO;
  if (puntosPrendidos === quiere) return;
  puntosPrendidos = quiere;
  for (const el of dosPuntos) el.setAttribute("color", quiere);
}
function mostrar(texto, color, puntos) {
  const s = String(texto).padStart(4, " ").slice(-4);
  for (let i = 0; i < 4; i++) pintarDigito(i, s[i], color);
  pintarPuntos(puntos, color);
}

// ── La hora ─────────────────────────────────────────────────────────────────
function ahora() {
  const d = new Date();
  const utc = typeof P.utc === "number" && Number.isFinite(P.utc) ? P.utc : null;
  if (utc == null) return { h: d.getHours(), m: d.getMinutes(), s: d.getSeconds() };
  const u = new Date(d.getTime() + utc * 3600000);
  return { h: u.getUTCHours(), m: u.getUTCMinutes(), s: u.getUTCSeconds() };
}
const dos = (n) => String(n).padStart(2, "0");

function leerAlarma(txt) {
  const m = /^(\d{1,2}):(\d{2})$/.exec(String(txt || ""));
  if (!m) return { h: 7, m: 0 };
  return { h: Math.min(23, +m[1]), m: Math.min(59, +m[2]) };
}
const guardada = memo.leer(null);
let alarma = guardada ? { h: guardada.h, m: guardada.m } : leerAlarma(P.alarma);
let activa = guardada ? !!guardada.activa : !!P.activa;
function guardar() { memo.guardar({ h: alarma.h, m: alarma.m, activa }); }

// ── Estados: la hora, la alarma a la vista, sonando ─────────────────────────
let viendoAlarma = 0;     // hasta cuándo se muestra la hora de la alarma
let sonando = false;
let ultimoMinutoSonado = -1;
let pitido = null;

function colorDigitos() { return Obj.color(P.digitos, "#FF3B30"); }

function refrescar() {
  const t = ahora();
  const c = colorDigitos();
  if (performance.now() < viendoAlarma) {
    mostrar(dos(alarma.h) + dos(alarma.m), Obj.tono(c, 0.35), true);
    leyenda.setAttribute("value", activa ? "alarma " + dos(alarma.h) + ":" + dos(alarma.m) : "alarma apagada");
  } else if (sonando) {
    // Titila entero, dos veces por segundo.
    const on = Math.floor(performance.now() / 250) % 2 === 0;
    mostrar(on ? dos(t.h) + dos(t.m) : "    ", c, on);
    leyenda.setAttribute("value", "¡arriba! tocá el botón");
  } else {
    mostrar(dos(t.h) + dos(t.m), c, t.s % 2 === 0);
    leyenda.setAttribute("value", "");
  }
  led.setAttribute("color", activa ? "#30D158" : "#3A3A3A");
  // ¿Hora de sonar? Una sola vez por minuto, y no si se acaba de apagar.
  if (activa && !sonando && t.h === alarma.h && t.m === alarma.m && ultimoMinutoSonado !== t.h * 60 + t.m) {
    ultimoMinutoSonado = t.h * 60 + t.m;
    sonar();
  }
}
...
function sonar() {
  sonando = true;
  Obj.emitir("alarma", { hora: dos(alarma.h) + ":" + dos(alarma.m) });
  let n = 0;
  clearInterval(pitido);
  // Tres pitidos, silencio, y otra vez: el ritmo de un despertador. Se apaga
  // solo al minuto si nadie lo toca.
  pitido = setInterval(() => {
    if (!sonando || n++ > 60) { apagar(); return; }
    for (let k = 0; k < 3; k++) sinte.nota(1760, 0.08, { tipo: "cuadrada", vol: 0.12, caida: 2, retraso: k * 0.13 });
  }, 1000);
}
function apagar() {
  sonando = false;
...
// ── Botones ─────────────────────────────────────────────────────────────────
function click() { sinte.nota(2400, 0.03, { tipo: "pulso", vol: 0.2 }); }
Obj.boton(Obj.$("b_apagar"), () => {
  click();
  if (sonando) { apagar(); return; }
  viendoAlarma = performance.now() + 2500;
  refrescar();
}, { color: "#F1FAEE" });
Obj.boton(Obj.$("b_hora"), () => {
  click();
  alarma.h = (alarma.h + 1) % 24; guardar();
  viendoAlarma = performance.now() + 2500; refrescar();
}, { color: "#1D3557" });
Obj.boton(Obj.$("b_minuto"), () => {
  click();
  alarma.m = (alarma.m + 5) % 60; guardar();
  viendoAlarma = performance.now() + 2500; refrescar();
}, { color: "#1D3557" });
Obj.boton(Obj.$("b_activa"), () => {
  click();
  activa = !activa; guardar();
  if (!activa) apagar();
  viendoAlarma = performance.now() + 2500; refrescar();
}, { color: "#457B9D" });
Obj.mensaje("apagar", apagar);

Obj.alCambiar((p, antes, cambios) => {
  Obj.$("cuerpo").setAttribute("color", Obj.color(p.color, "#E63946"));
  if (cambios.includes("alarma")) { alarma = leerAlarma(p.alarma); guardar(); }
  if (cambios.includes("activa")) { activa = !!p.activa; guardar(); }
  puntosPrendidos = null;
  for (const d of digitos) for (const el of Object.values(d)) el._prendido = null;
  refrescar();
});
Obj.$("cuerpo").setAttribute("color", Obj.color(P.color, "#E63946"));

// Un latido cada cuarto de segundo: alcanza para el titileo y los dos puntos,
// y no despierta al isolate en cada cuadro.
