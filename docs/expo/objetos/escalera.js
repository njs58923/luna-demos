...
;
// La escalera: sube hacia atrás desde el borde de adelante. Tocarla pasa al
// material siguiente (madera y metal con zancas; hormigón, maciza).
const P = Obj.props({ ancho: 1, alto: 1.2, escalones: 7, huella: 0.28, material: "madera", baranda: true, color: "" });
const ORDEN = ["madera", "hormigon", "metal"];
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let material = ORDEN.includes(P.material) ? P.material : "madera";

function dibujar() {
  const W = Obra.num(P.ancho, 1, 0.4, 6), H = Obra.num(P.alto, 1.2, 0.2, 8);
  const n = Math.round(Obra.num(P.escalones, 7, 2, 30)), D = n * Obra.num(P.huella, 0.28, 0.18, 0.5);
  const propio = material === P.material;
  armar(Obra.escalera({ ancho: W, alto: H, escalones: n, huella: P.huella, material, baranda: P.baranda !== false, color: propio ? P.color : "" }));
  toque.position = { x: 0, y: H / 2, z: -D / 2 };
  toque.scale = { x: W, y: H, z: D };
}
...
Obj.alCambiar((p, antes, cambios) => {
  if (cambios.includes("material") && ORDEN.includes(p.material)) material = p.material;
  dibujar();
});
dibujar();
