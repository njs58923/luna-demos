...
;
// La estantería: libros de adorno y libros que se leen.
//
// Los de adorno son cajas de colores con un azar fijo. Los que tienen texto
// llevan el lomo dorado; al tocar uno, el libro vuela del estante a un atril
// imaginario adelante y se abre en un "lector" de dos páginas. El texto se
// parte en renglones por palabras, y los renglones en páginas.
const P = Obj.props({
  madera: "#7F5539",
  libros: [
    { titulo: "Cómo llevar objetos", texto: "Cada objeto de esta calle es un documento que se puede incluir en cualquier mundo. En el atril de cada uno está su URL. Tocá COPIAR URL y apretá Ctrl+C: la dirección queda en el portapapeles. En tu mundo, tocá el buzón y apretá Ctrl+V. El objeto aparece, y con los mandos se lo acomoda: grip en el cuerpo para moverlo, en una esquina para escalarlo. Lo que el objeto guarda —una nota, un tanteo, una alarma— queda guardado aunque cierres todo." },
    { titulo: "Props", texto: "Un objeto se configura con props: un JSON en el atributo props del include. Colores, textos, tiempos. Si no las pasás, usa las suyas. También se pueden poner en la URL, después de un signo de pregunta: lampara.hsml?luz=%23FF0000 es una lámpara de luz roja. Así, copiar una URL es copiar un objeto ya configurado." },
    { titulo: "Eventos", texto: "Los objetos avisan lo que pasa: el dado avisa la tirada, el reloj la hora, el marcador el tanteo. Quien los incluye escucha component:tirada en el nodo include y hace lo que quiera con eso. Y se les puede hablar: include.send('tirar') tira el dado desde afuera. Así se arman cosas más grandes con piezas chicas." },
    { titulo: "Refranes", texto: "No por mucho madrugar amanece más temprano. Más vale tarde que nunca. En casa de herrero, cuchillo de palo. A caballo regalado no se le miran los dientes. El que busca, encuentra. Poco a poco se anda lejos. Cada loco con su tema." },
  ],
});

const W = 1.0, H = 1.8, D = 0.32, T = 0.025;
const mueble = Obj.$("mueble");
const grupoLibros = Obj.$("libros");
const lector = Obj.$("lector");
const renglones = Obj.$("renglones");
const madera = () => Obj.color(P.madera, "#7F5539");
const pintables = [];
const tabla = (a, k) => { const el = Obj.crear("box", Object.assign({ color: Obj.tono(madera(), k || 0) }, a), mueble); pintables.push([el, k || 0]); return el; };

tabla({ x: -W / 2 + T / 2, y: H / 2, sx: T, sy: H, sz: D });
tabla({ x: W / 2 - T / 2, y: H / 2, sx: T, sy: H, sz: D });
tabla({ y: H / 2, z: -D / 2 + 0.006, sx: W, sy: H, sz: 0.012 }, -0.3);
const ESTANTES = [0.06, 0.48, 0.9, 1.32, 1.76];
for (const y of ESTANTES) tabla({ y, sx: W, sy: T, sz: D }, 0.05);

// ── Los libros ──────────────────────────────────────────────────────────────
const COLORES = ["#6B1E2E", "#1D3557", "#2D6A4F", "#7F4F24", "#3C096C", "#9D0208", "#264653", "#B08968"];
const azar = Obj.azar(11);
function legibles() {
  const l = Array.isArray(P.libros) ? P.libros : [];
  return l.filter((b) => b && typeof b.titulo === "string").slice(0, 8).map((b) => ({ titulo: String(b.titulo).slice(0, 40), texto: String(b.texto || "").slice(0, 6000) }));
}
const libros = [];   // { el, x, y, w, h, color, lee: {titulo,texto} | null }
function armarLibros() {
  for (const l of libros) { l.el.remove(); if (l.lomo) l.lomo.remove(); }
  libros.length = 0;
  const lee = legibles();
  let cual = 0;
  for (let e = 0; e < 3; e++) {
    let x = -W / 2 + T + 0.02;
    const base = ESTANTES[e] + T / 2;
    while (x < W / 2 - T - 0.07) {
      const w = 0.025 + azar() * 0.035, h = 0.2 + azar() * 0.12;
      // Uno de cada cuatro, más o menos, es legible, hasta que se acaban.
      const esLegible = cual < lee.length && azar() < 0.3;
      const color = COLORES[Math.floor(azar() * COLORES.length)];
      const inclinado = !esLegible && azar() < 0.08;
      const el = Obj.crear("box", {
        x: x + w / 2, y: base + h / 2, z: 0.01, rz: inclinado ? -0.18 : 0,
        sx: w, sy: h, sz: 0.2, color,
      }, grupoLibros);
      let lomo = null;
      if (esLegible) {
        lomo = Obj.crear("box", { x: x + w / 2, y: base + h * 0.72, z: 0.111, sx: w * 0.8, sy: 0.02, sz: 0.004, color: "#D4AF37", "material-unlit": "true" }, grupoLibros);
      }
      const libro = { el, lomo, x: x + w / 2, y: base + h / 2, w, h, color, lee: esLegible ? lee[cual++] : null };
      if (libro.lee) Obj.boton(el, () => abrir(libro), { color });
      libros.push(libro);
      x += w + (inclinado ? 0.04 : 0.003);
    }
  }
}

// ── Leer ────────────────────────────────────────────────────────────────────
const SIZE = 0.017, POR_RENGLON = 30, POR_PAGINA = 14;
function paginar(texto) {
  const lineas = [];
  let actual = "";
  for (const p of texto.split(/\s+/).filter(Boolean)) {
    if ((actual + " " + p).trim().length > POR_RENGLON) { lineas.push(actual.trim()); actual = p; }
    else actual += " " + p;
  }
  if (actual.trim()) lineas.push(actual.trim());
  const paginas = [];
  for (let i = 0; i < lineas.length; i += POR_PAGINA) paginas.push(lineas.slice(i, i + POR_PAGINA));
  return paginas.length ? paginas : [[""]];
}
const nodosRenglon = [];
for (const lado of [-1, 1]) {
  for (let i = 0; i < POR_PAGINA + 1; i++) {
    nodosRenglon.push(Obj.crear("text", { x: lado * 0.152, y: 0.16 - i * 0.022, value: "", size: SIZE, color: "#2A2016" }, renglones));
  }
}

let abierto = null;   // { libro, paginas, hoja }
let vuelo = null;     // { libro, t0, ida }

/** Dónde queda el lector respecto del estante: vuela desde el lugar del libro. */
/** Cuánto mide un renglón: con el layout de la fuente si está, o a ojo. */
function ancho(t) {
  if (typeof TextLayout !== "undefined") {
    try { return TextLayout.create(t || " ", SIZE).w; } catch (e) { /* a ojo */ }
  }
  return t.length * SIZE * 0.5;
}
function dibujarPaginas() {
  const { libro, paginas, hoja } = abierto;
  // Hoja 0: a la izquierda el título, a la derecha la primera página. Las
  // siguientes: dos páginas de texto por hoja.
  const titulo = Array(POR_PAGINA + 1).fill("");
  titulo[5] = libro.lee.titulo.toUpperCase();
  const textoIzq = hoja === 0 ? titulo : (paginas[hoja * 2 - 1] || []);
  const textoDer = hoja === 0 ? paginas[0] : (paginas[hoja * 2] || []);
  for (let i = 0; i < POR_PAGINA + 1; i++) {
    for (const [lado, lineas, k] of [[-1, textoIzq, i], [1, textoDer, POR_PAGINA + 1 + i]]) {
      const txt = String(lineas[i] || "").slice(0, 36);
      nodosRenglon[k].setAttribute("value", txt);
      // El texto se centra solo en x: se lo corre para alinearlo a la izquierda,
      // salvo el título, que va centrado.
      const x = hoja === 0 && lado < 0 ? -0.152 : lado * 0.152 - 0.13 + ancho(txt) / 2;
      nodosRenglon[k].position = { x, y: 0.16 - i * 0.022, z: 0 };
    }
  }
  Obj.$("folio_izq").setAttribute("value", hoja === 0 ? "" : String(hoja * 2));
  Obj.$("folio_der").setAttribute("value", String(hoja * 2 + 1));
  Obj.$("tapa_izq").setAttribute("color", libro.color);
  Obj.$("tapa_der").setAttribute("color", libro.color);
}
function hojas() { return abierto ? Math.ceil((abierto.paginas.length + 1) / 2) : 0; }

const anim = Obj.animador((t) => {
  if (!vuelo) return false;
  if (!vuelo.t0) vuelo.t0 = t;
  const k = Math.min(1, (t - vuelo.t0) / 0.45);
  const e = Obj.suave(vuelo.ida ? k : 1 - k);
  const l = vuelo.libro;
  // El libro sale del estante (z sube) y vuela hacia el lector.
  l.el.position = { x: l.x * (1 - e), y: l.y + (1.25 - l.y) * e, z: 0.01 + 0.54 * e };
  l.el.rotation = { x: 0, y: -e * Math.PI / 2, z: 0 };
  if (k >= 1) {
    if (vuelo.ida) {
      l.el.setAttribute("visible", "false");
      lector.setAttribute("visible", "inherit");
    } else {
      l.el.position = { x: l.x, y: l.y, z: 0.01 };
      l.el.rotation = { x: 0, y: 0, z: 0 };
      if (l.lomo) l.lomo.setAttribute("visible", "inherit");
    }
    vuelo = null;
    return false;
  }
...
function abrir(libro) {
  if (vuelo || abierto) { if (abierto && abierto.libro === libro) cerrar(); return; }
  abierto = { libro, paginas: paginar(libro.lee.texto), hoja: 0 };
  dibujarPaginas();
  if (libro.lomo) libro.lomo.setAttribute("visible", "false");
  vuelo = { libro, t0: 0, ida: true };
  anim.iniciar();
  Obj.emitir("libro", { titulo: libro.lee.titulo, pagina: 1 });
}
function cerrar() {
  if (!abierto || vuelo) return;
  const libro = abierto.libro;
  abierto = null;
  lector.setAttribute("visible", "false");
  libro.el.setAttribute("visible", "inherit");
  vuelo = { libro, t0: 0, ida: false };
  anim.iniciar();
}
function pasar(k) {
  if (!abierto) return;
  const n = Math.max(0, Math.min(hojas() - 1, abierto.hoja + k));
  if (n === abierto.hoja) { if (k > 0) cerrar(); return; }
  abierto.hoja = n;
  dibujarPaginas();
  Obj.emitir("libro", { titulo: abierto.libro.lee.titulo, pagina: n * 2 + 1 });
}

Obj.boton(Obj.$("pagina_der"), () => pasar(1));
Obj.boton(Obj.$("pagina_izq"), () => pasar(-1));
Obj.boton(Obj.$("cinta"), cerrar, { color: "#C8102E" });
Obj.mensaje("cerrar", cerrar);
Obj.alCambiar((p, a, cambios) => {
  for (const [el, k] of pintables) el.setAttribute("color", Obj.tono(madera(), k));
  if (cambios.includes("libros")) { if (abierto) { abierto = null; lector.setAttribute("visible", "false"); } armarLibros(); }
});
armarLibros();
