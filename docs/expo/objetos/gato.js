...
;
// El gato: mirar, parpadear, mover la cola, ronronear y dormir.
//
// Todo corre en un solo intervalo de 20 cuadros por segundo. La cabeza busca
// un blanco —la mano que lo acaricia, si hay; si no, el visitante (con
// read_camera_pose); si no, un lugar al azar cada tanto— y va hacia él con
// suavidad. La cola es una cadena de tramos sobre el almohadón cuya punta se
// levanta y se mece: despacio si está contento, rápido si lo tocan de más.
// El ronroneo y el maullido son síntesis: ruido grave pulsado a 25 Hz para
// el primero, y armónicos con la altura que sube y baja ("mi-au") el segundo.
const P = Obj.props({ pelaje: "#F4A261", pecho: "#FFF1E0", ojos: "#9ACD32", cojin: "#8E7DBE", "sueño": 60 });

const sinte = Obj.sinte({ volumen: 0.8 });
const donde = Obj.marco();
const cabeza = Obj.$("cabeza"), ojos = Obj.$("ojos"), torso = Obj.$("torso_g"), colaG = Obj.$("cola");
const PIVOTE = { x: 0, y: 0.07 + 0.29, z: 0.05 };   // la cabeza, en coordenadas del objeto

// ── La cola ────────────────────────────────────────────────────────────────
const TRAMOS = 7;
const cola = [];
for (let k = 0; k < TRAMOS; k++) {
  const g = 0.034 - k * 0.002;
  cola.push({
    tramo: Obj.crear("cylinder", { sx: g, sy: 0.05, sz: g, color: "#F4A261", class: "pelaje" }, colaG),
    nudo: Obj.crear("sphere", { sx: g, sy: g, sz: g, color: "#F4A261", class: "pelaje" }, colaG),
    g,
  });
}
function dibujarCola(t, mecer, rapidez) {
  const pts = [];
  for (let k = 0; k <= TRAMOS; k++) {
    const s = k / TRAMOS;
    const f = -Math.PI / 2 + s * Math.PI * 0.65 + mecer * Math.pow(s, 1.5) * Math.sin(t * rapidez);
    const r = 0.15 + 0.02 * s;
    pts.push({ x: r * Math.cos(f), y: 0.025 + 0.06 * s * s * (0.5 + 0.5 * Math.sin(t * rapidez * 0.5)), z: r * Math.sin(f) - 0.02 });
  }
  cola.forEach((c, k) => {
    const l = Obj.tender(c.tramo, pts[k], pts[k + 1]);
    c.tramo.scale = { x: c.g, y: l + 0.004, z: c.g };
    c.nudo.position = pts[k + 1];
  });
}

// ── Estado ─────────────────────────────────────────────────────────────────
let durmiendo = false;
let ultimaAtencion = performance.now();
let ronroneaHasta = 0, ronroneando = false;
let toques = [];
let parpadeo = 0, proximoParpadeo = performance.now() + 3000;
let blanco = { yaw: 0, pitch: 0 }, mirada = { yaw: 0, pitch: 0 };
let proximoCapricho = 0;
let manoCerca = null, manoHasta = 0;
let enojo = 0;
let zetas = -1;

function mirarA(p) {
  const v = { x: p.x - PIVOTE.x, y: p.y - PIVOTE.y, z: p.z - PIVOTE.z };
  const yaw = Math.atan2(v.x, v.z);
  if (Math.abs(yaw) > 1.8) return false;   // detrás: no se da vuelta
  blanco = {
    yaw: Math.max(-1.1, Math.min(1.1, yaw)),
    pitch: Math.max(-0.5, Math.min(0.45, -Math.atan2(v.y, Math.hypot(v.x, v.z)))),
  };
  return true;
}
...
function paso() {
  ...
  // ¿A quién mira?
  const quien = Obj.visitante(donde);
  const distVisitante = quien ? Math.hypot(quien.x, quien.z) : Infinity;
  if (durmiendo && distVisitante < 1.0) despertar();
  // Alguien cerca lo mantiene despierto.
  if (!durmiendo && distVisitante < 1.5) ultimaAtencion = ahora;
  if (!durmiendo) {
    let mira = false;
    if (manoCerca && ahora < manoHasta) mira = mirarA(manoCerca);
    if (!mira && quien && distVisitante < 4) mira = mirarA(quien);
    if (!mira && ahora > proximoCapricho) {
      proximoCapricho = ahora + 2500 + Math.random() * 3000;
      blanco = { yaw: (Math.random() - 0.5) * 1.4, pitch: (Math.random() - 0.5) * 0.4 };
    }
    if (ahora - ultimaAtencion > 1000 * Math.max(5, Number(P["sueño"]) || 60)) dormir();
  } else blanco = { yaw: 0.35, pitch: 0.5 };
  const k = 1 - Math.exp(-dt * (durmiendo ? 1.5 : 5));
  mirada.yaw += (blanco.yaw - mirada.yaw) * k;
  mirada.pitch += (blanco.pitch - mirada.pitch) * k;
  cabeza.rotation = Obj.euler(Obj.qMul(Obj.qEje(Obj.Y, mirada.yaw), Obj.qEje(Obj.X, mirada.pitch)));

  // Ojos: dormido cerrados, ronroneando entornados, si no parpadea.
  if (!durmiendo && ahora > proximoParpadeo) { parpadeo = ahora; proximoParpadeo = ahora + 2500 + Math.random() * 4000; }
  let abre = 1;
  if (durmiendo) abre = 0.1;
  else if (ahora < ronroneaHasta) abre = 0.35;
  const b = (ahora - parpadeo) / 150;
  if (b < 1) abre = Math.min(abre, 0.1 + 0.9 * Math.abs(1 - 2 * b));
  ojos.scale = { x: 1, y: abre, z: 1 };

  // Respira.
  const r = 1 + (durmiendo ? 0.035 : 0.02) * Math.sin(t * (durmiendo ? 1.6 : 2.4));
  torso.scale = { x: 1, y: r, z: 1 };

  // La cola: tranquila, contenta o molesta.
  enojo = Math.max(0, enojo - dt * 0.5);
  if (durmiendo) dibujarCola(t, 0.05, 0.8);
  else if (enojo > 1) dibujarCola(t, 0.5, 9);
  else if (ahora < ronroneaHasta) dibujarCola(t, 0.35, 1.2);
  else dibujarCola(t, 0.22, 2.2);

  // El ronroneo se corta solo.
  if (ronroneando && ahora > ronroneaHasta) { ronroneando = false; sinte.flujo(null); }

  // Las zetas.
  if (durmiendo) {
    const n = Math.floor(t * 0.8) % 3;
    const z = Obj.$("zzz");
    if (n !== zetas) { zetas = n; z.setAttribute("value", ["z", "z Z", "z Z z"][n]); }
    z.position = { x: 0.1, y: 0.42 + ((t * 0.8) % 1) * 0.05, z: 0.05 };
  }
}
setInterval(paso, 50);
...
function ronroneo() {
  ...
  return (t) => {
    ...
    const n = semilla / 0x3fffffff - 1;
    lp += (n - lp) * 0.06;
    // Pulsos a 25 Hz, y una respiración que lo hace ir y venir.
    const pulso = Math.pow(0.5 + 0.5 * Math.sin(2 * Math.PI * 25 * t), 3);
    const resp = 0.55 + 0.45 * Math.sin(2 * Math.PI * 0.45 * t);
    const entra = Math.min(1, t * 3);
    return lp * pulso * resp * entra * 1.6;
  };
}
function maullido() {
  let fase = 0;
  ...
  return (t) => {
    if (t > dura) return 0;
    const s = t / dura;
    const f = 480 + 380 * Math.sin(Math.PI * Math.min(1, s * 1.25));
    fase += (2 * Math.PI * f) / 22050;
    // "mi" oscuro, "a" abierto, "u" que se cierra: el brillo sube y baja.
    const brillo = Math.sin(Math.PI * s);
    const v = Math.sin(fase) + 0.6 * brillo * Math.sin(2 * fase) + 0.4 * brillo * Math.sin(3 * fase) + 0.2 * brillo * Math.sin(4 * fase);
    const env = Math.min(1, t / 0.05) * Math.min(1, (dura - t) / 0.15);
    return v * env * 0.18;
  };
}
...
function acariciar() {
  const ahora = performance.now();
  ultimaAtencion = ahora;
  if (durmiendo) { despertar(); return; }
  ronroneaHasta = Math.max(ronroneaHasta, ahora + 4000);
  if (!ronroneando) {
    ronroneando = true;
    sinte.flujo(ronroneo());
    Obj.emitir("ronroneo", {});
  }
}
function maullar() {
  ultimaAtencion = performance.now();
  if (durmiendo) despertar();
  ronroneando = false;
  ronroneaHasta = 0;
  sinte.flujo(maullido());
  setTimeout(() => { if (!ronroneando) sinte.flujo(null); }, 900);
  Obj.emitir("miau", {});
}
function dormir() {
  if (durmiendo) return;
  durmiendo = true;
  ronroneando = false; ronroneaHasta = 0;
  sinte.flujo(null);
  Obj.emitir("duerme", {});
}
function despertar() {
  ...
  durmiendo = false;
  ultimaAtencion = performance.now();
  Obj.$("zzz").setAttribute("value", "");
  zetas = -1;
  Obj.emitir("despierta", {});
}

Obj.boton(Obj.$("toque"), () => {
  const ahora = performance.now();
  toques = toques.filter((x) => ahora - x < 2000);
  toques.push(ahora);
  if (toques.length >= 4) { toques = []; enojo = 3; maullar(); return; }
  acariciar();
});

// En VR: la mano que pasa despacio por el lomo o la cabeza lo acaricia.
const zona = Obj.$("zona");
let manoAntes = null, ultimaCaricia = 0;
if (zona) {
  zona.addEventListener("posemove", (e) => {
    const l = donde.aLocal({ x: e.px, y: e.py, z: e.pz });
    const ahora = performance.now();
    const dCabeza = Math.hypot(l.x - PIVOTE.x, l.y - PIVOTE.y, l.z - PIVOTE.z);
    if (dCabeza < 0.35) { manoCerca = l; manoHasta = ahora + 500; }
    // El lomo: un elipsoide un poco más grande que el torso.
    const q = { x: l.x / 0.14, y: (l.y - 0.2) / 0.17, z: (l.z + 0.02) / 0.18 };
    const enLomo = q.x * q.x + q.y * q.y + q.z * q.z < 1.6;
    const antes = manoAntes && manoAntes.mano === e.hand ? manoAntes : null;
    manoAntes = { mano: e.hand, p: l, t: ahora };
    if (!antes || (!enLomo && dCabeza > 0.11)) return;
    const v = Math.hypot(l.x - antes.p.x, l.y - antes.p.y, l.z - antes.p.z) / Math.max(0.001, (ahora - antes.t) / 1000);
    if (v > 0.1 && v < 2.5 && ahora - ultimaCaricia > 300) { ultimaCaricia = ahora; acariciar(); }
  });
}

Obj.mensaje("maullar", maullar);
Obj.mensaje("dormir", dormir);
Obj.mensaje("despertar", despertar);

function pintar() {
  const pel = Obj.color(P.pelaje, "#F4A261"), pe = Obj.color(P.pecho, "#FFF1E0"), oj = Obj.color(P.ojos, "#9ACD32");
  for (const el of Obj.root.getElementsByClass("pelaje")) el.setAttribute("color", pel);
  for (const c of cola) { c.tramo.setAttribute("color", pel); c.nudo.setAttribute("color", pel); }
  Obj.$("torso").setAttribute("color", pel);
  Obj.$("pecho").setAttribute("color", pe);
  for (const el of Obj.root.getElementsByClass("pecho")) el.setAttribute("color", pe);
  for (const el of Obj.root.getElementsByClass("ojo")) el.setAttribute("color", oj);
  Obj.$("cojin").setAttribute("color", Obj.color(P.cojin, "#8E7DBE"));
}
