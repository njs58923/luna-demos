...
;
// El globo terráqueo: manchas de tierra sobre una esfera, un eje inclinado y
// un giro con rozamiento.
//
// Los continentes no son un mapa: son grupos de parches (discos chatos
// tangentes a la esfera) ubicados por latitud y longitud aproximadas, lo
// justo para que se reconozca qué es. El giro va alrededor del eje propio,
// que está inclinado 23.4° como el de la Tierra.
const P = Obj.props({ mar: "#1E6091", tierra: "#76C893", pie: "#6B4A30" });

const R = 0.15;
const tierra = Obj.$("tierra");
const continentes = Obj.$("continentes");
const donde = Obj.marco();

// [latitud, longitud, radio angular] de cada parche, en grados.
const MANCHAS = [
  // América del Norte
  [50, -100, 16], [40, -95, 13], [60, -120, 12], [65, -150, 8], [30, -105, 9], [20, -100, 7], [55, -75, 10], [70, -80, 7],
  // América del Sur
  [0, -60, 12], [-10, -55, 12], [-25, -60, 9], [-35, -65, 6], [-45, -70, 4], [5, -73, 6],
  // Europa
  [50, 10, 9], [45, 25, 8], [60, 25, 7], [40, -3, 5], [55, -3, 3],
  // África
  [10, 20, 14], [0, 25, 12], [-15, 25, 10], [-28, 25, 7], [20, 0, 10], [25, 30, 7], [5, 40, 6],
  // Asia
  [55, 90, 18], [45, 70, 14], [35, 100, 13], [25, 80, 9], [60, 130, 12], [30, 45, 8], [15, 100, 6], [35, 135, 4],
  // Oceanía
  [-25, 135, 11], [-20, 125, 7], [-40, 175, 3], [-6, 140, 4],
  // Antártida
  [-82, 0, 14], [-80, 90, 14], [-80, 180, 14], [-80, -90, 14],
  // Groenlandia
  [72, -40, 7],
];
const parches = [];
const rad = (g) => (g * Math.PI) / 180;
for (const [lat, lon, ang] of MANCHAS) {
  const la = rad(lat), lo = rad(lon);
  const n = { x: Math.cos(la) * Math.sin(lo), y: Math.sin(la), z: Math.cos(la) * Math.cos(lo) };
  const q = Obj.qMul(Obj.qEje(Obj.Y, lo), Obj.qEje(Obj.X, -la));
  const e = Obj.euler(q);
  const d = 2 * R * Math.sin(rad(ang));
  parches.push(Obj.crear("sphere", {
    x: n.x * R * 0.97, y: n.y * R * 0.97, z: n.z * R * 0.97, rx: e.x, ry: e.y, rz: e.z,
    sx: d, sy: d, sz: 0.012, color: lat < -65 ? "#F1F5F9" : "#76C893",
  }, continentes));
}
// El meridiano: medio aro de metal que sostiene el globo, alrededor del eje.
for (let i = 0; i <= 16; i++) {
  const a = (i / 16) * Math.PI;
  Obj.crear("box", { x: Math.sin(a) * (R + 0.012), y: Math.cos(a) * (R + 0.012), rz: -a, sx: 0.008, sy: 0.022, sz: 0.012, color: "#C9A45C" }, Obj.$("meridiano"));
}

let giro = 0, omega = 0;
let enMano = false, manoAntes = null;
const anim = Obj.animador((t, dt) => {
  if (!enMano) omega *= Math.exp(-0.35 * dt);
  giro += omega * dt;
  tierra.rotation = { x: 0, y: giro, z: 0 };
  if (!enMano && Math.abs(omega) < 0.02) { omega = 0; return false; }
  return true;
});

function empujar(v) {
  omega = Math.max(-12, Math.min(12, omega + v));
  anim.iniciar();
  Obj.emitir("gira", { velocidad: Math.round(omega * 100) / 100 });
}
Obj.boton(Obj.$("toque"), (e) => {
  // Empujado del lado derecho gira para un lado; del izquierdo, para el otro.
  let s = 1;
  if (Number.isFinite(e.x)) { const l = donde.aLocal({ x: e.x, y: e.y, z: e.z }); s = l.x >= 0 ? -1 : 1; }
  empujar(3 * s);
});
Obj.mensaje("girar", (d) => empujar(d && Number.isFinite(d.velocidad) ? d.velocidad : 3));

// En VR: agarrarlo y arrastrar la mano lo hace girar; al soltar sigue.
const CENTRO = { x: 0, y: 0.28, z: 0 };
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca: (m) => Math.hypot(m.x - CENTRO.x, m.y - CENTRO.y, m.z - CENTRO.z) < R + 0.08,
  tomar: (m) => { enMano = true; omega = 0; manoAntes = { ...m, t: performance.now() }; anim.iniciar(); },
  mover: (m) => {
    const ahora = performance.now();
    // El ángulo de la mano alrededor del eje vertical, de un cuadro al otro.
    const a0 = Math.atan2(manoAntes.x - CENTRO.x, manoAntes.z - CENTRO.z);
    const a1 = Math.atan2(m.x - CENTRO.x, m.z - CENTRO.z);
    let d = a1 - a0;
    while (d > Math.PI) d -= Math.PI * 2;
    while (d < -Math.PI) d += Math.PI * 2;
    giro += d;
    omega = d / Math.max(0.001, (ahora - manoAntes.t) / 1000);
    manoAntes = { ...m, t: ahora };
  },
  soltar: () => { enMano = false; Obj.emitir("gira", { velocidad: Math.round(omega * 100) / 100 }); },
});

function pintar() {
  Obj.$("mar").setAttribute("color", Obj.color(P.mar, "#1E6091"));
  Obj.$("pie").setAttribute("color", Obj.color(P.pie, "#6B4A30"));
  MANCHAS.forEach(([lat], i) => { if (lat >= -65) parches[i].setAttribute("color", Obj.color(P.tierra, "#76C893")); });
}
