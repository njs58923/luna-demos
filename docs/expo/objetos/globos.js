...
;
// El ramo de globos: cada globo es un péndulo invertido.
//
// El globo tira hacia arriba (flota) y el hilo lo ata a la pesa: en reposo
// queda derecho sobre el nudo, y la brisa —senos lentos distintos para cada
// uno— lo empuja de costado. Entre ellos se empujan un poco para no
// encimarse. Al reventar suena un estallido, quedan unos jirones cayendo y a
// los doce segundos se vuelve a inflar.
const P = Obj.props({ cantidad: 5, colores: ["#FF3B30", "#FFD60A", "#0A84FF", "#30D158", "#BF5AF2", "#FF9F0A"], alto: 1.4, brisa: 0.5 });

const sinte = Obj.sinte({ volumen: 0.8 });
const donde = Obj.marco();
const ramo = Obj.$("ramo");
const NUDO = { x: 0, y: 0.1, z: 0 };
const N = Math.max(1, Math.min(9, Math.round(Number(P.cantidad) || 5)));
const R = 0.15;   // radio del globo

function colorDe(i) {
  const c = Array.isArray(P.colores) ? P.colores.map((x) => Obj.color(x, null)).filter(Boolean) : [];
  const l = c.length ? c : ["#FF3B30", "#FFD60A", "#0A84FF"];
  return l[i % l.length];
}
const largo = () => Math.max(0.5, Math.min(2.5, Number(P.alto) || 1.4));
const largoDe = (i) => largo() * (0.85 + 0.3 * ((i * 37) % 10) / 10);

const globos = [];
for (let i = 0; i < N; i++) {
  const g = Obj.crear("group", {}, ramo);
  const cuerpo = Obj.crear("sphere", { sx: R * 2, sy: R * 2.3, sz: R * 2, color: colorDe(i) }, g);
  Obj.crear("sphere", { x: -0.05, y: 0.08, z: 0.1, sx: 0.04, sy: 0.06, sz: 0.02, color: "#FFFFFF90", "material-alpha": "blend", "material-unlit": "true" }, g);
  const nudo = Obj.crear("cylinder", { y: -R * 1.18, sx: 0.03, sy: 0.03, sz: 0.03, color: colorDe(i) }, g);
  const hilo = Obj.crear("cylinder", { sx: 0.003, sy: 1, sz: 0.003, color: "#F0F0F0" }, ramo);
  const blanco = Obj.crear("sphere", { sx: R * 2.2, sy: R * 2.5, sz: R * 2.2, color: "transparent", "material-alpha": "blend" }, g);
  // Arranca repartido en abanico.
  const a = (i / N) * Math.PI * 2, abre = N === 1 ? 0 : 0.18;
  const gl = { i, g, cuerpo, nudo, hilo, a: { x: Math.sin(a) * abre, z: Math.cos(a) * abre }, v: { x: 0, z: 0 }, vivo: true, fase: i * 2.39, largo: largoDe(i) };
  Obj.boton(blanco, () => reventar(gl));
  globos.push(gl);
}

// Jirones del último reventado.
const jirones = [];
for (let i = 0; i < 6; i++) jirones.push({ el: Obj.crear("box", { sx: 0.05, sy: 0.04, sz: 0.004, visible: "false" }, ramo), p: null, v: null });
let jironesHasta = 0;

/** La punta del hilo (donde está el nudo del globo) para los ángulos a:
 *  dos inclinaciones, hacia +X y hacia +Z. */
function punta(gl) {
  const dx = Math.sin(gl.a.x), dz = Math.sin(gl.a.z);
  const y = Math.sqrt(Math.max(0.01, 1 - dx * dx - dz * dz));
  return { x: NUDO.x + dx * gl.largo, y: NUDO.y + y * gl.largo, z: NUDO.z + dz * gl.largo };
}
const centroDe = (gl) => { const p = punta(gl); return { x: p.x, y: p.y + R * 1.18, z: p.z }; };

function dibujar(gl) {
  const p = punta(gl);
  gl.g.position = { x: p.x, y: p.y + R * 1.18, z: p.z };
  // El globo se inclina con el hilo.
  gl.g.rotation = { x: gl.a.z * 0.8, y: 0, z: -gl.a.x * 0.8 };
  const l = Obj.tender(gl.hilo, NUDO, p);
  gl.hilo.scale = { x: 0.003, y: l, z: 0.003 };
}

const anim = Obj.animador((t, dt) => {
  const brisa = Math.max(0, Math.min(1, Number(P.brisa) || 0));
  const flota = 9.8 * 0.25;   // lo que lo endereza
  for (const gl of globos) {
    if (!gl.vivo) continue;
    const f = gl.fase;
    const bx = brisa * 0.35 * (Math.sin(t * 0.7 + f) + 0.5 * Math.sin(t * 1.9 + f * 2));
    const bz = brisa * 0.25 * (Math.sin(t * 0.5 + f * 3) + 0.5 * Math.sin(t * 1.3 + f));
    // Se empujan entre ellos.
    let ex = 0, ez = 0;
    const p = punta(gl);
    for (const o of globos) {
      if (o === gl || !o.vivo) continue;
      const q = punta(o);
      const dx = p.x - q.x, dz = p.z - q.z, d = Math.hypot(dx, p.y - q.y, dz);
      if (d < R * 2.2 && d > 1e-4) { ex += (dx / d) * (R * 2.2 - d) * 20; ez += (dz / d) * (R * 2.2 - d) * 20; }
    }
    for (const k of ["x", "z"]) {
      const empuje = k === "x" ? bx + ex : bz + ez;
      gl.v[k] += ((-flota / gl.largo) * Math.sin(gl.a[k]) + empuje / gl.largo - gl.v[k] * 1.2) * dt;
      gl.a[k] = Math.max(-0.9, Math.min(0.9, gl.a[k] + gl.v[k] * dt));
    }
    dibujar(gl);
  }
  if (jironesHasta && performance.now() < jironesHasta) {
    for (const j of jirones) {
      if (!j.p) continue;
      j.v.y -= 3 * dt;
      j.p.x += j.v.x * dt; j.p.y = Math.max(0.003, j.p.y + j.v.y * dt); j.p.z += j.v.z * dt;
      j.v.x *= 0.97; j.v.z *= 0.97;
      j.el.position = j.p;
      j.el.rotation = { x: t * 5 + j.v.x, y: t * 3, z: 0 };
    }
  } else if (jironesHasta) {
    jironesHasta = 0;
    for (const j of jirones) j.el.setAttribute("visible", "false");
  }
  return true;
});

function reventar(gl) {
  if (!gl.vivo) return;
  gl.vivo = false;
  const c = centroDe(gl);
  gl.g.setAttribute("visible", "false");
  gl.hilo.setAttribute("visible", "false");
  sinte.nota(900, 0.12, { tipo: "ruido", vol: 0.6, caida: 30 });
  sinte.nota(140, 0.1, { tipo: "seno", vol: 0.3, caida: 30 });
  const col = colorDe(gl.i);
  jirones.forEach((j, k) => {
    const a = (k / jirones.length) * Math.PI * 2;
    j.p = { x: c.x, y: c.y, z: c.z };
    j.v = { x: Math.cos(a) * 1.2, y: 0.8 + Math.random(), z: Math.sin(a) * 1.2 };
    j.el.setAttribute("color", col);
    j.el.setAttribute("visible", "inherit");
  });
  jironesHasta = performance.now() + 2500;
  Obj.emitir("pop", { quedan: globos.filter((g) => g.vivo).length });
  setTimeout(() => inflar(gl), 12000 + Math.random() * 4000);
}

function inflar(gl) {
  if (gl.vivo) return;
  gl.vivo = true;
  gl.a = { x: (Math.random() - 0.5) * 0.2, z: (Math.random() - 0.5) * 0.2 };
  gl.v = { x: 0, z: 0 };
  gl.g.setAttribute("visible", "inherit");
  gl.hilo.setAttribute("visible", "inherit");
  sinte.nota(300, 0.5, { tipo: "ruido", vol: 0.06, caida: 4 });
  // Crece desde chiquito.
  const desde = performance.now();
  const crecer = setInterval(() => {
    const k = Obj.salida(Math.min(1, (performance.now() - desde) / 600));
    gl.cuerpo.scale = { x: R * 2 * Math.max(0.05, k), y: R * 2.3 * Math.max(0.05, k), z: R * 2 * Math.max(0.05, k) };
    if (k >= 1) clearInterval(crecer);
  }, 33);
}

// En VR: la mano que entra a un globo lo pincha; rozarlo lo empuja.
const zona = Obj.$("zona");
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    for (const gl of globos) {
      if (!gl.vivo) continue;
      const c = centroDe(gl);
      const d = Math.hypot(l.x - c.x, (l.y - c.y) / 1.15, l.z - c.z);
      if (d < R * 0.9) { reventar(gl); continue; }
      if (d < R * 1.6) {
        const k = (R * 1.6 - d) * 3;
        gl.v.x += (c.x - l.x) * k;
        gl.v.z += (c.z - l.z) * k;
...
Obj.mensaje("inflar", () => globos.forEach(inflar));
Obj.alCambiar((p, a, cambios) => {
  for (const gl of globos) {
    if (cambios.includes("colores")) {
      const c = colorDe(gl.i);
      gl.cuerpo.setAttribute("color", c);
      gl.nudo.setAttribute("color", c);
    }
    if (cambios.includes("alto")) gl.largo = largoDe(gl.i);
  }
});
globos.forEach(dibujar);
anim.iniciar();
