...
;
// La guirnalda: lamparitas en una catenaria y patrones que se turnan.
//
// Cada lamparita es un foco sin iluminación (material-unlit) que cambia de
// color: prendida es su color, apagada un gris del mismo tono. Los patrones
// corren con un temporizador de 120 ms y sólo escriben las lamparitas cuyo
// color cambió desde la vez anterior.
const P = Obj.props({ paleta: "navidad", largo: 1.6, luces: 16, patron: "persecucion", clave: "" });

const PALETAS = {
  navidad: ["#FF3B30", "#30D158", "#FFD60A", "#0A84FF"],
  calida: ["#FFD27A", "#FFB347", "#FFE8B0", "#FF9F45"],
  arcoiris: ["#FF3B30", "#FF9F0A", "#FFD60A", "#30D158", "#0A84FF", "#BF5AF2"],
  fria: ["#E0F4FF", "#9AD7FF", "#FFFFFF", "#6EC6FF"],
};
const PATRONES = ["fija", "alternada", "persecucion", "respiracion", "apagada"];
const memo = Obj.memoria("patron");
const tira = Obj.$("tira");
...
function armar() {
  for (const el of focos.concat(cables)) el.f ? el.f.remove() : el.remove();
  for (const f of focos) { f.casquillo.remove(); }
  if (blanco) blanco.remove();
  focos = []; cables = [];
  const L = Math.max(0.6, Math.min(4, Number(P.largo) || 1.6));
  const N = Math.max(6, Math.min(40, Math.round(Number(P.luces) || 16)));
  const flecha = Math.min(0.3, L * 0.14);
  // La catenaria como parábola: y = -flecha·(1 − (2x/L)²).
  const y = (x) => -flecha * (1 - Math.pow((2 * x) / L, 2));
  // El cable, en tramos.
  const TRAMOS = 24;
  for (let i = 0; i < TRAMOS; i++) {
    const x0 = -L / 2 + (L * i) / TRAMOS, x1 = -L / 2 + (L * (i + 1)) / TRAMOS;
    const dx = x1 - x0, dy = y(x1) - y(x0);
    cables.push(Obj.crear("box", {
      x: (x0 + x1) / 2, y: (y(x0) + y(x1)) / 2, z: 0.02, rz: Math.atan2(dy, dx),
      sx: Math.hypot(dx, dy) + 0.002, sy: 0.004, sz: 0.004, color: "#1E2A1E",
    }, tira));
  }
  for (const s of [-1, 1]) cables.push(Obj.crear("sphere", { x: s * L / 2, y: 0, z: 0.01, sx: 0.02, sy: 0.02, sz: 0.02, color: "#8A8A8A" }, tira));
  for (let i = 0; i < N; i++) {
    const x = -L / 2 + (L * (i + 0.5)) / N;
    const casquillo = Obj.crear("cylinder", { x, y: y(x) - 0.012, z: 0.02, sx: 0.012, sy: 0.016, sz: 0.012, color: "#2E3A2E" }, tira);
    const f = Obj.crear("sphere", { x, y: y(x) - 0.034, z: 0.02, sx: 0.024, sy: 0.034, sz: 0.024, color: "#555555", "material-unlit": "true" }, tira);
    focos.push({ f, casquillo, color: null });
  }
  // Un solo blanco para toda la tira: una caja que la cubre.
  blanco = Obj.crear("box", { y: -flecha / 2 - 0.02, z: 0.03, sx: L, sy: flecha + 0.1, sz: 0.06, color: "transparent", "material-alpha": "blend", touchable: "true" }, tira);
  blanco.addEventListener("toque", siguiente);
}

let patron = PATRONES.includes(memo.leer(null)) ? memo.leer(null) : (PATRONES.includes(P.patron) ? P.patron : "persecucion");
let paso = 0;

function paleta() { return PALETAS[P.paleta] || PALETAS.navidad; }
function pintarFoco(f, c) { if (f.color !== c) { f.color = c; f.f.setAttribute("color", c); } }
const apagado = (c) => Obj.mezcla(c, "#2A2A2A", 0.82);
...
function cuadro() {
  ...
  focos.forEach((f, i) => {
    ...
    let on = true, k = 1;
    if (patron === "apagada") on = false;
    else if (patron === "alternada") on = (i + Math.floor(paso / 5)) % 2 === 0;
    else if (patron === "persecucion") on = (i - paso) % 4 === 0 || (i - paso + 1) % 4 === 0;
    else if (patron === "respiracion") k = 0.35 + 0.65 * (0.5 + 0.5 * Math.sin(paso * 0.18 + i * 0.1));
    pintarFoco(f, on ? (k < 1 ? Obj.mezcla(apagado(c), c, k) : c) : apagado(c));
  });
}
...
function aplicarPatron() {
  clearInterval(reloj);
  cuadro();
  // Fija y apagada no cambian: no hace falta seguir escribiendo.
  reloj = patron === "fija" || patron === "apagada" ? null : setInterval(cuadro, 120);
}
function siguiente() {
  patron = PATRONES[(PATRONES.indexOf(patron) + 1) % PATRONES.length];
  memo.guardar(patron);
  aplicarPatron();
  Obj.emitir("patron", { nombre: patron });
}

Obj.mensaje("patron", (d) => {
  if (!d || !PATRONES.includes(d.nombre)) return;
  patron = d.nombre; memo.guardar(patron); aplicarPatron();
});
Obj.alCambiar((p, antes, cambios) => {
  if (cambios.includes("largo") || cambios.includes("luces")) armar();
  for (const f of focos) f.color = null;
  aplicarPatron();
});
