  ...
  function pared(op) {
    ...
    for (const lado of caras) {
      ...
      for (const p of piezas) {
        if (p.linea) {
          for (const s of solidos) {
            if (p.y > s.y0 && p.y < s.y1) out.push(caja({ x: (s.x0 + s.x1) / 2, y: p.y, z: z + lado * 0.002, sx: s.x1 - s.x0, sy: 0.018, sz: 0.006, color: p.color }));
          }
          continue;
        }
        if (p.vlinea) {
          for (const s of solidos) {
            if (p.x > s.x0 && p.x < s.x1) out.push(caja({ x: p.x, y: (s.y0 + s.y1) / 2, z: z + lado * 0.002, sx: 0.018, sy: s.y1 - s.y0, sz: 0.006, color: p.color }));
          }
          continue;
        }
        if (p.punto) {
  ...
  function ventana(op) {
    ...
    if (op.postigos) {
      ...
      for (const s of [-1, 1]) {
        ...
        postigos.push({ bisagra: { x: s * (W / 2 + M), y: 0, z: 0.06 }, dir: -s, nodos });
      }
    }
...
;
// El hámster: una rutina (comer, correr, dormir) y una rueda con inercia.
//
// La rueda gira sobre Z. Mientras el hámster corre, su velocidad angular va
// hacia la que hace que el piso de la rueda pase bajo sus patas (ω = -v/R);
// sin él, frena sola. Un empujón fuerte con el hámster adentro se lo lleva:
// da la vuelta entera pegado a la rueda y sale mareado.
const P = Obj.props({ pelaje: "#D4A373", rueda: "#E74C3C" });

const sinte = Obj.sinte({ volumen: 0.7 });
const donde = Obj.marco();
const ruedaG = Obj.$("rueda"), ham = Obj.$("hamster");
const CENTRO = { x: 0.08, y: 0.14 }, R = 0.1, ANCHO = 0.07;
const PISO = 0.022, FONDO_RUEDA = CENTRO.y - R + 0.004;
const LUGAR = {
  comedero: { x: -0.05, y: PISO, z: 0.07, giro: Math.PI },
  nido: { x: -0.12, y: PISO + 0.01, z: -0.07, giro: 0.5 },
  rueda: { x: CENTRO.x, y: FONDO_RUEDA, z: 0, giro: 0 },
};

// ── La rueda ───────────────────────────────────────────────────────────────
const partesRueda = [];   // las que llevan el color de la rueda
const N = 20;
for (let k = 0; k < N; k++) {
  const a0 = (k / N) * Math.PI * 2, a1 = ((k + 1) / N) * Math.PI * 2;
  for (const z of [-ANCHO / 2, ANCHO / 2]) {
    const s = Obj.crear("cylinder", { sx: 0.005, sy: 0.01, sz: 0.005, color: "#E74C3C" }, ruedaG);
    const l = Obj.tender(s, { x: Math.cos(a0) * R, y: Math.sin(a0) * R, z }, { x: Math.cos(a1) * R, y: Math.sin(a1) * R, z });
    s.scale = { x: 0.005, y: l, z: 0.005 };
    partesRueda.push(s);
  }
  Obj.crear("cylinder", { x: Math.cos(a0) * R, y: Math.sin(a0) * R, rx: Math.PI / 2, sx: 0.003, sy: ANCHO, sz: 0.003, color: "#BDC3C7" }, ruedaG);
}
for (let k = 0; k < 4; k++) {
  const a = (k / 4) * Math.PI * 2;
  const s = Obj.crear("cylinder", { sx: 0.004, sy: 0.01, sz: 0.004, color: "#E74C3C" }, ruedaG);
  const l = Obj.tender(s, { x: 0, y: 0, z: -ANCHO / 2 }, { x: Math.cos(a) * R, y: Math.sin(a) * R, z: -ANCHO / 2 });
  s.scale = { x: 0.004, y: l, z: 0.004 };
  partesRueda.push(s);
}

// ── El hámster (mira a +X) ─────────────────────────────────────────────────
const cuerpo = Obj.crear("group", {}, ham);
const pel = [];
pel.push(Obj.crear("sphere", { y: 0.028, sx: 0.075, sy: 0.05, sz: 0.052, color: "#D4A373" }, cuerpo));
Obj.crear("sphere", { y: 0.02, x: 0.005, sx: 0.06, sy: 0.034, sz: 0.044, color: "#FFF3E0" }, cuerpo);
const cabezaH = Obj.crear("group", { x: 0.034, y: 0.036 }, cuerpo);
pel.push(Obj.crear("sphere", { sx: 0.042, sy: 0.038, sz: 0.042, color: "#D4A373" }, cabezaH));
for (const z of [-0.014, 0.014]) {
  Obj.crear("sphere", { x: 0.008, y: -0.008, z, sx: 0.022, sy: 0.02, sz: 0.018, color: "#FFF3E0" }, cabezaH);
  Obj.crear("sphere", { x: 0.016, y: 0.007, z: z * 0.85, sx: 0.007, sy: 0.007, sz: 0.007, color: "#111111" }, cabezaH);
  pel.push(Obj.crear("sphere", { x: -0.004, y: 0.019, z: z * 1.1, sx: 0.014, sy: 0.014, sz: 0.006, color: "#D4A373" }, cabezaH));
}
Obj.crear("sphere", { x: 0.022, y: -0.002, sx: 0.006, sy: 0.005, sz: 0.006, color: "#F4A6B5" }, cabezaH);
const patas = [];
for (const [x, z] of [[0.02, -0.018], [0.02, 0.018], [-0.02, -0.018], [-0.02, 0.018]]) {
  patas.push({ el: Obj.crear("sphere", { x, y: 0.004, z, sx: 0.014, sy: 0.008, sz: 0.012, color: "#F4C7B0" }, cuerpo), x, z });
}
const blanco = Obj.crear("box", { y: 0.03, sx: 0.1, sy: 0.07, sz: 0.07, color: "transparent", "material-alpha": "blend" }, ham);

// ── Estado ─────────────────────────────────────────────────────────────────
let estado = "comer", hasta = performance.now() + 5000;
let pos = { ...LUGAR.comedero }, giro = LUGAR.comedero.giro;
let destino = null, luego = null;
let omega = 0, angulo = 0, recorrido = 0, vueltas = 0, ultimoChirrido = 0;
let velCorrer = 0.35;
let tumbo = 0;           // ángulo recorrido en la vuelta entera
let mano = null, manoHasta = 0;

function ir(lugar, despues) {
  destino = LUGAR[lugar];
  luego = despues;
  estado = "caminar";
}
function llegar() {
  ...
  destino = null;
  const ahora = performance.now();
  if (l === "correr") { estado = "rueda"; hasta = ahora + 8000 + Math.random() * 6000; Obj.emitir("corre", {}); }
  else if (l === "comer") { estado = "comer"; hasta = ahora + 5000 + Math.random() * 3000; }
  else if (l === "dormir") { estado = "dormir"; hasta = ahora + 15000 + Math.random() * 10000; }
}
...
function paso() {
  ...
  const t = ahora / 1000;
  const dt = 0.033;
  let paseo = 0;        // velocidad de las patas

  // Una mano cerca: se queda quieto olfateándola (salvo en la rueda).
  const olfatea = mano && ahora < manoHasta && (estado === "comer" || estado === "caminar" || estado === "dormir");

  if (estado === "caminar" && destino && !olfatea) {
    const d = { x: destino.x - pos.x, z: destino.z - pos.z };
    const l = Math.hypot(d.x, d.z);
    const v = 0.12;
    if (l < 0.005) { pos = { ...destino }; giro = destino.giro; llegar(); }
    else {
      const k = Math.min(1, (v * dt) / l);
      pos = { x: pos.x + d.x * k, y: destino === LUGAR.rueda && l < 0.03 ? FONDO_RUEDA : PISO, z: pos.z + d.z * k };
      const g = Math.atan2(-d.z, d.x);
      giro += Math.atan2(Math.sin(g - giro), Math.cos(g - giro)) * Math.min(1, dt * 10);
      paseo = 14;
    }
  } else if (estado === "rueda") {
    paseo = 10 + velCorrer * 40;
    omega += (-velCorrer / R - omega) * Math.min(1, dt * 3);
    pos = { ...LUGAR.rueda, y: FONDO_RUEDA + 0.003 * Math.abs(Math.sin(t * paseo)) };
    giro = 0;
    if (ahora > hasta) {
      velCorrer = 0.35;
      const siesta = Math.random() < 0.3;
      ir(siesta ? "nido" : "comedero", siesta ? "dormir" : "comer");
    }
  } else if (estado === "vuelta") {
    // Pegado a la rueda: gira con ella hasta completar la vuelta.
    const d = Math.abs(omega) * dt;
    tumbo += d;
    const fi = -Math.PI / 2 + Math.sign(omega || 1) * tumbo;
    pos = { x: CENTRO.x + Math.cos(fi) * (R - 0.004), y: CENTRO.y + Math.sin(fi) * (R - 0.004), z: 0 };
    ham.rotation = { x: 0, y: 0, z: fi + Math.PI / 2 };
    if (tumbo >= Math.PI * 2 || Math.abs(omega) < 1.5) {
      estado = "mareado"; hasta = ahora + 2000;
      pos = { ...LUGAR.rueda };
      Obj.emitir("vuelta_entera", {});
      sinte.nota(2600, 0.1, { tipo: "seno", vol: 0.2, caida: 10 });
    }
  } else if (estado === "mareado") {
    if (ahora > hasta) ir("comedero", "comer");
  } else if (estado === "comer" || estado === "dormir") {
    if (ahora > hasta && !olfatea) {
      if (estado === "dormir") ir("comedero", "comer");
      else ir("rueda", "correr");
    }
  }

  // La rueda: frena sola si nadie la mueve.
  if (estado !== "rueda") omega *= Math.exp(-dt * (estado === "vuelta" ? 0.3 : 0.9));
  if (Math.abs(omega) < 0.01) omega = 0;
  angulo += omega * dt;
  recorrido += Math.abs(omega * dt);
  ruedaG.rotation = { x: 0, y: 0, z: angulo };
  if (recorrido >= Math.PI * 2) {
    recorrido -= Math.PI * 2;
    vueltas++;
    Obj.emitir("vuelta", { vueltas });
  }
  if (Math.abs(omega) > 1 && ahora - ultimoChirrido > Math.PI / Math.abs(omega) * 1000) {
    ultimoChirrido = ahora;
    sinte.nota(2300, 0.03, { tipo: "pulso", vol: 0.06 });
  }

  // El cuerpo.
  ham.position = pos;
  if (estado !== "vuelta") ham.rotation = { x: 0, y: giro, z: 0 };
  const dormido = estado === "dormir" && !destino;
  cuerpo.scale = dormido ? { x: 0.8, y: 0.9 + 0.04 * Math.sin(t * 2), z: 1.1 } : { x: 1, y: 1 + (paseo ? 0.03 * Math.sin(t * paseo) : 0.015 * Math.sin(t * 5)), z: 1 };
  patas.forEach((p, i) => {
    const s = paseo ? Math.max(0, Math.sin(t * paseo + (i % 2 ? Math.PI : 0) + (i > 1 ? Math.PI / 2 : 0))) * 0.006 : 0;
    p.el.position = { x: p.x, y: 0.004 + s, z: p.z };
  });
  // La cabeza: come, duerme, olfatea la mano o mira al frente.
  if (olfatea) {
    const l = { x: mano.x - pos.x, z: mano.z - pos.z };
    const ang = Math.atan2(-l.z, l.x) - giro;
    cabezaH.rotation = { x: 0, y: Math.max(-0.8, Math.min(0.8, Math.atan2(Math.sin(ang), Math.cos(ang)))), z: 0.1 * Math.sin(t * 20) };
  } else if (estado === "comer") cabezaH.rotation = { x: 0, y: 0, z: -0.25 - 0.15 * Math.abs(Math.sin(t * 9)) };
  else if (dormido) cabezaH.rotation = { x: 0, y: 0, z: -0.5 };
  else if (estado === "mareado") cabezaH.rotation = { x: 0.3 * Math.sin(t * 12), y: 0.3 * Math.cos(t * 12), z: 0 };
  else cabezaH.rotation = { x: 0, y: 0, z: 0 };
}
setInterval(paso, 33);

function chillido() {
  sinte.nota(3100, 0.06, { tipo: "seno", vol: 0.18, caida: 25 });
  sinte.nota(3500, 0.06, { tipo: "seno", vol: 0.18, caida: 25, retraso: 0.08 });
}
function aCorrer() {
  chillido();
  velCorrer = 0.55;
  if (estado === "rueda") { hasta = performance.now() + 10000; return; }
  if (estado === "vuelta") return;
  ir("rueda", "correr");
}
function empujar(fuerza) {
  omega += fuerza;
  sinte.nota(1800, 0.05, { tipo: "madera", vol: 0.3 });
  if (estado === "rueda" && Math.abs(omega) > 7) { estado = "vuelta"; tumbo = 0; chillido(); }
}

Obj.boton(blanco, aCorrer);
Obj.boton(Obj.$("toque_rueda"), (e) => {
  // Si está en la rueda y el toque cae sobre él, es a él.
  if (estado === "rueda" && Number.isFinite(e.x)) {
    const l = donde.aLocal({ x: e.x, y: e.y, z: e.z });
    if (Math.abs(l.x - pos.x) < 0.05 && Math.abs(l.y - (pos.y + 0.03)) < 0.04) { aCorrer(); return; }
  }
  empujar(omega <= 0 ? -9 : 9);
});

// En VR: la mano sobre el borde de la rueda la arrastra; cerca del hámster, él la olfatea.
const zona = Obj.$("zona");
let manoAntes = null;
if (zona) {
  zona.addEventListener("posemove", (e) => {
    const l = donde.aLocal({ x: e.px, y: e.py, z: e.pz });
    const ahora = performance.now();
    if (Math.hypot(l.x - pos.x, l.y - (pos.y + 0.03), l.z - pos.z) < 0.14) { mano = l; manoHasta = ahora + 400; }
    const antes = manoAntes && manoAntes.mano === e.hand ? manoAntes : null;
    manoAntes = { mano: e.hand, p: l, t: ahora };
    if (!antes) return;
    const rx = l.x - CENTRO.x, ry = l.y - CENTRO.y;
    const r = Math.hypot(rx, ry);
    if (Math.abs(r - R) > 0.03 || Math.abs(l.z) > ANCHO / 2 + 0.03) return;
    const dt = Math.max(0.001, (ahora - antes.t) / 1000);
    const v = { x: (l.x - antes.p.x) / dt, y: (l.y - antes.p.y) / dt };
    // La parte tangencial de la velocidad de la mano, en radianes por segundo.
    const w = (v.x * (-ry / r) + v.y * (rx / r)) / R;
    if (Math.abs(w) > 0.5) {
      omega = w;
      if (estado === "rueda" && Math.abs(omega) > 7) { estado = "vuelta"; tumbo = 0; chillido(); }
    }
...
Obj.mensaje("correr", aCorrer);
function pintar() {
  const c = Obj.color(P.pelaje, "#D4A373"), r = Obj.color(P.rueda, "#E74C3C");
  for (const el of pel) el.setAttribute("color", c);
  for (const el of partesRueda) el.setAttribute("color", r);
}
