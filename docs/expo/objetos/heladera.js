...
;
// La heladera: dos puertas con bisagra a la derecha, luz y estantes.
//
// La luz de adentro es el fondo y los estantes pasando a sin iluminación y a
// un tono cálido mientras alguna puerta está abierta. Las puertas giran hacia
// la derecha, como casi todas. Un imán por puerta, de colores.
const P = Obj.props({ color: "#F8F9FA", alarma: 30 });

const W = 0.7, H = 1.8, D = 0.7, T = 0.03;
const sinte = Obj.sinte({ volumen: 0.6 });
const cuerpo = Obj.$("cuerpo");
const adentro = Obj.$("adentro");
const color = () => Obj.color(P.color, "#F8F9FA");
const carcasa = [];
const caja = (a, padre, lista) => { const el = Obj.crear("box", a, padre); if (lista) lista.push(el); return el; };

// La carcasa (abierta adelante) y el separador entre freezer y heladera.
caja({ x: -W / 2 + T / 2, y: H / 2, sx: T, sy: H, sz: D, color: color() }, cuerpo, carcasa);
caja({ x: W / 2 - T / 2, y: H / 2, sx: T, sy: H, sz: D, color: color() }, cuerpo, carcasa);
caja({ y: H - T / 2, sx: W, sy: T, sz: D, color: color() }, cuerpo, carcasa);
caja({ y: 0.06, sx: W, sy: 0.12, sz: D, color: "#343A40" }, cuerpo);
caja({ y: 1.24, sx: W, sy: 0.04, sz: D, color: color() }, cuerpo, carcasa);
const fondo = caja({ y: H / 2, z: -D / 2 + 0.01, sx: W - 2 * T, sy: H - 0.1, sz: 0.02, color: "#DDE3E8" }, adentro);
const estantes = [0.45, 0.8, 1.52].map((y) => caja({ y, z: 0.02, sx: W - 2 * T - 0.01, sy: 0.012, sz: D - 0.1, color: "#C9D6DF" }, adentro));
// Lo que hay: botellas, frascos, una fruta, helado en el freezer.
const COSAS = [
  ["cylinder", -0.2, 0.12 + 0.13, 0.05, 0.07, 0.26, "#2B8A3E"],
  ["cylinder", -0.1, 0.12 + 0.11, 0.08, 0.07, 0.22, "#E03131"],
  ["box", 0.12, 0.12 + 0.07, 0.05, 0.16, 0.14, "#F08C00"],
  ["cylinder", -0.15, 0.45 + 0.06, 0.02, 0.1, 0.1, "#FFD43B"],
  ["sphere", 0.05, 0.45 + 0.045, 0.1, 0.09, 0.09, "#E8590C"],
  ["sphere", 0.15, 0.45 + 0.04, 0.0, 0.08, 0.08, "#74B816"],
  ["box", 0.0, 0.8 + 0.05, 0.0, 0.3, 0.09, "#FFF3BF"],
  ["box", -0.12, 1.28 + 0.06, 0.02, 0.18, 0.12, "#A5D8FF"],
  ["box", 0.12, 1.28 + 0.05, 0.05, 0.14, 0.1, "#F783AC"],
];
for (const [tag, x, y, z, a, h, c] of COSAS) Obj.crear(tag, { x, y, z, sx: a, sy: h, sz: tag === "box" ? a * 0.7 : a, color: c }, adentro);

// Las puertas: grupo con origen en la bisagra (a la derecha), que se extiende
// hacia -X.
const IMANES = ["#FF6B6B", "#4DABF7", "#FFD43B", "#69DB7C"];
function puerta(id, alto, conManija) {
  const g = Obj.$("bisagra_" + id);
  const hoja = caja({ x: -W / 2, sx: W, sy: alto, sz: 0.05, color: color(), "border-radius": 0.01 }, g, carcasa);
  caja({ x: -W + 0.06, y: conManija, z: 0.04, sx: 0.025, sy: 0.3, sz: 0.03, color: "#ADB5BD" }, g);
  const i = id === "freezer" ? 0 : 1;
  Obj.crear("box", { x: -0.25 - i * 0.1, y: alto / 2 - 0.12 - i * 0.2, z: 0.03, rz: 0.2 * (i ? -1 : 1), sx: 0.06, sy: 0.05, sz: 0.01, color: IMANES[i * 2], "border-radius": 0.01 }, g);
  Obj.crear("sphere", { x: -0.15, y: alto / 2 - 0.3 - i * 0.1, z: 0.03, sx: 0.04, sy: 0.04, sz: 0.015, color: IMANES[i * 2 + 1] }, g);
  const blanco = Obj.crear("plane", { x: -W / 2, z: 0.03, sx: W, sy: alto, color: "transparent", "material-alpha": "blend" }, g);
  blanco.setAttribute("manos", "false");   // con la mano se toma la hoja entera
  return { g, hoja, blanco, alto, abierta: false, k: 0, desde: 0, mano: null };
}
const puertas = { freezer: puerta("freezer", 0.52, -0.1), heladera: puerta("heladera", 1.2, 0.3) };

function luz() {
  const on = Object.values(puertas).some((p) => p.abierta);
  for (const el of [fondo, ...estantes]) el.setAttribute("material-unlit", on ? "true" : "false");
  fondo.setAttribute("color", on ? "#FFF8E1" : "#DDE3E8");
}

let pitando = null;
const anim = Obj.animador((t, dt) => {
  let seguir = false;
  for (const p of Object.values(puertas)) {
    const meta = p.abierta ? 1 : 0;
    if (p.mano === null && p.k !== meta) {
      p.k = meta > p.k ? Math.min(1, p.k + dt / 0.5) : Math.max(0, p.k - dt / 0.4);
      seguir = true;
    }
    // Bisagra a la derecha: abre girando en +Y.
    p.g.rotation = { x: 0, y: p.mano !== null ? p.mano : 1.9 * Obj.suave(p.k), z: 0 };
  }
  return seguir;
});

function vigilar() {
  // Una puerta abierta más de `alarma` segundos: pitidos hasta cerrarla.
  const s = Number(P.alarma) || 0;
  const ahora = performance.now();
  const olvidada = s > 0 && Object.values(puertas).some((p) => p.abierta && ahora - p.desde > s * 1000);
  if (olvidada && !pitando) {
    pitando = setInterval(() => sinte.nota(2600, 0.12, { tipo: "cuadrada", vol: 0.05, caida: 1 }), 900);
  } else if (!olvidada && pitando) { clearInterval(pitando); pitando = null; }
}
setInterval(vigilar, 1000);

function poner(cual, abierta) {
  const p = puertas[cual];
  if (!p || p.abierta === abierta) return;
  p.abierta = abierta;
  p.desde = performance.now();
  luz();
  anim.iniciar();
  vigilar();
  Obj.emitir("puerta", { cual, abierta });
}
for (const [cual, p] of Object.entries(puertas)) Obj.boton(p.blanco, () => poner(cual, !p.abierta));

// En VR: cada puerta se toma de cualquier punto (la puerta de las manos,
// _vr.js). La luz se prende apenas se entreabre, como en una de verdad.
const BISAGRAS = { freezer: { x: 0.345, y: 1.52, z: 0.35 }, heladera: { x: 0.345, y: 0.62, z: 0.35 } };
for (const [cual, p] of Object.entries(puertas)) {
  Obj.manos().puerta({
    pivote: BISAGRAS[cual], eje: { x: 0, y: 1, z: 0 }, brazo: { x: -1, y: 0, z: 0 },
    largo: W, alto: p.alto, max: 1.9,
    angulo: () => (p.mano !== null ? p.mano : 1.9 * Obj.suave(p.k)),
    mover(a) {
      p.mano = a;
      p.g.rotation = { x: 0, y: a, z: 0 };
      if (a > 0.15 && !p.abierta) poner(cual, true);
    },
    soltar(abierta, a) {
      p.mano = null;
      p.k = Obj.suaveInversa(Math.min(1, a / 1.9));
      if (abierta !== p.abierta) poner(cual, abierta); else anim.iniciar();
    },
  });
}
Obj.mensaje("cerrar", () => { poner("freezer", false); poner("heladera", false); });
Obj.alCambiar(() => { for (const el of carcasa) el.setAttribute("color", color()); });
