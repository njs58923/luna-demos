...
;
// El joystick: dos inclinaciones que tienden a cero.
//
// x e y van de -1 a 1 y son la inclinación sobre Y y sobre X dividida por la
// máxima (MAX); la bocha queda hacia (x, y) visto de frente. Tomado en VR
// apunta a la mano (vista desde el pivote); suelto, vuelve al centro con un
// resorte amortiguado que se pasa un poquito, como uno de verdad.
const P = Obj.props({ retorno: true, color: "#7B2FF7", etiqueta: "" });

const sinte = Obj.sinte({ volumen: 0.5 });
const donde = Obj.marco();
const palo = Obj.$("palo");
const MAX = 0.45, PIVOTE = 0.028, LARGO = 0.09;

let x = 0, y = 0, vx = 0, vy = 0;
let mx = 0, my = 0;          // la meta (lo que pide la mano o el clic)
let tomado = false, soltarEn = 0;
let avisado = { x: 0, y: 0 };

function dibujar() { palo.rotation = { x: -y * MAX, y: x * MAX, z: 0 }; }
function avisar(forzar) {
  if (!forzar && Math.abs(x - avisado.x) < 0.02 && Math.abs(y - avisado.y) < 0.02) return;
  avisado = { x, y };
  const r = (v) => +v.toFixed(3);
  Obj.emitir("eje", { x: r(x), y: r(y) });
  Obj.emitir("valor", { valor: r(Math.min(1, Math.hypot(x, y))), x: r(x), y: r(y) });
}

const anim = Obj.animador((t, dt) => {
  if (soltarEn && performance.now() > soltarEn) { soltarEn = 0; if (P.retorno !== false) { mx = 0; my = 0; } }
  if (tomado) { x = mx; y = my; vx = vy = 0; }
  else {
    // Resorte hacia la meta: rígido y con poca amortiguación.
    const k = 220, c = 18;
    vx += ((mx - x) * k - vx * c) * dt;
    vy += ((my - y) * k - vy * c) * dt;
    x += vx * dt; y += vy * dt;
  ...
  dibujar();
  avisar();
  const quieto = Math.abs(mx - x) < 1e-3 && Math.abs(my - y) < 1e-3 && Math.abs(vx) + Math.abs(vy) < 1e-3;
  if (quieto && !tomado) { x = mx; y = my; dibujar(); avisar(true); }
  return !quieto || tomado || !!soltarEn;
});
function acotar(ax, ay) {
  const m = Math.hypot(ax, ay);
  return m > 1 ? [ax / m, ay / m] : [ax, ay];
}

// Escritorio: un clic en la placa lo inclina hacia ahí un rato.
Obj.boton(Obj.$("toque"), (e) => {
  if (tomado || !Number.isFinite(e.x)) return;
  const l = donde.aLocal({ x: e.x, y: e.y, z: e.z });
  [mx, my] = acotar(l.x / 0.055, l.y / 0.055);
  soltarEn = performance.now() + 800;
  sinte.nota(500, 0.03, { tipo: "madera", vol: 0.2 });
  anim.iniciar();
});
Obj.boton(Obj.$("toque_bocha"), () => {
  sinte.nota(1500, 0.03, { tipo: "pulso", vol: 0.3 });
  Obj.emitir("boton", {});
});

// VR: la bocha en la mano.
Obj.agarre({
  zona: Obj.$("zona"), donde, gatillo: true,
  cerca(p) {
    const b = { x: Math.sin(x * MAX) * LARGO, y: Math.sin(y * MAX) * LARGO, z: PIVOTE + LARGO };
    return Math.hypot(p.x - b.x, p.y - b.y, p.z - b.z) < 0.06;
  },
  tomar(p) { tomado = true; soltarEn = 0; this.mover(p); anim.iniciar(); },
  mover(p) {
    const dz = Math.max(0.01, p.z - PIVOTE);
    [mx, my] = acotar(Math.atan2(p.x, dz) / MAX, Math.atan2(p.y, dz) / MAX);
    x = mx; y = my;
    dibujar();
    avisar();
  },
  soltar() {
    tomado = false;
    if (P.retorno !== false) { mx = 0; my = 0; }
    sinte.nota(300, 0.04, { tipo: "madera", vol: 0.25 });
    anim.iniciar();
  },
});

Obj.mensaje("centrar", () => { mx = 0; my = 0; anim.iniciar(); });
function pintar() {
  Obj.$("bocha").setAttribute("color", Obj.color(P.color, "#7B2FF7"));
