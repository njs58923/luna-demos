...
;
// La lámpara: prender, apagar, y recordarlo.
//
// No hay luces en el motor, así que "prendida" es una cuestión de color: la
// lamparita pasa a material sin iluminación, la pantalla se aclara como si la
// luz la atravesara, y aparecen un halo y un círculo de luz sobre la mesa,
// los dos translúcidos.
const P = Obj.props({ pantalla: "#2E6B5E", luz: "#FFD27A", pie: "#3A3530", encendida: false, clave: "" });
...
const base = Obj.$("base");
const llave = Obj.$("llave");
const memo = Obj.memoria("encendida");

let encendida = memo.leer(!!P.encendida);

function pintar() {
  const luz = Obj.color(P.luz, "#FFD27A");
  const tela = Obj.color(P.pantalla, "#2E6B5E");
  if (base) base.setAttribute("color", Obj.color(P.pie, "#3A3530"));
  foco.setAttribute("color", encendida ? Obj.tono(luz, 0.55) : "#8A8578");
  foco.setAttribute("material-unlit", encendida ? "true" : "false");
  for (const t of telas) {
    t.setAttribute("color", encendida ? Obj.mezcla(tela, luz, 0.45) : tela);
    t.setAttribute("material-unlit", encendida ? "true" : "false");
  }
  // El halo y el charco llevan la luz con alfa: 0x30 y 0x40 de 0xFF.
  halo.setAttribute("color", luz + "30");
  charco.setAttribute("color", luz + "40");
  halo.setAttribute("visible", encendida ? "inherit" : "false");
  charco.setAttribute("visible", encendida ? "inherit" : "false");
  // La llave marca el estado: corrida a un lado o al otro.
  llave.position = { x: 0.055, y: 0.034, z: encendida ? 0.041 : 0.029 };
}

function poner(v, avisar) {
  if (v === encendida) return;
  encendida = v;
  memo.guardar(encendida);
  pintar();
  if (avisar !== false) Obj.emitir("cambio", { encendida });
}

Obj.boton(Obj.$("toque"), () => poner(!encendida));
Obj.mensaje("encender", () => poner(true));
Obj.mensaje("apagar", () => poner(false));
Obj.mensaje("alternar", () => poner(!encendida));
