...
;
// La lámpara de pie: tres intensidades y una rueda de colores.
//
// Como la de mesa, "prendida" es una cuestión de color: la lamparita y la
// pantalla pasan a sin iluminación y se tiñen de la luz, y un halo y un charco
// translúcidos crecen con la intensidad. La intensidad es el alfa del halo y
// del charco —dos hexadecimales al final del color— y el tamaño del charco.
const P = Obj.props({ pantalla: "#F2E8CF", metal: "#B8923E", clave: "" });

const LUCES = ["#FFD27A", "#FFF1D6", "#CFE8FF", "#FF9EC7", "#B4F0A8", "#C9B6FF"];
const memo = Obj.memoria("estado");
const g = memo.leer(null);
let nivel = g && Number.isInteger(g.nivel) ? Math.max(0, Math.min(3, g.nivel)) : 0;
let color = g && Number.isInteger(g.color) ? ((g.color % LUCES.length) + LUCES.length) % LUCES.length : 0;

const foco = Obj.$("foco");
const halo = Obj.$("halo");
const charco = Obj.$("charco");
const perilla = Obj.$("perilla");
const telas = Obj.root.getElementsByClass("tela");

const ALFA_HALO = ["00", "14", "24", "38"];
const ALFA_CHARCO = ["00", "20", "34", "4C"];

function pintar() {
  const luz = LUCES[color];
  const tela = Obj.color(P.pantalla, "#F2E8CF");
  const on = nivel > 0;
  foco.setAttribute("color", on ? Obj.tono(luz, 0.6) : "#8A8578");
  foco.setAttribute("material-unlit", on ? "true" : "false");
  for (const t of telas) {
    t.setAttribute("color", on ? Obj.mezcla(tela, luz, 0.25 + nivel * 0.1) : tela);
    t.setAttribute("material-unlit", on ? "true" : "false");
  }
  halo.setAttribute("visible", on ? "inherit" : "false");
  charco.setAttribute("visible", on ? "inherit" : "false");
  halo.setAttribute("color", luz + ALFA_HALO[nivel]);
  charco.setAttribute("color", luz + ALFA_CHARCO[nivel]);
  const r = 1.0 + nivel * 0.35;
  charco.scale = { x: r, y: r, z: 1 };
  perilla.setAttribute("color", luz);
  Obj.$("cano").setAttribute("color", Obj.color(P.metal, "#B8923E"));
}

function cambio() {
  memo.guardar({ nivel, color });
  pintar();
  Obj.emitir("cambio", { nivel, color: LUCES[color] });
}

Obj.boton(Obj.$("toque"), () => { nivel = (nivel + 1) % 4; cambio(); });
Obj.boton(perilla, () => { color = (color + 1) % LUCES.length; if (nivel === 0) nivel = 2; cambio(); });
Obj.mensaje("nivel", (d) => { if (d && Number.isInteger(d.nivel)) { nivel = Math.max(0, Math.min(3, d.nivel)); cambio(); } });
Obj.mensaje("color", (d) => {
  const i = d ? LUCES.indexOf(String(d.color).toUpperCase()) : -1;
  if (i >= 0) { color = i; cambio(); }
});
