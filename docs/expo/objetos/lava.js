...
;
// La lámpara de lava: burbujas con temperatura propia.
//
// Cada burbuja tiene una temperatura: abajo se calienta, arriba se enfría.
// Caliente es menos densa y sube; fría, baja. La forma se estira en la
// dirección en que se mueve (sube alargada, baja achatada) y late un poco,
// que es lo que la hace parecer cera y no pelotitas.
const P = Obj.props({ cera: "#FF4D6D", liquido: "#3A0CA3", metal: "#B8C0C8", encendida: true });

const ALTO = 0.28, RADIO = 0.035;
const grupo = Obj.$("burbujas");
const azar = Obj.azar(21);
const burbujas = [];
for (let i = 0; i < 6; i++) {
  const r = 0.012 + azar() * 0.014;
  const el = Obj.crear("sphere", { sx: 2 * r, sy: 2 * r, sz: 2 * r, color: "#FF4D6D", "material-unlit": "true" }, grupo);
  burbujas.push({ el, r, y: azar() * 0.05, v: 0, temp: azar(), x: (azar() - 0.5) * RADIO, z: (azar() - 0.5) * RADIO, fase: azar() * 6 });
}
// La cera del fondo, que se ve siempre.
const charco = Obj.crear("sphere", { y: 0.004, sx: 0.09, sy: 0.02, sz: 0.09, color: "#FF4D6D", "material-unlit": "true" }, grupo);

let encendida = P.encendida !== false;
const anim = Obj.animador((t, dt) => {
  let moviendo = false;
  for (const b of burbujas) {
    // Temperatura: abajo calienta (si está prendida), arriba enfría.
    const altura = b.y / ALTO;
    const calor = encendida ? (1 - altura) * 0.5 : -0.2;
    b.temp = Math.max(0, Math.min(1, b.temp + (calor - altura * 0.35) * dt));
    // Flotación: la caliente sube; la fría baja; con rozamiento.
    const empuje = (b.temp - 0.45) * 0.08;
    b.v += (empuje - b.v * 1.8) * dt;
    b.y = Math.max(0, Math.min(ALTO, b.y + b.v * dt));
    if (b.y <= 0 && b.v < 0) b.v = 0;
    if (b.y >= ALTO && b.v > 0) b.v = 0;
    // Un vaivén lateral lento.
    const x = b.x + 0.006 * Math.sin(t * 0.7 + b.fase), z = b.z + 0.006 * Math.cos(t * 0.5 + b.fase);
    // Se estira al moverse, y late.
    const estira = Math.max(-0.35, Math.min(0.6, b.v * 18));
    const late = 1 + 0.06 * Math.sin(t * 2 + b.fase);
    const d = 2 * b.r * late;
    b.el.position = { x, y: b.y + b.r, z };
    b.el.scale = { x: d * (1 - estira * 0.3), y: d * (1 + estira), z: d * (1 - estira * 0.3) };
    if (Math.abs(b.v) > 0.0005 || b.y > 0.001) moviendo = true;
  }
  return encendida || moviendo;
});

function pintar() {
  const cera = Obj.color(P.cera, "#FF4D6D");
  for (const b of burbujas) b.el.setAttribute("color", cera);
  charco.setAttribute("color", cera);
  Obj.$("liquido").setAttribute("color", Obj.color(P.liquido, "#3A0CA3").slice(0, 7) + (encendida ? "C0" : "90"));
  for (const id of ["base_a", "base_b", "tapa"]) Obj.$(id).setAttribute("color", Obj.color(P.metal, "#B8C0C8"));
}
function poner(v) {
  if (v === encendida) return;
  encendida = v;
  pintar();
  anim.iniciar();
  Obj.emitir("cambio", { encendida });
}
Obj.boton(Obj.$("toque"), () => poner(!encendida));
Obj.mensaje("encender", () => poner(true));
