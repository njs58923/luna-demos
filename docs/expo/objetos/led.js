...
;
// El led: un brillo de 0 a 1 que se pinta mezclando el color apagado con el prendido.
//
// Apagado no es negro: es el mismo color muy oscuro, como una lámpara de
// panel sin corriente. Prendido va sin iluminación (brilla) y con un halo
// translúcido alrededor. Parpadeando alterna entre el brillo pedido y nada.
const P = Obj.props({ color: "#30D158", etiqueta: "", inicial: 0 });

const luz = Obj.$("luz"), halo = Obj.$("halo");
let brillo = Math.max(0, Math.min(1, Number(P.inicial) || 0));
let color = Obj.color(P.color, "#30D158");
let hz = 0;
let pintado = "";

function pintar(b) {
  const apagado = Obj.tono(color, -0.75);
  const c = Obj.mezcla(apagado, color, b);
  const alfa = Math.round(Math.max(0, b - 0.2) * 0.5 * 255).toString(16).padStart(2, "0");
  const firma = c + alfa;
  if (firma === pintado) return;
  pintado = firma;
  luz.setAttribute("color", c);
  luz.setAttribute("material-unlit", b > 0.05 ? "true" : "false");
  halo.setAttribute("color", color + alfa);
}
// Parpadeo: sólo corre mientras parpadea.
let reloj = null;
function parpadear(f) {
  hz = Math.max(0, Math.min(20, Number(f) || 0));
  if (reloj) { clearInterval(reloj); reloj = null; }
  if (hz > 0) reloj = setInterval(() => pintar(Math.floor(performance.now() / 1000 * hz * 2) % 2 ? 0 : brillo), 1000 / (hz * 4));
  else pintar(brillo);
}
function poner(b) { brillo = Math.max(0, Math.min(1, b)); if (!hz) pintar(brillo); }

Obj.mensaje("valor", (d) => {
  if (d == null) return;
  if (typeof d === "number" || typeof d === "boolean") return poner(Number(d));
  if (d.fraccion != null && Number.isFinite(Number(d.fraccion))) poner(Number(d.fraccion));
  else if (Number.isFinite(Number(d.valor))) poner(Number(d.valor));
});
Obj.mensaje("color", (d) => { const c = Obj.color(d && d.color, null); if (c) { color = c.slice(0, 7); pintado = ""; pintar(brillo); } });
Obj.mensaje("parpadear", (d) => parpadear(d && d.hz));
Obj.boton(Obj.$("toque"), () => poner(brillo > 0.5 ? 0 : 1));
Obj.alCambiar(() => { color = Obj.color(P.color, "#30D158").slice(0, 7); pintado = ""; Obj.$("etiqueta").setAttribute("value", String(P.etiqueta || "").slice(0, 10)); pintar(brillo); });
color = color.slice(0, 7);
Obj.$("etiqueta").setAttribute("value", String(P.etiqueta || "").slice(0, 10));
pintar(brillo);
