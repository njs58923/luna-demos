...
;
// La licuadora: pedazos en remolino que se vuelven líquido.
//
// Cada fruta agregada suma cinco pedazos de su color y un volumen. Prendida,
// los pedazos giran alrededor del eje con una velocidad que depende de la
// marcha, suben y bajan (el remolino) y se achican según cuánto se licuó;
// el licuado crece con lo que se deshizo y su color es el promedio de lo que
// hay, pesado por volumen (acá sí sirve el promedio: no son pigmentos).
const sinte = Obj.sinte({ volumen: 0.6 });
const licuado = Obj.$("licuado"), cuchillas = Obj.$("cuchillas");
const FRUTAS = {
  banana: { color: "#F6E27F", vol: 1 },
  frutilla: { color: "#E63946", vol: 1 },
  kiwi: { color: "#8DB600", vol: 1 },
  arandano: { color: "#4B3F72", vol: 0.6 },
  leche: { color: "#FAF7F0", vol: 1.4 },
};
const ALTO = 0.17, RADIO = 0.042, MAX_VOL = 6;

// Las cuchillas: una cruz.
for (let k = 0; k < 2; k++) Obj.crear("box", { ry: k * Math.PI / 2 + 0.3, rz: k ? 0.25 : -0.25, sx: 0.07, sy: 0.002, sz: 0.012, color: "#C0C0C0" }, cuchillas);
Obj.crear("cylinder", { sx: 0.012, sy: 0.012, sz: 0.012, color: "#8C8C8C" }, cuchillas);

// El frutero: una de cada, para tocar.
const frutero = Obj.$("frutero");
Object.entries(FRUTAS).forEach(([id, f], i) => {
  const a = (i / 5) * Math.PI * 2;
  const g = Obj.crear("group", { x: Math.cos(a) * 0.04, y: 0.05, z: Math.sin(a) * 0.04 }, frutero);
  if (id === "banana") Obj.crear("cylinder", { rz: 1.2, sx: 0.022, sy: 0.07, sz: 0.022, color: f.color }, g);
  else if (id === "leche") { Obj.crear("box", { y: 0.02, sx: 0.03, sy: 0.06, sz: 0.03, color: "#FFFFFF" }, g); Obj.crear("box", { y: 0.02, z: 0.0155, sx: 0.03, sy: 0.02, sz: 0.001, color: "#0A84FF" }, g); }
  else Obj.crear("sphere", { sx: id === "arandano" ? 0.018 : 0.035, sy: id === "arandano" ? 0.018 : 0.032, sz: id === "arandano" ? 0.018 : 0.035, color: f.color }, g);
  const b = Obj.crear("box", { y: 0.015, sx: 0.05, sy: 0.07, sz: 0.05, color: "transparent", "material-alpha": "blend" }, g);
  Obj.boton(b, () => agregar(id));
});

// Los botones.
const botonesG = Obj.$("botones");
[0, 1, 2, 3].forEach((n, i) => {
  const x = -0.045 + i * 0.03;
  const b = Obj.crear("box", { x, sx: 0.022, sy: 0.022, sz: 0.008, color: n ? "#F1FAEE" : "#1C1C1C", "border-radius": 0.004 }, botonesG);
  Obj.crear("text", { x, z: 0.0045, value: String(n), size: 0.013, color: n ? "#1C1C1C" : "#FFFFFF" }, botonesG);
  Obj.boton(b, () => velocidad(n), { color: n ? "#F1FAEE" : "#1C1C1C" });
});

// ── Estado ─────────────────────────────────────────────────────────────────
let contenido = [];     // { fruta, vol }
let pedazos = [];       // { el, color, a (ángulo), r (radio), y, vy, fase, entero (1 → 0), vol }
let licuadoVol = 0;     // cuánto ya es líquido
let marcha = 0, giro = 0;
const volTotal = () => contenido.reduce((a, c) => a + c.vol, 0);

function colorMezcla() {
  const t = volTotal();
  if (!t) return "#FFFFFF";
  const c = [0, 0, 0];
  for (const x of contenido) { const [r, g, b] = Obj.rgb(FRUTAS[x.fruta].color); c[0] += r * x.vol; c[1] += g * x.vol; c[2] += b * x.vol; }
  return Obj.hex(c[0] / t, c[1] / t, c[2] / t);
}
/** setAttribute sólo si cambió. */
function poner(el, k, v) {
  el.puesto = el.puesto || {};
  if (el.puesto[k] === v) return;
  el.puesto[k] = v;
  el.setAttribute(k, v);
}
function cartel(t) {
  Obj.$("cartel").setAttribute("value", t);
  setTimeout(() => Obj.$("cartel").setAttribute("value", ""), 2000);
}

function agregar(id) {
  const f = FRUTAS[id];
  if (!f) return;
  if (volTotal() + f.vol > MAX_VOL) { cartel("¡Está lleno!"); return; }
  contenido.push({ fruta: id, vol: f.vol });
  // Cinco pedazos que caen desde arriba.
  for (let k = 0; k < 5; k++) {
    const el = Obj.crear(id === "leche" ? "sphere" : "box", { sx: 0.02, sy: 0.02, sz: 0.02, color: f.color }, Obj.$("pedazos"));
    pedazos.push({ el, color: f.color, a: Math.random() * Math.PI * 2, r: 0.01 + Math.random() * 0.028, y: 0.22 + k * 0.02, vy: 0, fase: Math.random() * 6, entero: 1, vol: f.vol / 5 });
  }
  sinte.nota(300 + Math.random() * 100, 0.12, { tipo: "madera", vol: 0.35 });
  anim.iniciar();
}

function motor(n) {
  let fase = 0;
  const f0 = [0, 95, 140, 200][n];
  return () => {
    fase += (2 * Math.PI * f0) / 22050;
    const saw = ((fase / (2 * Math.PI)) % 1) * 2 - 1;
    return (saw * 0.5 + (Math.random() - 0.5) * 0.6) * 0.1;
  };
}
function velocidad(n) {
  n = Math.max(0, Math.min(3, Math.round(Number(n) || 0)));
  if (n === marcha) return;
  marcha = n;
  sinte.flujo(n ? motor(n) : null);
  sinte.nota(900, 0.03, { tipo: "pulso", vol: 0.3 });
  Obj.emitir("velocidad", { n });
  anim.iniciar();
}

const anim = Obj.animador((t, dt) => {
  let seguir = marcha > 0;
  giro += [0, 18, 30, 45][marcha] * dt;
  cuchillas.rotation = { x: 0, y: giro, z: 0 };
  // El líquido: su altura según lo que ya se deshizo.
  const alto = Math.max(0.001, (licuadoVol / MAX_VOL) * ALTO);
  poner(licuado, "visible", licuadoVol > 0.01 ? "inherit" : "false");
  licuado.position = { x: 0, y: 0.012 + alto / 2, z: 0 };
  licuado.scale = { x: 0.1, y: alto, z: 0.1 };
  const piso = 0.012 + alto;
  for (const p of pedazos) {
    if (p.entero <= 0) continue;
    if (marcha) {
      // Se deshace: más rápido cuanto más fuerte.
      const antes = p.entero;
      p.entero = Math.max(0, p.entero - dt * [0, 0.12, 0.22, 0.35][marcha]);
      licuadoVol += (antes - p.entero) * p.vol;
      p.a += dt * (4 + marcha * 3) / Math.max(0.3, p.r * 30);
      p.y = 0.03 + (0.5 + 0.5 * Math.sin(t * (2 + marcha) + p.fase)) * Math.min(ALTO, piso + 0.05);
      p.r = Math.min(RADIO, p.r + dt * 0.01);
      seguir = true;
    } else if (p.y > Math.max(0.02, piso - 0.01)) {
      // Caen hasta el fondo (o flotan en el licuado).
      p.vy -= 3 * dt;
      p.y = Math.max(Math.max(0.02, piso - 0.01), p.y + p.vy * dt);
      if (p.y <= Math.max(0.02, piso - 0.01)) p.vy = 0;
      seguir = true;
    }
    const s = 0.02 * Math.max(0.15, p.entero);
    p.el.position = { x: Math.cos(p.a) * p.r, y: p.y, z: Math.sin(p.a) * p.r };
    p.el.rotation = { x: p.fase + t * marcha, y: p.a, z: 0 };
    p.el.scale = { x: s, y: s, z: s };
    if (p.entero <= 0) p.el.setAttribute("visible", "false");
  }
  if (licuadoVol > 0.01) poner(licuado, "color", colorMezcla());
  return seguir;
});

function servir() {
  if (marcha) { cartel("Primero apagala"); return; }
  if (!contenido.length) return;
  const color = colorMezcla();
  const frutas = contenido.map((c) => c.fruta);
  Obj.emitir("licuado", { color, frutas, listo: pedazos.every((p) => p.entero <= 0) });
  for (const p of pedazos) p.el.remove();
  pedazos = [];
  contenido = [];
  licuadoVol = 0;
  sinte.nota(200, 0.6, { tipo: "ruido", vol: 0.08, caida: 3 });
  cartel("¡Salud!");
  anim.iniciar();
}
Obj.boton(Obj.$("vidrio"), servir);
Obj.mensaje("agregar", (d) => agregar(d && d.fruta));
Obj.mensaje("velocidad", (d) => velocidad(d && d.n));
Obj.mensaje("servir", servir);
