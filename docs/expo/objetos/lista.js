...
;
// La lista de tareas: ocho renglones con casilla, y una línea para agregar.
//
// Agregar usa el teclado del sistema con Obj.teclado(): el texto que se va
// escribiendo aparece en el renglón de "+ agregar", y Enter lo suma a la
// lista. Una tarea tildada queda tachada (una raya encima) hasta "limpiar".
const P = Obj.props({ titulo: "Para hacer", tareas: ["regar las plantas", "llamar a la abuela"], color: "#A47148", clave: "" });

const MAX = 8;
const memo = Obj.memoria("tareas");
const renglones = Obj.$("renglones");
const agregarTxt = Obj.$("agregar_txt");

let tareas = memo.leer(null);
if (!Array.isArray(tareas)) {
  tareas = (Array.isArray(P.tareas) ? P.tareas : []).slice(0, MAX).map((t) => ({ t: String(t).slice(0, 34), h: false }));
}
tareas = tareas.filter((x) => x && typeof x.t === "string").slice(0, MAX);

/** Cuánto mide un texto: con la fuente si está, o a ojo. */
function mide(t, size) {
...
const filas = [];
for (let i = 0; i < MAX; i++) {
  const y = 0.12 - i * 0.038;
  const caja = Obj.crear("box", { x: -0.125, y, sx: 0.022, sy: 0.022, sz: 0.003, color: "#FFFFFF" }, renglones);
  const borde = Obj.crear("box", { x: -0.125, y, z: -0.001, sx: 0.026, sy: 0.026, sz: 0.002, color: "#1D3557" }, renglones);
  // El tilde: dos palitos en V, en vez de un "✓" que la fuente puede no tener.
  const tilde = Obj.crear("group", { x: -0.125, y, z: 0.003, visible: "false" }, renglones);
  Obj.crear("box", { x: -0.005, y: -0.002, rz: 0.8, sx: 0.012, sy: 0.004, sz: 0.001, color: "#1E8E3E" }, tilde);
  Obj.crear("box", { x: 0.004, y: 0.003, rz: -0.9, sx: 0.02, sy: 0.004, sz: 0.001, color: "#1E8E3E" }, tilde);
  const texto = Obj.crear("text", { x: 0, y, z: 0.002, value: "", size: 0.017, color: "#1C1C1C" }, renglones);
  const tachado = Obj.crear("box", { x: 0, y, z: 0.003, sx: 0.1, sy: 0.002, sz: 0.001, color: "#6A6A6A", visible: "false" }, renglones);
  const blanco = Obj.crear("plane", { x: 0, y, z: 0.006, sx: 0.3, sy: 0.036, color: "transparent", "material-alpha": "blend" }, renglones);
  const f = { caja, borde, tilde, texto, tachado, blanco, y };
  Obj.boton(blanco, () => alternar(i));
  filas.push(f);
}

function dibujar() {
  Obj.$("titulo").setAttribute("value", String(P.titulo || "").slice(0, 22));
  filas.forEach((f, i) => {
    const t = tareas[i];
    const vis = t ? "inherit" : "false";
    for (const el of [f.caja, f.borde, f.texto]) el.setAttribute("visible", vis);
    if (!t) { f.tilde.setAttribute("visible", "false"); f.tachado.setAttribute("visible", "false"); return; }
    const w = mide(t.t, 0.017);
    f.texto.setAttribute("value", t.t);
    f.texto.setAttribute("color", t.h ? "#8A8A8A" : "#1C1C1C");
    // Alineado a la izquierda, después de la casilla.
    f.texto.position = { x: -0.1 + w / 2, y: f.y, z: 0.002 };
    f.tilde.setAttribute("visible", t.h ? "inherit" : "false");
    f.tachado.setAttribute("visible", t.h ? "inherit" : "false");
    f.tachado.position = { x: -0.1 + w / 2, y: f.y, z: 0.003 };
    f.tachado.scale = { x: w + 0.006, y: 0.002, z: 0.001 };
  });
}
function avisar() {
  memo.guardar(tareas);
  Obj.emitir("cambio", { hechas: tareas.filter((t) => t.h).length, total: tareas.length });
}
function alternar(i) {
  if (!tareas[i]) return;
  tareas[i].h = !tareas[i].h;
  dibujar(); avisar();
}
function agregar(texto) {
  const t = String(texto || "").replace(/\s+/g, " ").trim().slice(0, 34);
  if (!t || tareas.length >= MAX) return false;
  tareas.push({ t, h: false });
  dibujar(); avisar();
  return true;
}
function limpiar() {
  const antes = tareas.length;
  tareas = tareas.filter((t) => !t.h);
  if (tareas.length !== antes) { dibujar(); avisar(); }
}

// ── Agregar con el teclado ──────────────────────────────────────────────────
const editor = Obj.teclado(Obj.$("agregar"), {
  unaLinea: true, max: 34,
  cambio: (t) => agregarTxt.setAttribute("value", (t || "") + "|"),
  foco: (si) => {
    if (si) { editor.texto = ""; agregarTxt.setAttribute("value", tareas.length >= MAX ? "lista llena" : "|"); return; }
    // Al soltar (Enter): si hay texto, se suma.
    if (editor.texto.trim()) agregar(editor.texto);
    editor.texto = "";
    agregarTxt.setAttribute("value", "+ agregar");
  },
});
Obj.boton(Obj.$("agregar"), () => editor.tomar(), { color: "#E9F5EC" });
Obj.boton(Obj.$("limpiar"), limpiar, { color: "#FDECEC" });
Obj.mensaje("agregar", (d) => agregar(d && d.texto));
Obj.mensaje("limpiar", limpiar);
Obj.alCambiar(() => { Obj.$("tabla").setAttribute("color", Obj.color(P.color, "#A47148")); dibujar(); });
Obj.$("tabla").setAttribute("color", Obj.color(P.color, "#A47148"));
dibujar();
