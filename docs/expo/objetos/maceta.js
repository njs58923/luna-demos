...
;
// La maceta: una planta que crece con los riegos y se seca con los días.
//
// La planta se arma entera una vez —tallo, hojas, pimpollo y pétalos— y lo
// que cambia con el crecimiento es la escala de cada parte. El crecimiento es
// un número de 0 a 1 que sube con cada riego y baja despacio con el tiempo sin
// agua (se guarda la fecha del último riego). Las hojas se inclinan cuando
// falta agua: es lo primero que se nota en una planta de verdad.
const P = Obj.props({ flor: "#FF6B9A", maceta: "#C1652F", riegos: 6, clave: "" });

const memo = Obj.memoria("planta");
const planta = Obj.$("planta");
const regadera = Obj.$("regadera");
const gotasG = Obj.$("gotas");
const DIA = 86400000;

const g = memo.leer(null);
let crecido = g && Number.isFinite(g.c) ? Math.max(0, Math.min(1, g.c)) : 0.12;
let ultimoRiego = g && Number.isFinite(g.t) ? g.t : Date.now();
let riegos = g && Number.isInteger(g.r) ? g.r : 0;

// Sed: pasados dos días sin agua, pierde un poco de crecimiento por día.
function sed() {
  const dias = (Date.now() - ultimoRiego) / DIA;
  return Math.max(0, Math.min(1, (dias - 2) / 5));
}

const tallo = Obj.crear("cylinder", { sx: 0.012, sy: 1, sz: 0.012, color: "#3A7D44" }, planta);
const hojas = [];
for (let i = 0; i < 6; i++) {
  const a = i * 2.2;
  const h = Obj.crear("group", { ry: a }, planta);
  const hoja = Obj.crear("box", { x: 0.05, sx: 0.1, sy: 0.006, sz: 0.045, color: "#4C9A55", "border-radius": 0.02 }, h);
  hojas.push({ g: h, hoja, altura: 0.08 + i * 0.07, a });
}
const flor = Obj.crear("group", {}, planta);
const centro = Obj.crear("sphere", { sx: 0.04, sy: 0.03, sz: 0.04, color: "#FFD60A" }, flor);
const petalos = [];
for (let i = 0; i < 7; i++) {
  const a = (i / 7) * Math.PI * 2;
  const p = Obj.crear("box", { x: Math.sin(a) * 0.035, z: Math.cos(a) * 0.035, ry: a, rx: -0.3, sx: 0.035, sy: 0.006, sz: 0.06, color: "#FF6B9A", "border-radius": 0.015 }, flor);
  petalos.push(p);
}

function dibujar() {
  const c = Math.max(0, crecido - sed() * 0.3);
  const alto = 0.05 + c * 0.45;
  tallo.position = { x: 0, y: alto / 2, z: 0 };
  tallo.scale = { x: 0.012, y: alto, z: 0.012 };
  const caida = sed();
  hojas.forEach((h) => {
    const visible = h.altura < alto - 0.02;
    h.g.setAttribute("visible", visible ? "inherit" : "false");
    h.g.position = { x: 0, y: Math.min(h.altura, alto - 0.03), z: 0 };
    // Con sed, las hojas se caen hacia abajo.
    h.g.rotation = { x: 0, y: h.a, z: -0.2 - caida * 0.9 };
    const k = Math.min(1, 0.4 + c);
    h.hoja.scale = { x: 0.1 * k, y: 0.006, z: 0.045 * k };
    h.hoja.setAttribute("color", Obj.mezcla("#4C9A55", "#A68A3A", caida));
  });
  // Florece al final: primero un pimpollo verde, después los pétalos.
  flor.position = { x: 0, y: alto, z: 0 };
  const abre = Math.max(0, (c - 0.75) / 0.25);
  flor.setAttribute("visible", c > 0.55 ? "inherit" : "false");
  centro.setAttribute("color", abre > 0.2 ? "#FFD60A" : "#5A9A45");
  petalos.forEach((p, i) => {
    const a = (i / petalos.length) * Math.PI * 2;
    p.setAttribute("visible", abre > 0 ? "inherit" : "false");
    p.rotation = { x: -1.2 + abre * 0.9, y: a, z: 0 };
    p.setAttribute("color", Obj.mezcla(Obj.color(P.flor, "#FF6B9A"), "#8A6A4A", caida));
  });
}

// ── Regar: la regadera aparece, se inclina, caen gotas ─────────────────────
const gotas = [];
for (let i = 0; i < 10; i++) gotas.push(Obj.crear("sphere", { sx: 0.012, sy: 0.02, sz: 0.012, color: "#6EC6FF", visible: "false" }, gotasG));
let riego = null;
const anim = Obj.animador((t) => {
  if (!riego) return false;
  if (!riego.t0) riego.t0 = t;
  const k = (t - riego.t0) / 1.6;
  regadera.setAttribute("visible", k < 1 ? "inherit" : "false");
  regadera.rotation = { x: 0, y: 0, z: 0.6 * Math.sin(Math.min(1, k * 3) * Math.PI / 2) };
  gotas.forEach((g, i) => {
    const kk = ((k * 3 + i / gotas.length) % 1);
    const vis = k > 0.2 && k < 0.95;
    g.setAttribute("visible", vis ? "inherit" : "false");
    if (vis) g.position = { x: 0.1 - kk * 0.04 + (i % 3) * 0.01, y: 0.42 - kk * 0.25, z: ((i * 7) % 5 - 2) * 0.008 };
  });
  if (k >= 1) {
    riego = null;
    for (const gg of gotas) gg.setAttribute("visible", "false");
    return false;
  }
  return true;
});
...
function regar() {
  if (riego) return;
  riegos++;
  const paso = 1 / Math.max(2, Math.min(30, Number(P.riegos) || 6));
  crecido = Math.min(1, Math.max(0, crecido - sed() * 0.3) + paso);
  ultimoRiego = Date.now();
  memo.guardar({ c: crecido, t: ultimoRiego, r: riegos });
  riego = { t0: 0 };
  anim.iniciar();
  // La planta crece cuando termina de caer el agua.
  setTimeout(dibujar, 900);
  Obj.emitir("riego", { etapa: crecido >= 1 ? "flor" : crecido > 0.55 ? "pimpollo" : "creciendo", riegos });
}

Obj.boton(Obj.$("toque"), regar);
Obj.mensaje("regar", regar);
function pintar() {
  const c = Obj.color(P.maceta, "#C1652F");
  Obj.$("maceta").setAttribute("color", c);
  Obj.$("borde").setAttribute("color", Obj.tono(c, -0.12));
  dibujar();
}
Obj.alCambiar(pintar);
pintar();
// La sed avanza sola: se revisa cada tanto.
setInterval(dibujar, 60000);
