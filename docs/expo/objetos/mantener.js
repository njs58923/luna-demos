...
;
// El botón de mantener: una carga que sube apretado y baja suelto.
//
// Sube a razón de 1/segundos por segundo; suelto, baja al doble de rápido.
// El aro de veinticuatro luces muestra la carga; al completar se pone verde
// y el botón queda trabado hasta el mensaje `reiniciar` (o, con `repetir`,
// dos segundos).
const P = Obj.props({ segundos: 3, color: "#F77F00", etiqueta: "", repetir: true });

const sinte = Obj.sinte({ volumen: 0.6 });
const movil = Obj.$("movil");
const LUCES = 24, R = 0.07;
const luces = [];
for (let i = 0; i < LUCES; i++) {
  const a = (i / LUCES) * Math.PI * 2;
  luces.push(Obj.crear("box", { x: Math.sin(a) * R, y: Math.cos(a) * R, rz: -a, sx: 0.012, sy: 0.02, sz: 0.003, color: "#3A3F4B" }, Obj.$("aro")));
}

let carga = 0, apretado = false, trabado = false, completo = false;
let avisada = -1, prendidas = -1, zumbando = false;
const segundos = () => Math.max(0.5, Math.min(30, Number(P.segundos) || 3));

function pintar() {
  const n = Math.round(carga * LUCES);
  if (n !== prendidas) {
    prendidas = n;
    const c = completo ? "#30D158" : Obj.color(P.color, "#F77F00");
    luces.forEach((l, i) => l.setAttribute("color", i < n ? c : "#3A3F4B"));
  }
  movil.position = { x: 0, y: 0.01, z: apretado || completo ? 0.028 : 0.036 };
}
function zumbido() {
  let fase = 0;
  return () => { fase += (2 * Math.PI * (120 + carga * 260)) / 22050; return Math.sin(fase) * 0.06 + Math.sin(fase * 2.01) * 0.03; };
}
const anim = Obj.animador((t, dt) => {
  if (completo) return false;
  const antes = carga;
  if (apretado) carga = Math.min(1, carga + dt / segundos());
  else carga = Math.max(0, carga - (2 * dt) / segundos());
  const sonar = apretado && carga < 1;
  if (sonar !== zumbando) { zumbando = sonar; sinte.flujo(sonar ? zumbido() : null); }
  const pasos = Math.round(carga * 20);
  if (pasos !== avisada) { avisada = pasos; Obj.emitir("valor", { valor: +carga.toFixed(3), fraccion: +carga.toFixed(3) }); }
  if (carga >= 1) {
    completo = true;
    prendidas = -1;
    sinte.flujo(null); zumbando = false;
    for (let i = 0; i < 3; i++) sinte.nota(Obj.frecuencia(i * 4 + 5), 0.3, { tipo: "campana", vol: 0.2, retraso: i * 0.1 });
    Obj.emitir("completo", {});
    if (P.repetir !== false) setTimeout(reiniciar, 2000);
  }
  pintar();
  return completo ? false : apretado || carga > 0 || antes !== carga;
});

function apretar(si) {
  if (completo || apretado === si) return;
  apretado = si;
  sinte.nota(si ? 1300 : 900, 0.02, { tipo: "pulso", vol: 0.25 });
  pintar();
  anim.iniciar();
}
function reiniciar() {
  completo = false; trabado = false; apretado = false; carga = 0; prendidas = -1; avisada = -1;
  pintar();
  Obj.emitir("valor", { valor: 0, fraccion: 0 });
}

// Escritorio: un clic lo traba apretado, otro lo suelta.
Obj.boton(Obj.$("toque"), () => {
  if (completo) return;
  trabado = !trabado;
  apretar(trabado);
...
// VR: el botón de las manos (_vr.js): empujado con el mando o con grip o
// gatillo encima, carga mientras dure.
Obj.manos().boton({
  centro: { x: 0, y: 0.01, z: 0.046 }, radio: 0.038, recorrido: 0.008,
  alApretar() { if (!trabado) apretar(true); },
  alSoltar() { if (!trabado) apretar(false); },
});

Obj.mensaje("reiniciar", reiniciar);
Obj.alCambiar(() => { prendidas = -1; Obj.$("etiqueta").setAttribute("value", String(P.etiqueta || "").slice(0, 16)); Obj.$("tapa").setAttribute("color", Obj.color(P.color, "#F77F00")); pintar(); });
Obj.$("etiqueta").setAttribute("value", String(P.etiqueta || "").slice(0, 16));
Obj.$("tapa").setAttribute("color", Obj.color(P.color, "#F77F00"));
pintar();
