...
;
// La máquina de escribir: el texto se escribe siempre en el mismo punto y lo
// que se mueve es el papel.
//
// El punto de escritura está fijo en el medio de la máquina (la marquita
// roja). A cada letra el carro corre hacia la izquierda lo que mide el
// renglón escrito, y a cada Enter vuelve a la derecha y el papel sube un
// renglón. Los renglones son textos centrados: para que queden alineados a la
// izquierda se corren la mitad de su ancho, medido con la fuente.
const P = Obj.props({ clave: "", color: "#1F3B2F", tinta: "#1C1C1C" });

const sinte = Obj.sinte({ volumen: 0.8 });
const guardado = Obj.memoria("hoja");
const carro = Obj.$("carro"), papel = Obj.$("papel"), renglonesG = Obj.$("renglones");
const SIZE = 0.0085, LH = 0.0125, MARGEN = -0.09, VISIBLES = 18, COLS = 32, CAMPANA = 28;

const nodos = [];
for (let i = 0; i < VISIBLES; i++) nodos.push(Obj.crear("text", { value: "", size: SIZE, z: 0.0008, color: "#1C1C1C" }, renglonesG));

// Las teclas: cuatro hileras en escalera.
const teclas = [];
[10, 10, 9, 7].forEach((n, fila) => {
  for (let i = 0; i < n; i++) {
    const x = (i - (n - 1) / 2) * 0.026 + (fila % 2) * 0.006;
    const y = 0.078 + fila * 0.011, z = 0.1 - fila * 0.026;
    const g = Obj.crear("group", { x, y, z }, Obj.$("teclas"));
    Obj.crear("cylinder", { y: -0.008, sx: 0.003, sy: 0.016, sz: 0.003, color: "#8C8C8C" }, g);
    Obj.crear("cylinder", { sx: 0.017, sy: 0.005, sz: 0.017, color: "#111111" }, g);
    Obj.crear("cylinder", { y: 0.0027, sx: 0.013, sy: 0.0006, sz: 0.013, color: "#E8E4D8" }, g);
    teclas.push({ g, y, x, z });
  }
});
function hundir(t) {
  t.g.position = { x: t.x, y: t.y - 0.005, z: t.z };
  setTimeout(() => { t.g.position = { x: t.x, y: t.y, z: t.z }; }, 90);
}

/** Cuánto mide un texto, con la fuente si está o a ojo. */
function mide(t) {
  if (!t) return 0;
  if (typeof TextLayout !== "undefined") { try { return TextLayout.create(t, SIZE).w; } catch (e) { /* a ojo */ } }
  return t.length * SIZE * 0.55;
}
/** Renglones: los saltos de línea y, si uno se pasa de COLS, se corta ahí. */
function renglones(texto) {
  const out = [];
  for (const p of String(texto).split(String.fromCharCode(10))) {
    const letras = Array.from(p);
    if (!letras.length) { out.push(""); continue; }
    for (let i = 0; i < letras.length; i += COLS) out.push(letras.slice(i, i + COLS).join(""));
    if (letras.length % COLS === 0) out.push("");
  }
  return out;
}

let texto = String(guardado.leer("") || "");
let carroX = 0, carroMeta = 0, arribaPapel = 0.02;
function dibujar() {
  const r = renglones(texto);
  const c = r.length - 1;               // el renglón donde se escribe
  const desde = Math.max(0, c - VISIBLES + 1);
  nodos.forEach((n, i) => {
    const k = desde + i;
    const t = k <= c ? r[k] : "";
    if (n.valor !== t) { n.valor = t; n.setAttribute("value", t); }
    n.position = { x: MARGEN + mide(t) / 2, y: 0.006 + (c - k) * LH, z: 0.0008 };
  });
  const arriba = 0.006 + Math.min(c, VISIBLES - 1) * LH + 0.014;
  arribaPapel = arriba;
  papel.position = { x: 0, y: (arriba - 0.006) / 2, z: 0 };
  papel.scale = { x: 0.216, y: arriba + 0.006, z: 0.0012 };
  carroMeta = -MARGEN - mide(r[c]);
  anim.iniciar();
  return r;
}
const anim = Obj.animador((t, dt) => {
  // El carro va a su lugar: de a saltitos al escribir, deslizando al volver.
  const d = carroMeta - carroX;
  carroX += Math.abs(d) < 0.001 ? d : d * Math.min(1, dt * (d > 0 ? 9 : 30));
  carro.position = { x: carroX, y: 0.13, z: -0.07 };
  return Math.abs(carroMeta - carroX) > 1e-4;
});

// ── Escribir ───────────────────────────────────────────────────────────────
function sonarLetra(ch) {
  if (ch === " ") sinte.nota(140, 0.04, { tipo: "madera", vol: 0.25 });
  else {
    sinte.nota(160 + Math.random() * 30, 0.06, { tipo: "madera", vol: 0.5 });
    sinte.nota(2500, 0.02, { tipo: "ruido", vol: 0.12, caida: 60 });
    hundir(teclas[Math.floor(Math.random() * teclas.length)]);
  }
}
function sonarRetorno() {
  sinte.nota(600, 0.35, { tipo: "ruido", vol: 0.12, caida: 5 });
  for (let i = 0; i < 3; i++) sinte.nota(1200, 0.02, { tipo: "pulso", vol: 0.2, retraso: 0.25 + i * 0.04 });
}
function alCambiarTexto(nuevo) {
  const antes = texto;
  texto = nuevo;
  const r = dibujar();
  if (nuevo.length > antes.length && nuevo.startsWith(antes)) {
    const agregado = nuevo.slice(antes.length);
    const ult = agregado[agregado.length - 1];
    if (ult === String.fromCharCode(10)) sonarRetorno();
    else {
      sonarLetra(ult);
      if (Array.from(r[r.length - 1]).length === CAMPANA) sinte.nota(2100, 0.8, { tipo: "campana", vol: 0.3, caida: 3 });
    }
  } else if (nuevo.length < antes.length) sinte.nota(900, 0.03, { tipo: "pulso", vol: 0.2 });
  guardado.guardar(texto);
}

const editor = Obj.teclado(Obj.$("cuerpo"), {
  texto, max: 2000,
  cambio: alCambiarTexto,
});
Obj.boton(Obj.$("cuerpo"), () => editor.tomar());
Obj.boton(Obj.$("espacio"), () => editor.tomar());
// La palanca: retorno de carro.
Obj.boton(Obj.$("toque_palanca"), () => { editor.texto = texto + String.fromCharCode(10); alCambiarTexto(editor.texto); });

// Arrancar la hoja: el primer toque pregunta, el segundo (enseguida) arranca.
let confirmar = -Infinity;
const aviso = Obj.crear("text", { y: 0.03, z: 0.001, value: "", size: 0.01, color: "#D62828" }, Obj.$("hoja"));
Obj.boton(papel, () => {
  const ahora = performance.now();
  if (ahora - confirmar < 2500) { confirmar = -Infinity; aviso.setAttribute("value", ""); nueva(); return; }
  confirmar = ahora;
  aviso.position = { x: 0, y: arribaPapel + 0.01, z: 0.001 };
  aviso.setAttribute("value", "tocá de nuevo para arrancar la hoja");
  setTimeout(() => { if (performance.now() - confirmar >= 2500) aviso.setAttribute("value", ""); }, 2600);
});
function nueva() {
  if (texto.trim()) Obj.emitir("hoja", { texto });
  sinte.nota(3000, 0.4, { tipo: "ruido", vol: 0.15, caida: 6 });
  editor.texto = "";
  texto = "";
  guardado.guardar("");
  dibujar();
...
// Escribir un texto que llega por mensaje, letra por letra.
let dictado = [];
Obj.mensaje("escribir", (d) => {
  const t = d && typeof d.texto === "string" ? d.texto : typeof d === "string" ? d : "";
  dictado.push(...Array.from(t));
  if (dictado.length === Array.from(t).length) teclear();
});
function teclear() {
  if (!dictado.length) return;
  const ch = dictado.shift();
  editor.texto = texto + ch;
  alCambiarTexto(editor.texto);
  setTimeout(teclear, ch === String.fromCharCode(10) ? 450 : 110);
}

function pintar() {
  Obj.$("cuerpo").setAttribute("color", Obj.color(P.color, "#1F3B2F"));
  for (const n of nodos) n.setAttribute("color", Obj.color(P.tinta, "#1C1C1C"));
}
