...
;
// El marcador: dos números, cuatro botones, y un tanteo que no se pierde.
//
// Cada cambio hace latir el número —crece un poco y vuelve—, que es lo que
// dice "anotaron" sin tener que leer.
const P = Obj.props({ local: "LOCAL", visita: "VISITA", color_local: "#FF3B30", color_visita: "#0A84FF", clave: "" });

const memo = Obj.memoria("tanteo");
const g = memo.leer(null);
let tanteo = { local: g && Number.isInteger(g.local) ? g.local : 0, visita: g && Number.isInteger(g.visita) ? g.visita : 0 };
const num = { local: Obj.$("puntos_local"), visita: Obj.$("puntos_visita") };
const latido = { local: 0, visita: 0 };

function dibujar() {
  for (const k of ["local", "visita"]) num[k].setAttribute("value", String(tanteo[k]));
  Obj.$("nombre_local").setAttribute("value", String(P.local || "LOCAL").slice(0, 12));
  Obj.$("nombre_visita").setAttribute("value", String(P.visita || "VISITA").slice(0, 12));
  num.local.setAttribute("color", Obj.color(P.color_local, "#FF3B30"));
  num.visita.setAttribute("color", Obj.color(P.color_visita, "#0A84FF"));
}

const anim = Obj.animador(() => {
  const ahora = performance.now();
  let seguir = false;
  for (const k of ["local", "visita"]) {
    const q = Math.max(0, (latido[k] - ahora) / 300);
    num[k].setAttribute("size", String(0.2 * (1 + 0.25 * Math.sin(q * Math.PI))));
    if (q > 0) seguir = true;
  }
  return seguir;
});

function sumar(equipo, n) {
  if (equipo !== "local" && equipo !== "visita") return;
  const antes = tanteo[equipo];
  tanteo[equipo] = Math.max(0, Math.min(999, antes + n));
  if (tanteo[equipo] === antes) return;
  memo.guardar(tanteo);
  latido[equipo] = performance.now() + 300;
  anim.iniciar();
  dibujar();
  Obj.emitir("punto", { local: tanteo.local, visita: tanteo.visita });
}
function cero() {
  tanteo = { local: 0, visita: 0 };
  memo.guardar(tanteo);
  dibujar();
  Obj.emitir("punto", { local: 0, visita: 0 });
}

Obj.boton(Obj.$("mas_local"), () => sumar("local", 1), { color: "#30D158" });
Obj.boton(Obj.$("menos_local"), () => sumar("local", -1), { color: "#3A3F4B" });
Obj.boton(Obj.$("mas_visita"), () => sumar("visita", 1), { color: "#30D158" });
Obj.boton(Obj.$("menos_visita"), () => sumar("visita", -1), { color: "#3A3F4B" });
Obj.boton(Obj.$("cero"), cero, { color: "#FFD60A" });
Obj.mensaje("sumar", (d) => { if (d && Number.isFinite(d.puntos)) sumar(d.equipo, Math.round(d.puntos)); });
Obj.mensaje("cero", cero);
Obj.alCambiar(dibujar);
dibujar();
