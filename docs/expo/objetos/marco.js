...
;
// El marco de fotos: un <image> y una lista.
//
// <image> tiene la API de un HTMLImageElement: cambiar `src` pide la imagen
// nueva y avisa con load o error. Mientras carga, el nodo no se dibuja (lo
// oculta el host); una que falla muestra el aviso en el paspartú y se sigue
// con la próxima. Abajo, un punto por foto marca cuál es.
const P = Obj.props({
  // Relativas a propósito: se resuelven contra este documento, o sea contra
  // el servidor de la calle, que tiene estas vistas de la expo.
  imagenes: ["../preview.png", "../preview_aerea.png", "../preview_fogata.png", "../preview_mariposa.png", "../preview_farol.png"],
  segundos: 0, marco: "#3A2A1A", paspartu: "#F4F1EA",
});

const foto = Obj.$("foto");
const aviso = Obj.$("cargando");
const puntosG = Obj.$("puntos");
let indice = 0;
let puntos = [];
let pasador = null;

function lista() {
  const l = Array.isArray(P.imagenes) ? P.imagenes.filter((s) => typeof s === "string" && s.trim()) : [];
  return l.slice(0, 24);
}
function absoluta(src) {
  try { return new URL(src, location.href).href; } catch (e) { return src; }
}
function armarPuntos() {
  for (const p of puntos) p.remove();
  const n = lista().length;
  puntos = [];
  for (let i = 0; i < n; i++) {
    puntos.push(Obj.crear("plane", { x: (i - (n - 1) / 2) * 0.022, sx: 0.012, sy: 0.012, "border-radius": 0.5, color: "#B0A89A" }, puntosG));
  }
}
function mostrar(i) {
  const l = lista();
  if (!l.length) { aviso.setAttribute("value", "sin imágenes"); return; }
  indice = ((i % l.length) + l.length) % l.length;
  aviso.setAttribute("value", "cargando…");
  foto.src = absoluta(l[indice]);
  puntos.forEach((p, k) => p.setAttribute("color", k === indice ? "#1C1C1C" : "#B0A89A"));
  Obj.emitir("foto", { indice, src: absoluta(l[indice]) });
}
foto.onload = () => aviso.setAttribute("value", "");
foto.onerror = () => aviso.setAttribute("value", "no cargó: " + String(lista()[indice] || "").slice(-28));

function programar() {
  clearInterval(pasador);
  const s = Number(P.segundos) || 0;
  pasador = s > 0 ? setInterval(() => mostrar(indice + 1), Math.max(2, s) * 1000) : null;
}

Obj.boton(Obj.$("toque"), () => { mostrar(indice + 1); programar(); });
Obj.mensaje("siguiente", () => mostrar(indice + 1));
Obj.mensaje("anterior", () => mostrar(indice - 1));
function pintar() {
  Obj.$("marco").setAttribute("color", Obj.color(P.marco, "#3A2A1A"));
  Obj.$("paspartu").setAttribute("color", Obj.color(P.paspartu, "#F4F1EA"));
}
Obj.alCambiar((p, a, cambios) => {
  pintar();
  if (cambios.includes("imagenes")) { armarPuntos(); mostrar(0); }
  if (cambios.includes("segundos")) programar();
});
pintar();
armarPuntos();
mostrar(0);
