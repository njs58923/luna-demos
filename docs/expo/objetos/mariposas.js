...
;
// Las mariposas: vuelo errático hacia un blanco que cambia, y posarse.
//
// Cada mariposa tiene un blanco (una flor o un punto del aire alrededor) y
// una velocidad que gira hacia él con algo de ruido: nunca va derecho. Si el
// blanco es una flor y llega, se posa unos segundos abriendo y cerrando las
// alas despacio. Una mano quieta un rato cerca de las flores se vuelve un
// blanco más: la primera mariposa libre va y se queda en ella mientras la
// mano no se mueva rápido.
const P = Obj.props({ cantidad: 3, colores: ["#FF9F1C", "#4CC9F0", "#F72585", "#FFD60A", "#80ED99"], maceta: "#C0663A" });

const donde = Obj.marco();
const N = Math.max(1, Math.min(5, Math.round(Number(P.cantidad) || 3)));
const CENTRO = { x: 0, y: 0.45, z: 0 };

// ── Las flores ─────────────────────────────────────────────────────────────
const FLORES = [
  { x: -0.05, z: 0.02, alto: 0.46, color: "#FF70A6" },
  { x: 0.05, z: -0.03, alto: 0.54, color: "#FFFFFF" },
  { x: 0.01, z: 0.06, alto: 0.4, color: "#B388EB" },
];
for (const f of FLORES) {
  const base = { x: f.x * 0.4, y: 0.24, z: f.z * 0.4 };
  const cima = { x: f.x, y: f.alto, z: f.z };
  const tallo = Obj.crear("cylinder", { sx: 0.008, sy: 0.1, sz: 0.008, color: "#3A7D44" }, Obj.$("flores"));
  const l = Obj.tender(tallo, base, cima);
  tallo.scale = { x: 0.008, y: l, z: 0.008 };
  Obj.crear("sphere", { x: (base.x + cima.x) / 2 + 0.02, y: (base.y + cima.y) / 2, z: (base.z + cima.z) / 2, sx: 0.04, sy: 0.006, sz: 0.02, rz: 0.4, color: "#4F9D5B" }, Obj.$("flores"));
  const cab = Obj.crear("group", { x: cima.x, y: cima.y, z: cima.z }, Obj.$("flores"));
  for (let k = 0; k < 6; k++) {
    const a = (k / 6) * Math.PI * 2;
    Obj.crear("sphere", { x: Math.cos(a) * 0.022, z: Math.sin(a) * 0.022, ry: -a, sx: 0.035, sy: 0.006, sz: 0.018, color: f.color }, cab);
  }
  Obj.crear("sphere", { y: 0.004, sx: 0.02, sy: 0.012, sz: 0.02, color: "#FFC300" }, cab);
  f.cima = { x: cima.x, y: cima.y + 0.012, z: cima.z };
}

// ── Las mariposas (miran a +Z) ─────────────────────────────────────────────
function colorDe(i) {
  const c = Array.isArray(P.colores) ? P.colores.map((x) => Obj.color(x, null)).filter(Boolean) : [];
  const l = c.length ? c : ["#FF9F1C", "#4CC9F0", "#F72585"];
  return l[i % l.length];
}
const bichos = [];
for (let i = 0; i < N; i++) {
  const g = Obj.crear("group", {}, Obj.$("bichos"));
  Obj.crear("cylinder", { rx: Math.PI / 2, sx: 0.006, sy: 0.04, sz: 0.006, color: "#2B2B2B" }, g);
  Obj.crear("cylinder", { y: 0.006, z: 0.026, rx: 1.1, x: -0.004, sx: 0.0015, sy: 0.018, sz: 0.0015, color: "#2B2B2B" }, g);
  Obj.crear("cylinder", { y: 0.006, z: 0.026, rx: 1.1, x: 0.004, sx: 0.0015, sy: 0.018, sz: 0.0015, color: "#2B2B2B" }, g);
  // Cada lado es una bisagra sobre el cuerpo con el ala de adelante y la de atrás.
  const alas = [], bisagras = [];
  for (const lado of [-1, 1]) {
    const bis = Obj.crear("group", {}, g);
    const c = colorDe(i);
    alas.push(Obj.crear("box", { x: lado * 0.021, z: 0.008, ry: lado * 0.25, sx: 0.038, sy: 0.001, sz: 0.03, color: c }, bis));
    alas.push(Obj.crear("box", { x: lado * 0.016, z: -0.014, ry: -lado * 0.3, sx: 0.028, sy: 0.001, sz: 0.022, color: Obj.tono(c, -0.2) }, bis));
    Obj.crear("sphere", { x: lado * 0.028, y: 0.001, z: 0.01, sx: 0.008, sy: 0.0015, sz: 0.008, color: "#FFFFFF" }, bis);
    bisagras.push({ el: bis, lado });
  }
  const a = (i / N) * Math.PI * 2;
  bichos.push({
    i, g, alas, bisagras,
    p: { x: Math.cos(a) * 0.25, y: 0.5 + 0.05 * i, z: Math.sin(a) * 0.25 },
    v: { x: 0, y: 0, z: 0 },
    blanco: null, posada: null, hasta: 0, fase: i * 1.3, rumbo: 0,
  });
}

function nuevoBlanco(b) {
  const ahora = performance.now();
  if (Math.random() < 0.35) {
    const f = FLORES[Math.floor(Math.random() * FLORES.length)];
    if (!bichos.some((o) => o !== b && o.posada === f)) { b.blanco = { ...f.cima, flor: f }; b.hasta = ahora + 8000; return; }
  }
  const a = Math.random() * Math.PI * 2, r = 0.12 + Math.random() * 0.25;
  b.blanco = { x: CENTRO.x + Math.cos(a) * r, y: 0.3 + Math.random() * 0.4, z: CENTRO.z + Math.sin(a) * r };
  b.hasta = ahora + 1500 + Math.random() * 2000;
}

// ── La mano (VR) ───────────────────────────────────────────────────────────
let mano = null;           // { p, quietaDesde, ultima }
let enMano = null;         // la mariposa posada en la mano

let espantadaHasta = 0;
...
function paso() {
  ...
  antes = ahora;
  const t = ahora / 1000;
  const espantadas = ahora < espantadaHasta;

  // Una mano quieta hace 1.5 s cerca de las flores llama a una mariposa.
  if (mano && !enMano && !espantadas && ahora - mano.ultima < 300 && mano.quietaDesde && ahora - mano.quietaDesde > 1500) {
    const libre = bichos.find((b) => !b.posada);
    if (libre) { enMano = libre; libre.blanco = { ...mano.p, y: mano.p.y + 0.03, enMano: true }; }
  }
  if (enMano && (!mano || ahora - mano.ultima > 400)) { soltarMano(); }

  for (const b of bichos) {
    let aleteo = 22;
    if (b.posada) {
      aleteo = 0;
      if (b.posada === "mano" && mano) b.p = { x: mano.p.x, y: mano.p.y + 0.03, z: mano.p.z };
      if (espantadas || (b.posada !== "mano" && ahora > b.hasta)) { b.posada = null; nuevoBlanco(b); b.v.y = 0.4; }
    } else {
      if (!b.blanco || (!b.blanco.enMano && ahora > b.hasta) || (espantadas && b.blanco.flor)) nuevoBlanco(b);
      if (espantadas) b.blanco = { x: b.p.x * 3, y: 0.8, z: b.p.z * 3 };
      if (b.blanco.enMano && mano) b.blanco = { ...mano.p, y: mano.p.y + 0.03, enMano: true };
      const d = { x: b.blanco.x - b.p.x, y: b.blanco.y - b.p.y, z: b.blanco.z - b.p.z };
      const l = Math.hypot(d.x, d.y, d.z) || 1;
      const rapidez = espantadas ? 1.2 : Math.min(0.35, 0.1 + l);
      // Gira hacia el blanco con un zigzag propio.
      const zz = { x: Math.sin(t * 3.1 + b.fase) * 0.25, y: Math.sin(t * 4.3 + b.fase * 2) * 0.2, z: Math.cos(t * 2.7 + b.fase) * 0.25 };
      for (const k of ["x", "y", "z"]) b.v[k] += ((d[k] / l) * rapidez + zz[k] * (l > 0.05 ? 1 : 0.2) - b.v[k]) * Math.min(1, dt * 3);
      b.p = { x: b.p.x + b.v.x * dt, y: b.p.y + b.v.y * dt + Math.sin(t * 11 + b.fase) * 0.0015, z: b.p.z + b.v.z * dt };
      if (b.p.y < 0.27) b.p.y = 0.27;
      if (Math.hypot(b.v.x, b.v.z) > 0.02) b.rumbo = Math.atan2(b.v.x, b.v.z);
      // Llegó.
      if (l < 0.015 && b.blanco.flor && bichos.some((o) => o.posada === b.blanco.flor)) nuevoBlanco(b);
      else if (l < 0.015 && b.blanco.flor) { b.posada = b.blanco.flor; b.p = { ...b.blanco.flor.cima }; b.hasta = ahora + 3000 + Math.random() * 5000; b.v = { x: 0, y: 0, z: 0 }; Obj.emitir("posada", { donde: "flor" }); }
      else if (l < 0.02 && b.blanco.enMano) { b.posada = "mano"; b.p = { x: b.blanco.x, y: b.blanco.y, z: b.blanco.z }; b.v = { x: 0, y: 0, z: 0 }; Obj.emitir("posada", { donde: "mano" }); }
    }
    b.g.position = b.p;
    b.g.rotation = { x: 0, y: b.rumbo, z: 0 };
    // Alas: batiendo rápido en vuelo, abriendo y cerrando lento posada.
    const ang = aleteo ? 0.2 + 0.9 * (0.5 + 0.5 * Math.sin(t * aleteo + b.fase)) : 0.7 + 0.6 * (0.5 + 0.5 * Math.sin(t * 1.5 + b.fase));
    for (const h of b.bisagras) h.el.rotation = { x: 0, y: 0, z: h.lado * ang };
  }
}
setInterval(paso, 40);
...
function soltarMano() {
  ...
  const b = enMano;
  enMano = null;
  if (b.posada === "mano") b.posada = null;
  nuevoBlanco(b);
  b.v.y = 0.3;
}

function espantar() {
  espantadaHasta = performance.now() + 900;
  soltarMano();
  Obj.emitir("vuelan", {});
}
Obj.boton(Obj.$("toque"), espantar);
Obj.mensaje("espantar", espantar);
...
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    const ahora = performance.now();
    const cerca = Math.hypot(l.x - CENTRO.x, l.y - CENTRO.y, l.z - CENTRO.z) < 0.45;
    if (!cerca) { if (mano && mano.mano === e.hand) { mano = null; } return; }
    if (mano && mano.mano !== e.hand && ahora - mano.ultima < 300) return;   // una mano a la vez
    const antes = mano;
    const dt = antes ? Math.max(0.001, (ahora - antes.ultima) / 1000) : 1;
    const v = antes ? Math.hypot(l.x - antes.p.x, l.y - antes.p.y, l.z - antes.p.z) / dt : 0;
    // El temblor del seguimiento mueve la mano unos milímetros por cuadro:
    // "quieta" se mide con la velocidad promediada.
    const vs = antes ? antes.vs * 0.85 + v * 0.15 : 0;
    const quieta = vs < 0.12;
    mano = { mano: e.hand, p: l, ultima: ahora, vs, quietaDesde: quieta ? (antes && antes.quietaDesde) || ahora : 0 };
    // Un sacudón la espanta.
    if (v > 1.2 && enMano) espantar();
  });
}
...
function pintar() {
  bichos.forEach((b) => {
    const c = colorDe(b.i);
    b.alas.forEach((a, k) => a.setAttribute("color", k % 2 ? Obj.tono(c, -0.2) : c));
  });
  Obj.$("maceta").setAttribute("color", Obj.color(P.maceta, "#C0663A"));
}
