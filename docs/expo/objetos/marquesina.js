...
;
// La marquesina: casillas fijas, y el texto que pasa por ellas.
//
// No se mueve ningún texto por la pantalla —no hay recorte: una letra que se
// sale del letrero se vería flotando afuera—. Hay una fila de casillas, cada
// una con una letra, y lo que avanza es qué pedazo del mensaje cae en cada
// casilla. Una casilla sólo se reescribe si su letra cambió.
const P = Obj.props({ texto: "BIENVENIDOS A LA CALLE DE LOS OBJETOS", color: "#FF9F0A", velocidad: 6, editable: true, clave: "" });

const CASILLAS = 18, PASO = 1.08 / CASILLAS;
const memo = Obj.memoria("texto");
const grupo = Obj.$("letras");
const ayuda = Obj.$("ayuda");
const casillas = [];
for (let i = 0; i < CASILLAS; i++) {
  const x = -0.54 + PASO * (i + 0.5);
  // Una "lamparita" apagada de fondo en cada casilla, para que se lea como
  // un tablero de luces.
  Obj.crear("plane", { x, sx: PASO * 0.86, sy: 0.12, color: "#1A1208", "material-unlit": "true" }, grupo);
  casillas.push({ el: Obj.crear("text", { x, z: 0.001, value: "", size: 0.1, color: "#FF9F0A", "material-unlit": "true" }, grupo), letra: null });
}

let texto = String(memo.leer(null) ?? P.texto ?? "").toUpperCase().slice(0, 120);
let pos = 0;
let pausa = false;

function cinta() { return "      " + texto + "      "; }
function paso() {
  if (pausa || editor.enfocado) return;
  const c = cinta();
  pos = (pos + 1) % c.length;
  dibujar(c);
}
function dibujar(c) {
  const col = Obj.color(P.color, "#FF9F0A");
  for (let i = 0; i < CASILLAS; i++) {
    const letra = c[(pos + i) % c.length] || " ";
    const k = casillas[i];
    if (k.letra !== letra) { k.letra = letra; k.el.setAttribute("value", letra === " " ? "" : letra); }
  }
  if (col !== dibujar.col) { dibujar.col = col; for (const k of casillas) k.el.setAttribute("color", col); }
}
let reloj = null;
function programar() {
  clearInterval(reloj);
  const v = Math.max(1, Math.min(20, Number(P.velocidad) || 6));
  reloj = setInterval(paso, 1000 / v);
}

const editor = Obj.teclado(Obj.$("letras"), {
  unaLinea: true, max: 120,
  cambio: (t) => {
    // Mientras se escribe, se ve el final de lo escrito, quieto.
    const vista = (t.toUpperCase() + "_").slice(-CASILLAS).padEnd(CASILLAS, " ");
    pos = 0;
    dibujar(vista);
  },
  foco: (si) => {
    if (si) { editor.texto = ""; ayuda.setAttribute("value", "escribí el mensaje y Enter"); dibujar("_".padEnd(CASILLAS, " ")); return; }
    ayuda.setAttribute("value", "");
    const t = editor.texto.trim().toUpperCase();
    if (t) { texto = t.slice(0, 120); memo.guardar(texto); Obj.emitir("cambio", { texto }); }
    pos = 0;
    dibujar(cinta());
  },
});
Obj.boton(Obj.$("toque"), () => { if (P.editable !== false) editor.tomar(); else pausa = !pausa; });
Obj.mensaje("texto", (d) => {
  if (!d || typeof d.texto !== "string") return;
  texto = d.texto.toUpperCase().slice(0, 120);
  memo.guardar(texto);
  pos = 0;
  dibujar(cinta());
});
Obj.alCambiar((p, a, cambios) => {
  if (cambios.includes("velocidad")) programar();
  dibujar(cinta());
});
dibujar(cinta());
programar();
