...
;
// La mecedora: balancines en arco y un péndulo amortiguado.
//
// Un balancín de verdad rueda sobre el piso: al girar un ángulo θ, el punto de
// apoyo se corre R·θ y el centro del arco no se mueve de altura. Se hace así:
// la silla gira alrededor del centro del arco (a R metros sobre el piso) y
// se traslada R·θ en Z. Con eso no atraviesa el piso ni flota.
const P = Obj.props({ madera: "#8B5A2B", almohadon: "#9A2A2A" });

const silla = Obj.$("silla");
const R = 1.1;   // radio de los balancines
const pintMadera = [], pintTela = [];
const madera = (attrs) => { const el = Obj.crear("box", Object.assign({ color: Obj.color(P.madera, "#8B5A2B") }, attrs), cuerpo); pintMadera.push(el); return el; };

// El cuerpo se arma en coordenadas de la silla parada y se lo cuelga de un
// grupo cuyo origen es el centro del arco.
// (Referencias propias y no `cuerpo.parent`: un nodo recién creado no conoce
// a su padre hasta que el motor lo resuelve.)
const giro = Obj.crear("group", { y: R }, silla);
const cuerpo = Obj.crear("group", { y: -R }, giro);

// Balancines: arcos de cajas, a cada lado.
for (const s of [-1, 1]) {
  for (let i = -4; i <= 4; i++) {
    const a = (i / 4) * 0.36;
    const z = Math.sin(a) * R, y = R - Math.cos(a) * R;
    madera({ x: s * 0.26, y: y + 0.015, z, rx: a, sx: 0.03, sy: 0.03, sz: (0.72 * R) / 9 + 0.02 });
  }
}
// Patas, asiento, respaldo y brazos.
for (const s of [-1, 1]) for (const z of [-0.22, 0.2]) madera({ x: s * 0.26, y: 0.25, z, sx: 0.035, sy: 0.4, sz: 0.035 });
madera({ y: 0.45, sx: 0.56, sy: 0.04, sz: 0.5 });
for (let i = 0; i < 5; i++) madera({ x: -0.2 + i * 0.1, y: 0.78, z: -0.27, rx: -0.2, sx: 0.05, sy: 0.62, sz: 0.02 });
madera({ y: 1.08, z: -0.33, rx: -0.2, sx: 0.6, sy: 0.06, sz: 0.04 });
for (const s of [-1, 1]) {
  madera({ x: s * 0.29, y: 0.66, z: -0.02, sx: 0.05, sy: 0.03, sz: 0.5 });
  madera({ x: s * 0.29, y: 0.56, z: 0.2, sx: 0.03, sy: 0.2, sz: 0.03 });
}
pintTela.push(Obj.crear("box", { y: 0.48, z: 0.01, sx: 0.5, sy: 0.05, sz: 0.44, color: Obj.color(P.almohadon, "#9A2A2A"), "border-radius": 0.02 }, cuerpo));
pintTela.push(Obj.crear("box", { y: 0.8, z: -0.24, rx: -0.2, sx: 0.46, sy: 0.4, sz: 0.04, color: Obj.color(P.almohadon, "#9A2A2A"), "border-radius": 0.02 }, cuerpo));

let angulo = 0, vel = 0;
const anim = Obj.animador((t, dt) => {
  // Péndulo amortiguado: el respaldo pesa y vuelve.
  const acc = -4.5 * Math.sin(angulo) - 0.6 * vel;
  vel += acc * dt;
  angulo += vel * dt;
  angulo = Math.max(-0.3, Math.min(0.3, angulo));
  giro.rotation = { x: angulo, y: 0, z: 0 };
  giro.position = { x: 0, y: R, z: R * angulo };
  if (Math.abs(angulo) < 0.002 && Math.abs(vel) < 0.01) {
    angulo = 0; vel = 0;
    giro.rotation = { x: 0, y: 0, z: 0 };
    giro.position = { x: 0, y: R, z: 0 };
    return false;
  }
  return true;
});
...
function empujar(f) {
  const fuerza = f == null ? 1 : Math.max(0, Math.min(2, f));
  // El empujón va a favor del movimiento: nunca frena la hamaca.
  vel += (vel >= 0 ? 1 : -1) * 0.9 * fuerza;
  anim.iniciar();
  Obj.emitir("hamaca", { fuerza });
}
Obj.boton(Obj.$("toque"), () => empujar(1));
Obj.mensaje("empujar", (d) => empujar(d && Number.isFinite(d.fuerza) ? d.fuerza : 1));
Obj.alCambiar(() => {
  for (const el of pintMadera) el.setAttribute("color", Obj.color(P.madera, "#8B5A2B"));
  for (const el of pintTela) el.setAttribute("color", Obj.color(P.almohadon, "#9A2A2A"));
});
