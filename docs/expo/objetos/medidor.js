...
;
// El medidor: una aguja con resorte hacia la fracción pedida.
//
// La aguja barre de -80° a +80° (en el sentido del reloj desde arriba). Va
// hacia su meta como un resorte poco amortiguado: se pasa un poquito y
// vuelve, que es lo que hace que una aguja se lea como aguja.
const P = Obj.props({ min: 0, max: 100, unidad: "", titulo: "", zonas: true });

const aguja = Obj.$("aguja"), zonasG = Obj.$("zonas");
const BARRIDO = (80 * Math.PI) / 180, R = 0.068;
const min = () => Number(P.min) || 0;
const max = () => (Number.isFinite(Number(P.max)) && Number(P.max) !== min() ? Number(P.max) : min() + 1);
const angDe = (f) => -BARRIDO + f * 2 * BARRIDO;

let meta = 0, f = 0, v = 0;
const zonas = [];
function armarZonas() {
  for (const z of zonas) z.remove();
  zonas.length = 0;
  const N = 32;
  for (let i = 0; i < N; i++) {
    const fr = (i + 0.5) / N, a = angDe(fr);
    const color = P.zonas === false ? "#3A3F4B" : fr < 0.6 ? "#52B788" : fr < 0.85 ? "#FFD166" : "#E63946";
    zonas.push(Obj.crear("box", { x: Math.sin(a) * R, y: Math.cos(a) * R, rz: -a, sx: (2 * BARRIDO * R) / N + 0.0008, sy: 0.008, sz: 0.001, color }, zonasG));
    if (i % 8 === 0 || i === N - 1) {
      const a2 = angDe(i === N - 1 ? 1 : i / N);
      zonas.push(Obj.crear("box", { x: Math.sin(a2) * (R - 0.009), y: Math.cos(a2) * (R - 0.009), rz: -a2, sx: 0.0018, sy: 0.008, sz: 0.001, color: "#1C1C1C" }, zonasG));
    }
  }
}
function texto() {
  const val = min() + (max() - min()) * meta;
  const d = Math.abs(max() - min()) >= 20 ? 0 : 1;
  return val.toFixed(d) + (P.unidad ? " " + String(P.unidad).slice(0, 6) : "");
}
...
const anim = Obj.animador((t, dt) => {
  ...
  v += ((meta - f) * k - v * c) * dt;
  f += v * dt;
  aguja.rotation = { x: 0, y: 0, z: -angDe(Math.max(-0.03, Math.min(1.03, f))) };
  const quieto = Math.abs(meta - f) < 5e-4 && Math.abs(v) < 5e-3;
  if (quieto) { f = meta; v = 0; aguja.rotation = { x: 0, y: 0, z: -angDe(f) }; }
  return !quieto;
});
function poner(fr) {
  meta = Math.max(0, Math.min(1, fr));
  Obj.$("lectura").setAttribute("value", texto());
  anim.iniciar();
}
Obj.mensaje("valor", (d) => {
  if (d == null) return;
  if (typeof d === "number") return poner((d - min()) / (max() - min()));
  if (Number.isFinite(Number(d.valor)) && d.fraccion == null) poner((Number(d.valor) - min()) / (max() - min()));
  else if (Number.isFinite(Number(d.fraccion))) poner(Number(d.fraccion));
});
Obj.boton(Obj.$("toque"), () => poner(Math.random()));
function pintar() {
  Obj.$("titulo").setAttribute("value", String(P.titulo || "").slice(0, 18));
  Obj.$("lectura").setAttribute("value", texto());
}
Obj.alCambiar(() => { armarZonas(); pintar(); });
armarZonas();
pintar();
aguja.rotation = { x: 0, y: 0, z: -angDe(0) };
