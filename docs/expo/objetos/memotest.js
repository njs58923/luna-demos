...
;
// El memotest: fichas que se dan vuelta sobre su eje X.
//
// Cada ficha es un grupo con dos caras: el dorso (arriba) y el dibujo (abajo,
// mirando al piso). Darla vuelta es girar el grupo media vuelta sobre X,
// levantándola un poco en el medio del giro. Los dibujos son ocho figuras
// hechas con primitivas, de ocho colores: no dependen de ninguna fuente.
const P = Obj.props({ clave: "", pares: 8 });

const sinte = Obj.sinte({ volumen: 0.6 });
const record = Obj.memoria("record");
const fichasG = Obj.$("fichas");
const N_PARES = () => Math.max(4, Math.min(8, Math.round(Number(P.pares) || 8)));
const LADO = 0.085, PASO = 0.095;
const FIGURAS = [
  { color: "#E63946", forma: "esfera" }, { color: "#457B9D", forma: "cubo" },
  { color: "#2A9D8F", forma: "cilindro" }, { color: "#F4A261", forma: "cruz" },
  { color: "#7B2FF7", forma: "anillo" }, { color: "#FFD60A", forma: "estrella" },
  { color: "#FF70A6", forma: "dos" }, { color: "#1C1C1C", forma: "barra" },
];

// Dieciséis fichas creadas una vez; la figura se arma al mezclar.
const fichas = [];
for (let i = 0; i < 16; i++) {
  const g = Obj.crear("group", {}, fichasG);
  Obj.crear("box", { y: 0.003, sx: LADO, sy: 0.006, sz: LADO, color: "#F1FAEE", "border-radius": 0.008 }, g);
  Obj.crear("box", { y: 0.0062, sx: LADO * 0.8, sy: 0.0006, sz: LADO * 0.8, color: "#A8DADC", "border-radius": 0.006 }, g);
  // La cara del dibujo mira hacia abajo del grupo (-Y): al girar π queda arriba.
  const dibujo = Obj.crear("group", { y: -0.0005, rx: Math.PI }, g);
  const blanco = Obj.crear("box", { y: 0.004, sx: LADO, sy: 0.012, sz: LADO, color: "transparent", "material-alpha": "blend" }, g);
  const f = { i, g, dibujo, partes: [], fig: null, estado: "tapada", ang: 0, meta: 0, x: 0, z: 0 };
  Obj.boton(blanco, () => tocar(f));
  fichas.push(f);
}
function dibujar(f) {
  for (const p of f.partes) p.remove();
  f.partes = [];
  const c = f.fig.color, d = f.dibujo;
  const add = (tag, a) => f.partes.push(Obj.crear(tag, Object.assign({ color: c }, a), d));
  switch (f.fig.forma) {
    case "esfera": add("sphere", { y: 0.01, sx: 0.04, sy: 0.02, sz: 0.04 }); break;
    case "cubo": add("box", { y: 0.008, ry: 0.4, sx: 0.036, sy: 0.016, sz: 0.036 }); break;
    case "cilindro": add("cylinder", { y: 0.008, sx: 0.045, sy: 0.016, sz: 0.045 }); break;
    case "cruz": add("box", { y: 0.006, sx: 0.05, sy: 0.012, sz: 0.014 }); add("box", { y: 0.006, sx: 0.014, sy: 0.012, sz: 0.05 }); break;
    case "anillo": for (let k = 0; k < 8; k++) { const a = (k / 8) * Math.PI * 2; add("sphere", { x: Math.cos(a) * 0.02, y: 0.006, z: Math.sin(a) * 0.02, sx: 0.012, sy: 0.012, sz: 0.012 }); } break;
    case "estrella": for (let k = 0; k < 3; k++) add("box", { y: 0.006, ry: (k * Math.PI) / 3, sx: 0.055, sy: 0.01, sz: 0.012 }); break;
    case "dos": add("sphere", { x: -0.013, y: 0.009, sx: 0.022, sy: 0.018, sz: 0.022 }); add("sphere", { x: 0.013, y: 0.009, sx: 0.022, sy: 0.018, sz: 0.022 }); break;
    default: add("box", { y: 0.006, ry: 0.785, sx: 0.06, sy: 0.012, sz: 0.012 });
  }
}

// ── La partida ─────────────────────────────────────────────────────────────
let abiertas = [], jugadas = 0, pares = 0, inicio = 0, bloqueo = false, activas = [];
function mezclar() {
  const n = N_PARES();
  const figs = [];
  for (let k = 0; k < n; k++) figs.push(FIGURAS[k], FIGURAS[k]);
  for (let i = figs.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [figs[i], figs[j]] = [figs[j], figs[i]]; }
  // Las fichas que se usan, centradas en una grilla de 4 columnas.
  activas = fichas.slice(0, figs.length);
  const filas = Math.ceil(figs.length / 4);
  fichas.forEach((f, i) => {
    const usada = i < figs.length;
    f.g.setAttribute("visible", usada ? "inherit" : "false");
    if (!usada) return;
    f.fig = figs[i];
    f.estado = "tapada"; f.ang = 0; f.meta = 0;
    dibujar(f);
    const col = i % 4, fila = Math.floor(i / 4);
    f.x = (col - 1.5) * PASO; f.z = (fila - (filas - 1) / 2) * PASO - 0.01;
    f.g.position = { x: f.x, y: 0, z: f.z };
    f.g.rotation = { x: 0, y: 0, z: 0 };
  });
  abiertas = []; jugadas = 0; pares = 0; inicio = 0; bloqueo = false;
  marcador();
  sinte.nota(600, 0.2, { tipo: "ruido", vol: 0.1, caida: 8 });
}
function marcador(txt) {
  const r = record.leer(null);
  Obj.$("marcador").setAttribute("value", txt || ("jugadas: " + jugadas + (r ? "   récord: " + r : "")));
}

const anim = Obj.animador((t, dt) => {
  let seguir = false;
  for (const f of activas) {
    if (f.ang === f.meta) continue;
    const d = f.meta - f.ang;
    f.ang += Math.sign(d) * Math.min(Math.abs(d), dt * 9);
    const k = f.ang / Math.PI;
    // Gira sobre su borde de abajo: dada vuelta, se sube su propio espesor.
    f.g.position = { x: f.x, y: Math.sin(Math.PI * k) * 0.05 + 0.0062 * k, z: f.z };
    f.g.rotation = { x: f.ang, y: 0, z: 0 };
    seguir = true;
  }
  return seguir;
});
function voltear(f, abierta) {
  f.meta = abierta ? Math.PI : 0;
  sinte.nota(abierta ? 900 : 600, 0.05, { tipo: "madera", vol: 0.35 });
  anim.iniciar();
}

function tocar(f) {
  if (bloqueo || f.estado !== "tapada") return;
  ...
  abiertas = [];
  if (a.fig === b.fig) {
    a.estado = b.estado = "hecha";
    pares++;
    sinte.nota(Obj.frecuencia(7), 0.3, { tipo: "campana", vol: 0.2, retraso: 0.2 });
    Obj.emitir("par", { jugadas });
    if (pares === N_PARES()) ganar();
    else marcador();
  } else {
    bloqueo = true;
    marcador();
    setTimeout(() => {
      a.estado = b.estado = "tapada";
      voltear(a, false); voltear(b, false);
      bloqueo = false;
    }, 1000);
  }
}
function ganar() {
  ...
  const r = record.leer(null);
  const nuevo = !r || jugadas < r;
  if (nuevo) record.guardar(jugadas);
  for (let i = 0; i < 4; i++) sinte.nota(Obj.frecuencia(i * 4), 0.4, { tipo: "campana", vol: 0.2, retraso: 0.3 + i * 0.12 });
  marcador("¡Ganaste en " + jugadas + " jugadas!" + (nuevo ? " récord" : ""));
  Obj.emitir("ganado", { jugadas, segundos, record: nuevo });
}

Obj.boton(Obj.$("mezclar"), mezclar, { color: "#E63946" });
Obj.mensaje("mezclar", mezclar);
Obj.alCambiar((p, a, cambios) => { if (cambios.includes("pares")) mezclar(); });
mezclar();
