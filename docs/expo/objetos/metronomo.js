...
;
// El metrónomo: un péndulo al tempo y un tic por pulso.
//
// El sonido manda. Cada tic se agenda en el sintetizador con su retraso exacto
// desde el anterior, y el péndulo sólo dibuja dónde "debería" estar: si un
// cuadro llega tarde, el pulso no se corre.
const P = Obj.props({ bpm: 90, compas: 4, madera: "#7A4A2A", clave: "" });

const memo = Obj.memoria("bpm");
const sinte = Obj.sinte({ volumen: 0.8 });
const pendulo = Obj.$("pendulo");
const lectura = Obj.$("lectura");
const pesa = Obj.$("pesa");

const acotar = (v) => Math.max(30, Math.min(240, Math.round(v)));
let bpm = acotar(memo.leer(Number(P.bpm) || 90));
let andando = false;
let inicio = 0;        // performance.now() del primer pulso
let pulsos = 0;        // cuántos se agendaron

function compas() { return Math.max(1, Math.min(12, Math.round(Number(P.compas) || 4))); }
function dibujarLectura() { lectura.setAttribute("value", String(bpm)); }
// La pesa sube cuanto más lento: como en uno de verdad.
function dibujarPesa() { pesa.position = { x: 0, y: 0.06 + 0.12 * (1 - (bpm - 30) / 210), z: 0 }; }

let agenda = null;
function agendar() {
  // Se agendan los tics del próximo medio segundo, de a uno por pulso.
  const periodo = 60000 / bpm;
  const horizonte = performance.now() + 500;
  while (inicio + pulsos * periodo < horizonte) {
    const cuando = inicio + pulsos * periodo;
    const retraso = Math.max(0, (cuando - performance.now()) / 1000);
    const fuerte = pulsos % compas() === 0;
    sinte.nota(fuerte ? 1600 : 1100, 0.05, { tipo: "madera", vol: fuerte ? 0.35 : 0.22, caida: 60, retraso });
    pulsos++;
  }
}

const anim = Obj.animador(() => {
  if (!andando) { pendulo.rotation = { x: 0, y: 0, z: 0 }; return false; }
  const periodo = 60000 / bpm;
  // Un pulso por cada extremo: medio vaivén por pulso.
  const fase = ((performance.now() - inicio) / periodo) * Math.PI;
  pendulo.rotation = { x: 0, y: 0, z: 0.42 * Math.cos(fase) };
  return true;
});
...
function largar() {
  if (andando) return;
  andando = true;
  inicio = performance.now() + 60;
  pulsos = 0;
  agenda = setInterval(agendar, 200);
  agendar();
  anim.iniciar();
  Obj.emitir("cambio", { bpm, andando });
}
function parar() {
  if (!andando) return;
  andando = false;
...
function cambiar(nuevo) {
  const b = acotar(nuevo);
  if (b === bpm) return;
  // Se re-ancla en el pulso que viene, para no saltar.
  if (andando) {
    const periodo = 60000 / bpm;
    const proximo = inicio + pulsos * periodo;
    bpm = b;
    inicio = proximo;
    pulsos = 0;
  } else bpm = b;
  memo.guardar(bpm);
  dibujarLectura(); dibujarPesa();
  Obj.emitir("cambio", { bpm, andando });
}

Obj.boton(Obj.$("toque"), () => (andando ? parar() : largar()));
Obj.boton(Obj.$("mas"), () => cambiar(bpm + 5), { color: "#2A2A2A" });
Obj.boton(Obj.$("menos"), () => cambiar(bpm - 5), { color: "#2A2A2A" });
Obj.mensaje("largar", largar);
Obj.mensaje("parar", parar);
Obj.mensaje("bpm", (d) => { if (d && Number.isFinite(d.bpm)) cambiar(d.bpm); });
Obj.alCambiar((p, a, cambios) => {
  for (const m of Obj.root.getElementsByClass("madera")) m.setAttribute("color", Obj.color(p.madera, "#7A4A2A"));
  if (cambios.includes("bpm")) cambiar(Number(p.bpm) || bpm);
});
for (const m of Obj.root.getElementsByClass("madera")) m.setAttribute("color", Obj.color(P.madera, "#7A4A2A"));
dibujarLectura();
dibujarPesa();
