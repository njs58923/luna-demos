...
;
// Mezclas: pigmentos que se suman como pintura, no como luz.
//
// Mezclar RGB promediando da barro (azul + amarillo = gris). La pintura se
// piensa en rojo-amarillo-azul: se guardan las cantidades de cada pigmento y
// el color sale de interpolar entre los ocho vértices del cubo RYB (el
// método de Gossett y Chen), donde amarillo + azul es verde y los tres juntos
// casi negro. El blanco no es un pigmento del cubo: aclara el resultado según
// cuánto hay.
const sinte = Obj.sinte({ volumen: 0.7 });
const donde = Obj.marco();
const VASO = { x: 0, y: 0.011, z: 0.07 };
const RADIO_VASO = 0.047, ALTO_MAX = 0.1, TRAGO = 0.018;

const PIGMENTOS = [
  { id: "rojo", color: "#E63946" },
  { id: "amarillo", color: "#FFD60A" },
  { id: "azul", color: "#1D4ED8" },
  { id: "blanco", color: "#F8F8F8" },
];
const hay = { rojo: 0, amarillo: 0, azul: 0, blanco: 0 };
...
function ryb(r, y, b) {
  const out = [0, 0, 0];
  for (const [k, c] of Object.entries(CUBO)) {
    const w = (k[0] === "1" ? r : 1 - r) * (k[1] === "1" ? y : 1 - y) * (k[2] === "1" ? b : 1 - b);
    for (let i = 0; i < 3; i++) out[i] += w * c[i];
  }
  return out;
}
function colorMezcla() {
  const total = hay.rojo + hay.amarillo + hay.azul + hay.blanco;
  if (total <= 0) return null;
  const m = Math.max(hay.rojo, hay.amarillo, hay.azul);
  let c = m > 0 ? ryb(hay.rojo / m, hay.amarillo / m, hay.azul / m) : [1, 1, 1];
  const f = (hay.blanco / total) * 0.85;
  c = c.map((v) => v + (1 - v) * f);
  return Obj.hex(c[0] * 255, c[1] * 255, c[2] * 255);
}

// ── Los frascos ────────────────────────────────────────────────────────────
const frascos = PIGMENTOS.map((pg, i) => {
  const casa = { x: -0.18 + i * 0.12, y: 0.03, z: -0.07 };
  const g = Obj.crear("group", casa, Obj.$("frascos"));
  Obj.crear("cylinder", { y: 0.02, sx: 0.064, sy: 0.034, sz: 0.064, color: pg.color }, g);
  Obj.crear("cylinder", { y: 0.02, sx: 0.07, sy: 0.04, sz: 0.07, color: "#DDEEFF40", "material-alpha": "blend" }, g);
  Obj.crear("cylinder", { y: 0.055, sx: 0.05, sy: 0.03, sz: 0.05, color: "#DDEEFF40", "material-alpha": "blend" }, g);
  Obj.crear("cylinder", { y: 0.095, sx: 0.022, sy: 0.05, sz: 0.022, color: "#DDEEFF40", "material-alpha": "blend" }, g);
  Obj.crear("cylinder", { y: 0.12, sx: 0.026, sy: 0.004, sz: 0.026, color: "#DDEEFF60", "material-alpha": "blend" }, g);
  const blanco = Obj.crear("box", { y: 0.06, sx: 0.09, sy: 0.13, sz: 0.09, color: "transparent", "material-alpha": "blend" }, g);
  const f = { pg, casa, g, ocupado: false };
  Obj.boton(blanco, () => volcar(f));
  return f;
});
const porId = (id) => frascos.find((f) => f.pg.id === id);

// ── El vaso ────────────────────────────────────────────────────────────────
const liquido = Obj.$("liquido"), chorro = Obj.$("chorro");
const nivel = () => Math.min(ALTO_MAX, (hay.rojo + hay.amarillo + hay.azul + hay.blanco) * TRAGO);
let nivelVisto = 0;
function pintarVaso() {
  const h = nivelVisto;
  liquido.setAttribute("visible", h > 0.0005 ? "inherit" : "false");
  liquido.position = { x: 0, y: 0.011 + h / 2, z: 0 };
  liquido.scale = { x: 0.094, y: Math.max(0.001, h), z: 0.094 };
  const c = colorMezcla();
  if (c) liquido.setAttribute("color", c);
}
function lleno() { return nivel() >= ALTO_MAX - 1e-6; }

// Burbujas que suben al mezclar.
const burbujas = [];
for (let i = 0; i < 8; i++) burbujas.push({ el: Obj.crear("sphere", { sx: 0.006, sy: 0.006, sz: 0.006, color: "#FFFFFF90", "material-alpha": "blend", visible: "false" }, Obj.$("burbujas")), p: null });
let burbujasHasta = 0;

const anim = Obj.animador((t, dt) => {
  let seguir = false;
  // El nivel sube (o baja) hacia el que corresponde.
  const meta = nivel();
  if (Math.abs(meta - nivelVisto) > 0.0002) {
    nivelVisto += Math.sign(meta - nivelVisto) * Math.min(Math.abs(meta - nivelVisto), dt * (meta > nivelVisto ? 0.06 : 0.1));
    pintarVaso();
    seguir = true;
  }
  if (performance.now() < burbujasHasta) {
    seguir = true;
    burbujas.forEach((b, i) => {
      if (!b.p || b.p.y > VASO.y + nivelVisto) {
        const a = i * 2.4 + t, r = (i % 3) * 0.013;
        b.p = { x: VASO.x + Math.cos(a) * r, y: VASO.y + 0.004, z: VASO.z + Math.sin(a) * r };
      }
      b.p.y += dt * (0.05 + (i % 3) * 0.02);
      b.el.position = b.p;
      b.el.setAttribute("visible", nivelVisto > 0.01 ? "inherit" : "false");
    });
  } else if (burbujasHasta) {
    burbujasHasta = 0;
    for (const b of burbujas) b.el.setAttribute("visible", "false");
  }
  return seguir;
});

function echar(id, cuanto) {
  if (lleno()) { sinte.nota(300, 0.1, { tipo: "madera", vol: 0.3 }); return false; }
  hay[id] += cuanto;
  const c = colorMezcla();
  burbujasHasta = performance.now() + 2000;
  sinte.nota(500 + nivel() * 3000, 0.15, { tipo: "seno", vol: 0.2, caida: 12 });
  Obj.emitir("mezcla", { color: c, rojo: +hay.rojo.toFixed(2), amarillo: +hay.amarillo.toFixed(2), azul: +hay.azul.toFixed(2), blanco: +hay.blanco.toFixed(2) });
  anim.iniciar();
  return true;
}
function vaciar() {
  for (const k of Object.keys(hay)) hay[k] = 0;
  sinte.nota(180, 0.8, { tipo: "ruido", vol: 0.08, caida: 2 });
  Obj.emitir("vaciado", {});
  anim.iniciar();
}

/** El chorro de la boca del frasco a la superficie del vaso. */
function mostrarChorro(boca, color) {
  if (!boca) { chorro.setAttribute("visible", "false"); return; }
  const abajo = { x: boca.x, y: VASO.y + nivelVisto, z: boca.z };
  const l = Obj.tender(chorro, boca, abajo);
  chorro.scale = { x: 0.007, y: Math.max(0.001, l), z: 0.007 };
  chorro.setAttribute("color", color);
  chorro.setAttribute("visible", "inherit");
}

// ── Volcar tocando: el frasco va al vaso, se inclina, sirve y vuelve ───────
function volcar(f) {
  if (f.ocupado || frascos.some((k) => k.ocupado)) return;
  f.ocupado = true;
  const t0 = performance.now();
  const desde = f.casa, sobre = { x: VASO.x - 0.07, y: 0.17, z: VASO.z };
  const INCL = -1.9;
  let servido = false, entro = false;
  const tick = setInterval(() => {
    const t = (performance.now() - t0) / 1000;
    let k, incl;
    if (t < 0.4) { k = Obj.suave(t / 0.4); incl = 0; }
    else if (t < 0.7) { k = 1; incl = INCL * Obj.suave((t - 0.4) / 0.3); }
    else if (t < 1.3) { k = 1; incl = INCL; }
    else if (t < 1.6) { k = 1; incl = INCL * (1 - Obj.suave((t - 1.3) / 0.3)); }
    else { k = 1 - Obj.suave(Math.min(1, (t - 1.6) / 0.4)); incl = 0; }
    const p = { x: desde.x + (sobre.x - desde.x) * k, y: desde.y + (sobre.y - desde.y) * k, z: desde.z + (sobre.z - desde.z) * k };
    f.g.position = p;
    f.g.rotation = { x: 0, y: 0, z: incl };
    const boca = { x: p.x - Math.sin(incl) * 0.12, y: p.y + Math.cos(incl) * 0.12, z: p.z };
    if (t >= 0.7 && t < 1.3) {
      if (!servido) { servido = true; entro = echar(f.pg.id, 1); }
      mostrarChorro(entro ? boca : null, f.pg.color);
    } else mostrarChorro(null);
    if (t >= 2) {
      clearInterval(tick);
      f.g.position = f.casa;
      f.g.rotation = { x: 0, y: 0, z: 0 };
      f.ocupado = false;
    }
  }, 33);
}

// ── En VR: tomar un frasco e inclinarlo sobre el vaso ─────────────────────
let enMano = null, servidoVR = 0, pendiente = 0;
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) {
    enMano = null;
    ...
    for (const f of frascos) {
      if (f.ocupado) continue;
      const d = Math.hypot(p.x - f.casa.x, p.y - (f.casa.y + 0.06), p.z - f.casa.z);
      if (d < dm) { dm = d; enMano = f; }
    }
    return !!enMano;
  },
  tomar(p, mano, e) { enMano.ocupado = true; servidoVR = performance.now(); pendiente = 0; seguir(p, e); },
  mover(p, mano, e) { seguir(p, e); },
  soltar() {
    const f = enMano;
    enMano = null;
    if (pendiente > 0.02) echar(f.pg.id, pendiente);
    pendiente = 0;
    mostrarChorro(null);
    f.g.position = f.casa;
    f.g.rotation = { x: 0, y: 0, z: 0 };
...
function seguir(p, e) {
  const f = enMano;
  const q = { x: e.qx || 0, y: e.qy || 0, z: e.qz || 0, w: e.qw == null ? 1 : e.qw };
  const arribaMundo = Obj.rotarV(q, { x: 0, y: 1, z: 0 });
  const a = donde.dirLocal(arribaMundo);
  const m = Math.hypot(a.x, a.y, a.z) || 1;
  const eje = { x: a.x / m, y: a.y / m, z: a.z / m };
  // El frasco se toma del cuello: la mano queda a 6 cm del fondo.
  const base = { x: p.x - eje.x * 0.06, y: p.y - eje.y * 0.06, z: p.z - eje.z * 0.06 };
  f.g.position = base;
  f.g.rotation = Obj.apuntarY(eje);
  const boca = { x: base.x + eje.x * 0.12, y: base.y + eje.y * 0.12, z: base.z + eje.z * 0.12 };
  // Sirve si está bien inclinado y la boca cae adentro del vaso.
  const dentro = Math.hypot(boca.x - VASO.x, boca.z - VASO.z) < RADIO_VASO && boca.y > VASO.y + nivelVisto;
  const ahora = performance.now();
  if (eje.y < 0.1 && dentro) {
    const dt = Math.min(0.1, (ahora - servidoVR) / 1000);
    // Dos tragos por segundo; se echa de a cuartos para no avisar a cada cuadro.
    if (!lleno()) {
      pendiente += dt * 2;
      if (pendiente >= 0.25) { echar(f.pg.id, pendiente); pendiente = 0; }
      mostrarChorro(boca, f.pg.color);
    } else mostrarChorro(null);
  } else mostrarChorro(null);
  servidoVR = ahora;
}

Obj.boton(Obj.$("vidrio"), vaciar);
Obj.mensaje("vaciar", vaciar);
Obj.mensaje("echar", (d) => { const f = d && porId(d.pigmento); if (f) volcar(f); });
pintarVaso();
