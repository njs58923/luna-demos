...
;
// El microondas: una puerta, un temporizador y un plato que gira.
//
// No arranca con la puerta abierta, y abrirla lo pausa, como uno de verdad.
// Mientras anda, el interior se ilumina y el vidrio se aclara para dejar ver
// el plato girando; al terminar, tres pitidos y el visor dice "LISTO".
const P = Obj.props({ color: "#DEE2E6" });

const sinte = Obj.sinte({ volumen: 0.7 });
const bisagra = Obj.$("bisagra");
const plato = Obj.$("plato");
const visor = Obj.$("visor");
const vidrio = Obj.$("vidrio");
const interior = Obj.$("interior");

let restante = 0;      // ms
let andando = false;
let abierta = false, apertura = 0;
let giro = 0, ultimo = 0, listoHasta = 0;

const dos = (n) => String(Math.floor(n)).padStart(2, "0");
function dibujarVisor() {
  if (performance.now() < listoHasta) { visor.setAttribute("value", "LISTO"); return; }
  const s = Math.ceil(restante / 1000);
  visor.setAttribute("value", Math.floor(s / 60) + ":" + dos(s % 60));
}
function luz(on) {
  interior.setAttribute("color", on ? "#FFE8A3" : "#2A2A2A");
  interior.setAttribute("material-unlit", on ? "true" : "false");
  vidrio.setAttribute("color", on ? "#8A7A40" : "#1E2A30");
}

let enMano = null;   // el ángulo de la puerta mientras la sostienen
const anim = Obj.animador((t, dt) => {
  // La puerta.
  const meta = abierta ? 1 : 0;
  if (enMano === null && apertura !== meta) apertura = meta > apertura ? Math.min(1, apertura + dt / 0.4) : Math.max(0, apertura - dt / 0.3);
  bisagra.rotation = { x: 0, y: enMano !== null ? enMano : -1.7 * Obj.suave(apertura), z: 0 };
  // El plato y el tiempo.
  if (andando) {
    const ahora = performance.now();
    restante = Math.max(0, restante - (ahora - ultimo));
    ultimo = ahora;
    giro += dt * 0.8;
    plato.rotation = { x: 0, y: giro, z: 0 };
    dibujarVisor();
    if (restante <= 0) terminar();
  }
  return andando || apertura !== meta || enMano !== null || (performance.now() < listoHasta);
});

function arrancar() {
  if (andando || abierta || restante <= 0) return;
  andando = true;
...
function parar() {
  if (!andando) { restante = 0; dibujarVisor(); return; }
  andando = false;
  luz(false);
  dibujarVisor();
}
function terminar() {
  andando = false;
  luz(false);
  listoHasta = performance.now() + 3000;
  for (let i = 0; i < 3; i++) sinte.nota(1800, 0.25, { tipo: "cuadrada", vol: 0.08, caida: 1, retraso: i * 0.4 });
  Obj.emitir("fin", { segundos: 0 });
  dibujarVisor();
  setTimeout(dibujarVisor, 3100);
}
function bip() { sinte.nota(2200, 0.06, { tipo: "cuadrada", vol: 0.06, caida: 1 }); }

function ponerPuerta(v) {
  abierta = v;
  if (abierta && andando) { andando = false; luz(false); dibujarVisor(); }
  anim.iniciar();
}
Obj.boton(Obj.$("puerta"), () => ponerPuerta(!abierta), { vr: false });
// En VR: la puerta se toma de cualquier punto (la puerta de las manos,
// _vr.js). Entreabrirla ya lo pausa.
Obj.manos().puerta({
  pivote: { x: -0.25, y: 0.15, z: 0.195 }, eje: { x: 0, y: 1, z: 0 }, brazo: { x: 1, y: 0, z: 0 },
  largo: 0.38, alto: 0.28, max: -1.7,
  angulo: () => (enMano !== null ? enMano : -1.7 * Obj.suave(apertura)),
  alTomar() { enMano = -1.7 * Obj.suave(apertura); anim.iniciar(); },
  mover(a) {
    enMano = a;
    bisagra.rotation = { x: 0, y: a, z: 0 };
    if (a < -0.1 && !abierta) ponerPuerta(true);
  },
  soltar(v, a) {
    enMano = null;
    apertura = Obj.suaveInversa(Math.min(1, -a / 1.7));
    ponerPuerta(v);
  },
});
Obj.boton(Obj.$("mas30"), () => { bip(); restante = Math.min(99 * 60000, restante + 30000); listoHasta = 0; dibujarVisor(); }, { color: "#495057" });
Obj.boton(Obj.$("arrancar"), () => { bip(); arrancar(); }, { color: "#2B8A3E" });
Obj.boton(Obj.$("parar"), () => { bip(); parar(); }, { color: "#C92A2A" });
Obj.mensaje("sumar", (d) => { if (d && Number.isFinite(d.segundos)) { restante = Math.max(0, restante + d.segundos * 1000); dibujarVisor(); } });
Obj.mensaje("arrancar", arrancar);
Obj.mensaje("parar", parar);
Obj.alCambiar(() => Obj.$("caja").setAttribute("color", Obj.color(P.color, "#DEE2E6")));
Obj.$("caja").setAttribute("color", Obj.color(P.color, "#DEE2E6"));
dibujarVisor();
