...
;
// El mini golf: una pelota en un plano con rozamiento, paredes y un hoyo.
//
// La pelota vive en XZ: frena con una desaceleración constante (la del
// césped) y rebota en las paredes y los obstáculos, que son cajas: se busca
// el punto de la caja más cercano al centro de la pelota y, si está a menos
// del radio, se la saca y se refleja la velocidad sobre esa normal. Al hoyo
// entra si pasa por encima despacio; rápido, lo "labia" y sigue desviada.
// Tocar el césped tira hacia ese punto con v = √(2·a·d): justo para llegar.
const P = Obj.props({ clave: "", barrera: true });

const sinte = Obj.sinte({ volumen: 0.7 });
const donde = Obj.marco();
const record = Obj.memoria("record");
const pelota = Obj.$("pelota"), barrera = Obj.$("barrera"), palo = Obj.$("palo");
const R = 0.021, A = 0.5, E = 0.7;
const TEE = { x: -0.6, z: 0 }, HOYO = { x: 0.55, z: 0, r: 0.03 };
const LIM = { x0: -0.8, x1: 0.8, z0: -0.245, z1: 0.245 };
const FIJOS = [
  { x0: 0.18, x1: 0.22, z0: -0.25, z1: -0.11 },
  { x0: 0.18, x1: 0.22, z0: 0.11, z1: 0.25 },
];

let p = { ...TEE }, v = { x: 0, z: 0 };
let y = 0.041, cayendo = 0;
let golpes = 0, terminado = false;
let zBarrera = 0;

function cajaBarrera() { return { x0: 0.18, x1: 0.22, z0: zBarrera - 0.06, z1: zBarrera + 0.06 }; }
function chocar(c) {
  const qx = Math.max(c.x0, Math.min(c.x1, p.x)), qz = Math.max(c.z0, Math.min(c.z1, p.z));
  const dx = p.x - qx, dz = p.z - qz, d = Math.hypot(dx, dz);
  if (d >= R || d === 0) return false;
  const nx = dx / d, nz = dz / d;
  p.x = qx + nx * R; p.z = qz + nz * R;
  const vn = v.x * nx + v.z * nz;
  if (vn < 0) { v.x -= (1 + E) * vn * nx; v.z -= (1 + E) * vn * nz; golpeSeco(-vn); }
  return true;
}
function golpeSeco(fuerza) { if (fuerza > 0.1) sinte.nota(260, 0.05, { tipo: "madera", vol: Math.min(0.5, fuerza * 0.3) }); }

function fisica(dt) {
  const vel = Math.hypot(v.x, v.z);
  if (cayendo) return;
  if (vel > 0) {
    const nv = Math.max(0, vel - A * dt);
    v.x *= nv / vel; v.z *= nv / vel;
    p.x += v.x * dt; p.z += v.z * dt;
  }
  // Las paredes de la cancha.
  if (p.x < LIM.x0 + R) { p.x = LIM.x0 + R; if (v.x < 0) { golpeSeco(-v.x); v.x = -v.x * E; } }
  if (p.x > LIM.x1 - R) { p.x = LIM.x1 - R; if (v.x > 0) { golpeSeco(v.x); v.x = -v.x * E; } }
  if (p.z < LIM.z0 + R) { p.z = LIM.z0 + R; if (v.z < 0) { golpeSeco(-v.z); v.z = -v.z * E; } }
  if (p.z > LIM.z1 - R) { p.z = LIM.z1 - R; if (v.z > 0) { golpeSeco(v.z); v.z = -v.z * E; } }
  for (const c of FIJOS) chocar(c);
  if (P.barrera !== false) chocar(cajaBarrera());
  // El hoyo.
  const dh = Math.hypot(p.x - HOYO.x, p.z - HOYO.z);
  const vv = Math.hypot(v.x, v.z);
  if (dh < HOYO.r) {
    if (vv < 1.0) entrar();
    else { v.x += (p.z - HOYO.z) * 3; v.z -= (p.x - HOYO.x) * 3; }   // lo labia
  }
}
function entrar() {
  cayendo = performance.now();
  v = { x: 0, z: 0 };
  p = { x: HOYO.x, z: HOYO.z };
  terminado = true;
  sinte.nota(180, 0.2, { tipo: "madera", vol: 0.5 });
  sinte.nota(Obj.frecuencia(7), 0.5, { tipo: "campana", vol: 0.2, retraso: 0.25 });
  const r = record.leer(null);
  const nuevo = !r || golpes < r;
  if (nuevo) record.guardar(golpes);
  marcador(golpes === 1 ? "¡Hoyo en uno!" : "¡Adentro! " + golpes + " golpes" + (nuevo ? " (récord)" : ""));
  Obj.emitir("hoyo", { golpes, record: nuevo });
  setTimeout(reiniciar, 2500);
}
function reiniciar() {
  p = { ...TEE }; v = { x: 0, z: 0 }; y = 0.041; cayendo = 0; golpes = 0; terminado = false;
  marcador();
}
function marcador(t) {
  const r = record.leer(null);
  Obj.$("marcador").setAttribute("value", t || ("golpes: " + golpes + (r ? "   récord: " + r : "")));
}

setInterval(() => {
  const t = performance.now() / 1000;
  zBarrera = P.barrera !== false ? 0.05 * Math.sin(t * 1.5) : 0;
  barrera.position = { x: 0.2, y: 0.04, z: zBarrera };
  for (let k = 0; k < 4; k++) fisica(0.033 / 4);
  if (cayendo) y = Math.max(-0.01, 0.041 - ((performance.now() - cayendo) / 1000) * 0.3);
  pelota.position = { x: p.x, y, z: p.z };
}, 33);

const quieta = () => !cayendo && Math.hypot(v.x, v.z) < 0.01;
function pegar(vx, vz) {
  if (!quieta() || terminado) return;
  v = { x: vx, z: vz };
  golpes++;
  sinte.nota(420, 0.06, { tipo: "madera", vol: 0.5 });
  marcador();
  Obj.emitir("golpe", { golpes });
}
Obj.boton(Obj.$("cesped"), (e) => {
  if (!Number.isFinite(e.x)) return pegar(1.2, 0);
  const l = donde.aLocal({ x: e.x, y: e.y, z: e.z });
  const d = { x: l.x - p.x, z: l.z - p.z };
  const dist = Math.hypot(d.x, d.z);
  if (dist < 0.02) return;
  const vel = Math.sqrt(2 * A * dist) * (1 + (Math.random() - 0.5) * 0.08);
  pegar((d.x / dist) * vel, (d.z / dist) * vel);
});
Obj.boton(pelota, () => pegar(1.2, 0));
Obj.mensaje("reiniciar", reiniciar);

// En VR: el palo. La mano toma el mango; la cabeza queda 80 cm más allá, hacia donde apunta el mando.
const REPOSO = { x: -0.75, y: 0, z: 0.2 };
let paloAntes = null, paloPos = { ...REPOSO };
function ponerPalo(pos, rot) { paloPos = pos; palo.position = pos; palo.rotation = rot; }
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(q) { return Math.hypot(q.x - paloPos.x, q.y - (paloPos.y + 0.8), q.z - paloPos.z) < 0.12; },
  tomar() { paloAntes = null; },
  mover(q, mano, e) {
    const f0 = donde.dirLocal({ x: e.dx, y: e.dy, z: e.dz });
    const m = Math.hypot(f0.x, f0.y, f0.z) || 1;
    const f = { x: f0.x / m, y: f0.y / m, z: f0.z / m };
    const base = { x: q.x + f.x * 0.8, y: q.y + f.y * 0.8, z: q.z + f.z * 0.8 };
    ponerPalo(base, Obj.apuntarY({ x: -f.x, y: -f.y, z: -f.z }));
    const ahora = performance.now();
    if (paloAntes) {
      const dt = Math.max(0.001, (ahora - paloAntes.t) / 1000);
      const vh = { x: (base.x - paloAntes.p.x) / dt, z: (base.z - paloAntes.p.z) / dt };
      const cerca = Math.hypot(base.x - p.x, base.z - p.z) < 0.05 && base.y < 0.09;
      const rapidez = Math.hypot(vh.x, vh.z);
      if (cerca && rapidez > 0.1 && quieta()) {
        const k = Math.min(3, rapidez * 1.2) / rapidez;
        pegar(vh.x * k, vh.z * k);
      }
    }
    paloAntes = { p: base, t: ahora };
  },
  soltar() { ponerPalo(REPOSO, { x: 0, y: 0, z: 0 }); },
});

function ponerBarrera() { barrera.setAttribute("visible", P.barrera !== false ? "inherit" : "false"); }
Obj.alCambiar(ponerBarrera);
ponerBarrera();
marcador();
