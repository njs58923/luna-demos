...
;
// El molinete: cuatro aspas, un soplido y rozamiento.
//
// Cada aspa es un triángulo hecho de tres tiras escalonadas, torcidas para
// que parezcan dobladas hacia el centro como las de papel. Un soplido le suma
// velocidad angular; el rozamiento se la come de a poco.
const P = Obj.props({ colores: ["#FF595E", "#FFCA3A", "#8AC926", "#1982C4"], palo: "#E9D8A6" });
...
function colores() {
  ...
  return c.length >= 1 ? c : ["#FF595E", "#FFCA3A", "#8AC926", "#1982C4"];
}
function armar() {
  for (const a of aspas) a.remove();
  aspas = [];
  const cs = colores();
  for (let i = 0; i < 4; i++) {
    const g = Obj.crear("group", { rz: (i * Math.PI) / 2 }, rueda);
    aspas.push(g);
    const c = cs[i % cs.length];
    for (let k = 0; k < 3; k++) {
      const r = 0.02 + k * 0.035;
      Obj.crear("box", { x: r + 0.017, y: 0.012 * k, ry: 0.35, sx: 0.036, sy: 0.12 - k * 0.035, sz: 0.002, color: k === 2 ? Obj.tono(c, -0.15) : c }, g);
    }
  }
}
...
const anim = Obj.animador((t, dt) => {
  vel *= Math.exp(-0.45 * dt);
  angulo += vel * dt;
  rueda.rotation = { x: 0, y: 0, z: angulo };
  if (Math.abs(vel) < 0.05) { vel = 0; return false; }
  return true;
});
function soplar(fuerza) {
  const f = Math.max(0, Math.min(3, fuerza));
  vel = Math.min(40, vel + 12 * f);
  anim.iniciar();
  Obj.emitir("gira", { velocidad: Math.round(vel * 10) / 10 });
}
Obj.boton(Obj.$("toque"), () => soplar(1));
Obj.mensaje("soplar", (d) => soplar(d && Number.isFinite(d.fuerza) ? d.fuerza : 1));

// En VR: una mano que pasa rápido por delante sopla, más cuanto más rápido.
const zona = Obj.$("zona");
const manos = { left: null, right: null };
let ultimoSoplo = 0;
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    if (!antes || ahora - antes.t > 200 || ahora - ultimoSoplo < 150) return;
    const dt = (ahora - antes.t) / 1000;
    const v = Math.hypot(l.x - antes.x, l.y - antes.y) / Math.max(0.001, dt);
    if (v > 0.8 && Math.hypot(l.x, l.y - 0.5) < 0.2) { ultimoSoplo = ahora; soplar(v / 2); }
  });
}

Obj.alCambiar((p, a, cambios) => {
  Obj.$("palo").setAttribute("color", Obj.color(p.palo, "#E9D8A6"));
  if (cambios.includes("colores")) armar();
});
Obj.$("palo").setAttribute("color", Obj.color(P.palo, "#E9D8A6"));
armar();
