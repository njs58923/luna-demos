...
;
// La moneda: al aire, girando, y cae de un lado.
//
// El resultado se decide al tirar, y el giro se arma para terminar ahí: un
// número entero de medias vueltas en X —par, cara arriba; impar, cruz—.
const P = Obj.props({ metal: "#D4AF37", cara: "CARA", cruz: "CRUZ" });

const salto = Obj.$("salto");
const moneda = Obj.$("moneda");
const resultado = Obj.$("resultado");
let lado = 0;          // 0 cara arriba, 1 cruz arriba
let tiro = null;

const anim = Obj.animador((t) => {
  if (!tiro) return false;
  if (!tiro.t0) tiro.t0 = t;
  const k = Math.min(1, (t - tiro.t0) / tiro.dur);
  // Una parábola: sube y cae en el mismo tiempo que gira.
  salto.position = { x: 0, y: 0.009 + 4 * 0.32 * k * (1 - k), z: 0 };
  const a = Math.PI * (tiro.desde + (tiro.medias) * Obj.salida(k));
  moneda.rotation = { x: a, y: tiro.ry * k, z: 0 };
  if (k >= 1) {
    lado = tiro.final;
    const txt = lado === 0 ? String(P.cara || "CARA") : String(P.cruz || "CRUZ");
    resultado.setAttribute("value", txt.slice(0, 12));
    resultado.setAttribute("visible", "inherit");
    Obj.emitir("moneda", { resultado: lado === 0 ? "cara" : "cruz" });
    tiro = null;
...
function tirar() {
  if (tiro) return;
  const final = Math.random() < 0.5 ? 0 : 1;
  // Entre 7 y 10 vueltas enteras, más media si cambia de lado.
  const vueltas = 7 + Math.floor(Math.random() * 4);
  const medias = vueltas * 2 + (final !== lado ? 1 : 0);
  tiro = { t0: 0, dur: 1.0, desde: lado, medias, final, ry: (Math.random() - 0.5) * 2 };
...
function pintar() {
  const c = Obj.color(P.metal, "#D4AF37");
  Obj.$("disco").setAttribute("color", c);
  Obj.$("borde_a").setAttribute("color", Obj.tono(c, -0.15));
  Obj.$("borde_b").setAttribute("color", Obj.tono(c, -0.15));
  Obj.$("cara").setAttribute("value", String(P.cara || "CARA").slice(0, 8));
  Obj.$("cruz").setAttribute("value", String(P.cruz || "CRUZ").slice(0, 8));
  Obj.$("cara").setAttribute("color", Obj.tono(c, -0.6));
  Obj.$("cruz").setAttribute("color", Obj.tono(c, -0.6));
}
