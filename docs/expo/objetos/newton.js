...
;
// El péndulo de Newton: una cadena de choques elásticos.
//
// No se simula cada bolita contra su vecina: con masas iguales y choques
// elásticos, el efecto es siempre el mismo —las que golpean se quedan
// quietas y salen tantas como llegaron por la otra punta—. Así que se simula
// un solo péndulo por lado: el grupo de la izquierda y el de la derecha se
// turnan, cada uno con su ángulo, y en el medio las bolitas quedan colgando.
// Pierde un poco de energía en cada choque, hasta quedarse quieto.
const P = Obj.props({ metal: "#C0C0C0", bolitas: 5 });

const N = Math.max(3, Math.min(7, Math.round(Number(P.bolitas) || 5)));
const L = 0.16, R = 0.022, SEP = 2 * R + 0.0005;
const sinte = Obj.sinte({ volumen: 0.6 });
const grupo = Obj.$("bolitas");
const marco = Obj.$("marco");
const X0 = -((N - 1) * SEP) / 2;

// El marco: dos arcos de caño, y los hilos en V hacia cada bolita.
for (const z of [-0.06, 0.06]) {
  for (const x of [-0.14, 0.14]) Obj.crear("cylinder", { x, y: 0.12, z, sx: 0.008, sy: 0.22, sz: 0.008, color: "#9A9A9A" }, marco);
  Obj.crear("cylinder", { y: 0.23, z, rz: Math.PI / 2, sx: 0.008, sy: 0.28, sz: 0.008, color: "#9A9A9A" }, marco);
}

const bolitas = [];
for (let i = 0; i < N; i++) {
  const g = Obj.crear("group", { x: X0 + i * SEP }, grupo);   // el pivote, arriba
  for (const z of [-0.06, 0.06]) {
    const dz = z, dy = -L;
    // Del caño (0, 0, ±0.06), sobre el eje de giro, a la bolita (0, -L, 0):
    // el eje Y del cilindro girado en X hasta quedar a lo largo del hilo.
    Obj.crear("cylinder", { y: dy / 2, z: dz / 2, rx: Math.atan2(dz, -dy), sx: 0.0015, sy: Math.hypot(dy, dz), sz: 0.0015, color: "#D0D0D0" }, g);
  }
  const bola = Obj.crear("sphere", { y: -L, sx: 2 * R, sy: 2 * R, sz: 2 * R, color: Obj.color(P.metal, "#C0C0C0") }, g);
  bolitas.push({ g, bola, i });
}

// Estado: cuántas bolitas forman el grupo que se mueve, de qué lado, y su
// ángulo (negativo a la izquierda, positivo a la derecha).
let cuantas = 1, lado = -1;
let ang = 0, vel = 0;
const G = 9.8;

function dibujar() {
  bolitas.forEach((b) => b.g.rotation = { x: 0, y: 0, z: 0 });
  if (!ang) return;
  // El grupo que está afuera: las `cuantas` de la punta de su lado.
  const idx = lado < 0 ? [...Array(cuantas).keys()] : [...Array(cuantas).keys()].map((k) => N - 1 - k);
  for (const i of idx) bolitas[i].g.rotation = { x: 0, y: 0, z: ang };
}
...
const anim = Obj.animador((t, dt) => {
  ...
  for (let k = 0; k < PASOS; k++) {
    const antes = ang;
    vel += -(G / L) * Math.sin(ang) * h;
    ang += vel * h;
    // Cruzó el centro: choque. El impulso pasa al otro lado.
    if ((antes < 0 && ang >= 0 && lado < 0) || (antes > 0 && ang <= 0 && lado > 0)) {
      lado = -lado;
      vel *= 0.97;
      ang = 0;
      sinte.nota(3100, 0.03, { tipo: "pulso", vol: Math.min(0.3, Math.abs(vel) * 0.12) });
      Obj.emitir("choque", {});
    }
  }
  dibujar();
  if (Math.abs(vel) < 0.02 && Math.abs(ang) < 0.003) { ang = 0; vel = 0; dibujar(); return false; }
  return true;
});

function soltar(n, desde) {
  cuantas = Math.max(1, Math.min(N - 1, n));
  lado = desde;
  // Levantadas 45°: el ángulo es hacia afuera de la fila.
  ang = desde < 0 ? -0.8 : 0.8;
  vel = 0;
  dibujar();
  anim.iniciar();
...
// Tocar una bolita de la punta suelta una; la segunda, dos; y así.
bolitas.forEach((b) => {
  Obj.boton(b.bola, () => {
    const i = b.i;
    const izq = i < N / 2;
    const n = izq ? i + 1 : N - i;
    soltar(Math.min(n, N - 1), izq ? -1 : 1);
  });
});
Obj.mensaje("soltar", (d) => soltar(d && Number.isInteger(d.cuantas) ? d.cuantas : 1, -1));
Obj.alCambiar(() => { for (const b of bolitas) b.bola.setAttribute("color", Obj.color(P.metal, "#C0C0C0")); });
