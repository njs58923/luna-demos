...
;
// El nivel: lee hacia dónde queda "arriba" en sus propios ejes.
//
// Obj.marco() da los ejes del objeto en el mundo; la vertical del mundo vista
// desde el objeto dice cuánto está inclinado. Su componente sobre X es el
// seno de la inclinación a lo largo del cuerpo, y sobre Z la de costado. La
// burbuja sube: se corre hacia el lado alto, lo mismo que en una ampolla de
// verdad.
const P = Obj.props({ color: "#F2C200", tolerancia: 0.25 });

const donde = Obj.marco();
const burbuja = Obj.$("burbuja"), burbuja2 = Obj.$("burbuja2");
const K = 0.6;          // metros de corrimiento por radián
const TOPE = 0.036;     // media ampolla
const TOPE2 = 0.009;    // radio útil del ojo de buey
const grad = (r) => (r * 180) / Math.PI;

let bx = 0, ox = 0, oz = 0, antes = performance.now();
let nivelado = null, ultimoAviso = null;
// Un texto sólo se reescribe si cambió: rearmar la tipografía cuesta.
const puesto = {};
function poner(id, attr, v) {
  if (puesto[id + attr] === v) return;
  puesto[id + attr] = v;
  Obj.$(id).setAttribute(attr, v);
}

function leer() {
  const up = donde.dirLocal({ x: 0, y: 1, z: 0 });
  const ux = Math.max(-1, Math.min(1, up.x)), uz = Math.max(-1, Math.min(1, up.z));
  return { largo: Math.asin(ux), costado: Math.asin(uz), up };
}

function paso() {
  const ahora = performance.now();
  const dt = Math.min(0.2, (ahora - antes) / 1000);
  antes = ahora;
  const { largo, costado, up } = leer();
  // La burbuja no se mueve si el nivel está patas arriba: se va a la cara de abajo.
  const derecho = up.y > 0;
  const suave = 1 - Math.exp(-dt * 8);
  bx += (Math.max(-TOPE, Math.min(TOPE, derecho ? largo * K : 0)) - bx) * suave;
  let tx = largo * K, tz = costado * K;
  const m = Math.hypot(tx, tz);
  if (m > TOPE2) { tx *= TOPE2 / m; tz *= TOPE2 / m; }
  ox += ((derecho ? tx : 0) - ox) * suave;
  oz += ((derecho ? tz : 0) - oz) * suave;
  burbuja.position = { x: bx, y: 0.025, z: 0.0215 };
  burbuja2.position = { x: 0.2 + ox, y: 0.0545, z: oz };

  const g = grad(largo);
  const tol = Math.max(0.05, Number(P.tolerancia) || 0.25);
  const parejo = Math.abs(g) < tol && Math.abs(grad(costado)) < tol * 4;
  poner("grados", "value", mostrar(largo));
  poner("costado", "value", "costado " + mostrar(costado));
  poner("visor", "color", parejo ? "#9BE58A" : "#E8F0E0");
  if (parejo && nivelado !== true) Obj.emitir("nivelado", { grados: +g.toFixed(2) });
  nivelado = parejo;
  if (ultimoAviso === null || Math.abs(g - ultimoAviso) > 0.5) {
    ultimoAviso = g;
    Obj.emitir("inclinado", { grados: +g.toFixed(1), costado: +grad(costado).toFixed(1) });
  }
}
setInterval(paso, 50);

// Tocar el visor cambia la unidad: grados, pendiente en % o milímetros por metro.
const MODOS = ["°", "%", "mm/m"];
let modo = 0;
function mostrar(r) {
  let v = MODOS[modo] === "°" ? grad(r) : MODOS[modo] === "%" ? Math.tan(r) * 100 : Math.tan(r) * 1000;
  const cifras = MODOS[modo] === "mm/m" ? 0 : 1;
  if (Math.abs(v) < 0.5 * Math.pow(10, -cifras)) v = 0;
  return v.toFixed(cifras) + (MODOS[modo] === "°" ? "°" : " " + MODOS[modo]);
}
Obj.boton(Obj.$("visor"), () => { modo = (modo + 1) % MODOS.length; paso(); });

function pintar() { Obj.$("cuerpo").setAttribute("color", Obj.color(P.color, "#F2C200")); }
