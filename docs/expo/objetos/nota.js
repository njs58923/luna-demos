...
;
// La nota adhesiva: tocar y escribir.
//
// El teclado sólo le llega al documento que el usuario tocó. Tocar la nota le
// pide el foco editable al motor; en VR eso además abre el teclado virtual.
// Obj.teclado() se engancha a los hooks del runtime —insertText para el texto
// y defaultKeyDown para Backspace, Enter y Ctrl+V— así que no hay un editor
// aparte, sólo una cadena y siete renglones.
const P = Obj.props({ color: "#FFE66D", texto: "", tinta: "#1D3557", clave: "" });
...
for (let i = 0; i < RENGLONES; i++) {
  // El texto se centra en x: cada renglón va corrido a la derecha la mitad de
  // su ancho, así queda alineado a la izquierda. Se corrige al escribir.
  nodos.push(Obj.crear("text", { x: 0, y: 0.058 - i * 0.022, value: "", size: SIZE, color: Obj.color(P.tinta, "#1D3557") }, lineas));
}

/** Cuánto mide un texto: con el layout de la fuente si está, o a ojo. */
function mide(t) {
...
/** Parte en renglones por palabras; una palabra más larga que el renglón se
 *  corta por letras. Los saltos de línea del texto se respetan. */
function partir(texto) {
  const out = [];
  for (const parrafo of String(texto).split(String.fromCharCode(10))) {
    let actual = "";
    for (const palabra of parrafo.split(" ")) {
      const prueba = actual ? actual + " " + palabra : palabra;
      if (mide(prueba) <= ANCHO) { actual = prueba; continue; }
      if (actual) out.push(actual);
      actual = palabra;
      while (mide(actual) > ANCHO && actual.length > 1) {
        let n = actual.length - 1;
        while (n > 1 && mide(actual.slice(0, n)) > ANCHO) n--;
        out.push(actual.slice(0, n));
        actual = actual.slice(n);
      }
    }
    out.push(actual);
  }
...
function dibujar() {
  let r = partir(editor.texto);
  if (editor.enfocado && cursorVisible) r[r.length - 1] += "|";
  // Si no entra, se ven los últimos renglones: los que se están escribiendo.
  if (r.length > RENGLONES) r = r.slice(r.length - RENGLONES);
  for (let i = 0; i < RENGLONES; i++) {
    const t = r[i] || "";
    nodos[i].setAttribute("value", t);
    nodos[i].position = { x: -ANCHO / 2 + mide(t) / 2, y: 0.058 - i * 0.022, z: 0 };
  }
  if (ayuda) {
    ayuda.setAttribute("value", editor.enfocado ? "escribiendo · Enter: renglón · Esc: listo" : (editor.texto ? "" : "tocá para escribir"));
  }
}

let guardarLuego = null;
const editor = Obj.teclado(Obj.$("papel"), {
  texto: memo.leer(String(P.texto || "")),
  max: 280,
  cambio: () => {
    cursorVisible = true;
    dibujar();
    clearTimeout(guardarLuego);
    guardarLuego = setTimeout(() => memo.guardar(editor.texto), 400);
  },
  foco: (si) => {
    cursorVisible = si;
    clearInterval(parpadeo);
    parpadeo = si ? setInterval(() => { cursorVisible = !cursorVisible; dibujar(); }, 530) : null;
    if (!si) {
      memo.guardar(editor.texto);
      Obj.emitir("cambio", { texto: editor.texto });
    }
    dibujar();
  },
});
...
Obj.boton(Obj.$("toque"), () => editor.tomar());

function colorear() {
  const c = Obj.color(P.color, "#FFE66D");
  Obj.$("papel").setAttribute("color", c);
  Obj.$("franja").setAttribute("color", Obj.tono(c, -0.08));
  for (const n of nodos) n.setAttribute("color", Obj.color(P.tinta, "#1D3557"));
}
Obj.alCambiar(colorear);
Obj.mensaje("escribir", (d) => {
  if (!d || typeof d.texto !== "string") return;
  editor.texto = d.texto;
  memo.guardar(editor.texto);
  dibujar();
});
Obj.mensaje("borrar", () => { editor.texto = ""; memo.guardar(""); dibujar(); });
