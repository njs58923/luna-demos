...
;
// La palanca: un ángulo entre dos topes sobre el eje X.
//
// Con el ángulo en ±MAX (medido desde la perpendicular a la pared), arriba
// es -MAX y abajo +MAX. En VR apunta a donde está la mano, proyectada en el
// plano del giro (YZ): no importa cuánto se aleje. Es el mismo criterio que
// la palanca del kit de consola de la nave.
const P = Obj.props({ modo: "dos", inicial: 0, color: "#D62828", etiqueta: "", clave: "" });

const sinte = Obj.sinte({ volumen: 0.6 });
const donde = Obj.marco();
const guardado = Obj.memoria("fraccion");
const eje = Obj.$("eje");
const MAX = 0.75, LARGO = 0.125, EJE_Z = 0.03;
const angulo = (f) => -MAX + f * 2 * MAX;

let fraccion = (() => {
  const g = P.modo !== "retorno" ? guardado.leer(null) : null;
  return Number.isFinite(g) ? g : Math.max(0, Math.min(1, Number(P.inicial) || 0));
})();
let meta = null, tomada = false, sostenerHasta = 0, lado = null, porClic = false;

function poner(f, avisar) {
  f = Math.max(0, Math.min(1, f));
  const cambio = Math.abs(f - fraccion) > 1e-4;
  fraccion = f;
  eje.rotation = { x: angulo(f), y: 0, z: 0 };
  if (cambio && avisar !== false) Obj.emitir("valor", { valor: +f.toFixed(4), fraccion: +f.toFixed(4) });
  const l = f <= 0.02 ? "arriba" : f >= 0.98 ? "abajo" : null;
  if (l && l !== lado) {
    lado = l;
    sinte.nota(l === "abajo" ? 220 : 300, 0.08, { tipo: "madera", vol: 0.5 });
    if (avisar !== false) Obj.emitir("lado", { lado: l });
  } else if (f > 0.1 && f < 0.9) lado = null;
}
/** Dónde está la bocha, en coordenadas del objeto. */
function bocha() {
  const a = angulo(fraccion);
  return { x: 0, y: -Math.sin(a) * LARGO, z: EJE_Z + Math.cos(a) * LARGO };
}

const anim = Obj.animador((t, dt) => {
  if (tomada) return false;
  if (sostenerHasta) {
    if (performance.now() < sostenerHasta) return true;
    sostenerHasta = 0;
    meta = 0;
  }
  if (meta === null) return false;
  let f = fraccion + (meta - fraccion) * (1 - Math.exp(-dt * (P.modo === "retorno" && meta === 0 ? 7 : 12)));
  if (Math.abs(f - meta) < 0.004) f = meta;
  poner(f);
  if (f === meta) {
    const llegada = meta;
    meta = null;
    if (P.modo !== "retorno") guardado.guardar(fraccion);
    // Un clic en retorno: abajo, se sostiene y vuelve.
    if (P.modo === "retorno" && llegada === 1 && porClic) { porClic = false; sostenerHasta = performance.now() + 700; return true; }
    return false;
  }
  return true;
});

// Escritorio: al otro tope; en retorno, baja, se sostiene un rato y sube.
Obj.boton(Obj.$("toque"), () => {
  if (tomada) return;
  if (P.modo === "retorno") { meta = 1; porClic = true; }
  else meta = fraccion < 0.5 ? 1 : 0;
  anim.iniciar();
});

// VR: la palanca de las manos (_vr.js), en el eje X: se toma la bocha y
// sigue a la mano entre los topes.
Obj.manos().palanca({
  pivote: { x: 0, y: 0, z: EJE_Z }, eje: { x: 1, y: 0, z: 0 }, brazo: { x: 0, y: 0, z: 1 },
  largo: LARGO, min: -MAX, max: MAX, alcance: 0.07,
  angulo: () => angulo(fraccion),
  alTomar() { tomada = true; meta = null; sostenerHasta = 0; },
  alMover(a) { poner((a + MAX) / (2 * MAX)); },
  alSoltar() {
    tomada = false;
    if (P.modo === "dos") meta = fraccion < 0.5 ? 0 : 1;
    else if (P.modo === "retorno") meta = 0;
    else guardado.guardar(fraccion);
    anim.iniciar();
  },
});

Obj.mensaje("poner", (d) => { if (d && Number.isFinite(Number(d.fraccion))) { meta = Math.max(0, Math.min(1, Number(d.fraccion))); anim.iniciar(); } });
function pintar() {
  Obj.$("bocha").setAttribute("color", Obj.color(P.color, "#D62828"));
  Obj.$("etiqueta").setAttribute("value", String(P.etiqueta || "").slice(0, 12));
}
Obj.alCambiar(pintar);
pintar();
lado = fraccion <= 0.02 ? "arriba" : fraccion >= 0.98 ? "abajo" : null;
poner(fraccion, false);
