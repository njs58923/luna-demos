...
;
// La pantalla: dos renglones de hasta 16 letras, en mayúsculas.
//
// Un texto más largo se parte en el espacio más cercano; lo que no entra en
// dos renglones se corta con "..". Los números se muestran con los decimales
// pedidos y la unidad, si viene.
const P = Obj.props({ tinta: "verde", inicial: "", decimales: 2 });

const TINTAS = { verde: ["#0F2A1D", "#7CFC8A"], ambar: ["#2A1D0B", "#FFB000"], azul: ["#0B1A2A", "#6EC6FF"] };
const COLS = 16;

function partir(t) {
  const lineas = [];
  for (const parrafo of String(t).toUpperCase().split(String.fromCharCode(10))) {
    let resto = parrafo.trim();
    while (resto.length > COLS) {
      let corte = resto.lastIndexOf(" ", COLS);
      if (corte < 4) corte = COLS;
      lineas.push(resto.slice(0, corte).trim());
      resto = resto.slice(corte).trim();
    }
    lineas.push(resto);
  }
  if (lineas.length > 2) lineas[1] = lineas[1].slice(0, COLS - 2) + "..";
  return [lineas[0] || "", lineas[1] || ""];
}
function mostrar(t) {
  const [a, b] = partir(t);
  Obj.$("renglon1").setAttribute("value", a);
  Obj.$("renglon2").setAttribute("value", b);
}
function pintar() {
  const [fondo, tinta] = TINTAS[P.tinta] || TINTAS.verde;
  Obj.$("vidrio").setAttribute("color", fondo);
  Obj.$("renglon1").setAttribute("color", tinta);
  Obj.$("renglon2").setAttribute("color", tinta);
}

Obj.mensaje("texto", (d) => mostrar(d == null ? "" : typeof d === "string" ? d : d.texto != null ? d.texto : ""));
Obj.mensaje("valor", (d) => {
  if (d == null) return;
  const v = typeof d === "number" ? d : Number(d.valor != null ? d.valor : d.fraccion);
  if (!Number.isFinite(v)) return;
  const dec = Math.max(0, Math.min(6, Number.isFinite(Number(d.decimales)) ? Number(d.decimales) : Number(P.decimales) || 0));
  const unidad = d && d.unidad ? " " + String(d.unidad).slice(0, 6) : "";
  mostrar((d && d.titulo ? String(d.titulo) + String.fromCharCode(10) : "") + v.toFixed(dec) + unidad);
});
Obj.mensaje("limpiar", () => mostrar(""));
let prueba = 0;
Obj.boton(Obj.$("toque"), () => {
  const PRUEBAS = ["HOLA MUNDO", "0123456789ABCDEF" + String.fromCharCode(10) + "GHIJKLMNOPQRSTUV", "LISTO"];
  mostrar(PRUEBAS[prueba++ % PRUEBAS.length]);
});
