...
;
// Pava y mate: la temperatura del agua es lo que importa.
//
// El agua se calienta con el fuego hacia 100 °C siguiendo la ley de Newton
// (más rápido cuanto más lejos está) y sin fuego se enfría hacia 20 °C. El
// termómetro de la pava es una aguja entre 20 y 100. Al hervir sale vapor y
// silba: un seno agudo con vibrato, que crece. Cada cebada lava un poco la
// yerba; el agua demasiado caliente la quema.
const sinte = Obj.sinte({ volumen: 0.7 });
const aguja = Obj.$("aguja"), fuegoG = Obj.$("fuego"), vaporG = Obj.$("vapor");

let temp = 20, fuego = false, silbando = false;
let cebadas = 0, quemada = false;
let proximoAviso = 0;

const llamas = [];
for (let i = 0; i < 8; i++) {
  const a = (i / 8) * Math.PI * 2;
  llamas.push(Obj.crear("cylinder", { x: Math.cos(a) * 0.06, z: Math.sin(a) * 0.06, sx: 0.01, sy: 0.018, sz: 0.01, color: "#4895EF", "material-unlit": "true", visible: "false" }, fuegoG));
}
const vapor = [];
for (let i = 0; i < 6; i++) vapor.push(Obj.crear("sphere", { sx: 0.02, sy: 0.02, sz: 0.02, color: "#FFFFFF70", "material-alpha": "blend", visible: "false" }, vaporG));
// La punta del pico, de donde sale el vapor.
const PICO = { x: -0.08 + 0.1 + Math.sin(0.9) * 0.045, y: 0.046 + 0.08 + Math.cos(0.9) * 0.045, z: 0 };

function silbido() {
  let fase = 0;
  return (t) => {
    const f = 2150 + 60 * Math.sin(2 * Math.PI * 6 * t) + 150 * Math.min(1, t / 3);
    fase += (2 * Math.PI * f) / 22050;
    const sube = Math.min(1, t / 2.5);
    return Math.sin(fase) * 0.16 * sube + (Math.random() - 0.5) * 0.03 * sube;
  };
}
...
setInterval(() => {
  ...
  // Newton: hacia 100 con fuego (unos 40 s de 20 a hervir), hacia 20 sin él.
  const meta = fuego ? 104 : 20;
  temp += (meta - temp) * (fuego ? 0.07 : 0.008) * dt;
  temp = Math.min(100, temp);
  // Aguja: -60° a 20 °C, +60° a 100 °C.
  aguja.rotation = { x: 0, y: 0, z: -((temp - 20) / 80) * 2.1 + 1.05 };
  const txt = Math.round(temp) + "°";
  if (txt !== ultimoTexto) { ultimoTexto = txt; Obj.$("grados").setAttribute("value", txt); }
  // Llamas.
  if (fuego) llamas.forEach((l, i) => { l.scale = { x: 0.01, y: 0.014 + 0.01 * Math.abs(Math.sin(t * 13 + i)), z: 0.01 }; });
  // Vapor: más cuanto más caliente.
  const cuanto = Math.max(0, (temp - 75) / 25);
  vapor.forEach((v, i) => {
    const k = (t * 0.7 + i / vapor.length) % 1;
    const ve = cuanto > 0 && i / vapor.length < cuanto;
    if (v.visto !== ve) { v.visto = ve; v.setAttribute("visible", ve ? "inherit" : "false"); }
    if (!ve) return;
    v.position = { x: PICO.x + k * 0.06, y: PICO.y + k * 0.12, z: Math.sin(i * 2 + t) * 0.01 };
    const s = 0.015 + k * 0.04;
    v.scale = { x: s, y: s, z: s };
  });
  // Hierve: silba.
  if (temp >= 99 && !silbando) { silbando = true; sinte.flujo(silbido()); Obj.emitir("hierve", {}); }
  if (temp < 96 && silbando) { silbando = false; sinte.flujo(null); }
}, 100);

function prender(si) {
  fuego = si == null ? !fuego : !!si;
  for (const l of llamas) l.setAttribute("visible", fuego ? "inherit" : "false");
...
// Tocar la pava la saca del fuego (y deja de silbar al rato).
Obj.boton(Obj.$("toque_pava"), () => { if (fuego) prender(false); });

function cartel(t) {
  Obj.$("cartel").setAttribute("value", t);
  proximoAviso = performance.now() + 2500;
  setTimeout(() => { if (performance.now() >= proximoAviso) Obj.$("cartel").setAttribute("value", ""); }, 2600);
}
function cebar() {
  let estado;
  if (cebadas >= 12) estado = "lavado";
  else if (temp > 88) { estado = "quemado"; quemada = true; }
  else if (temp < 65) estado = "frio";
  else estado = quemada ? "quemado" : "bien";
  cebadas++;
  // La yerba se va lavando: de verde a amarillenta.
  const lav = Math.min(1, cebadas / 12);
  Obj.$("yerba").setAttribute("color", Obj.mezcla(quemada ? "#556B2F" : "#6B8E23", "#C8B560", lav));
  Obj.$("agua").setAttribute("color", Obj.mezcla("#8DB600", "#D8C878", lav));
  Obj.$("agua").setAttribute("visible", "inherit");
  sinte.nota(600, 0.3, { tipo: "ruido", vol: 0.06, caida: 6 });
  sinte.nota(320, 0.15, { tipo: "seno", vol: 0.1, caida: 10, retraso: 0.25 });
  cartel({ bien: "¡Rico!", quemado: "Se quemó la yerba", frio: "Agua fría…", lavado: "Está lavado" }[estado]);
  Obj.emitir("cebado", { temperatura: Math.round(temp), estado });
}
Obj.boton(Obj.$("toque_mate"), cebar);
function yerba() {
  cebadas = 0; quemada = false;
  Obj.$("yerba").setAttribute("color", "#6B8E23");
  Obj.$("agua").setAttribute("visible", "false");
  sinte.nota(2500, 0.25, { tipo: "ruido", vol: 0.06, caida: 10 });
  cartel("Yerba nueva");
}
Obj.boton(Obj.$("toque_yerbera"), yerba);
Obj.mensaje("fuego", (d) => prender(d ? d.prendido !== false : true));
Obj.mensaje("yerba", yerba);
