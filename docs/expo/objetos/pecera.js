...
;
// La pecera: peces con rumbo propio, algas que se mecen y burbujas.
//
// Cada pez sigue un blanco que cambia cada tanto (un punto al azar adentro
// del vidrio) y dobla de a poco hacia él: nada de líneas rectas. Cuando hay
// comida flotando, el blanco pasa a ser la comida más cercana, y un pez que
// llega hasta ella se la come. Todo en un solo bucle de cuadros, que es lo
// que hace falta para que se vean vivos.
const P = Obj.props({ peces: 5, agua: "#4FC3F7" });

const W = 0.52, H = 0.3, D = 0.24;     // el volumen donde nadan
const COLORES = ["#FF7F11", "#FFD60A", "#E63946", "#4CC9F0", "#B5179E", "#F4F1EA", "#80ED99", "#FF9F1C", "#7209B7", "#06D6A0"];
const azar = Obj.azar(5);
const dentro = () => ({ x: (Math.random() - 0.5) * W, y: 0.03 + Math.random() * (H - 0.06), z: (Math.random() - 0.5) * D });

// Algas, piedras y un castillito.
const fondo = Obj.$("fondo");
const algas = [];
for (let i = 0; i < 7; i++) {
  const x = (azar() - 0.5) * 0.5, z = (azar() - 0.5) * 0.2;
  const g = Obj.crear("group", { x, z }, fondo);
  const alto = 0.12 + azar() * 0.14;
  for (let k = 0; k < 3; k++) Obj.crear("box", { y: alto * (k + 0.5) / 3, sx: 0.012, sy: alto / 3 + 0.01, sz: 0.004, color: "#2D6A4F" }, g);
  algas.push({ g, fase: azar() * 6 });
}
for (let i = 0; i < 9; i++) Obj.crear("sphere", { x: (azar() - 0.5) * 0.5, y: 0.005, z: (azar() - 0.5) * 0.24, sx: 0.03, sy: 0.018, sz: 0.025, color: ["#8D99AE", "#6C757D", "#ADB5BD"][i % 3] }, fondo);
Obj.crear("box", { x: 0.16, y: 0.04, z: -0.06, sx: 0.06, sy: 0.08, sz: 0.05, color: "#B8B8B8" }, fondo);
Obj.crear("box", { x: 0.16, y: 0.085, z: -0.06, sx: 0.07, sy: 0.012, sz: 0.06, color: "#9A9A9A" }, fondo);

// Los peces: cuerpo, cola y un ojo, en un grupo que se orienta al nadar.
const peces = [];
const N = Math.max(1, Math.min(10, Math.round(Number(P.peces) || 5)));
for (let i = 0; i < N; i++) {
  const g = Obj.crear("group", {}, Obj.$("peces"));
  const c = COLORES[i % COLORES.length];
  const tam = 0.8 + azar() * 0.5;
  Obj.crear("sphere", { sx: 0.05 * tam, sy: 0.028 * tam, sz: 0.015 * tam, color: c }, g);
  const cola = Obj.crear("box", { x: -0.03 * tam, sx: 0.022 * tam, sy: 0.024 * tam, sz: 0.003, rz: 0, color: Obj.tono(c, -0.2) }, g);
  Obj.crear("sphere", { x: 0.014 * tam, y: 0.004 * tam, z: 0.007 * tam, sx: 0.006, sy: 0.006, sz: 0.004, color: "#101010" }, g);
  const p = dentro();
  peces.push({ g, cola, p, dir: Math.random() * Math.PI * 2, meta: dentro(), vel: 0.04 + azar() * 0.03, fase: azar() * 6, cambio: 0 });
}

// Burbujas que suben del castillito.
const burbujas = [];
for (let i = 0; i < 6; i++) {
  burbujas.push({ el: Obj.crear("sphere", { sx: 0.01, sy: 0.01, sz: 0.01, color: "#E0F7FF" }, Obj.$("burbujas")), t: i * 0.5 });
}
// La comida: copos que flotan y bajan despacio.
const copos = [];
for (let i = 0; i < 8; i++) copos.push({ el: Obj.crear("box", { sx: 0.008, sy: 0.003, sz: 0.008, color: "#C49A6C", visible: "false" }, Obj.$("comida")), vivo: false, p: null });

const anim = Obj.animador((t, dt) => {
  for (const a of algas) a.g.rotation = { x: 0, y: 0, z: 0.12 * Math.sin(t * 1.3 + a.fase) };
  for (const b of burbujas) {
    b.t += dt;
    const k = (b.t % 3) / 3;
    b.el.position = { x: 0.16 + 0.01 * Math.sin(b.t * 6), y: 0.1 + k * (H - 0.1), z: -0.06 };
  }
  for (const c of copos) {
    if (!c.vivo) continue;
    c.p.y = Math.max(0.01, c.p.y - dt * 0.012);
    c.el.position = c.p;
  }
  for (const f of peces) {
    // ¿Comida? El blanco es el copo más cercano.
    let meta = f.meta, comida = null, mejor = 1e9;
    for (const c of copos) {
      if (!c.vivo) continue;
      const d = Math.hypot(c.p.x - f.p.x, c.p.y - f.p.y, c.p.z - f.p.z);
      if (d < mejor) { mejor = d; comida = c; meta = c.p; }
    }
    if (comida && mejor < 0.02) { comida.vivo = false; comida.el.setAttribute("visible", "false"); }
    f.cambio -= dt;
    if (!comida && (f.cambio <= 0 || Math.hypot(meta.x - f.p.x, meta.z - f.p.z) < 0.03)) { f.meta = dentro(); f.cambio = 3 + Math.random() * 4; }
    // Dobla hacia el blanco de a poco (en el plano), y sube o baja aparte.
    const quiere = Math.atan2(-(meta.z - f.p.z), meta.x - f.p.x);
    let d = quiere - f.dir;
    while (d > Math.PI) d -= Math.PI * 2;
    while (d < -Math.PI) d += Math.PI * 2;
    f.dir += Math.max(-2 * dt, Math.min(2 * dt, d));
    const v = f.vel * (comida ? 2.2 : 1);
    f.p.x = Math.max(-W / 2, Math.min(W / 2, f.p.x + Math.cos(f.dir) * v * dt));
    f.p.z = Math.max(-D / 2, Math.min(D / 2, f.p.z - Math.sin(f.dir) * v * dt));
    f.p.y = Math.max(0.02, Math.min(H - 0.02, f.p.y + Math.sign(meta.y - f.p.y) * Math.min(Math.abs(meta.y - f.p.y), 0.03 * dt)));
    f.g.position = f.p;
    f.g.rotation = { x: 0, y: f.dir, z: 0 };
    f.fase += dt * (comida ? 16 : 9);
    f.cola.rotation = { x: 0, y: 0.5 * Math.sin(f.fase), z: 0 };
  }
  return true;
});
anim.iniciar();
...
function comida() {
  let n = 0;
  for (const c of copos) {
    if (c.vivo || n >= 4) continue;
    c.vivo = true; n++;
    c.p = { x: (Math.random() - 0.5) * 0.3, y: H - 0.01, z: (Math.random() - 0.5) * 0.14 };
    c.el.position = c.p;
    c.el.setAttribute("visible", "inherit");
  }
  Obj.emitir("comida", {});
}
Obj.boton(Obj.$("toque"), comida);
Obj.mensaje("comida", comida);
Obj.alCambiar(() => Obj.$("agua").setAttribute("color", Obj.color(P.agua, "#4FC3F7").slice(0, 7) + "40"));
Obj.$("agua").setAttribute("color", Obj.color(P.agua, "#4FC3F7").slice(0, 7) + "40");
