...
;
// La pelota: una física chica, hecha a mano.
//
// Gravedad, rebote contra el piso (pierde energía), rozamiento al rodar y un
// borde circular que la devuelve. El giro que se ve es el de rodar sin
// deslizar: la pelota gira alrededor del eje perpendicular a su marcha, lo
// que avanza dividido el radio. Todo en coordenadas del objeto.
const P = Obj.props({ color: "#FFFFFF", manchas: "#1C1C1C", radio: 2, rebote: 0.6 });
...
// Manchas: doce parches oscuros repartidos sobre la esfera (un icosaedro).
const manchas = [];
const PHI = (1 + Math.sqrt(5)) / 2;
for (const [a, b, c] of [[0, 1, PHI], [0, -1, PHI], [0, 1, -PHI], [0, -1, -PHI], [1, PHI, 0], [-1, PHI, 0], [1, -PHI, 0], [-1, -PHI, 0], [PHI, 0, 1], [-PHI, 0, 1], [PHI, 0, -1], [-PHI, 0, -1]]) {
  const m = Math.hypot(a, b, c);
  const n = { x: a / m, y: b / m, z: c / m };
  // Un disco chato tangente a la esfera: una esfera aplastada en la normal.
  const q = Obj.qMul(Obj.qEje({ x: 0, y: 1, z: 0 }, Math.atan2(n.x, n.z)), Obj.qEje({ x: 1, y: 0, z: 0 }, -Math.asin(n.y)));
  const e = Obj.euler(q);
  manchas.push(Obj.crear("sphere", { x: n.x * R * 0.93, y: n.y * R * 0.93, z: n.z * R * 0.93, rx: e.x, ry: e.y, rz: e.z, sx: 0.07, sy: 0.07, sz: 0.02, color: "#1C1C1C" }, giro));
}
...
function dibujar() {
  pelota.position = p;
  giro.rotation = Obj.euler(q);
  toque.position = p;
  const alto = Math.max(0, p.y - R);
  const s = Math.max(0.08, 0.2 - alto * 0.08);
  sombra.position = { x: p.x, y: 0.002, z: p.z };
  sombra.scale = { x: s, y: s, z: 1 };
}

const anim = Obj.animador((t, dt) => {
  if (enMano) { dibujar(); return true; }
  const radio = Math.max(0.5, Math.min(10, Number(P.radio) || 2));
  const rebote = Math.max(0, Math.min(0.95, Number(P.rebote) || 0.6));
  // Subpasos: a la velocidad de una patada, un cuadro es mucho.
  ...
  for (let i = 0; i < PASOS; i++) {
    ...
    if (p.y < R) {
      p.y = R;
      if (v.y < 0) v.y = -v.y * rebote;
      if (Math.abs(v.y) < 0.3) v.y = 0;
      // Rozamiento al tocar el piso.
      const f = Math.exp(-1.2 * h);
      v.x *= f; v.z *= f;
    }
    // El borde: se refleja la velocidad radial.
    const d = Math.hypot(p.x, p.z);
    if (d > radio - R) {
      const nx = p.x / d, nz = p.z / d;
      const vr = v.x * nx + v.z * nz;
      if (vr > 0) { v.x -= 2 * vr * nx * 0.8; v.z -= 2 * vr * nz * 0.8; }
      p.x = nx * (radio - R); p.z = nz * (radio - R);
    }
  }
  // Rodar: eje = arriba × velocidad, ángulo = recorrido / R.
  const vh = Math.hypot(v.x, v.z);
  if (vh > 1e-4 && p.y <= R + 0.001) {
    const eje = { x: v.z / vh, y: 0, z: -v.x / vh };
    q = Obj.qMul(Obj.qEje(eje, (vh * dt) / R), q);
  }
  dibujar();
  const quieta = vh < 0.02 && Math.abs(v.y) < 0.01 && p.y <= R + 0.001;
  if (quieta) { v.x = v.z = v.y = 0; return false; }
  return true;
});

function patear(dx, dz, fuerza) {
  const m = Math.hypot(dx, dz) || 1;
  const f = Math.max(0.2, Math.min(3, fuerza));
  v.x += (dx / m) * 3.2 * f; v.z += (dz / m) * 3.2 * f; v.y += 1.6 * f;
  anim.iniciar();
  Obj.emitir("patada", { fuerza: Math.round(f * 100) / 100 });
}

// Con el mouse: se patea desde el punto tocado hacia el centro de la pelota.
Obj.boton(toque, (e) => {
  let dx = 0, dz = -1;
  if (Number.isFinite(e.x)) {
    const l = donde.aLocal({ x: e.x, y: e.y, z: e.z });
    dx = p.x - l.x; dz = p.z - l.z;
    if (Math.hypot(dx, dz) < 1e-3) { dx = 0; dz = -1; }
  }
  patear(dx, dz, 1);
});

// En VR: agarrarla y tirarla.
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca: (m) => Math.hypot(m.x - p.x, m.y - p.y, m.z - p.z) < R + 0.1,
  tomar: (m) => { enMano = true; v.x = v.y = v.z = 0; p.x = m.x; p.y = Math.max(R, m.y); p.z = m.z; anim.iniciar(); },
  mover: (m) => { p.x = m.x; p.y = Math.max(R, m.y); p.z = m.z; dibujar(); },
  soltar: (m, vel) => {
    enMano = false;
    v.x = vel.x; v.y = vel.y; v.z = vel.z;
    anim.iniciar();
    Obj.emitir("patada", { fuerza: Math.round(Math.hypot(vel.x, vel.y, vel.z) * 100) / 100 });
  },
});

Obj.mensaje("patear", (d) => { if (d) patear(Number(d.x) || 0, Number(d.z) || -1, Number(d.fuerza) || 1); });
Obj.mensaje("volver", () => { p.x = 0; p.y = R; p.z = 0; v.x = v.y = v.z = 0; dibujar(); });
function pintar() {
  Obj.$("cuero").setAttribute("color", Obj.color(P.color, "#FFFFFF"));
  for (const m of manchas) m.setAttribute("color", Obj.color(P.manchas, "#1C1C1C"));
}
