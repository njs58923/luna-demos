...
;
// La perilla: un ángulo entre dos topes, que se lee como un valor.
//
// El recorrido va de -135° a +135°, medidos en el sentido del reloj desde
// arriba y mirando la perilla de frente. Girar la muñeca en VR suma su giro
// (Obj.giroMuneca) al ángulo que tenía al agarrarla; un clic en el borde la
// lleva al ángulo del punto tocado. Con muescas, el valor salta de a una y
// cada salto hace un clic; el ángulo que se ve se asienta en la muesca.
const P = Obj.props({ min: 0, max: 10, pasos: 10, inicial: null, decimales: null, color: "#3A86FF", etiqueta: "", clave: "" });

const sinte = Obj.sinte({ volumen: 0.6 });
const donde = Obj.marco();
const guardado = Obj.memoria("fraccion");
const perilla = Obj.$("perilla"), escala = Obj.$("escala");
const TOPE = (135 * Math.PI) / 180;
const C = { x: 0, y: 0.01 };
const R = 0.034;

const min = () => Number(P.min) || 0;
const max = () => (Number.isFinite(Number(P.max)) && Number(P.max) !== min() ? Number(P.max) : min() + 1);
const pasos = () => Math.max(0, Math.min(100, Math.round(Number(P.pasos) || 0)));
const angDe = (f) => -TOPE + f * 2 * TOPE;
const fracDe = (a) => Math.max(0, Math.min(1, (a + TOPE) / (2 * TOPE)));
function ajustar(f) {
  f = Math.max(0, Math.min(1, f));
  const n = pasos();
  return n ? Math.round(f * n) / n : f;
}
const valorDe = (f) => min() + (max() - min()) * f;
function texto(v) {
  const d = P.decimales != null ? Number(P.decimales) : pasos() && Number.isInteger((max() - min()) / pasos()) ? 0 : 2;
  return v.toFixed(Math.max(0, Math.min(4, d)));
}

// ── Lo que se ve ───────────────────────────────────────────────────────────
// La escala: rayitas en el arco (una por muesca, o once si es continua).
function armarEscala() {
  for (const c of [...marcas]) c.remove();
  marcas.length = 0;
  const n = pasos() || 10;
  for (let i = 0; i <= n; i++) {
    const a = angDe(i / n);
    const larga = i === 0 || i === n || (n > 10 ? i % 5 === 0 : true);
    const r0 = R + 0.008, l = larga ? 0.012 : 0.007;
    marcas.push(Obj.crear("box", { x: Math.sin(a) * (r0 + l / 2), y: Math.cos(a) * (r0 + l / 2), rz: -a, sx: 0.0025, sy: l, sz: 0.002, color: "#C8CCD8" }, escala));
  }
}
const marcas = [];
// La perilla: cuerpo, estrías y la raya que indica.
Obj.crear("cylinder", { z: 0.012, rx: Math.PI / 2, sx: R * 2, sy: 0.024, sz: R * 2, color: "#2B2F3A" }, perilla);
const cara = Obj.crear("cylinder", { z: 0.0245, rx: Math.PI / 2, sx: R * 1.7, sy: 0.002, sz: R * 1.7, color: "#3A86FF" }, perilla);
for (let i = 0; i < 18; i++) {
  const a = (i / 18) * Math.PI * 2;
  Obj.crear("box", { x: Math.sin(a) * R, y: Math.cos(a) * R, z: 0.012, rz: -a, sx: 0.004, sy: 0.004, sz: 0.022, color: "#1C1F27" }, perilla);
}
Obj.crear("box", { y: R * 0.5, z: 0.026, sx: 0.005, sy: R * 0.8, sz: 0.002, color: "#FFFFFF" }, perilla);

// ── Estado ─────────────────────────────────────────────────────────────────
let fraccion = (() => {
  const g = guardado.leer(null);
  if (Number.isFinite(g)) return ajustar(g);
  if (P.inicial != null && Number.isFinite(Number(P.inicial))) return ajustar((Number(P.inicial) - min()) / (max() - min()));
  return 0;
})();
let angulo = angDe(fraccion);    // lo que se ve (se asienta hacia la muesca)
let meta = angulo;
let tomada = false;

function poner(f, avisar) {
  const nf = ajustar(f);
  if (Math.abs(nf - fraccion) < 1e-9) return;
  const pasoAntes = Math.round(fraccion * (pasos() || 50));
  fraccion = nf;
  guardado.guardar(fraccion);
  if (Math.round(fraccion * (pasos() || 50)) !== pasoAntes) sinte.nota(pasos() ? 2200 : 3000, 0.012, { tipo: "pulso", vol: pasos() ? 0.25 : 0.08 });
  Obj.$("lectura").setAttribute("value", texto(valorDe(fraccion)));
  if (avisar !== false) Obj.emitir("valor", { valor: +valorDe(fraccion).toFixed(6), fraccion: +fraccion.toFixed(6) });
}

const anim = Obj.animador((t, dt) => {
  if (!tomada) meta = angDe(fraccion);
  angulo += (meta - angulo) * (1 - Math.exp(-dt * 16));
  if (Math.abs(meta - angulo) < 1e-4) angulo = meta;
  perilla.rotation = { x: 0, y: 0, z: -angulo };
  return angulo !== meta || tomada;
});

// Escritorio: el borde lleva a ese ángulo; el centro, una muesca más (y al pasar el tope, vuelve al principio).
Obj.boton(Obj.$("toque"), (e) => {
  let dx = 0, dy = 0;
  if (Number.isFinite(e.x)) { const l = donde.aLocal({ x: e.x, y: e.y, z: e.z }); dx = l.x - C.x; dy = l.y - C.y; }
  if (Math.hypot(dx, dy) < R * 0.45) {
    const paso = pasos() ? 1 / pasos() : 0.1;
    poner(fraccion >= 1 - 1e-6 ? 0 : fraccion + paso);
  } else {
    const a = Math.atan2(dx, dy);
    // Fuera del recorrido (abajo, entre los topes): al tope más cercano.
    poner(fracDe(Math.max(-TOPE, Math.min(TOPE, a))));
  }
  anim.iniciar();
});

// VR: la perilla de las manos (_vr.js): se agarra y se gira la muñeca; si el
// mando apunta derecho a la pared y no hay giro que leer, vale llevar la mano
// alrededor.
Obj.manos().perilla({
  centro: { x: C.x, y: C.y, z: 0.02 }, eje: { x: 0, y: 0, z: 1 }, radio: R,
  alTomar() { tomada = true; meta = angDe(fraccion); anim.iniciar(); },
  alGirar(d) { meta = Math.max(-TOPE, Math.min(TOPE, meta + d)); poner(fracDe(meta)); },
  alSoltar() { tomada = false; anim.iniciar(); },
});

Obj.mensaje("poner", (d) => {
  if (!d) return;
  if (Number.isFinite(Number(d.fraccion))) poner(Number(d.fraccion));
  else if (Number.isFinite(Number(d.valor))) poner((Number(d.valor) - min()) / (max() - min()));
  anim.iniciar();
});

function pintar() {
  cara.setAttribute("color", Obj.color(P.color, "#3A86FF"));
  Obj.$("etiqueta").setAttribute("value", String(P.etiqueta || "").slice(0, 16));
  Obj.$("lectura").setAttribute("value", texto(valorDe(fraccion)));
}
Obj.alCambiar((p, a, cambios) => { if (cambios.includes("pasos")) { armarEscala(); poner(fraccion, false); } pintar(); anim.iniciar(); });
armarEscala();
pintar();
perilla.rotation = { x: 0, y: 0, z: -angulo };
