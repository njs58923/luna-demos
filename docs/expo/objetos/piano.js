...
;
// El piano: quince blancas, diez negras, y tres timbres sintetizados.
//
// Una tecla baja al tocarla y vuelve sola. En VR, cada mano toca la tecla que
// tiene debajo cuando baja más rápido que un umbral: la misma idea que los
// palillos del xilófono, con las negras primero porque están arriba.
const P = Obj.props({ octava: 0, timbre: "piano", mueble: "#1C1C1C" });
...
const ANCHO = 0.052, LARGO_B = 0.2, LARGO_N = 0.12;
const N_BLANCAS = 15;
const X0 = -((N_BLANCAS - 1) * ANCHO) / 2;

const teclas = [];   // por semitono 0..24
let blanca = 0;
for (let s = 0; s <= 24; s++) {
  const esBlanca = BLANCAS.includes(s % 12);
  let x, el;
  if (esBlanca) {
    x = X0 + blanca * ANCHO;
    el = Obj.crear("box", { x, y: 0, z: 0, sx: ANCHO - 0.003, sy: 0.02, sz: LARGO_B, color: "#FAFAF5", "border-radius": 0.003 }, grupo);
    blanca++;
  } else {
    x = X0 + (blanca - 0.5) * ANCHO;
    el = Obj.crear("box", { x, y: 0.014, z: -(LARGO_B - LARGO_N) / 2, sx: ANCHO * 0.58, sy: 0.02, sz: LARGO_N, color: "#15151A", "border-radius": 0.003 }, grupo);
  }
  const t = { s, el, x, esBlanca, y: esBlanca ? 0 : 0.014, z: esBlanca ? 0 : -(LARGO_B - LARGO_N) / 2, color: esBlanca ? "#FAFAF5" : "#15151A", hasta: 0 };
  Obj.boton(el, () => tocar(s, 1), { color: t.color, realce: esBlanca ? "#E6EEF8" : "#3A3A48" });
  teclas.push(t);
}

function frecuencia(s) {
  const oct = Math.max(-2, Math.min(2, Math.round(Number(P.octava) || 0)));
  // Do4 está 9 semitonos debajo del La4.
  return Obj.frecuencia(s - 9 + 12 * oct);
}

function sonido(f, fuerza) {
  const vol = 0.22 * Math.max(0.3, Math.min(1, fuerza));
  if (P.timbre === "organo") {
    sinte.sumar((t) => {
      const env = Math.min(1, t / 0.02) * (t < 0.7 ? 1 : Math.max(0, 1 - (t - 0.7) / 0.3));
      return env * vol * 0.6 * (Math.sin(6.2832 * f * t) + 0.5 * Math.sin(6.2832 * f * 2 * t) + 0.25 * Math.sin(6.2832 * f * 4 * t));
    }, 1.0);
  } else if (P.timbre === "campana") {
    sinte.nota(f, 2.2, { tipo: "campana", vol, caida: 1.8 });
  } else {
    // Piano: ataque seco, armónicos que se apagan antes que la fundamental.
    sinte.sumar((t) => {
      const env = Math.min(1, t / 0.004) * Math.exp(-2.6 * t);
      return env * vol * (Math.sin(6.2832 * f * t) + 0.45 * Math.sin(6.2832 * f * 2 * t) * Math.exp(-3 * t) + 0.2 * Math.sin(6.2832 * f * 3 * t) * Math.exp(-5 * t));
    }, 1.8);
  }
}
...
const anim = Obj.animador(() => {
  ...
  for (const t of teclas) {
    if (!t.hasta) continue;
    const k = Math.max(0, (t.hasta - ahora) / 160);
    t.el.position = { x: t.x, y: t.y - 0.008 * k, z: t.z };
    if (k <= 0) t.hasta = 0; else seguir = true;
  }
  return seguir;
});

function tocar(s, fuerza) {
  const t = teclas[s];
  if (!t) return;
  const f = frecuencia(s);
  sonido(f, fuerza);
  t.hasta = performance.now() + 160;
  anim.iniciar();
  Obj.emitir("nota", { tecla: s, nombre: NOMBRES[s % 12], frecuencia: Math.round(f * 100) / 100 });
}
Obj.mensaje("tocar", (d) => { if (d && Number.isInteger(d.tecla)) tocar(d.tecla, 1); });

// ── Dedos en VR ─────────────────────────────────────────────────────────────
const Y_TECLA = 0.075 + 0.01;   // cara de arriba de las blancas, en el objeto
const manos = { left: null, right: null };
const zona = Obj.$("zona");
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    // Las negras están más arriba: se prueban primero.
    for (const negras of [true, false]) {
      const cara = Y_TECLA + (negras ? 0.014 : 0);
      if (!(antes.y > cara && l.y <= cara + 0.003)) continue;
      const v = (antes.y - l.y) / Math.max(0.001, (ahora - antes.t) / 1000);
      if (v < 0.2) return;
      const zl = l.z - 0.02;
      const t = teclas.find((k) => k.esBlanca !== negras && Math.abs(l.x - k.x) < (negras ? ANCHO * 0.3 : ANCHO / 2) && Math.abs(zl - k.z) < (negras ? LARGO_N : LARGO_B) / 2);
      if (t) { tocar(t.s, v / 2); return; }
    }
  });
}

function pintar() {
  const c = Obj.color(P.mueble, "#1C1C1C");
  Obj.$("mueble").setAttribute("color", c);
  Obj.$("atril").setAttribute("color", c);
}
