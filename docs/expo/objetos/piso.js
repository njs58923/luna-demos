...
;
// El piso: un tramo que apoya en y=0 y va desde su borde de adelante hacia
// atrás. Tocarlo pasa al dibujo siguiente.
const P = Obj.props({ ancho: 2, largo: 2, patron: "baldosa", color: "", color2: "", lado: 0.33, semilla: 3 });
const ORDEN = Object.keys(Obra.PISOS);
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let patron = ORDEN.includes(P.patron) ? P.patron : "baldosa";

function dibujar() {
  const W = Obra.num(P.ancho, 2, 0.2, 60), D = Obra.num(P.largo, 2, 0.2, 60);
  // Los colores de las props son del dibujo que se pidió: al cambiar de dibujo
  // con el toque se usan los del dibujo nuevo.
  const propio = patron === P.patron;
  armar(Obra.piso({ ancho: W, largo: D, patron, lado: P.lado, semilla: P.semilla, color: propio ? P.color : "", color2: propio ? P.color2 : "" }));
  toque.position = { x: 0, y: 0.06, z: -D / 2 };
  toque.scale = { x: W, y: 0.12, z: D };
}

Obj.boton(toque, () => {
  patron = Obra.siguiente(ORDEN, patron);
  dibujar();
  Obj.emitir("cambio", { patron });
});
Obj.alCambiar((p, antes, cambios) => {
  if (cambios.includes("patron") && ORDEN.includes(p.patron)) patron = p.patron;
  dibujar();
});
dibujar();
