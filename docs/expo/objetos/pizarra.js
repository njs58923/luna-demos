...
;
// La pizarra: trazos hechos de segmentos, dibujados con la mano o a toques.
//
// Cada segmento es una caja fina entre dos puntos, de un pozo de cajas que se
// reusan: cuando se acaba el pozo, los trazos más viejos se borran. Los
// puntos se guardan en milímetros, así lo dibujado entra holgado en
// localStorage y vuelve igual en la próxima carga.
//
// El toque trae `local`: el punto del impacto en coordenadas del nodo tocado
// (en unidades del plano, de -0.5 a 0.5). Con eso el mouse dibuja exacto sin
// tener que saber dónde quedó la pizarra en el mundo. Las manos, en cambio,
// llegan en coordenadas de mundo y se pasan con Obj.marco().
const P = Obj.props({ marco: "#B0B7BF", clave: "" });

const W = 1.2, H = 0.8, GROSOR = 0.007;
const MAX_SEG = 900;
const COLORES = ["#1C1C1C", "#C8102E", "#0A57C2", "#1E8E3E"];
const memo = Obj.memoria("trazos");
const donde = Obj.marco();
const grupo = Obj.$("trazos");
const ayuda = Obj.$("ayuda");

// ── Los trazos ──────────────────────────────────────────────────────────────
let trazos = [];          // { c, p: [x0,y0,x1,y1,...] en mm }
let color = 0;
const pozo = [];          // cajas
let usados = 0;

function caja(i) {
  if (!pozo[i]) pozo[i] = Obj.crear("box", { sx: GROSOR, sy: GROSOR, sz: 0.002, color: "#1C1C1C", visible: "false" }, grupo);
  return pozo[i];
}
function segmento(x0, y0, x1, y1, c) {
  if (usados >= MAX_SEG) return false;
  const el = caja(usados++);
  const dx = x1 - x0, dy = y1 - y0;
  const largo = Math.hypot(dx, dy);
  el.position = { x: (x0 + x1) / 2, y: (y0 + y1) / 2, z: 0 };
  el.rotation = { x: 0, y: 0, z: Math.atan2(dy, dx) };
  el.scale = { x: largo + GROSOR, y: GROSOR, z: 0.002 };
  el.setAttribute("color", COLORES[c] || COLORES[0]);
  el.setAttribute("visible", "inherit");
  return true;
}
function redibujar() {
  // Si no entra todo, se tiran los trazos más viejos.
  let total = 0;
  for (let i = trazos.length - 1; i >= 0; i--) {
    total += Math.max(1, trazos[i].p.length / 2 - 1);
    if (total > MAX_SEG) { trazos = trazos.slice(i + 1); break; }
  }
  usados = 0;
  for (const tr of trazos) {
    const p = tr.p;
    if (p.length === 2) segmento(p[0] / 1000, p[1] / 1000, p[0] / 1000, p[1] / 1000, tr.c);
    for (let i = 2; i < p.length; i += 2) segmento(p[i - 2] / 1000, p[i - 1] / 1000, p[i] / 1000, p[i + 1] / 1000, tr.c);
  }
  for (let i = usados; i < pozo.length; i++) pozo[i].setAttribute("visible", "false");
}
function guardar() {
  // Hasta 4000 puntos: holgado para una pizarra, chico para localStorage.
  let n = 0;
  const out = [];
  for (let i = trazos.length - 1; i >= 0 && n < 8000; i--) { out.unshift(trazos[i]); n += trazos[i].p.length; }
  memo.guardar(out);
}
const dentro = (x, y) => Math.abs(x) <= W / 2 && Math.abs(y) <= H / 2;
const mm = (v) => Math.round(v * 1000);

// Un trazo en curso: se le agregan puntos y se dibujan de a segmento.
let actual = null;
function empezar(x, y) {
  actual = { c: color, p: [mm(x), mm(y)] };
  trazos.push(actual);
  if (usados >= MAX_SEG) redibujar();
  segmento(x, y, x, y, color);
}
function seguir(x, y) {
  const p = actual.p;
  const lx = p[p.length - 2] / 1000, ly = p[p.length - 1] / 1000;
  if (Math.hypot(x - lx, y - ly) < 0.004) return;
  p.push(mm(x), mm(y));
  if (usados >= MAX_SEG) redibujar(); else segmento(lx, ly, x, y, actual.c);
}
function terminar() {
  if (!actual) return;
  Obj.emitir("trazo", { puntos: actual.p.length / 2, color: COLORES[actual.c] });
  actual = null;
  guardar();
}

// ── Con el mouse: toques que se unen ────────────────────────────────────────
let ultimoToque = 0;
Obj.boton(Obj.$("lienzo"), (e) => {
  const l = Obj.toqueLocal(e);
  if (!l) return;
  const x = l[0] * W, y = l[1] * H;
  if (!dentro(x, y)) return;
  const ahora = performance.now();
  if (actual && ahora - ultimoToque < 2000) seguir(x, y);
  else { terminar(); empezar(x, y); }
  ultimoToque = ahora;
  clearTimeout(cierre);
  cierre = setTimeout(terminar, 2000);
});
let cierre = null;

// ── Con la mano: gatillo cerca de la superficie ─────────────────────────────
const zona = Obj.$("zona");
const dibujando = { left: false, right: false };
if (zona) {
  zona.addEventListener("posemove", (e) => {
    const mano = e.hand === "left" ? "left" : "right";
    const l = donde.aLocal({ x: e.px, y: e.py, z: e.pz });
    // La punta del fibrón: hasta 4 cm delante de la superficie.
    const cerca = l.z > 0 && l.z < 0.045 + 0.023 && dentro(l.x, l.y);
    const aprieta = (e.trigger || 0) > 0.5;
    if (cerca && aprieta) {
      if (!dibujando[mano]) { terminar(); empezar(l.x, l.y); dibujando[mano] = true; }
      else seguir(l.x, l.y);
    } else if (dibujando[mano]) {
      dibujando[mano] = false;
      terminar();
    }
  });
}

// ── La bandeja: fibrones y borrador ─────────────────────────────────────────
const bandeja = Obj.$("bandeja");
const fibrones = COLORES.map((c, i) => {
  const g = Obj.crear("group", { x: -0.42 + i * 0.13 }, bandeja);
  const cuerpo = Obj.crear("cylinder", { rz: Math.PI / 2, sx: 0.022, sy: 0.11, sz: 0.022, color: "#F4F4F4" }, g);
  Obj.crear("cylinder", { x: 0.06, rz: Math.PI / 2, sx: 0.024, sy: 0.02, sz: 0.024, color: c }, g);
  Obj.boton(cuerpo, () => { color = i; marcar(); });
  return { g, cuerpo };
});
function marcar() {
  fibrones.forEach((f, i) => { f.g.position = { x: -0.42 + i * 0.13, y: i === color ? 0.02 : 0, z: 0 }; });
  ayuda.setAttribute("value", "VR: gatillo cerca de la pizarra · mouse: tocá puntos para unirlos");
}
const borrador = Obj.crear("box", { x: 0.4, y: 0.012, sx: 0.14, sy: 0.035, sz: 0.05, color: "#3A3F4B", "border-radius": 0.006 }, bandeja);
Obj.crear("box", { x: 0.4, y: -0.01, sx: 0.14, sy: 0.012, sz: 0.05, color: "#E8E8E8" }, bandeja);
// -Infinity y no 0: performance.now() arranca cerca de cero al cargar la
// página, y con 0 el primer toque ya contaba como confirmación.
...
Obj.boton(borrador, () => {
  const ahora = performance.now();
  if (ahora - confirmar > 2500) { confirmar = ahora; ayuda.setAttribute("value", "tocá el borrador otra vez para limpiar todo"); return; }
  confirmar = -Infinity;
  borrarTodo();
  marcar();
}, { color: "#3A3F4B" });
Obj.mensaje("borrar", borrarTodo);
Obj.alCambiar(() => Obj.$("marco").setAttribute("color", Obj.color(P.marco, "#B0B7BF")));

// Lo guardado.
const g = memo.leer([]);
if (Array.isArray(g)) trazos = g.filter((t) => t && Array.isArray(t.p) && t.p.length >= 2 && t.p.every(Number.isFinite)).map((t) => ({ c: Number.isInteger(t.c) ? t.c : 0, p: t.p }));
Obj.$("marco").setAttribute("color", Obj.color(P.marco, "#B0B7BF"));
redibujar();
marcar();
