...
;
// El planetario: longitudes medias de los planetas para una fecha.
//
// Cada planeta cuelga de un brazo que sale del eje: un grupo que gira en Y
// con la longitud heliocéntrica media del planeta (grados en J2000 más grados
// por día, de las tablas de Standish). Alcanza para ver en qué lado del Sol
// está cada uno hoy; no son efemérides exactas (no tienen en cuenta la
// excentricidad), pero Mercurio y Venus cambian en semanas y Saturno en años
// como en el cielo. Las distancias no van a escala: la de Saturno haría a
// Mercurio invisible.
const P = Obj.props({ dias: 36.5 });

const brazos = Obj.$("brazos");
const J2000 = Date.UTC(2000, 0, 1, 12);
const DIA = 86400000;
const PLANETAS = [
  { nombre: "Mercurio", l0: 252.2503, n: 4.0923344, r: 0.07, t: 0.018, color: "#A8A29A", y: 0.285 },
  { nombre: "Venus", l0: 181.9791, n: 1.6021302, r: 0.11, t: 0.026, color: "#E8C27A", y: 0.27 },
  { nombre: "Tierra", l0: 100.4645, n: 0.9856091, r: 0.16, t: 0.028, color: "#3D7DD8", y: 0.255, luna: true },
  { nombre: "Marte", l0: 355.4533, n: 0.5240329, r: 0.21, t: 0.022, color: "#C1440E", y: 0.24 },
  { nombre: "Júpiter", l0: 34.3515, n: 0.0830853, r: 0.28, t: 0.05, color: "#D8B98A", y: 0.225 },
  { nombre: "Saturno", l0: 50.0775, n: 0.0334443, r: 0.36, t: 0.042, color: "#E3CF96", y: 0.21, anillo: true },
];

for (const p of PLANETAS) {
  // El brazo: del eje al planeta, con una varilla que baja hasta él.
  p.g = Obj.crear("group", {}, brazos);
  Obj.crear("cylinder", { x: p.r / 2, y: p.y, rz: Math.PI / 2, sx: 0.005, sy: p.r, sz: 0.005, color: "#B08D57" }, p.g);
  Obj.crear("cylinder", { x: p.r, y: p.y + 0.03, sx: 0.004, sy: 0.06, sz: 0.004, color: "#B08D57" }, p.g);
  Obj.crear("cylinder", { y: p.y, sx: 0.016, sy: 0.012, sz: 0.016, color: "#8C6D3F" }, p.g);
  const cuerpo = Obj.crear("sphere", { x: p.r, y: p.y + 0.06 + p.t / 2, sx: p.t, sy: p.t, sz: p.t, color: p.color }, p.g);
  Obj.boton(cuerpo, () => mostrar(p));
  if (p.anillo) Obj.crear("cylinder", { x: p.r, y: p.y + 0.06 + p.t / 2, rx: 0.47, sx: p.t * 2.1, sy: 0.002, sz: p.t * 2.1, color: "#C9B47E" }, p.g);
  if (p.luna) {
    p.lunaG = Obj.crear("group", { x: p.r, y: p.y + 0.06 + p.t / 2 }, p.g);
    Obj.crear("sphere", { x: 0.03, sx: 0.009, sy: 0.009, sz: 0.009, color: "#D8D8D8" }, p.lunaG);
  }
}

// ── La fecha que se muestra ────────────────────────────────────────────────
let corriendo = false;
let fechaMs = Date.now();
let ultimoDia = null;
const MESES = ["ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"];

function colocar() {
  const d = (fechaMs - J2000) / DIA;
  for (const p of PLANETAS) {
    const l = (((p.l0 + p.n * d) % 360) + 360) % 360;
    p.grados = l;
    p.g.rotation = { x: 0, y: (l * Math.PI) / 180, z: 0 };
    if (p.lunaG) {
      // La Luna, con su longitud media geocéntrica; el brazo ya giró con la
      // Tierra, así que se le resta.
      const lm = 218.3165 + 13.17639648 * d;
      p.lunaG.rotation = { x: 0, y: ((lm - l) * Math.PI) / 180, z: 0 };
    }
  }
  ...
  if (dia !== ultimoDia) {
    ultimoDia = dia;
    Obj.$("fecha").setAttribute("value", f.getUTCDate() + " " + MESES[f.getUTCMonth()] + " " + f.getUTCFullYear() + (corriendo ? "  >>" : ""));
    if (corriendo) Obj.emitir("fecha", { iso: dia });
  }
}
...
const anim = Obj.animador(() => {
  ...
  const v = Math.max(1, Math.min(3650, Number(velocidad ?? P.dias) || 36.5));
  fechaMs += dt * v * DIA;
  colocar();
  return true;
});

let velocidad = null;     // la que pidió un mensaje; si no, la de las props
...
let nombreHasta = 0;
function mostrar(p) {
  Obj.$("nombre").setAttribute("value", p.nombre);
  Obj.emitir("planeta", { nombre: p.nombre, grados: +p.grados.toFixed(1) });
  nombreHasta = performance.now() + 2500;
  setTimeout(() => { if (performance.now() >= nombreHasta) Obj.$("nombre").setAttribute("value", ""); }, 2600);
}

Obj.boton(Obj.$("sol"), () => (corriendo ? hoy() : correr()));
Obj.mensaje("hoy", hoy);
Obj.mensaje("correr", (d) => correr(d && Number.isFinite(Number(d.dias)) ? Number(d.dias) : null));
// Quieto, igual se pone al día cada minuto.
setInterval(() => { if (!corriendo) { fechaMs = Date.now(); colocar(); } }, 60000);
colocar();
