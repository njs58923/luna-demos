...
;
// La planta: crece con el tiempo de verdad, sólo mientras la tierra está húmeda.
//
// El estado guardado es {crecimiento (0 a 1), humedad (0 a 1), secaHoras
// (horas seguidas sin agua), ultimo (ms)}. Al abrirla y cada pocos segundos se pone
// al día con lo que pasó desde `ultimo`: la humedad baja en línea recta hasta
// cero, y el crecimiento avanza sólo durante el rato en que todavía había
// agua. Así, volver después de tres días da la planta que corresponde.
const P = Obj.props({ clave: "", horas: 96, seca: 24, flor: "#FF70A6" });

const donde = Obj.marco();
const guardado = Obj.memoria("planta");
const plantaG = Obj.$("planta"), regadera = Obj.$("regadera");
const HORA = 3600 * 1000;
const ETAPAS = [[0, "semilla"], [0.1, "brote"], [0.35, "planta"], [0.75, "pimpollo"], [0.9, "flor"]];
const etapaDe = (g) => ETAPAS.filter(([u]) => g >= u).pop()[1];
const dura = () => Math.max(0.001, Number(P.seca) || 24);     // horas que dura un riego
const total = () => Math.max(0.001, Number(P.horas) || 96);   // horas de semilla a flor
// Más de un riego y medio sin agua, se marchita (y deja de crecer).
const marchitaYa = () => estado.secaHoras > dura() * 1.5;

let estado = Object.assign({ crecimiento: 0, humedad: 0, secaHoras: 0, ultimo: Date.now() }, guardado.leer({}) || {});
let etapaAntes = etapaDe(estado.crecimiento);

// ── La planta, armada una vez ──────────────────────────────────────────────
const tallo = Obj.crear("cylinder", { sx: 0.008, sy: 0.02, sz: 0.008, color: "#3A7D44" }, plantaG);
const hojas = [];
for (let k = 0; k < 6; k++) {
  const lado = k % 2 ? 1 : -1;
  const g = Obj.crear("group", {}, plantaG);
  const hoja = Obj.crear("sphere", { x: lado * 0.025, sx: 0.05, sy: 0.006, sz: 0.022, rz: lado * 0.35, color: "#52B788" }, g);
  hojas.push({ g, hoja, lado, k });
}
const flor = Obj.crear("group", {}, plantaG);
const petalos = [];
for (let k = 0; k < 7; k++) {
  const a = (k / 7) * Math.PI * 2;
  petalos.push(Obj.crear("sphere", { x: Math.cos(a) * 0.022, z: Math.sin(a) * 0.022, ry: -a, sx: 0.035, sy: 0.006, sz: 0.018, color: "#FF70A6" }, flor));
}
Obj.crear("sphere", { y: 0.004, sx: 0.02, sy: 0.012, sz: 0.02, color: "#FFC300" }, flor);
const pimpollo = Obj.crear("sphere", { sx: 0.018, sy: 0.026, sz: 0.018, color: "#3A7D44" }, plantaG);

function dibujar() {
  const g = estado.crecimiento;
  const marchita = marchitaYa();
  const alto = g < 0.1 ? 0 : 0.02 + Math.min(1, (g - 0.1) / 0.75) * 0.28;
  const verde = marchita ? "#8D7B4F" : "#3A7D44";
  const hojaC = marchita ? "#A68A64" : "#52B788";
  // Seca de verdad se dobla.
  const doblez = marchita ? 0.5 : estado.humedad <= 0 ? 0.15 : 0;
  tallo.setAttribute("visible", alto > 0 ? "inherit" : "false");
  tallo.position = { x: Math.sin(doblez) * alto / 2, y: alto / 2, z: 0 };
  tallo.rotation = { x: 0, y: 0, z: -doblez };
  tallo.scale = { x: 0.008 + g * 0.004, y: Math.max(0.001, alto), z: 0.008 + g * 0.004 };
  tallo.setAttribute("color", verde);
  const punta = { x: Math.sin(doblez) * alto, y: Math.cos(doblez) * alto };
  hojas.forEach((h) => {
    // La hoja k aparece cuando el crecimiento pasa 0.12 + k·0.1.
    const s = Math.max(0, Math.min(1, (g - (0.12 + h.k * 0.1)) / 0.1));
    h.g.setAttribute("visible", s > 0 ? "inherit" : "false");
    if (s <= 0) return;
    const f = (h.k + 1) / 7;
    h.g.position = { x: punta.x * f, y: punta.y * f, z: 0 };
    h.g.rotation = { x: 0, y: h.k * 1.3, z: marchita ? h.lado * -0.6 : 0 };
    h.g.scale = { x: s, y: s, z: s };
  });
  pintarHojas(hojaC);
  // Pimpollo y flor.
  const bp = Math.max(0, Math.min(1, (g - 0.75) / 0.15));
  pimpollo.setAttribute("visible", bp > 0 && g < 0.95 ? "inherit" : "false");
  pimpollo.position = { x: punta.x, y: punta.y + 0.01, z: 0 };
  pimpollo.scale = { x: 0.018 * bp, y: 0.026 * bp, z: 0.018 * bp };
  const fl = Math.max(0, Math.min(1, (g - 0.9) / 0.1));
  flor.setAttribute("visible", fl > 0 && !marchita ? "inherit" : "false");
  flor.position = { x: punta.x, y: punta.y + 0.01, z: 0 };
  flor.scale = { x: fl, y: fl, z: fl };
  for (const p of petalos) p.setAttribute("color", Obj.color(P.flor, "#FF70A6"));
  // La tierra: oscura mojada, clara seca.
  Obj.$("tierra").setAttribute("color", Obj.mezcla("#A0826D", "#4A3322", Math.min(1, estado.humedad * 1.5)));
  Obj.$("monticulo").setAttribute("visible", g < 0.1 ? "inherit" : "false");
  const nombre = marchita ? "marchita" : etapaDe(g);
  Obj.$("cartelito").setAttribute("value", nombre + " " + Math.round(g * 100) + "%" + (estado.humedad > 0 ? "" : " (seca)"));
}
let colorHojas = "";
function pintarHojas(c) {
  if (c === colorHojas) return;
  colorHojas = c;
  for (const h of hojas) h.hoja.setAttribute("color", c);
}

/** Ponerse al día con el tiempo que pasó. */
function alDia() {
  const ahora = Date.now();
  const dtH = Math.max(0, (ahora - (Number(estado.ultimo) || ahora)) / HORA);
  estado.ultimo = ahora;
  if (!dtH) return;
  const conAgua = Math.min(dtH, estado.humedad * dura());
  estado.humedad = Math.max(0, estado.humedad - dtH / dura());
  estado.secaHoras = estado.humedad > 0 ? 0 : (Number(estado.secaHoras) || 0) + (dtH - conAgua);
  // Lo que creció mientras tuvo agua cuenta aunque después se haya secado.
  estado.crecimiento = Math.min(1, estado.crecimiento + conAgua / total());
  const marchita = marchitaYa();
  const e = marchita ? "marchita" : etapaDe(estado.crecimiento);
  if (e !== etapaAntes) {
    etapaAntes = e;
    Obj.emitir(e === "marchita" ? "marchita" : "etapa", { nombre: e, crecimiento: +estado.crecimiento.toFixed(3) });
  }
}
function paso() { alDia(); dibujar(); guardado.guardar(estado); }
setInterval(paso, 2000);

// ── Regar ──────────────────────────────────────────────────────────────────
const gotas = [];
for (let i = 0; i < 10; i++) gotas.push({ el: Obj.crear("sphere", { sx: 0.006, sy: 0.01, sz: 0.006, color: "#6EC6FF", visible: "false" }, Obj.$("gotas")), vida: 0 });
function regar() {
  alDia();
  estado.humedad = 1;
  if (etapaAntes === "marchita" || estado.secaHoras > 0) { estado.secaHoras = 0; etapaAntes = etapaDe(estado.crecimiento); }
  Obj.emitir("riego", {});
  paso();
}
// Tocando: la regadera va, se inclina y vuelve.
const REPOSO = { x: -0.2, y: 0, z: 0.02 };
let regando = null;
const anim = Obj.animador(() => {
  let seguir = false;
  const ahora = performance.now();
  if (regando) {
    seguir = true;
    const t = (ahora - regando.t0) / 1000;
    const k = t < 0.4 ? Obj.suave(t / 0.4) : t < 1.6 ? 1 : 1 - Obj.suave(Math.min(1, (t - 1.6) / 0.4));
    regadera.position = { x: REPOSO.x + 0.12 * k, y: 0.14 * k, z: REPOSO.z };
    regadera.rotation = { x: 0, y: 0, z: -0.7 * k };
    if (t > 0.4 && t < 1.6) chorrear({ x: -0.2 + 0.12 + 0.13, y: 0.14 + 0.05, z: REPOSO.z });
    if (t >= 0.8 && !regando.hecho) { regando.hecho = true; regar(); }
    if (t >= 2) regando = null;
  }
  for (const g of gotas) {
    ...
    g.p.y -= 0.9 * 0.016;
    g.el.position = g.p;
    if (g.p.y < 0.15 || ahora > g.vida) { g.vida = 0; g.el.setAttribute("visible", "false"); }
  }
  return seguir;
});
let sigGota = 0;
function chorrear(p) {
  const g = gotas[sigGota++ % gotas.length];
  g.p = { x: p.x + (Math.random() - 0.5) * 0.02, y: p.y, z: p.z + (Math.random() - 0.5) * 0.02 };
  g.vida = performance.now() + 600;
  g.el.setAttribute("visible", "inherit");
}
Obj.boton(Obj.$("toque_regadera"), () => { if (!regando) { regando = { t0: performance.now(), hecho: false }; anim.iniciar(); } });
Obj.mensaje("regar", regar);
Obj.mensaje("replantar", () => {
  estado = { crecimiento: 0, humedad: 0, secaHoras: 0, ultimo: Date.now() };
  etapaAntes = "semilla";
  paso();
});

// En VR: tomar la regadera e inclinarla sobre la maceta.
let ultimoRiego = 0;
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) { return !regando && Math.hypot(p.x - (REPOSO.x - 0.045), p.y - 0.09, p.z - REPOSO.z) < 0.1; },
  mover(p, mano, e) {
    const q = { x: e.qx || 0, y: e.qy || 0, z: e.qz || 0, w: e.qw == null ? 1 : e.qw };
    let arriba = donde.dirLocal(Obj.rotarV(q, { x: 0, y: 1, z: 0 }));
    const m = Math.hypot(arriba.x, arriba.y, arriba.z) || 1;
    arriba = { x: arriba.x / m, y: arriba.y / m, z: arriba.z / m };
    // El giro que lleva +Y a "arriba" de la mano, como cuaternión, para ubicar el pico.
    const l = Math.hypot(arriba.x, arriba.z);
    const qR = l < 1e-6 ? { x: 0, y: 0, z: 0, w: 1 } : Obj.qEje({ x: arriba.z / l, y: 0, z: -arriba.x / l }, Math.acos(Math.max(-1, Math.min(1, arriba.y))));
    // La mano en el asa: el cuerpo 4.5 cm al costado.
    const base = { x: p.x + 0.045, y: p.y - 0.09, z: p.z };
    regadera.position = base;
    regadera.rotation = Obj.euler(qR);
    // Sirve si el pico apunta hacia abajo y cae sobre la maceta.
    const t = Obj.rotarV(qR, { x: 0.105, y: 0.11, z: 0 });
    const pico = { x: base.x + t.x, y: base.y + t.y, z: base.z + t.z };
    const dirPico = Obj.rotarV(qR, { x: 0.84, y: 0.54, z: 0 });
    const sobre = Math.hypot(pico.x, pico.z) < 0.09 && pico.y > 0.15;
    if (dirPico.y < -0.1 && sobre) {
      chorrear(pico);
      anim.iniciar();
      const ahora = performance.now();
      if (ahora - ultimoRiego > 1500) { ultimoRiego = ahora; regar(); }
    }
  },
  soltar() { regadera.position = REPOSO; regadera.rotation = { x: 0, y: 0, z: 0 }; },
});
