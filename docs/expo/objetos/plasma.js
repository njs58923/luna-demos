...
;
// La esfera de plasma: rayos quebrados del núcleo al vidrio.
//
// Cada rayo apunta a un lugar del vidrio que se pasea despacio (una caminata
// al azar sobre la esfera) y se dibuja con unos pocos tramos rectos: los
// puntos intermedios se corren de costado con senos de fase propia, más en el
// medio que en las puntas, y eso lo hace temblar. Tocar el vidrio (o acercar
// la mano) tira el primer rayo hacia ese punto y lo engrosa. Se redibuja a 15
// cuadros por segundo: alcanza para que parezca vivo y cuesta poco.
const P = Obj.props({ color: "#FF6BD6", rayos: 8, apagada: false });

const sinte = Obj.sinte({ volumen: 0.6 });
const donde = Obj.marco();
const grupo = Obj.$("rayos");
const C = { x: 0, y: 0.2, z: 0 };
const R = 0.114, R0 = 0.02;
const TRAMOS = 5;
const N = Math.max(4, Math.min(12, Math.round(Number(P.rayos) || 8)));
const azar = Obj.azar(7);
...
const rayos = [];
for (let i = 0; i < N; i++) {
  // Repartidos parejo sobre la esfera (espiral de Fibonacci), sin la parte de abajo.
  const y = 0.9 - (1.5 * (i + 0.5)) / N;
  const r = Math.sqrt(1 - y * y), a = i * 2.39996;
  const tramos = [];
  for (let k = 0; k < TRAMOS; k++) tramos.push(Obj.crear("cylinder", { sx: 0.003, sy: 0.01, sz: 0.003, color: "#FF6BD6", "material-unlit": "true" }, grupo));
  rayos.push({ dir: { x: Math.cos(a) * r, y, z: Math.sin(a) * r }, fase: i * 1.7, tramos, pintado: "" });
}

let prendida = !P.apagada;
let dedo = null;          // { dir, hasta }
let ultimoChispazo = 0;

function colorRayo() { return Obj.color(P.color, "#FF6BD6"); }

function dibujar() {
  if (!prendida) return;
  const t = performance.now() / 1000;
  const ahora = performance.now();
  if (dedo && ahora > dedo.hasta) dedo = null;
  rayos.forEach((ry, i) => {
    // La punta se pasea; el rayo del dedo va derecho al dedo.
    if (i === 0 && dedo) ry.dir = norm({ x: ry.dir.x * 0.3 + dedo.dir.x * 0.7, y: ry.dir.y * 0.3 + dedo.dir.y * 0.7, z: ry.dir.z * 0.3 + dedo.dir.z * 0.7 });
    else {
      ry.dir = norm({ x: ry.dir.x + (azar() - 0.5) * 0.12, y: ry.dir.y + (azar() - 0.5) * 0.12 + (ry.dir.y < -0.5 ? 0.05 : 0), z: ry.dir.z + (azar() - 0.5) * 0.12 });
    }
    const d = ry.dir;
    // Dos perpendiculares para correr los puntos de costado.
    const u = norm(Math.abs(d.y) < 0.9 ? cruz(d, { x: 0, y: 1, z: 0 }) : cruz(d, { x: 1, y: 0, z: 0 }));
    const w = cruz(d, u);
    const grueso = i === 0 && !!dedo;
    const pts = [];
    for (let k = 0; k <= TRAMOS; k++) {
      const s = k / TRAMOS;
      const rad = R0 + (R - R0) * s;
      const amp = (grueso ? 0.006 : 0.016) * Math.sin(Math.PI * s);
      const a = amp * Math.sin(t * 9 + ry.fase + k * 2.1), b = amp * Math.sin(t * 7.3 + ry.fase * 1.3 + k * 1.7);
      pts.push({ x: C.x + d.x * rad + u.x * a + w.x * b, y: C.y + d.y * rad + u.y * a + w.y * b, z: C.z + d.z * rad + u.z * a + w.z * b });
    }
    const g = grueso ? 0.007 : 0.0025;
    ry.tramos.forEach((tr, k) => {
      const l = Obj.tender(tr, pts[k], pts[k + 1]);
      // Más fino hacia el vidrio, como se abre la descarga.
      const gg = g * (grueso ? 1 : 1.3 - 0.6 * (k / TRAMOS));
      tr.scale = { x: gg, y: l, z: gg };
    });
    const c = grueso ? Obj.tono(colorRayo(), 0.6) : colorRayo();
    if (c !== ry.pintado) {
      ry.pintado = c;
      for (const tr of ry.tramos) tr.setAttribute("color", c);
    }
  });
}
setInterval(dibujar, 66);

function chispazo() {
  const ahora = performance.now();
  if (ahora - ultimoChispazo < 250) return;
  ultimoChispazo = ahora;
  sinte.nota(4000, 0.08, { tipo: "ruido", vol: 0.1, caida: 40 });
  sinte.nota(120, 0.2, { tipo: "cuadrada", vol: 0.04, caida: 12 });
}

/** El dedo en p (coordenadas del objeto). */
function tocar(p, ms) {
  if (!prendida) return;
  const d = norm({ x: p.x - C.x, y: p.y - C.y, z: p.z - C.z });
  const nuevo = !dedo;
  dedo = { dir: d, hasta: performance.now() + ms };
  chispazo();
  if (nuevo) {
    const q = { x: +(C.x + d.x * R).toFixed(4), y: +(C.y + d.y * R).toFixed(4), z: +(C.z + d.z * R).toFixed(4) };
    Obj.emitir("chispa", q);
    dibujar();
  }
}

Obj.boton(Obj.$("vidrio"), (e) => {
  if (!Number.isFinite(e.x)) return tocar({ x: 0, y: 0.2, z: 1 }, 1500);
  tocar(donde.aLocal({ x: e.x, y: e.y, z: e.z }), 1500);
});
...
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    if (d < R + 0.06 && d > 0.02) tocar(l, 250);
  });
}

function poner(v) {
  prendida = v;
  grupo.setAttribute("visible", v ? "inherit" : "false");
  Obj.$("nucleo").setAttribute("color", v ? "#FFD6F5" : "#555566");
  Obj.$("interruptor").setAttribute("color", v ? "#30D158" : "#8E8E93");
  if (v) dibujar();
}
Obj.boton(Obj.$("interruptor"), () => { poner(!prendida); sinte.nota(1500, 0.03, { tipo: "pulso", vol: 0.3 }); });
Obj.mensaje("prender", () => poner(true));
Obj.mensaje("apagar", () => poner(false));
Obj.alCambiar((p, a, cambios) => {
  if (cambios.includes("apagada")) poner(!P.apagada);
  dibujar();
});
poner(prendida);
dibujar();
