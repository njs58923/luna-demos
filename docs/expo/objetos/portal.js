...
;
// El portal: un aro de piedra y un remolino que lleva a otro lado.
//
...
const P = Obj.props({
  destino: "", titulo: "Portal", sub: "", color: "#7B2FF7", piedra: "#6E6A64",
});

const aro = Obj.$("aro");
const remolino = Obj.$("remolino");
const R = 0.8;          // radio interior del aro
const CENTRO = 1.2;

// El aro: dovelas alrededor de un círculo, apoyado sobre dos basas.
const piedras = [];
const DOVELAS = 22;
for (let i = 0; i < DOVELAS; i++) {
  const a = (i / DOVELAS) * Math.PI * 2;
  // Las de abajo del todo se omiten: ahí van las basas.
  if (Math.cos(a) < -0.92) continue;
  const r = R + 0.11;
  piedras.push(Obj.crear("box", {
    x: Math.sin(a) * r, y: CENTRO + Math.cos(a) * r, rz: -a,
    sx: (2 * Math.PI * r) / DOVELAS + 0.012, sy: 0.22, sz: 0.34,
    color: i % 2 ? "#6E6A64" : "#7C7770",
  }, aro));
}
for (const s of [-1, 1]) {
  piedras.push(Obj.crear("box", { x: s * 0.62, y: 0.2, sx: 0.5, sy: 0.4, sz: 0.46, color: "#5E5A54" }, aro));
}
// La clave, arriba: una piedra más grande con el color del remolino.
const clave = Obj.crear("box", { y: CENTRO + R + 0.14, sx: 0.2, sy: 0.3, sz: 0.4, color: "#7B2FF7" }, aro);

// El remolino: discos concéntricos que giran a distinta velocidad, con un
// hueco angular pintado en cada uno para que el giro se note.
const capas = [];
for (let i = 0; i < 6; i++) {
  const r = R * (1 - i * 0.15);
  const g = Obj.crear("group", { z: 0.01 + i * 0.004 }, remolino);
  const disco = Obj.crear("plane", { sx: r * 2, sy: r * 2, "border-radius": 0.5, "material-unlit": "true", color: "#7B2FF7" }, g);
  const marca = Obj.crear("plane", { x: r * 0.55, z: 0.001, sx: r * 0.5, sy: r * 0.12, "border-radius": 0.5, "material-unlit": "true", color: "#FFFFFF" }, g);
  capas.push({ g, disco, marca, vel: (i % 2 ? -1 : 1) * (0.4 + i * 0.25) });
}
// La otra cara del remolino: los planos son de doble cara, pero el orden de
// las capas se invierte visto desde atrás, así que atrás va un disco oscuro.
Obj.crear("plane", { z: -0.02, sx: R * 2, sy: R * 2, "border-radius": 0.5, color: "#14101C" }, remolino);

function pintar() {
  const c = Obj.color(P.color, "#7B2FF7");
  capas.forEach((k, i) => {
    k.disco.setAttribute("color", Obj.mezcla(Obj.tono(c, -0.45), Obj.tono(c, 0.5), i / (capas.length - 1)));
    k.marca.setAttribute("color", Obj.tono(c, 0.7));
  });
  clave.setAttribute("color", c);
  const pie = Obj.color(P.piedra, "#6E6A64");
  piedras.forEach((el, i) => el.setAttribute("color", i >= piedras.length - 2 ? Obj.tono(pie, -0.12) : (i % 2 ? pie : Obj.tono(pie, 0.08))));
...
// El remolino gira siempre, pero despacio: es lo que dice "esto es un portal".
let acelerar = 0;
const anim = Obj.animador((t, dt) => {
  const k = 1 + acelerar * 6;
  for (const c of capas) {
    c.ang = (c.ang || 0) + c.vel * dt * k;
