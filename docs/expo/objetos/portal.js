...
;
// El portal: un marco, un vano con un efecto que se mueve, y chispas o runas
// que giran alrededor. Tocarlo lleva a otro lado.
//
// Todo sale de Obra.portalMalla (_obra.js) y va en mallas propias, en el
// isolate del portal: ocho tipos de marco (aro, arco gótico, puerta,
// monolitos, espejo, hexágono, anillo de runas, grieta), cinco efectos
// (remolino, ondas, estrellas, plasma, vórtice), seis materiales y lo demás
// por props. El efecto se anima cambiando los colores de la malla del vano
// unas treinta veces por segundo; las chispas y las runas giran en grupos.
//
// Navega solo con hiperspace.world.navigate, que reemplaza el mundo entero
// desde adentro de un include. El permiso (`navigate_world`) no se puede
// consultar: si no lo delegaron, la llamada no hace nada y el rechazo queda
// en el log del host. Por eso el portal lo dice en su cartel al tocarlo sin
// éxito aparente, en vez de quedarse callado.
const P = Obj.props({
  destino: "", titulo: "Portal", sub: "",
  tipo: "aro", efecto: "remolino", color: "#7B2FF7", color2: "#40E0D0",
  material: "", piedra: "", tamano: 1, particulas: 60, velocidad: 1, runas: false, semilla: 3,
});

const marcoG = Obj.$("marco");
const cartel = Obj.$("cartel");
const toque = Obj.$("toque");
let hechos = [], recursos = [];
let vano = null, colores = null, giros = [];

function soltar() {
  for (const el of hechos) el.remove();
  for (const m of recursos) { try { m.dispose(); } catch (e) { /* ya no estaba */ } }
  hechos = []; recursos = []; giros = []; vano = null;
}
function modelo(b, padre) {
  const m = MeshResource.create(b);
  recursos.push(m);
  const el = Obj.crear("model", { touchable: "false", "material-unlit": "true" }, padre);
  el.src = m.src;
  return m;
}
...
function dibujar() {
  soltar();
  const p = Obra.portalMalla(P);
  // El cartel arriba del marco; el blanco del toque, sobre el vano.
  cartel.position = { x: 0, y: p.alto + 0.22, z: 0 };
  toque.position = { x: p.centro[0], y: p.centro[1], z: 0.02 };
  toque.scale = { x: p.ancho, y: p.centro[1] * 2, z: 1 };
  pintar();
  if (typeof MeshResource === "undefined") return;
  // El marco, quieto.
  const g = Obj.crear("group", {}, marcoG);
  hechos.push(g);
  for (const b of Obra.mallasDe(p.marco)) modelo(b, g);
  // Lo que gira va en grupos con el origen en el centro del vano.
  const centro = Obj.crear("group", { x: p.centro[0], y: p.centro[1] }, marcoG);
  hechos.push(centro);
  const capas = [[p.particulas[0], 0.35], [p.particulas[1], -0.22], [p.runas, 0.08]];
  for (const [L, vel] of capas) {
    const bs = Obra.mallasDe(L);
    if (!bs.length) continue;
    const gr = Obj.crear("group", {}, centro);
    for (const b of bs) modelo(b, gr);
    giros.push({ g: gr, vel, ang: 0 });
  }
  // El vano: su malla con los colores del efecto, que se actualizan.
  const a = p.abertura;
  colores = new Float32Array(a.vertices * 4);
  a.colores(0, colores);
  vano = { a, datos: { positions: a.positions, indices: a.indices, colors: colores } };
  vano.malla = modelo(vano.datos, g);
}

function pintar() {
  Obj.$("titulo").setAttribute("value", String(P.titulo || "").slice(0, 28));
  Obj.$("sub").setAttribute("value", String(P.sub || (P.destino ? "tocá para entrar" : "sin destino")).slice(0, 40));
}

// El efecto corre siempre, pero despacio: es lo que dice "esto es un portal".
let acelerar = 0, reloj = 0, ultimo = -1;
const anim = Obj.animador((t, dt) => {
  const k = 1 + acelerar * 5;
  reloj += dt * k;
  for (const c of giros) {
    c.ang += c.vel * dt * k;
    c.g.rotation = { x: 0, y: 0, z: c.ang };
  }
  // Unas treinta veces por segundo alcanza para el vano. update() manda los
  // cinco buffers siempre (guides/trampas.md): van posiciones e índices igual.
  if (vano && t - ultimo > 0.033) {
    ultimo = t;
    vano.a.colores(reloj, colores);
    try { vano.malla.update(vano.datos); } catch (e) { /* se soltó en un rearmado */ }
  }
  if (acelerar > 0) acelerar = Math.max(0, acelerar - dt * 0.6);
  return true;
});
anim.iniciar();

function destinoAbsoluto() {
  const d = String(P.destino || "").trim();
  if (!d) return "";
  try { return new URL(d, location.href).href; } catch (e) { return ""; }
}

let intentos = 0;
Obj.boton(toque, () => {
  const d = destinoAbsoluto();
  acelerar = 1;
  Obj.emitir("entrar", { destino: d });
  if (!d) return;
  intentos++;
  try {
    hiperspace.world.navigate(d);
  } catch (e) {
    console.warn("[portal] no se pudo navegar:", (e && e.message) || e);
  }
  // Si el mundo sigue acá un segundo y medio después, el permiso no llegó.
  const n = intentos;
  setTimeout(() => {
    if (n !== intentos) return;
    Obj.$("sub").setAttribute("value", "no me dejaron pasar: falta navigate_world");
  }, 1500);
});

Obj.alCambiar((p, antes, cambios) => {
  if (cambios.every((c) => ["titulo", "sub", "destino"].includes(c))) return void pintar();
  dibujar();
});
dibujar();
