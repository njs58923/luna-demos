...
;
// El portón de garage: tablillas que suben por sus rieles, doblan en el
// dintel y siguen hacia atrás acostadas, como un portón seccional.
//
// Cada tablilla tiene el origen en su borde de abajo. Con el portón subiendo
// `d` metros, la tablilla que estaba en y0 anda por y0 + d mientras le quede
// vano; pasado el dintel dobla por un cuarto de círculo de radio R y sigue
// derecho hacia -Z. La tablilla de arriba es la primera en doblar.
const P = Obj.props({ ancho: 2.4, alto: 2.1, tablillas: 5, color: "", marco: "", ventanitas: true, clave: "" });
const R = 0.35;
const memo = Obj.memoria("abierto");
const armarMarco = Obra.montar(Obj.$("obra"), Obj.crear);
const riel = Obj.$("tablillas");
const toque = Obj.$("toque");
let abierto = !!memo.leer(false);
let k = abierto ? 1 : 0;
let medidas = null, grupos = [], hechos = [];

function dibujar() {
  for (const el of hechos) el.remove();
  const p = Obra.porton(Object.assign({}, P, { ventanitas: P.ventanitas !== false }));
  armarMarco(p.marco);
  grupos = p.tablillas.map((t) => {
    const g = Obj.crear("group", { y: t.y }, riel);
    Obra.construir(t.nodos, g, Obj.crear);
    return { g, y0: t.y };
  });
  hechos = grupos.map((t) => t.g);
  medidas = p;
  toque.position = { x: 0, y: p.alto / 2, z: 0.06 };
  toque.scale = { x: p.ancho, y: p.alto, z: 0.12 };
  mover();
}
function mover() {
  const H = medidas.alto, h = medidas.paso;
  const d = Obj.suave(k) * (H + 0.02);
  const arco = (R * Math.PI) / 2;
  for (const t of grupos) {
    const p = t.y0 + d;
    const e = p - (H - h);
    if (e <= 0) {
      t.g.position = { x: 0, y: p, z: 0 };
      t.g.rotation = { x: 0, y: 0, z: 0 };
    } else if (e < arco) {
      const a = e / R;
      // El borde de abajo sobre un cuarto de círculo con centro en (H - h, -R).
      t.g.position = { x: 0, y: H - h + R * Math.sin(a), z: -R + R * Math.cos(a) };
      t.g.rotation = { x: -a, y: 0, z: 0 };
    } else {
      t.g.position = { x: 0, y: H - h + R, z: -R - (e - arco) };
      t.g.rotation = { x: -Math.PI / 2, y: 0, z: 0 };
    }
  }
}
...
const anim = Obj.animador((t, dt) => {
  ...
  k = meta > k ? Math.min(1, k + dt / 2.6) : Math.max(0, k - dt / 2.6);
  mover();
  return k !== meta;
});
function poner(v) {
