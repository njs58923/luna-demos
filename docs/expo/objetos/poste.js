...
;
// El poste indicador: flechas que señalan, y que pueden llevar.
//
// Cada flecha es un tablón con una punta (un cuadrado girado 45°, medio
// metido en el tablón) y su texto en las dos caras, porque un cartel de
// camino se lee de los dos lados. La flecha gira alrededor del palo según su
// ángulo; su texto de atrás va dado vuelta para no leerse al revés.
const P = Obj.props({
  // Rutas relativas a propósito: se resuelven contra este documento, o sea
  // contra el servidor de la calle, esté donde esté montado el poste.
  flechas: [
    { texto: "LA CALLE", angulo: 20, color: "#C9A86A", destino: "/calle.hsml" },
    { texto: "MI MUNDO", angulo: 160, color: "#A7C4A0", destino: "/mundo.hsml" },
    { texto: "LA EXPO", angulo: -60, color: "#D9B38C", destino: "/" },
  ],
  madera: "#6B4A30",
});

const grupo = Obj.$("flechas");
let partes = [];

function flechas() {
  const f = Array.isArray(P.flechas) ? P.flechas : [];
  return f.filter((x) => x && typeof x.texto === "string").slice(0, 4).map((x) => ({
    texto: x.texto.slice(0, 14),
    angulo: Number.isFinite(Number(x.angulo)) ? Number(x.angulo) : 0,
    color: Obj.color(x.color, "#C9A86A"),
    destino: typeof x.destino === "string" ? x.destino : "",
  }));
}

function destinoAbsoluto(d) {
  if (!d) return "";
...
function armar() {
  ...
  partes = [];
  flechas().forEach((f, i) => {
    // Un grupo que gira alrededor del palo; la flecha apunta hacia su +X.
    const g = Obj.crear("group", { y: 1.95 - i * 0.3, ry: (f.angulo * Math.PI) / 180 }, grupo);
    partes.push(g);
    const L = 0.72, A = 0.17;
    Obj.crear("box", { x: 0.06 + L / 2, sx: L, sy: A, sz: 0.035, color: f.color }, g);
    Obj.crear("box", { x: 0.06 + L, rz: Math.PI / 4, sx: A / Math.SQRT2, sy: A / Math.SQRT2, sz: 0.035, color: f.color }, g);
    // El borde de abajo, un poco más oscuro, para que se lea como madera.
    Obj.crear("box", { x: 0.06 + L / 2, y: -A / 2 + 0.01, sx: L, sy: 0.02, sz: 0.037, color: Obj.tono(f.color, -0.2) }, g);
    Obj.crear("text", { x: 0.06 + L / 2, z: 0.019, value: f.texto, size: 0.07, color: "#2A1E12" }, g);
    Obj.crear("text", { x: 0.06 + L / 2, z: -0.019, ry: Math.PI, value: f.texto, size: 0.07, color: "#2A1E12" }, g);
    const blanco = Obj.crear("box", { x: 0.06 + L / 2 + 0.05, sx: L + 0.1, sy: A, sz: 0.06, color: "transparent", "material-alpha": "blend" }, g);
    Obj.boton(blanco, () => elegir(f));
  });
}

function elegir(f) {
  const d = destinoAbsoluto(f.destino);
  Obj.emitir("elegir", { texto: f.texto, destino: d });
  if (!d) return;
  try { hiperspace.world.navigate(d); } catch (e) { console.warn("[poste] no se pudo navegar:", (e && e.message) || e); }
}

Obj.alCambiar((p, a, cambios) => {
  Obj.$("palo").setAttribute("color", Obj.color(p.madera, "#6B4A30"));
  if (cambios.includes("flechas")) armar();
});
Obj.$("palo").setAttribute("color", Obj.color(P.madera, "#6B4A30"));
armar();
