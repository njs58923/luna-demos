...
;
// La puerta: marco fijo y una hoja colgada de su bisagra, que es el origen
// del grupo `bisagra`. Girar el grupo en +Y la abre hacia atrás, lejos de
// quien la toca. En VR se toma la hoja de cualquier punto y se empuja (la
// puerta de las manos, _vr.js); al soltarla termina de abrirse o se cierra.
const P = Obj.props({ ancho: 0.9, alto: 2.05, estilo: "tableros", color: "", marco: "", picaporte: "", clave: "" });
const ABIERTA = 1.45;
const memo = Obj.memoria("abierta");
const bisagra = Obj.$("bisagra");
const armarMarco = Obra.montar(Obj.$("obra"), Obj.crear);
const armarHoja = Obra.montar(bisagra, Obj.crear);
let abierta = !!memo.leer(false);
let k = abierta ? 1 : 0;
let mano = null;
let medidas = null;

function dibujar() {
  const p = Obra.puerta(P);
  armarMarco(p.marco);
  const hechos = armarHoja(p.hoja);
  // El blanco del toque es la hoja entera: se crea con ella y se va con ella.
  const blanco = Obj.crear("box", { x: p.ancho / 2, y: p.alto / 2, sx: p.ancho, sy: p.alto, sz: 0.08, color: "transparent", "material-alpha": "blend" }, bisagra);
  blanco.setAttribute("manos", "false");
  hechos.push(blanco);
  Obj.boton(blanco, () => poner(!abierta));
  bisagra.position = p.bisagra;
  medidas = p;
  girar();
}
function girar() {
  bisagra.rotation = { x: 0, y: mano !== null ? mano : ABIERTA * Obj.suave(k), z: 0 };
}

const anim = Obj.animador((t, dt) => {
  const meta = abierta ? 1 : 0;
  if (mano === null && k !== meta) k = meta > k ? Math.min(1, k + dt / 0.9) : Math.max(0, k - dt / 0.7);
  girar();
  return mano === null && k !== meta;
});
function poner(v) {
  if (abierta === v) return;
  abierta = v;
  memo.guardar(abierta);
  anim.iniciar();
...
dibujar();
Obj.manos().puerta({
  pivote: { x: medidas.bisagra.x, y: medidas.alto / 2, z: 0 }, eje: { x: 0, y: 1, z: 0 }, brazo: { x: 1, y: 0, z: 0 },
  largo: medidas.ancho, alto: medidas.alto, max: ABIERTA,
  angulo: () => (mano !== null ? mano : ABIERTA * Obj.suave(k)),
  mover(a) { mano = a; girar(); },
  soltar(v, a) {
    mano = null;
    k = Obj.suaveInversa(Math.min(1, Math.abs(a) / ABIERTA));
    if (v !== abierta) poner(v); else anim.iniciar();
  },
});
