...
;
// La radio de ambientes: cuatro sonidos que se generan, no se bajan.
//
// Cada estación es una función de muestra en el tiempo que se le pasa al
// flujo del sintetizador (Obj.sinte().flujo): ruido filtrado para la lluvia,
// ruido con una ola lenta de volumen para el mar, chasquidos al azar sobre un
// rumor grave para la fogata, y arpegios pentatónicos para la cajita. El
// volumen se aplica adentro de la función, así cambiarlo no corta nada.
const P = Obj.props({ estacion: "lluvia", volumen: 0.6, color: "#8C5A3C", clave: "" });

const memo = Obj.memoria("radio");
const sinte = Obj.sinte({ volumen: 0.9 });
const ESTACIONES = ["lluvia", "olas", "fogata", "arpegios"];
const NOMBRES = { lluvia: "LLUVIA 88.1", olas: "OLAS 92.5", fogata: "FOGATA 97.3", arpegios: "ARPEGIOS 103.7" };

const g = memo.leer(null);
let estacion = ESTACIONES.includes(g && g.e) ? g.e : (ESTACIONES.includes(P.estacion) ? P.estacion : "lluvia");
let volumen = g && Number.isFinite(g.v) ? g.v : Math.max(0, Math.min(1, Number(P.volumen) || 0.6));
let prendida = false;

// La rejilla del parlante: barras horizontales.
for (let i = 0; i < 7; i++) Obj.crear("box", { y: -0.045 + i * 0.015, sx: 0.16, sy: 0.004, sz: 0.002, color: "#8A7A5A" }, Obj.$("rejilla"));

// ── Los generadores ─────────────────────────────────────────────────────────
function generador(nombre) {
  let s = 12345, lp = 0, lp2 = 0, chis = 0;
  const ruido = () => { s = (s * 1103515245 + 12345) & 0x7fffffff; return s / 0x3fffffff - 1; };
  const R = sinte.RATE;
  if (nombre === "lluvia") {
    return (t) => {
      // Ruido pasabajos + gotas sueltas (chasquidos agudos al azar).
      lp += (ruido() - lp) * 0.35;
      if (Math.random() < 0.0012) chis = 1;
      chis *= 0.985;
      return (lp * 0.35 + ruido() * chis * 0.25) * volumen;
    };
  }
  if (nombre === "olas") {
    return (t) => {
      lp += (ruido() - lp) * 0.08;
      lp2 += (lp - lp2) * 0.2;
      const ola = Math.pow(0.5 + 0.5 * Math.sin(t * 0.55 + Math.sin(t * 0.13) * 2), 2.2);
      return lp2 * (0.25 + 1.4 * ola) * volumen;
    };
  }
  if (nombre === "fogata") {
    return (t) => {
      lp += (ruido() - lp) * 0.03;
      if (Math.random() < 0.0009) chis = 0.6 + Math.random() * 0.4;
      chis *= 0.93;
      return (lp * 0.6 + ruido() * chis * 0.5) * volumen;
    };
  }
  // Arpegios: una nota pentatónica cada 0.3 s, con su campanita.
  const PENTA = [0, 2, 4, 7, 9, 12, 14, 16, 19];
  let nota = 0, inicio = 0, freq = 440;
  return (t) => {
    if (t - inicio >= 0.3) {
      inicio = t;
      nota = (nota + 1 + Math.floor(Math.random() * 3)) % PENTA.length;
      freq = Obj.frecuencia(PENTA[nota] - 9);
    }
    const u = t - inicio;
    const env = Math.min(1, u / 0.005) * Math.exp(-5 * u);
    return (Math.sin(6.2832 * freq * u) + 0.3 * Math.sin(6.2832 * freq * 2 * u) * Math.exp(-9 * u)) * env * 0.18 * volumen;
  };
}

// ── Estado ──────────────────────────────────────────────────────────────────
function guardar() { memo.guardar({ e: estacion, v: volumen }); }
function dibujar() {
  Obj.$("dial_txt").setAttribute("value", prendida ? NOMBRES[estacion] : "");
  Obj.$("luz").setAttribute("color", prendida ? "#FFB84D" : "#3A2A1A");
  // El dial gira un cuarto de vuelta por estación; la marca va con él.
  const i = ESTACIONES.indexOf(estacion);
  Obj.$("dial").rotation = { x: Math.PI / 2, y: i * (Math.PI / 2), z: 0 };
  const a = i * (Math.PI / 2);
  Obj.$("dial_marca").position = { x: 0.09 + Math.sin(a) * 0.02, y: 0.14 + Math.cos(a) * 0.02, z: 0.087 };
  Obj.$("dial_marca").rotation = { x: 0, y: 0, z: -a };
  Obj.$("volumen").rotation = { x: Math.PI / 2, y: -1.5 + volumen * 3, z: 0 };
}
function sintonizar() {
  if (!prendida) { sinte.flujo(null); return; }
  if (!sinte.flujo(generador(estacion))) {
    Obj.$("dial_txt").setAttribute("value", "SIN SONIDO");
  }
}
function prender(v) {
  if (v === prendida) return;
  prendida = v;
  sintonizar();
  dibujar();
  Obj.emitir("estacion", { nombre: estacion, prendida });
}
function cambiar(nombre) {
  if (!ESTACIONES.includes(nombre)) return;
  estacion = nombre;
  guardar();
  // Un chasquido de estática al cambiar, como una radio de verdad.
  if (prendida) sinte.nota(300, 0.12, { tipo: "ruido", vol: 0.15, caida: 25 });
  sintonizar();
  dibujar();
  Obj.emitir("estacion", { nombre: estacion, prendida });
}

Obj.boton(Obj.$("encendido"), () => prender(!prendida), { color: "#3A3A3A" });
Obj.boton(Obj.$("dial"), () => { if (!prendida) prender(true); else cambiar(ESTACIONES[(ESTACIONES.indexOf(estacion) + 1) % ESTACIONES.length]); }, { color: "#E9D8A6" });
Obj.boton(Obj.$("volumen"), () => { volumen = volumen >= 0.95 ? 0.2 : Math.min(1, volumen + 0.2); guardar(); dibujar(); }, { color: "#E9D8A6" });
Obj.mensaje("prender", () => prender(true));
Obj.mensaje("apagar", () => prender(false));
Obj.mensaje("estacion", (d) => { if (d) cambiar(d.nombre); });
Obj.alCambiar(() => Obj.$("gabinete").setAttribute("color", Obj.color(P.color, "#8C5A3C")));
Obj.$("gabinete").setAttribute("color", Obj.color(P.color, "#8C5A3C"));
dibujar();
