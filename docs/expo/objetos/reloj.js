...
;
// El reloj de pared: agujas que marcan la hora de verdad.
//
// No pide cuadros: el segundero salta una vez por segundo, así que alcanza con
// un temporizador alineado al cambio de segundo. Un reloj colgado en un mundo
// no tiene por qué despertar al isolate sesenta veces por segundo.
const P = Obj.props({
  marco: "#2E2A26", esfera: "#F4F1EA", agujas: "#1C1C1C", segundero: true,
  utc: null, ciudad: "", clave: "",
});

const marcas = Obj.$("marcas");
const R = 0.165;

// Las doce marcas y los cuatro números.
for (let i = 0; i < 60; i++) {
  const a = (i / 60) * Math.PI * 2;
  const hora = i % 5 === 0;
  const largo = hora ? 0.028 : 0.01;
  const r = R - largo / 2 + 0.01;
  Obj.crear("box", {
    x: Math.sin(a) * r, y: Math.cos(a) * r, z: 0.057, rz: -a,
    sx: hora ? 0.007 : 0.0025, sy: largo, sz: 0.002, color: "#2A2A2A",
  }, marcas);
}
for (const [n, a] of [["12", 0], ["3", 90], ["6", 180], ["9", 270]]) {
  const r = 0.128, ang = (a * Math.PI) / 180;
  Obj.crear("text", { x: Math.sin(ang) * r, y: Math.cos(ang) * r, z: 0.057, value: n, size: 0.03, color: "#2A2A2A" }, marcas);
}

/** Una aguja: una caja corrida hacia arriba, así el pivote queda en el eje. */
function aguja(grupo, largo, ancho, color, cola) {
  const g = Obj.$(grupo);
  const partes = [Obj.crear("box", { y: largo / 2 - cola, sx: ancho, sy: largo + cola, sz: 0.003, color }, g)];
  return { g, partes };
}
const h = aguja("aguja_h", 0.095, 0.011, P.agujas, 0.015);
const m = aguja("aguja_m", 0.145, 0.007, P.agujas, 0.02);
const s = aguja("aguja_s", 0.158, 0.0025, "#C8102E", 0.035);
s.partes.push(Obj.crear("cylinder", { y: -0.03, rx: Math.PI / 2, sx: 0.014, sy: 0.003, sz: 0.014, color: "#C8102E" }, s.g));

const digital = Obj.$("digital");
const dHora = Obj.$("digital_hora");
const dFecha = Obj.$("digital_fecha");
const ciudad = Obj.$("ciudad");

function aplicar(p) {
  Obj.$("marco").setAttribute("color", Obj.color(p.marco, "#2E2A26"));
  Obj.$("esfera").setAttribute("color", Obj.color(p.esfera, "#F4F1EA"));
  for (const a of [h, m]) for (const el of a.partes) el.setAttribute("color", Obj.color(p.agujas, "#1C1C1C"));
  s.g.setAttribute("visible", p.segundero === false ? "false" : "inherit");
  if (ciudad) ciudad.setAttribute("value", String(p.ciudad || "").slice(0, 24));
}
aplicar(P);
Obj.alCambiar((p) => { aplicar(p); marcar(); });

/** La hora a mostrar: local, o la de un huso fijo si vino `utc`. */
function ahora() {
  const d = new Date();
  const utc = typeof P.utc === "number" && Number.isFinite(P.utc) ? P.utc : null;
  if (utc == null) return { h: d.getHours(), m: d.getMinutes(), s: d.getSeconds(), dia: d.getDate(), mes: d.getMonth(), anio: d.getFullYear(), sem: d.getDay() };
  const u = new Date(d.getTime() + utc * 3600000);
  return { h: u.getUTCHours(), m: u.getUTCMinutes(), s: u.getUTCSeconds(), dia: u.getUTCDate(), mes: u.getUTCMonth(), anio: u.getUTCFullYear(), sem: u.getUTCDay() };
}
...
function marcar() {
  const t = ahora();
  // Las agujas giran en sentido horario visto desde +Z: rz negativo.
  h.g.rotation = { x: 0, y: 0, z: -((t.h % 12) + t.m / 60) / 12 * Math.PI * 2 };
  m.g.rotation = { x: 0, y: 0, z: -(t.m + t.s / 60) / 60 * Math.PI * 2 };
  s.g.rotation = { x: 0, y: 0, z: -t.s / 60 * Math.PI * 2 };
  if (mostrando) {
    dHora.setAttribute("value", dos(t.h) + ":" + dos(t.m) + ":" + dos(t.s));
    dFecha.setAttribute("value", DIAS[t.sem] + " " + t.dia + " " + MESES[t.mes] + " " + t.anio);
  }
  if (t.m !== minutoAvisado) {
    minutoAvisado = t.m;
    Obj.emitir("hora", { h: t.h, m: t.m, s: t.s });
  }
}

// Alineado al cambio de segundo: el salto del segundero cae cuando cambia el
// segundo, no en cualquier momento de él.
function latir() {
  marcar();
  const ms = 1000 - (Date.now() % 1000);
  setTimeout(latir, ms + 5);
}

let mostrando = false, ocultar = null;
Obj.boton(Obj.$("toque"), () => {
  mostrando = true;
  digital.setAttribute("visible", "inherit");
  marcar();
  clearTimeout(ocultar);
  ocultar = setTimeout(() => { mostrando = false; digital.setAttribute("visible", "false"); }, 5000);
});
