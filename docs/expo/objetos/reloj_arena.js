...
;
// El reloj de arena: dar vuelta y esperar.
//
// Las pilas de arena son conos escalonados —tres cilindros cada una, porque
// no hay cono—, y lo que baja o sube es la escala de cada grupo. Da igual que
// la arena "de arriba" esté físicamente arriba o abajo: al dar vuelta el
// reloj se intercambian los papeles, y el grupo que quedó arriba es el que se
// vacía. Así el giro de 180° no tiene que mover arena de lugar.
const P = Obj.props({ duracion: 60, arena: "#E9C46A", madera: "#6B4A30" });

const todo = Obj.$("todo");
const hilo = Obj.$("hilo");
const grupos = { a: Obj.$("arriba"), b: Obj.$("abajo") };
const granos = [];

/** Una pila cónica: tres discos que se angostan. `invertida` para la de
 *  arriba, que se vacía desde abajo, por el cuello. */
function pila(g, invertida) {
  const discos = [[0.078, 0.03], [0.056, 0.025], [0.032, 0.02]];
  let y = 0;
  for (const [d, h] of discos) {
    granos.push(Obj.crear("cylinder", { y: invertida ? -(y + h / 2) + 0.075 : y + h / 2, sx: d, sy: h, sz: d, color: Obj.color(P.arena, "#E9C46A") }, g));
    y += h;
  }
}
pila(grupos.a, true);
...
// Arena en cada ampolla, de 0 a 1. Arriba en el mundo está `arriba` salvo que
// el reloj esté dado vuelta.
let arena = { a: 0, b: 1 };
let volteado = false;
let giro = 0;        // 0..1 mientras gira
let girando = false;
let cayendo = false;
let ultimo = 0;
...
function dibujarArena() {
  for (const k of ["a", "b"]) {
    const v = Math.max(0.0001, arena[k]);
    grupos[k].scale = { x: Math.sqrt(v), y: v, z: Math.sqrt(v) };
  }
  hilo.setAttribute("visible", cayendo ? "inherit" : "false");
  // El hilo sale de la ampolla que está arriba en el mundo.
  hilo.position = { x: 0, y: volteado ? 0.06 : -0.06, z: 0 };
}

const anim = Obj.animador((t, dt) => {
  if (girando) {
    giro = Math.min(1, giro + dt / 0.7);
    const base = volteado ? 0 : Math.PI;   // de dónde viene
    todo.rotation = { x: 0, y: 0, z: base + Math.PI * Obj.suave(giro) };
    if (giro >= 1) {
      girando = false;
      cayendo = true;
      ultimo = t;
  ...
  if (cayendo) {
    const dur = Math.max(1, Math.min(3600, Number(P.duracion) || 60));
    const paso = (t - ultimo) / dur;
    ultimo = t;
    // La que está arriba en el mundo se vacía en la otra.
    const de = volteado ? "b" : "a", a = volteado ? "a" : "b";
    const mueve = Math.min(arena[de], paso);
    arena[de] -= mueve;
    arena[a] += mueve;
    if (arena[de] <= 0) {
      cayendo = false;
...
function darVuelta() {
  ...
  cayendo = false;
  giro = 0;
  Obj.emitir("vuelta", { duracion: Number(P.duracion) || 60 });
  anim.iniciar();
}

Obj.boton(Obj.$("toque"), darVuelta);
Obj.mensaje("dar_vuelta", darVuelta);

function pintar() {
  for (const g of granos) g.setAttribute("color", Obj.color(P.arena, "#E9C46A"));
  hilo.setAttribute("color", Obj.color(P.arena, "#E9C46A"));
  for (const m of Obj.root.getElementsByClass("madera")) m.setAttribute("color", Obj.color(P.madera, "#6B4A30"));
}
