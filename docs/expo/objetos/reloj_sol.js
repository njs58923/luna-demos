...
;
// El reloj de sol: la sombra sale del cálculo del sol, no de un reloj.
//
// Con la fecha, la hora y la longitud se saca el ángulo horario del sol
// (0 al mediodía solar, 15° por hora); con la latitud y la declinación del
// día, su altura. En un reloj de sol horizontal, la sombra de la hora H forma
// con la línea del mediodía un ángulo θ tal que tan θ = sen|φ| · tan H, y su
// largo es la altura del gnomon dividida por la tangente de la altura del
// sol. Las marcas de las horas se dibujan con esa misma fórmula.
const P = Obj.props({ latitud: -34.6, longitud: -58.4, utc: null, piedra: "#B8B0A0" });

const R = 0.17, ALTO_GNOMON = 0.08;
const rad = (g) => (g * Math.PI) / 180;
const marcas = Obj.$("marcas");
const gnomon = Obj.$("gnomon");
const sombra = Obj.$("sombra");
const lectura = Obj.$("lectura");

function latitud() { return Math.max(-66, Math.min(66, Number(P.latitud) || -34.6)); }
const s = () => (latitud() >= 0 ? 1 : -1);

/** La dirección de la sombra (x, z) para el ángulo horario H (radianes). */
function direccion(H) {
  const th = Math.atan(Math.sin(Math.abs(rad(latitud()))) * Math.tan(H));
  // Pasado el horizonte (|H| > 90°) la fórmula se da vuelta: se corrige.
  const t = Math.abs(H) > Math.PI / 2 ? th + Math.sign(H) * Math.PI : th;
  return { x: s() * Math.sin(t), z: -Math.cos(t) };
}

// Las marcas de 6 a 18, con su número.
let nodosMarcas = [];
function dibujarMarcas() {
  for (const n of nodosMarcas) n.remove();
  nodosMarcas = [];
  for (let h = 6; h <= 18; h++) {
    const d = direccion(rad((h - 12) * 15));
    const a = Math.atan2(d.x, -d.z);
    nodosMarcas.push(Obj.crear("box", { x: d.x * R * 0.8, z: d.z * R * 0.8, ry: -a, sx: 0.003, sy: 0.001, sz: R * 0.25, color: "#6A5E4E" }, marcas));
    nodosMarcas.push(Obj.crear("text", { x: d.x * R * 1.02, y: 0.001, z: d.z * R * 1.02, rx: -Math.PI / 2, value: String(h), size: 0.018, color: "#3A3228" }, marcas));
  }
  // El gnomon: una aleta triangular (escalones) sobre la línea del mediodía,
  // inclinada con la latitud, apuntando al polo (-Z).
  const inclinacion = rad(Math.abs(latitud()));
  for (let i = 0; i < 6; i++) {
    const k = (i + 0.5) / 6;
    const z = -k * R * 0.85;
    const h = Math.tan(inclinacion) * (k * R * 0.85);
    nodosMarcas.push(Obj.crear("box", { y: Math.min(h, ALTO_GNOMON) / 2, z, sx: 0.006, sy: Math.min(h, ALTO_GNOMON), sz: (R * 0.85) / 6 + 0.002, color: "#7A6A4A" }, gnomon));
  }
}

/** Posición del sol ahora: ángulo horario H y altura (radianes). */
function sol() {
  const d = new Date();
  const utc = typeof P.utc === "number" && Number.isFinite(P.utc) ? P.utc : -d.getTimezoneOffset() / 60;
  // Hora UTC en horas, y día del año.
  const horasUTC = d.getUTCHours() + d.getUTCMinutes() / 60 + d.getUTCSeconds() / 3600;
  const inicio = Date.UTC(d.getUTCFullYear(), 0, 0);
  const dia = Math.floor((d.getTime() - inicio) / 86400000);
  // Ecuación del tiempo (aprox., en minutos) y hora solar local.
  const B = rad((360 / 365) * (dia - 81));
  const ecuacion = 9.87 * Math.sin(2 * B) - 7.53 * Math.cos(B) - 1.5 * Math.sin(B);
  const longitud = Number.isFinite(Number(P.longitud)) ? Number(P.longitud) : utc * 15;
  const solar = ((horasUTC + longitud / 15 + ecuacion / 60) % 24 + 24) % 24;
  const H = rad((solar - 12) * 15);
  const dec = rad(23.44) * Math.sin(rad((360 / 365) * (dia - 81)));
  const lat = rad(latitud());
  const alt = Math.asin(Math.sin(lat) * Math.sin(dec) + Math.cos(lat) * Math.cos(dec) * Math.cos(H));
  return { H, alt, solar, legal: ((horasUTC + utc) % 24 + 24) % 24 };
}

function dibujar() {
  const { H, alt } = sol();
  if (alt <= rad(1)) { sombra.setAttribute("visible", "false"); return; }
  const d = direccion(H);
  const largo = Math.min(R * 1.1, ALTO_GNOMON / Math.tan(alt));
  sombra.setAttribute("visible", "inherit");
  sombra.position = { x: d.x * largo / 2, y: 0.0625, z: d.z * largo / 2 };
  sombra.rotation = { x: 0, y: -Math.atan2(d.x, -d.z), z: 0 };
  sombra.scale = { x: 0.012, y: 0.001, z: largo };
}
...
Obj.boton(Obj.$("toque"), () => {
  const { solar, alt } = sol();
  lectura.setAttribute("value", alt > 0 ? "hora solar " + hhmm(solar) : "de noche: sin sombra");
  Obj.emitir("hora", { solar: hhmm(solar) });
  clearTimeout(ocultar);
  ocultar = setTimeout(() => lectura.setAttribute("value", ""), 4000);
});
Obj.alCambiar((p, a, cambios) => {
  Obj.$("piedra").setAttribute("color", Obj.color(p.piedra, "#B8B0A0"));
  if (cambios.includes("latitud")) dibujarMarcas();
  dibujar();
});
Obj.$("piedra").setAttribute("color", Obj.color(P.piedra, "#B8B0A0"));
dibujarMarcas();
dibujar();
// La sombra se mueve un grado cada cuatro minutos: se redibuja cada minuto.
