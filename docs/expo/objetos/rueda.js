...
;
// La rueda: un ángulo acumulado, sin topes.
//
// El ángulo se mide en el sentido del reloj desde arriba, mirando la rueda de
// frente, y se acumula: dos vueltas son 4π. En VR la manija sigue al ángulo
// de la mano alrededor del centro (el paso de un cuadro al otro, que nunca
// es mayor que media vuelta). Con sentido "horario" sólo cuenta lo que avanza
// en el sentido del reloj; lo que retrocede no descuenta pero tampoco mueve
// la rueda, como un trinquete. En escritorio cada clic es un tramo animado
// hasta el ángulo del punto tocado, siempre por el lado permitido.
const P = Obj.props({ meta: 3, sentido: "horario", color: "#F77F00", etiqueta: "", retroceso: false });

const sinte = Obj.sinte({ volumen: 0.6 });
const donde = Obj.marco();
const ruedaG = Obj.$("rueda"), lucesG = Obj.$("luces");
const C = { x: 0, y: 0.01 };
const R = 0.1, R_MANIJA = 0.082;
const DOS_PI = Math.PI * 2;

// ── Lo que se ve ───────────────────────────────────────────────────────────
Obj.crear("cylinder", { z: 0.006, rx: Math.PI / 2, sx: R * 2, sy: 0.012, sz: R * 2, color: "#2B2F3A" }, ruedaG);
const aro = Obj.crear("cylinder", { z: 0.013, rx: Math.PI / 2, sx: R * 2 - 0.012, sy: 0.002, sz: R * 2 - 0.012, color: "#F77F00" }, ruedaG);
Obj.crear("cylinder", { z: 0.0142, rx: Math.PI / 2, sx: R * 1.3, sy: 0.002, sz: R * 1.3, color: "#2B2F3A" }, ruedaG);
for (let i = 0; i < 4; i++) {
  const a = (i / 4) * DOS_PI;
  Obj.crear("box", { x: Math.sin(a) * R * 0.33, y: Math.cos(a) * R * 0.33, z: 0.016, rz: -a, sx: 0.012, sy: R * 0.62, sz: 0.004, color: "#3A3F4B" }, ruedaG);
}
Obj.crear("cylinder", { z: 0.018, rx: Math.PI / 2, sx: 0.03, sy: 0.008, sz: 0.03, color: "#8C8C8C" }, ruedaG);
// La manija: un cilindro que sale hacia el que mira, arriba en el borde.
const manija = Obj.crear("group", { y: R_MANIJA, z: 0.016 }, ruedaG);
Obj.crear("cylinder", { z: 0.025, rx: Math.PI / 2, sx: 0.02, sy: 0.05, sz: 0.02, color: "#1C1F27" }, manija);
Obj.crear("sphere", { z: 0.052, sx: 0.026, sy: 0.026, sz: 0.026, color: "#1C1F27" }, manija);
// Las luces del progreso: un aro de veinte alrededor.
const LUCES = 20;
const luces = [];
for (let i = 0; i < LUCES; i++) {
  const a = (i / LUCES) * DOS_PI;
  luces.push(Obj.crear("sphere", { x: Math.sin(a) * (R + 0.018), y: Math.cos(a) * (R + 0.018), sx: 0.01, sy: 0.01, sz: 0.004, color: "#3A3F4B" }, lucesG));
}

// ── Estado ─────────────────────────────────────────────────────────────────
let angulo = 0;          // acumulado, lo que se ve
let cuenta = 0;          // lo que cuenta hacia la meta (en radianes)
let vueltas = 0, completo = false;
let meta = null;         // escritorio: el ángulo acumulado al que va
let tomada = false;
let ultimoClic = 0;
const vueltasMeta = () => Math.max(0, Number(P.meta) || 0);
const permitido = (d) => P.sentido === "antihorario" ? d < 0 : P.sentido === "ambos" ? true : d > 0;

function avanzar(d) {
  // d: cuánto gira ahora (+ = sentido del reloj).
  if (!d) return;
  if (!permitido(d) && P.sentido !== "ambos") return;
  angulo += d;
  cuenta = P.sentido === "ambos" ? cuenta + d : cuenta + Math.abs(d);
  if (P.sentido === "ambos") cuenta = Math.max(0, cuenta);
  // Un clic de trinquete cada doceavo de vuelta.
  if (Math.floor(Math.abs(angulo) / (DOS_PI / 12)) !== ultimoClic) {
    ultimoClic = Math.floor(Math.abs(angulo) / (DOS_PI / 12));
    sinte.nota(1600, 0.01, { tipo: "pulso", vol: 0.12 });
  }
  // (un pelo de tolerancia: la suma de muchos pasos de una vuelta justa da 0.99999…)
  const v = cuenta / DOS_PI + 1e-9;
  if (Math.floor(v) > vueltas) {
    vueltas = Math.floor(v);
    sinte.nota(Obj.frecuencia(vueltas % 8 * 2), 0.2, { tipo: "campana", vol: 0.15 });
    Obj.emitir("vuelta", { n: vueltas });
  } else if (Math.floor(v) < vueltas) vueltas = Math.floor(v);
  const m = vueltasMeta();
  const fraccion = m ? Math.min(1, v / m) : v - Math.floor(v);
  Obj.emitir("valor", { valor: +v.toFixed(4), fraccion: +fraccion.toFixed(4) });
  if (m && !completo && v >= m) {
    completo = true;
    for (let i = 0; i < 4; i++) sinte.nota(Obj.frecuencia(i * 4 + 3), 0.4, { tipo: "campana", vol: 0.18, retraso: i * 0.1 });
    Obj.emitir("completo", {});
  }
  pintar();
}
...
function pintar() {
  const m = vueltasMeta();
  const v = cuenta / DOS_PI;
  const encendidas = m ? Math.round(Math.min(1, v / m) * LUCES) : Math.round((v - Math.floor(v)) * LUCES);
  const color = Obj.color(P.color, "#F77F00");
  const firma = encendidas + color + completo;
  if (firma !== ultimoPintado) {
    ultimoPintado = firma;
    luces.forEach((l, i) => l.setAttribute("color", i < encendidas ? (completo ? "#30D158" : color) : "#3A3F4B"));
    aro.setAttribute("color", color);
  }
  Obj.$("lectura").setAttribute("value", m ? Math.min(m, v).toFixed(1) + " / " + m : v.toFixed(1) + " vueltas");
  ruedaG.rotation = { x: 0, y: 0, z: -angulo };
}

// Escritorio: un tramo animado; retroceso: sin tocarla, vuelve de a poco.
const anim = Obj.animador((t, dt) => {
  let seguir = false;
  if (meta !== null) {
    const falta = meta - angulo;
    const paso = Math.sign(falta) * Math.min(Math.abs(falta), dt * 7);
    avanzar(paso);
    if (Math.abs(meta - angulo) < 1e-4) meta = null; else seguir = true;
  } else if (P.retroceso && !tomada && cuenta > 0 && !completo) {
    const d = Math.min(cuenta, dt * 0.8);
    cuenta -= d;
    angulo -= (P.sentido === "antihorario" ? -1 : 1) * d;
    pintar();
    Obj.emitir("valor", { valor: +(cuenta / DOS_PI).toFixed(4), fraccion: vueltasMeta() ? +Math.min(1, cuenta / DOS_PI / vueltasMeta()).toFixed(4) : 0 });
    seguir = true;
  }
  return seguir;
});
Obj.boton(Obj.$("toque"), (e) => {
  ...
  const base = meta !== null ? meta : angulo;
  const dir = P.sentido === "antihorario" ? -1 : 1;
  let d;
  if (Math.hypot(dx, dy) < R * 0.35) d = dir * Math.PI / 2;
  else {
    // Del ángulo de la manija al del punto, dando la vuelta por el lado permitido.
    const actual = ((base % DOS_PI) + DOS_PI) % DOS_PI;
    const destino = ((Math.atan2(dx, dy) % DOS_PI) + DOS_PI) % DOS_PI;
    d = destino - actual;
    if (P.sentido === "ambos") d = Obj.difAngulo(destino, actual);
    else if (dir > 0 && d <= 0) d += DOS_PI;
    else if (dir < 0 && d >= 0) d -= DOS_PI;
  }
  meta = base + d;
  anim.iniciar();
...
// VR: la manivela de las manos (_vr.js): se toma la manija (o el aro) y la
// vuelta de la mano alrededor del centro la lleva.
Obj.manos().manivela({
  centro: { x: C.x, y: C.y, z: 0 }, eje: { x: 0, y: 0, z: 1 },
  radio: R_MANIJA, salida: 0.07, aro: true,
  angulo: () => angulo,
  alTomar() { tomada = true; meta = null; },
  alGirar(d) { avanzar(d); },
  alSoltar() { tomada = false; anim.iniciar(); },
});

function reiniciar() {
  cuenta = 0; vueltas = 0; completo = false; meta = null;
  Obj.emitir("valor", { valor: 0, fraccion: 0 });
  pintar();
}
Obj.mensaje("reiniciar", reiniciar);
Obj.alCambiar(() => { Obj.$("etiqueta").setAttribute("value", String(P.etiqueta || "").slice(0, 18)); ultimoPintado = ""; pintar(); anim.iniciar(); });
Obj.$("etiqueta").setAttribute("value", String(P.etiqueta || "").slice(0, 18));
pintar();
anim.iniciar();
