...
;
// La ruleta: gajos de colores, un giro que frena y un "tic" por cada gajo.
//
// Los gajos son abanicos de tiras radiales —no hay triángulos sueltos en el
// HSML— que se ensanchan hacia el borde. El giro tiene una velocidad inicial
// al azar y un rozamiento fijo; el resultado sale de dónde frenó, no se
// decide antes: así lo que se ve y lo que se avisa no pueden no coincidir.
const P = Obj.props({
  opciones: ["Sí", "No", "Tal vez", "Otra vez", "Mañana", "Ahora", "Nunca", "Siempre"],
  colores: ["#E63946", "#F4A261", "#E9C46A", "#2A9D8F", "#1982C4", "#6A4C93", "#8AC926", "#FF595E"],
  sonido: true,
});

const rueda = Obj.$("rueda");
const salio = Obj.$("salio");
const sinte = Obj.sinte({ volumen: 0.6 });
const R = 0.4;

function opciones() {
  const o = Array.isArray(P.opciones) ? P.opciones.map((v) => String(v).slice(0, 14)).filter(Boolean) : [];
  return o.length >= 2 ? o.slice(0, 12) : ["Sí", "No"];
}
function colores() {
  const c = Array.isArray(P.colores) ? P.colores.map((v) => Obj.color(v, null)).filter(Boolean) : [];
  return c.length ? c : ["#E63946", "#F4A261", "#E9C46A", "#2A9D8F", "#1982C4", "#6A4C93"];
}

let partes = [];
function armar() {
  for (const p of partes) p.remove();
  partes = [];
  const ops = opciones(), cols = colores();
  const n = ops.length, ancho = (Math.PI * 2) / n;
  for (let i = 0; i < n; i++) {
    // El gajo i está centrado en el ángulo -i·ancho (en sentido horario desde
    // arriba, visto desde +Z).
    const g = Obj.crear("group", { rz: -i * ancho }, rueda);
    partes.push(g);
    const TIRAS = 6;
    for (let k = 0; k < TIRAS; k++) {
      const r0 = (k / TIRAS) * R, r1 = ((k + 1) / TIRAS) * R;
      const w = 2 * Math.tan(ancho / 2) * r1 + 0.004;
      Obj.crear("box", { y: (r0 + r1) / 2, sx: Math.max(0.004, w), sy: r1 - r0 + 0.002, sz: 0.012, color: cols[i % cols.length] }, g);
    }
    // El texto, a lo largo del radio, mirando hacia afuera.
    Obj.crear("text", { y: R * 0.62, z: 0.008, rz: n > 6 ? Math.PI / 2 : 0, value: ops[i], size: n > 8 ? 0.028 : 0.036, color: "#FFFFFF" }, g);
    // El clavo del borde entre gajo y gajo, que hace "tic" contra la flecha.
    const b = Obj.crear("group", { rz: -i * ancho - ancho / 2 }, rueda);
    partes.push(b);
    Obj.crear("cylinder", { y: R - 0.012, z: 0.012, rx: Math.PI / 2, sx: 0.014, sy: 0.02, sz: 0.014, color: "#F4F1EA" }, b);
  }
}
...
let ultimoGajo = null;
function gajoArriba() {
  const n = opciones().length, ancho = (Math.PI * 2) / n;
  // Rotar la rueda +angulo trae al frente (arriba) el gajo que estaba en -angulo.
  const a = ((angulo % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
  return Math.round(a / ancho) % n;
}

const anim = Obj.animador((t, dt) => {
  // Rozamiento: frena más a alta velocidad y un mínimo constante para parar.
  vel = Math.max(0, vel - (0.35 * vel + 0.6) * dt);
  angulo += vel * dt;
  rueda.rotation = { x: 0, y: 0, z: angulo };
  const g = gajoArriba();
  if (g !== ultimoGajo) {
    ultimoGajo = g;
    if (P.sonido !== false && vel > 0.05) sinte.nota(1400, 0.03, { tipo: "pulso", vol: Math.min(0.3, 0.08 + vel * 0.03) });
  }
  if (vel <= 0) {
    const i = gajoArriba();
    const op = opciones()[i];
    salio.setAttribute("value", op);
    Obj.emitir("resultado", { opcion: op, indice: i });
    if (P.sonido !== false) sinte.nota(880, 0.6, { tipo: "campana", vol: 0.3 });
    return false;
  }
  return true;
});
...
function girar() {
  if (vel > 0) return;
  vel = 7 + Math.random() * 8;   // rad/s
  salio.setAttribute("value", "");
  anim.iniciar();
}

Obj.boton(Obj.$("toque"), girar);
Obj.mensaje("girar", girar);
Obj.alCambiar((p, a, cambios) => { if (cambios.includes("opciones") || cambios.includes("colores")) armar(); });
armar();
