...
;
// La sartén: un panqueque de dos caras que se cocinan por separado.
//
// Cada cara tiene su punto (0 crudo, 1 dorado, 2 quemado); se cocina la que
// toca la sartén, si el fuego está prendido y la sartén sobre el anafe. Dar
// vuelta es un tiro: el panqueque sale con una velocidad hacia arriba y gira
// sobre X media vuelta en lo que dura el vuelo (con algo de error). Si al
// bajar la sartén está debajo, lo atrapa: si giró cerca de media vuelta,
// cambió de cara; si no, cae como estaba. Si no está debajo, se cae afuera.
const sinte = Obj.sinte({ volumen: 0.7 });
const donde = Obj.marco();
const sartenG = Obj.$("sarten"), panq = Obj.$("panqueque"), fuegoG = Obj.$("fuego");
const caraA = Obj.$("cara_a"), caraB = Obj.$("cara_b");
const ANAFE = { x: 0, y: 0.06, z: 0 };
const MANGO = { x: 0.21, y: 0.03, z: 0 };
const ALTO_PANQ = 0.0105;
const G = 9.8;

let sarten = { ...ANAFE };
let fuego = false;
let punto = [0, 0];            // [cara_a, cara_b]
let arribaEs = 0;              // qué índice de `punto` está arriba ahora (0 o 1)
let panqEstado = "sarten";     // sarten | vuelo | afuera
let p = null, v = null, giro = 0, omega = 0;   // vuelo
let listoAvisado = false;

// El borde de la sartén: una corona de tablitas (un cilindro sería macizo y
// taparía el panqueque).
for (let i = 0; i < 28; i++) {
  const a = (i / 28) * Math.PI * 2;
  Obj.crear("box", { x: Math.cos(a) * 0.121, y: 0.02, z: Math.sin(a) * 0.121, ry: -a, sx: 0.006, sy: 0.03, sz: 0.029, color: "#2B2B2B" }, Obj.$("borde"));
}
...
for (let i = 0; i < 10; i++) {
  ...
  llamas.push({ el: Obj.crear("cylinder", { x: Math.cos(a) * 0.075, z: Math.sin(a) * 0.075, sx: 0.012, sy: 0.02, sz: 0.012, color: "#4895EF", "material-unlit": "true", visible: "false" }, fuegoG), a });
}
// Humo cuando se quema.
const humo = [];
for (let i = 0; i < 6; i++) humo.push(Obj.crear("sphere", { sx: 0.03, sy: 0.03, sz: 0.03, color: "#77777760", "material-alpha": "blend", visible: "false" }, Obj.$("humo")));

function colorDe(d) {
  if (d <= 1) return Obj.mezcla("#F3D9A4", "#D08C3A", d);
  return Obj.mezcla("#D08C3A", "#2E1B0E", Math.min(1, d - 1));
}
function pintar() {
  // cara_a es la de arriba del grupo; si el panqueque está dado vuelta, el grupo gira π.
  caraA.setAttribute("color", colorDe(punto[0]));
  caraB.setAttribute("color", colorDe(punto[1]));
}

/** Dónde apoya el panqueque en la sartén. */
const enSarten = () => ({ x: sarten.x, y: sarten.y + ALTO_PANQ, z: sarten.z });
function ponerPanq() {
  if (panqEstado === "sarten") {
    panq.position = enSarten();
    panq.rotation = { x: arribaEs === 0 ? 0 : Math.PI, y: 0, z: 0 };
  } else {
    panq.position = p;
    panq.rotation = { x: giro, y: 0, z: 0 };
  }
}

// ── El tiempo ──────────────────────────────────────────────────────────────
let sonandoFrito = false;
function frito() {
  let lp = 0, semilla = 7;
  return () => {
    ...
    lp += (n - lp) * 0.5;
    const chispa = Math.random() < 0.002 ? 1 : 0;
    return (n - lp) * 0.08 + chispa * 0.2 * n;
  };
}
const anim = Obj.animador((t, dt) => {
  let seguir = fuego || panqEstado === "vuelo";
  // Llamas que titilan.
  if (fuego) llamas.forEach((l, i) => { l.el.scale = { x: 0.012, y: 0.018 + 0.012 * Math.abs(Math.sin(t * 13 + i * 1.7)), z: 0.012 }; });
  // Cocinar la cara de abajo.
  const sobreAnafe = Math.hypot(sarten.x - ANAFE.x, sarten.z - ANAFE.z) < 0.06 && sarten.y < ANAFE.y + 0.06;
  const cocina = fuego && panqEstado === "sarten" && sobreAnafe;
  if (cocina) {
    const abajo = 1 - arribaEs;
    punto[abajo] = Math.min(2, punto[abajo] + dt / 25);
    pintar();
    if (!listoAvisado && punto[0] >= 0.9 && punto[1] >= 0.9) { listoAvisado = true; Obj.emitir("listo", { punto: +Math.max(punto[0], punto[1]).toFixed(2) }); }
  }
  if (cocina !== sonandoFrito) { sonandoFrito = cocina; sinte.flujo(cocina ? frito() : null); }
  // Humo si algo se quema.
  const quema = cocina && punto[1 - arribaEs] > 1.4;
  humo.forEach((h, i) => {
    if (!quema) { h.setAttribute("visible", "false"); return; }
    const k = ((t * 0.5 + i / humo.length) % 1);
    h.setAttribute("visible", "inherit");
    h.position = { x: sarten.x + Math.sin(i * 2 + t) * 0.03, y: sarten.y + 0.03 + k * 0.3, z: sarten.z + Math.cos(i * 3) * 0.03 };
    const s = 0.02 + k * 0.06;
    h.scale = { x: s, y: s, z: s };
  });
  // El vuelo.
  if (panqEstado === "vuelo") {
    ...
    const base = enSarten();
    if (v.y < 0 && p.y <= base.y) {
      const dx = Math.hypot(p.x - base.x, p.z - base.z);
      if (dx < 0.09) aterrizar(true);
      else if (p.y <= 0.045) aterrizar(false);
    }
  }
  ponerPanq();
  return seguir;
...
function tirar(vy, vx, vz, error) {
  if (panqEstado !== "sarten") return;
  panqEstado = "vuelo";
  p = enSarten();
  v = { x: vx || 0, y: vy, z: vz || 0 };
  giro = arribaEs === 0 ? 0 : Math.PI;
  // Media vuelta en el tiempo de vuelo, con el error que corresponda.
  const tv = (2 * vy) / G;
  omega = (Math.PI * (1 + (error || 0))) / tv;
  sinte.nota(500, 0.15, { tipo: "ruido", vol: 0.12, caida: 12 });
  anim.iniciar();
}
function aterrizar(enLaSarten) {
  // La cara que quedó arriba: la más cercana según el giro acumulado.
  const vueltas = Math.round(giro / Math.PI);
  const torcido = Math.abs(giro - vueltas * Math.PI) > 0.45;
  arribaEs = ((vueltas % 2) + 2) % 2;
  if (enLaSarten) {
    panqEstado = "sarten";
    sinte.nota(130, 0.12, { tipo: "madera", vol: 0.45 });
    Obj.emitir("vuelta", { atrapado: true, torcido });
  } else {
    panqEstado = "afuera";
    p = { x: p.x, y: 0.045, z: p.z };
    giro = arribaEs * Math.PI;
    sinte.nota(90, 0.15, { tipo: "madera", vol: 0.5 });
    Obj.emitir("vuelta", { atrapado: false });
    // Vuelve solo al rato (el cocinero lo levanta).
    setTimeout(() => { if (panqEstado === "afuera") { panqEstado = "sarten"; ponerPanq(); } }, 2500);
  }
}

// ── Tocar ──────────────────────────────────────────────────────────────────
function prender(si) {
  fuego = si == null ? !fuego : !!si;
  for (const l of llamas) l.el.setAttribute("visible", fuego ? "inherit" : "false");
  Obj.$("perilla").rotation = { x: Math.PI / 2, y: fuego ? -1.2 : 0, z: 0 };
  sinte.nota(fuego ? 800 : 400, 0.05, { tipo: "pulso", vol: 0.3 });
  if (fuego) sinte.nota(200, 0.4, { tipo: "ruido", vol: 0.1, caida: 4, retraso: 0.05 });
  Obj.emitir("fuego", { prendido: fuego });
  anim.iniciar();
}
Obj.boton(Obj.$("perilla"), () => prender());
Obj.boton(Obj.$("toque"), () => {
  if (panqEstado !== "sarten" || agarre.tomado) return;
  // Casi siempre sale bien; a veces gira de más o de menos, y de vez en cuando se escapa.
  const r = Math.random();
  const error = r < 0.75 ? (Math.random() - 0.5) * 0.2 : (Math.random() < 0.5 ? -1 : 1) * (0.35 + Math.random() * 0.3);
  const vx = r > 0.92 ? (Math.random() < 0.5 ? -1 : 1) * 0.35 : 0;
  tirar(1.5, vx, 0, error);
});
function nuevo() {
  punto = [0, 0]; arribaEs = 0; listoAvisado = false;
  panqEstado = "sarten";
  pintar(); ponerPanq();
}
Obj.mensaje("nuevo", nuevo);
Obj.mensaje("fuego", (d) => prender(d ? d.prendido !== false : true));

// ── En VR: la sartén en la mano ────────────────────────────────────────────
let manoAntes = null;
const agarre = Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(q) {
    const m = { x: sarten.x + MANGO.x, y: sarten.y + MANGO.y, z: sarten.z + MANGO.z };
    return Math.hypot(q.x - m.x, q.y - m.y, q.z - m.z) < 0.08;
  },
  tomar(q) { manoAntes = { q, t: performance.now() }; anim.iniciar(); },
  mover(q) {
    const ahora = performance.now();
    const dt = Math.max(0.001, (ahora - manoAntes.t) / 1000);
    const vel = { x: (q.x - manoAntes.q.x) / dt, y: (q.y - manoAntes.q.y) / dt, z: (q.z - manoAntes.q.z) / dt };
    manoAntes = { q, t: ahora };
    sarten = { x: q.x - MANGO.x, y: Math.max(ANAFE.y, q.y - MANGO.y), z: q.z - MANGO.z };
    sartenG.position = sarten;
    // Un golpe de muñeca hacia arriba lo tira; el error depende de lo prolijo.
    if (panqEstado === "sarten" && vel.y > 1.2) {
      const lateral = Math.hypot(vel.x, vel.z);
      tirar(Math.min(2.5, vel.y * 0.9), vel.x * 0.3, vel.z * 0.3, Math.min(0.8, lateral / vel.y * 0.4));
    }
    if (panqEstado === "sarten") ponerPanq();
  },
  soltar() {
    sarten = { ...ANAFE };
    sartenG.position = sarten;
