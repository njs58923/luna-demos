...
;
// El sello: ir a la almohadilla, ir al punto, bajar y dejar la marca.
//
// Una marca es un grupo chato sobre la hoja: un rectángulo de tinta con otro
// más chico color papel encima (el borde) y el texto, girado un poco al azar.
// Hay un máximo; pasado ese, la más vieja se reusa. La tinta se gasta: cada
// entintada alcanza para tres marcas, y la última sale más pálida.
const P = Obj.props({ texto: "APROBADO", color: "#C1121F", clave: "" });

const sinte = Obj.sinte({ volumen: 0.7 });
const donde = Obj.marco();
const guardado = Obj.memoria("marcas");
const sello = Obj.$("sello"), marcasG = Obj.$("marcas");
const HOJA = { x: 0.06, z: 0, lado: 0.24 };
const PAD = { x: -0.13, y: 0.0135, z: -0.04 };
const REPOSO = { x: -0.13, y: 0, z: 0.018 };
const MAX = 16;

let marcas = [];            // { x, z, giro, fuerza }
let tinta = 0;
const nodos = [];
for (let i = 0; i < MAX; i++) {
  const g = Obj.crear("group", { visible: "false" }, marcasG);
  const borde = Obj.crear("box", { sx: 0.05, sy: 0.0004, sz: 0.03, color: "#C1121F" }, g);
  Obj.crear("box", { y: 0.0003, sx: 0.044, sy: 0.0004, sz: 0.024, color: "#FBF8EF" }, g);
  const texto = Obj.crear("text", { y: 0.0008, rx: -Math.PI / 2, value: "APROBADO", size: 0.0075, color: "#C1121F" }, g);
  nodos.push({ g, borde, texto });
}
function colorTinta(fuerza) { return Obj.mezcla(Obj.color(P.color, "#C1121F"), "#FBF8EF", fuerza < 1 ? 0.45 : 0.05); }
function dibujarMarcas() {
  nodos.forEach((n, i) => {
    const m = marcas[i];
    n.g.setAttribute("visible", m ? "inherit" : "false");
    if (!m) return;
    n.g.position = { x: m.x, y: 0, z: m.z };
    n.g.rotation = { x: 0, y: m.giro, z: 0 };
    const c = colorTinta(m.fuerza);
    n.borde.setAttribute("color", c);
    n.texto.setAttribute("color", c);
    n.texto.setAttribute("value", String(P.texto || "APROBADO").slice(0, 12));
  });
}
function sellar(x, z, giro) {
  if (tinta <= 0) { sinte.nota(200, 0.06, { tipo: "madera", vol: 0.3 }); return false; }
  const g = Number.isFinite(giro) ? giro : (Math.random() - 0.5) * 0.4;
  const m = { x: +x.toFixed(4), z: +z.toFixed(4), giro: +g.toFixed(3), fuerza: tinta > 1 ? 1 : 0.5 };
  tinta--;
  marcas.push(m);
  if (marcas.length > MAX) marcas.shift();
  dibujarMarcas();
  guardado.guardar(marcas);
  sinte.nota(120, 0.1, { tipo: "madera", vol: 0.6 });
  Obj.emitir("sellado", { x: m.x, z: m.z });
  return true;
}
function entintar() {
  tinta = 3;
  sinte.nota(300, 0.06, { tipo: "ruido", vol: 0.15, caida: 30 });
}
function limpiar() {
  marcas = [];
  guardado.guardar(marcas);
  dibujarMarcas();
  sinte.nota(2000, 0.3, { tipo: "ruido", vol: 0.08, caida: 8 });
}
const enHoja = (x, z) => Math.abs(x - HOJA.x) < HOJA.lado / 2 - 0.02 && Math.abs(z - HOJA.z) < HOJA.lado / 2 - 0.01;

// ── Tocando: un viaje animado ──────────────────────────────────────────────
let viaje = null;
let selloPos = { ...REPOSO };
function ponerSello(p, giro) {
  selloPos = { x: p.x, y: p.y, z: p.z };
  sello.position = p;
  sello.rotation = { x: 0, y: giro || 0, z: 0 };
}
const anim = Obj.animador(() => {
  if (!viaje) return false;
  const t = (performance.now() - viaje.t0) / 1000;
  // Lo que pasa al final de cada tramo (entintar, sellar), aunque el cuadro lo saltee.
  for (const x of viaje.tramos) if (t >= x.hasta && x.alLlegar && !x.hecho) { x.hecho = true; x.alLlegar(); }
  const tramo = viaje.tramos.find((x) => t < x.hasta) || viaje.tramos[viaje.tramos.length - 1];
  const k = Math.min(1, Math.max(0, (t - tramo.desde) / (tramo.hasta - tramo.desde)));
  const s = Obj.suave(k);
  ponerSello({ x: tramo.a.x + (tramo.b.x - tramo.a.x) * s, y: tramo.a.y + (tramo.b.y - tramo.a.y) * s, z: tramo.a.z + (tramo.b.z - tramo.a.z) * s }, tramo.giro || 0);
  if (t >= viaje.tramos[viaje.tramos.length - 1].hasta) { viaje = null; ponerSello(REPOSO); return false; }
  return true;
});
function viajar(x, z) {
  if (viaje || agarre.tomado) return;
  const arriba = (p) => ({ x: p.x, y: 0.08, z: p.z });
  const pad = { ...PAD, y: PAD.y };
  const punto = { x, y: 0.0025, z };
  const giro = (Math.random() - 0.5) * 0.4;
  const pasos = [
    [REPOSO, arriba(pad)], [arriba(pad), pad, entintar], [pad, arriba(pad)],
    [arriba(pad), arriba(punto)], [arriba(punto), punto, () => sellar(x, z, giro)], [punto, arriba(punto)], [arriba(punto), REPOSO],
  ];
  const DUR = [0.2, 0.12, 0.12, 0.3, 0.12, 0.12, 0.3];
  let t = 0;
  viaje = { t0: performance.now(), tramos: pasos.map(([a, b, f], i) => { const x0 = t; t += DUR[i]; return { a, b, desde: x0, hasta: t, alLlegar: f, giro: i >= 3 && i <= 5 ? giro : 0 }; }) };
  anim.iniciar();
}
Obj.boton(Obj.$("hoja"), (e) => {
  if (!Number.isFinite(e.x)) return viajar(HOJA.x, HOJA.z);
  const l = donde.aLocal({ x: e.x, y: e.y, z: e.z });
  const x = Math.max(HOJA.x - 0.09, Math.min(HOJA.x + 0.09, l.x)), z = Math.max(-0.1, Math.min(0.1, l.z));
  viajar(x, z);
});
Obj.boton(Obj.$("goma"), limpiar);
Obj.mensaje("limpiar", limpiar);

// ── En VR: tomarlo y apretar ───────────────────────────────────────────────
let apoyado = false;
const agarre = Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) {
    const s = selloPos;
    return Math.hypot(p.x - s.x, p.y - (s.y + 0.05), p.z - s.z) < 0.08;
  },
  tomar() { viaje = null; },
  mover(p) {
    // La mano toma el mango: la goma queda 5 cm abajo. No atraviesa la mesa.
    const base = { x: p.x, y: Math.max(0.0025, p.y - 0.05), z: p.z };
    const toca = base.y <= 0.004 || (Math.abs(base.x - PAD.x) < 0.045 && Math.abs(base.z - PAD.z) < 0.03 && base.y <= PAD.y + 0.002);
    if (toca && !apoyado) {
      apoyado = true;
      if (Math.abs(base.x - PAD.x) < 0.045 && Math.abs(base.z - PAD.z) < 0.03) entintar();
      else if (enHoja(base.x, base.z)) sellar(base.x, base.z);
    } else if (!toca && base.y > 0.012) apoyado = false;
    if (Math.abs(base.x - PAD.x) < 0.045 && Math.abs(base.z - PAD.z) < 0.03) base.y = Math.max(base.y, PAD.y);
    ponerSello(base);
  },
  soltar() { apoyado = false; setTimeout(() => { if (!agarre.tomado) ponerSello(REPOSO); }, 600); },
});

Obj.alCambiar(dibujarMarcas);
const previas = guardado.leer([]);
if (Array.isArray(previas)) marcas = previas.filter((m) => m && Number.isFinite(m.x) && Number.isFinite(m.z)).slice(-MAX);
dibujarMarcas();
ponerSello(REPOSO);
