...
;
// El semáforo: tres luces, un ciclo con sus tiempos, y un modo manual.
//
// Cada luz es una esfera sin iluminación con su visera; apagada es un tono
// oscuro del mismo color. El ciclo corre con un temporizador por luz, no por
// cuadro: un semáforo cambia cada varios segundos.
const P = Obj.props({ verde: 8, amarillo: 2, rojo: 8, manual: false });

const LUCES = [
  { nombre: "rojo", on: "#FF3B30", y: 0.24 },
  { nombre: "amarillo", on: "#FFCC00", y: 0 },
  { nombre: "verde", on: "#30D158", y: -0.24 },
];
const grupo = Obj.$("luces");
const modo = Obj.$("modo");
for (const l of LUCES) {
  l.el = Obj.crear("sphere", { y: l.y, sx: 0.17, sy: 0.17, sz: 0.06, color: Obj.mezcla(l.on, "#101010", 0.8), "material-unlit": "true" }, grupo);
  Obj.crear("box", { y: l.y + 0.1, z: 0.05, rx: 0.4, sx: 0.2, sy: 0.012, sz: 0.1, color: "#141414" }, grupo);
}
...
function pintar() {
  for (const l of LUCES) l.el.setAttribute("color", l.nombre === actual ? l.on : Obj.mezcla(l.on, "#101010", 0.8));
  modo.setAttribute("value", manual ? "manual" : "");
}
function poner(nombre, avisar) {
  if (!ORDEN.includes(nombre)) return;
  actual = nombre;
  pintar();
  if (avisar !== false) Obj.emitir("luz", { color: actual });
}
function segundos(nombre) { return Math.max(0.5, Math.min(600, Number(P[nombre]) || { verde: 8, amarillo: 2, rojo: 8 }[nombre])); }
function programar() {
  clearTimeout(reloj);
  if (manual) return;
  reloj = setTimeout(() => {
    poner(ORDEN[(ORDEN.indexOf(actual) + 1) % 3]);
    programar();
  }, segundos(actual) * 1000);
}
...
Obj.mensaje("luz", (d) => { if (d && ORDEN.includes(d.color)) { manual = true; clearTimeout(reloj); poner(d.color); } });
Obj.mensaje("auto", () => { manual = false; pintar(); programar(); });
Obj.alCambiar((p, a, cambios) => { if (cambios.includes("manual")) manual = !!p.manual; pintar(); programar(); });
pintar();
programar();
