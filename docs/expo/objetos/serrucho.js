...
;
// El serrucho: una ranura que baja con cada pasada, hasta que la punta cae.
//
// El serrucho se arma en su marco: +Y es la hoja (del mango a la punta) y +X
// el filo. Corta cuando el medio del filo está sobre la línea, apoyado en el
// fondo de la ranura, y la hoja va y viene a lo largo de sí misma: cada
// centímetro de ida o de vuelta baja la ranura un poco.
const P = Obj.props({ madera: "#D2A86E" });

const sinte = Obj.sinte({ volumen: 0.8 });
const donde = Obj.marco();
const serrucho = Obj.$("serrucho"), punta = Obj.$("punta"), ranura = Obj.$("ranura");
const CORTE = 0.3, ARRIBA = 0.65, GRUESO = 0.1, ZC = 0;
const FILO = 0.045;
const PUNTA0 = { x: 0.4, y: 0.6, z: 0 };

let hondo = 0;          // de 0 a GRUESO
let cortado = false;

// ── Aserrín ────────────────────────────────────────────────────────────────
const aserrin = [];
for (let i = 0; i < 24; i++) {
  aserrin.push({ el: Obj.crear("box", { sx: 0.004, sy: 0.004, sz: 0.004, color: "#E8C99A", visible: "false" }, Obj.$("aserrin")), p: null, v: null, vive: 0 });
}
let sigAserrin = 0;
function soltarAserrin(n) {
  for (let k = 0; k < n; k++) {
    const a = aserrin[sigAserrin++ % aserrin.length];
    const lado = Math.random() < 0.5 ? -1 : 1;
    a.p = { x: CORTE + (Math.random() - 0.5) * 0.01, y: ARRIBA - hondo, z: ZC + lado * 0.052 };
    a.v = { x: (Math.random() - 0.5) * 0.1, y: Math.random() * 0.2, z: lado * (0.05 + Math.random() * 0.1) };
    a.vive = performance.now() + 2500;
...
const anim = Obj.animador((t, dt) => {
  let seguir = false;
  const ahora = performance.now();
  for (const a of aserrin) {
    if (!a.p) continue;
    if (ahora > a.vive) { a.p = null; a.el.setAttribute("visible", "false"); continue; }
    seguir = true;
    if (a.p.y > 0.002) {
      a.v.y -= 4 * dt;
      a.p.x += a.v.x * dt; a.p.y = Math.max(0.002, a.p.y + a.v.y * dt); a.p.z += a.v.z * dt;
      a.el.position = a.p;
    }
  }
  if (cae) {
    seguir = true;
    const k = Math.min(1, (ahora - cae) / 450);
    // Cae con gravedad (y = y0 - ½ g t²) y se inclina hasta quedar plana.
    const tt = k * 0.45;
    const y = Math.max(0.05, PUNTA0.y - 0.5 * 9.8 * tt * tt * 0.6);
    punta.position = { x: PUNTA0.x + 0.12 * k, y, z: PUNTA0.z + 0.03 * k };
    punta.rotation = { x: 0, y: 0.3 * k, z: -1.2 * k * (1 - k) * 2 };
    if (k >= 1) { cae = null; sinte.nota(90, 0.25, { tipo: "madera", vol: 0.8 }); }
  }
  return seguir;
});

// ── Cortar ─────────────────────────────────────────────────────────────────
function avanzar(cuanto) {
  if (cortado) return;
  hondo = Math.min(GRUESO, hondo + cuanto);
  ranura.setAttribute("visible", "inherit");
  ranura.position = { x: CORTE, y: ARRIBA - hondo / 2, z: ZC };
  ranura.scale = { x: 0.004, y: Math.max(0.001, hondo), z: 0.102 };
  Obj.emitir("corte", { profundidad: +(hondo / GRUESO).toFixed(3) });
  if (hondo >= GRUESO - 1e-6) {
    cortado = true;
    cae = performance.now();
    Obj.emitir("cortado", {});
    anim.iniciar();
    setTimeout(reponer, 6000);
  }
}
function reponer() {
  hondo = 0; cortado = false; cae = null;
  ranura.setAttribute("visible", "false");
  punta.position = PUNTA0;
  punta.rotation = { x: 0, y: 0, z: 0 };
  if (!agarre.tomado && !animando) REPOSO();
}
...
/** El mango en `mano`, la hoja hacia f y el filo hacia c. Devuelve el medio del filo. */
function pose(mano, f, c) {
  f = norm(f);
  const d = c.x * f.x + c.y * f.y + c.z * f.z;
  const x = norm({ x: c.x - d * f.x, y: c.y - d * f.y, z: c.z - d * f.z });
  serrucho.position = mano;
  serrucho.rotation = Obj.deEjes(x, f, cruz(x, f));
  return { p: { x: mano.x + f.x * 0.28 + x.x * FILO, y: mano.y + f.y * 0.28 + x.y * FILO, z: mano.z + f.z * 0.28 + x.z * FILO }, f };
}
// En reposo: acostado sobre el tirante, con el filo hacia adelante.
const REPOSO = () => pose({ x: -0.45, y: ARRIBA + 0.014, z: -0.01 }, { x: 1, y: 0, z: 0 }, { x: 0, y: 0, z: 1 });

// Dos pasadas animadas (sin VR).
let animando = false;
function pasadas() {
  if (animando || cortado || agarre.tomado) return;
  animando = true;
  const t0 = performance.now(), dura = 900;
  const inicio = hondo, meta = Math.min(GRUESO, hondo + GRUESO / 5 + 1e-6);
  const tick = setInterval(() => {
    const k = Math.min(1, (performance.now() - t0) / dura);
    const h = inicio + (meta - inicio) * k;
    // El medio del filo sobre el fondo de la ranura, yendo y viniendo en z.
    const z = ZC + 0.1 * Math.sin(k * Math.PI * 4);
    pose({ x: CORTE, y: ARRIBA - h + FILO, z: z + 0.28 }, { x: 0, y: 0, z: -1 }, { x: 0, y: -1, z: 0 });
    if (Math.random() < 0.5) sinte.nota(3000, 0.05, { tipo: "ruido", vol: 0.15, caida: 30 });
    if (k >= 1) {
      clearInterval(tick);
      animando = false;
      soltarAserrin(8);
      avanzar(meta - hondo);
      setTimeout(() => { if (!animando && !agarre.tomado) REPOSO(); }, 400);
    }
  }, 33);
}
Obj.boton(Obj.$("toque"), pasadas);
Obj.boton(Obj.$("agarrar"), pasadas);

// ── En VR ──────────────────────────────────────────────────────────────────
let antes = null, sonido = 0;
const agarre = Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) { return Math.hypot(p.x - -0.4, p.y - (ARRIBA + 0.014), p.z - 0) < 0.12; },
  tomar(p, mano, e) { antes = null; seguir(p, e); },
  mover(p, mano, e) { seguir(p, e); },
  soltar() { antes = null; setTimeout(() => { if (!agarre.tomado) REPOSO(); }, 800); },
});
function seguir(p, e) {
  const f = donde.dirLocal({ x: e.dx, y: e.dy, z: e.dz });
  const q = { x: e.qx || 0, y: e.qy || 0, z: e.qz || 0, w: e.qw == null ? 1 : e.qw };
  const arriba = Obj.rotarV(q, { x: 0, y: 1, z: 0 });
  const abajo = donde.dirLocal({ x: -arriba.x, y: -arriba.y, z: -arriba.z });
  const c = Math.abs(abajo.x * f.x + abajo.y * f.y + abajo.z * f.z) > 0.95 ? { x: 0, y: -1, z: 0 } : abajo;
  const { p: filo, f: fn } = pose(p, f, c);
  if (antes && !cortado) {
    const d = { x: p.x - antes.x, y: p.y - antes.y, z: p.z - antes.z };
    const recorrido = Math.abs(d.x * fn.x + d.y * fn.y + d.z * fn.z);
    const fondo = ARRIBA - hondo;
    const sobre = Math.abs(filo.x - CORTE) < 0.04 && Math.abs(filo.z - ZC) < 0.12;
    const apoya = filo.y < fondo + 0.03 && filo.y > fondo - 0.05;
    if (sobre && apoya && recorrido > 0.002) {
      avanzar(recorrido * 0.05);
      const ahora = performance.now();
      if (ahora - sonido > 60) { sonido = ahora; sinte.nota(2500 + recorrido * 20000, 0.07, { tipo: "ruido", vol: 0.18, caida: 25 }); soltarAserrin(1); }
    }
  }
  antes = p;
}

Obj.mensaje("reponer", reponer);
function pintar() {
  const c = Obj.color(P.madera, "#D2A86E");
  Obj.$("tirante").setAttribute("color", c);
  punta.setAttribute("color", c);
}
