...
;
// El seto: una caja redondeada con bultos de hojas encima y a los costados.
// Tocarlo lo vuelve a podar: otra semilla, otros bultos.
const P = Obj.props({ largo: 1.6, alto: 0.9, espesor: 0.6, color: "", semilla: 9 });
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let semilla = Number(P.semilla) || 9;

function dibujar() {
  const L = Obra.num(P.largo, 1.6, 0.3, 40), H = Obra.num(P.alto, 0.9, 0.2, 4), E = Obra.num(P.espesor, 0.6, 0.2, 3);
  armar(Obra.seto({ largo: L, alto: H, espesor: E, color: P.color, semilla }));
  toque.position = { x: 0, y: H / 2, z: 0 };
  toque.scale = { x: L, y: H + 0.1, z: E + 0.1 };
}
...
Obj.alCambiar((p, antes, cambios) => {
  if (cambios.includes("semilla")) semilla = Number(p.semilla) || 9;
  dibujar();
});
dibujar();
