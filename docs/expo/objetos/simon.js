...
;
// Simón: una secuencia que crece y el jugador que la repite.
//
// Cuatro teclas en cruz, cada una con su color y su nota (las de siempre: un
// acorde mayor, para que la secuencia suene a algo). La máquina muestra la
// secuencia prendiendo cada tecla; mientras tanto no se puede tocar. Después
// cada toque se compara con el que toca; al completar la ronda suma una.
const P = Obj.props({ meta: 5, velocidad: 1 });

const sinte = Obj.sinte({ volumen: 0.6 });
const TECLAS = [
  { x: 0, y: 0.06, color: "#30D158", semi: 7 },
  { x: 0.06, y: 0, color: "#E63946", semi: 4 },
  { x: 0, y: -0.06, color: "#FFD60A", semi: 0 },
  { x: -0.06, y: 0, color: "#0A84FF", semi: -5 },
];
const REPOSO = 0.008, LADO = 0.05;

const teclas = TECLAS.map((t, i) => {
  const g = Obj.crear("group", { x: t.x, y: t.y, z: REPOSO }, Obj.$("teclas"));
  const tapa = Obj.crear("box", { sx: LADO, sy: LADO, sz: 0.014, color: Obj.tono(t.color, -0.55), "border-radius": 0.008 }, g);
  const k = { i, ...t, g, tapa, abajo: false };
  Obj.boton(tapa, () => tocar(k));
  return k;
});
const meta = () => Math.max(2, Math.min(20, Math.round(Number(P.meta) || 5)));
const vel = () => Math.max(0.5, Math.min(2, Number(P.velocidad) || 1));

let secuencia = [], indice = 0, estado = "quieto";   // quieto | mostrando | jugando | fin
let temporizadores = [];

function prender(k, ms) {
  k.tapa.setAttribute("color", k.color);
  k.tapa.setAttribute("material-unlit", "true");
  k.g.position = { x: k.x, y: k.y, z: REPOSO - 0.004 };
  sinte.nota(Obj.frecuencia(k.semi), Math.min(0.6, ms / 1000 + 0.1), { tipo: "campana", vol: 0.22, caida: 3 });
  temporizadores.push(setTimeout(() => apagar(k), ms));
}
function apagar(k) {
  k.tapa.setAttribute("color", Obj.tono(k.color, -0.55));
  k.tapa.setAttribute("material-unlit", "false");
  k.g.position = { x: k.x, y: k.y, z: REPOSO };
}
function limpiar() { for (const t of temporizadores) clearTimeout(t); temporizadores = []; teclas.forEach(apagar); }
function texto(id, t) { Obj.$(id).setAttribute("value", t); }

function mostrar() {
  estado = "mostrando";
  texto("estado", "mirá…");
  const paso = 600 / vel();
  secuencia.forEach((i, n) => temporizadores.push(setTimeout(() => prender(teclas[i], paso * 0.6), 500 + n * paso)));
  temporizadores.push(setTimeout(() => { estado = "jugando"; indice = 0; texto("estado", "tu turno"); }, 500 + secuencia.length * paso));
}
function empezar() {
  limpiar();
  secuencia = [Math.floor(Math.random() * 4)];
  texto("ronda", "1");
  Obj.emitir("ronda", { n: 1 });
  Obj.emitir("valor", { valor: 1, fraccion: +(1 / meta()).toFixed(3) });
  mostrar();
}
function tocar(k) {
  if (estado !== "jugando") return;
  prender(k, 250);
  if (k.i !== secuencia[indice]) {
    estado = "fin";
    texto("estado", "¡error! otra vez");
    sinte.nota(110, 0.5, { tipo: "cuadrada", vol: 0.12, caida: 3, retraso: 0.1 });
    Obj.emitir("error", {});
    Obj.emitir("valor", { valor: 0, fraccion: 0 });
    temporizadores.push(setTimeout(empezar, 1500));
    return;
  }
  indice++;
  if (indice < secuencia.length) return;
  // Ronda completa.
  if (secuencia.length >= meta()) {
    estado = "fin";
    texto("estado", "¡completo!");
    texto("ronda", "OK");
    for (let n = 0; n < 4; n++) temporizadores.push(setTimeout(() => prender(teclas[n], 300), 400 + n * 120));
    Obj.emitir("valor", { valor: secuencia.length, fraccion: 1 });
    Obj.emitir("completo", {});
    return;
  }
  secuencia.push(Math.floor(Math.random() * 4));
  texto("ronda", String(secuencia.length));
  Obj.emitir("ronda", { n: secuencia.length });
  Obj.emitir("valor", { valor: secuencia.length, fraccion: +(secuencia.length / meta()).toFixed(3) });
  estado = "mostrando";
  temporizadores.push(setTimeout(mostrar, 500));
}

Obj.boton(Obj.$("centro"), empezar);
Obj.mensaje("empezar", empezar);

// VR: las teclas y el centro son blancos de toque, y la capa de las manos
// (_vr.js) los aprieta con la punta del mando.
texto("estado", "tocá el centro");
