...
const op = Deno.core.ops.op_hsml_get_global_position;
const g = (el) => { const a = op(el.nodeId); return "(" + f(a[0]) + " " + f(a[1]) + " " + f(a[2]) + ")"; };
let n = 0;
setInterval(() => {
  const o = donde.aMundo({ x: 0, y: 0, z: 0 }), x = donde.aMundo({ x: 1, y: 0, z: 0 });
  const t = "o " + f(o.x) + " " + f(o.y) + " " + f(o.z) + String.fromCharCode(10) + "x " + f(x.x - o.x) + " " + f(x.y - o.y) + " " + f(x.z - o.z) + String.fromCharCode(10) + "esc " + f(donde.escala);
  Obj.$("txt").setAttribute("value", t);
  console.log("[sonda] " + t.split(String.fromCharCode(10)).join(" | ") + " | caja " + g(Obj.$("caja")) + " raiz " + g(Obj.root) + " rootId " + Obj.root.nodeId + " cajaId " + Obj.$("caja").nodeId + " parent " + (Obj.$("caja").parent && Obj.$("caja").parent.nodeId));
  if (++n === 3) { const b = Obj.crear("group", { x: 0.5 }); setTimeout(() => console.log("[sonda] nuevo " + g(b)), 500); }
}, 1000);
