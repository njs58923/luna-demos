...
;
// La tabla de picar: una verdura que se acorta y rodajas que se apilan.
//
// La verdura está a lo largo de X, del cabo (x = 0 de su grupo) a la punta
// (x = largo). Cortar en una x separa lo que va de ahí a la punta: si es
// finito, es una rodaja que se acuesta sobre la tabla; si es largo, un trozo
// que queda de costado. El cuchillo tiene su marco: +X del mango a la punta
// y el filo abajo (-Y); en VR sigue a la mano como un martillo, y corta
// cuando el filo cruza hacia abajo la línea de la verdura.
const P = Obj.props({ verdura: "zanahoria" });

const sinte = Obj.sinte({ volumen: 0.7 });
const donde = Obj.marco();
const verduraG = Obj.$("verdura"), rodajasG = Obj.$("rodajas"), cuchillo = Obj.$("cuchillo");
const X0 = -0.1, EJE_Y = 0.034;
const TIPOS = {
  zanahoria: { color: "#F77F00", adentro: "#FFB703", r: 0.013, largo: 0.17, hojas: true },
  pepino: { color: "#3A5A40", adentro: "#D8F3DC", r: 0.018, largo: 0.19, hojas: false },
};
const tipo = () => TIPOS[P.verdura] || TIPOS.zanahoria;

// ── La verdura ─────────────────────────────────────────────────────────────
const cuerpo = Obj.crear("cylinder", { rz: Math.PI / 2, sx: 0.026, sy: 0.17, sz: 0.026, color: "#F77F00" }, verduraG);
const cara = Obj.crear("cylinder", { rz: Math.PI / 2, sx: 0.02, sy: 0.001, sz: 0.02, color: "#FFB703", visible: "false" }, verduraG);
const punta = [0, 1, 2].map((k) => Obj.crear("cylinder", { rz: Math.PI / 2, sx: 0.02, sy: 0.012, sz: 0.02, color: "#F77F00" }, verduraG));
const hojas = [-0.5, 0, 0.5].map((a) => Obj.crear("box", { x: -0.03, y: 0.005 + a * 0.01, rz: a * 0.6, sx: 0.06, sy: 0.004, sz: 0.012, color: "#52B788" }, verduraG));

let largo = 0.17, entera = true;
function dibujarVerdura() {
  const t = tipo();
  const d = t.r * 2;
  cuerpo.position = { x: largo / 2, y: 0, z: 0 };
  cuerpo.scale = { x: d, y: Math.max(0.001, largo), z: d };
  cuerpo.setAttribute("visible", largo > 0.002 ? "inherit" : "false");
  cuerpo.setAttribute("color", t.color);
  // La punta: tres cilindros cada vez más finos, sólo si nunca se cortó.
  punta.forEach((p, k) => {
    p.setAttribute("visible", entera ? "inherit" : "false");
    p.setAttribute("color", t.color);
    const r = d * (0.75 - k * 0.22);
    p.position = { x: largo + 0.006 + k * 0.012, y: 0, z: 0 };
    p.scale = { x: r, y: 0.012, z: r };
  });
  // La cara del corte se ve del color de adentro.
  cara.setAttribute("visible", !entera && largo > 0.002 ? "inherit" : "false");
  cara.setAttribute("color", t.adentro);
  cara.position = { x: largo + 0.0005, y: 0, z: 0 };
  cara.scale = { x: d * 0.8, y: 0.001, z: d * 0.8 };
  for (const h of hojas) h.setAttribute("visible", t.hojas ? "inherit" : "false");
}
...
const rodajas = [];
let sig = 0;
for (let i = 0; i < MAX; i++) rodajas.push({ el: Obj.crear("cylinder", { sx: 0.026, sy: 0.01, sz: 0.026, color: "#F77F00", visible: "false" }, rodajasG), anim: null });
function lugarEnPila(i) {
  // En filas a la derecha de la tabla.
  const col = i % 4, fila = Math.floor(i / 4);
  return { x: 0.08 + col * 0.03, y: 0.022, z: -0.07 + fila * 0.03 };
}
function soltarPedazo(x, largoPedazo) {
  const t = tipo();
  const r = rodajas[sig % MAX];
  const idx = sig % MAX;
  sig++;
  const acostada = largoPedazo <= 0.02;
  r.el.setAttribute("color", t.color);
  r.el.setAttribute("visible", "inherit");
  const desde = { x: X0 + x + largoPedazo / 2, y: EJE_Y, z: 0 };
  const hasta = lugarEnPila(idx);
  const d = t.r * 2;
  r.el.scale = { x: d, y: largoPedazo, z: d };
  if (acostada) hasta.y = 0.02 + largoPedazo / 2; else hasta.y = 0.02 + t.r;
  r.anim = { desde, hasta, t0: performance.now(), acostada };
  anim.iniciar();
}
const anim = Obj.animador(() => {
  ...
  for (const r of rodajas) {
    if (!r.anim) continue;
    const k = Math.min(1, (ahora - r.anim.t0) / 450);
    const s = Obj.suave(k);
    const a = r.anim.desde, b = r.anim.hasta;
    r.el.position = { x: a.x + (b.x - a.x) * s, y: a.y + (b.y - a.y) * s + Math.sin(Math.PI * k) * 0.03, z: a.z + (b.z - a.z) * s };
    // Se da vuelta: de acostada a lo largo (eje X) a parada sobre la cara (eje Y).
    r.el.rotation = { x: 0, y: 0, z: r.anim.acostada ? (Math.PI / 2) * (1 - s) : Math.PI / 2 };
    if (k >= 1) r.anim = null; else seguir = true;
  }
  return seguir;
});
...
// ── Cortar ─────────────────────────────────────────────────────────────────
function cortar(x) {
  // x: dónde, medido desde el cabo.
  if (largo <= 0.02) return false;
  x = Math.max(0.015, Math.min(largo - 0.004, x));
  const pedazo = largo - x + (entera ? 0.03 : 0);
  largo = x;
  entera = false;
  soltarPedazo(x, pedazo);
  dibujarVerdura();
  sinte.nota(160, 0.06, { tipo: "madera", vol: 0.6 });
  sinte.nota(1800, 0.03, { tipo: "ruido", vol: 0.1, caida: 60 });
  Obj.emitir("corte", { queda: +(largo * 100).toFixed(1) });
  if (largo <= 0.02) Obj.emitir("terminada", {});
  return true;
}
function nueva() {
  largo = tipo().largo;
  entera = true;
  for (const r of rodajas) { r.el.setAttribute("visible", "false"); r.anim = null; }
  sig = 0;
  dibujarVerdura();
}

// ── El cuchillo ────────────────────────────────────────────────────────────
// Acostado atrás, con el mango (el más grueso) apoyado en la tabla.
const REPOSO = { p: { x: 0.02, y: 0.029, z: -0.1 }, r: { x: Math.PI / 2, y: 0, z: 0 } };
let cuchilloPos = { ...REPOSO.p };
function ponerCuchillo(p, r) { cuchilloPos = { x: p.x, y: p.y, z: p.z }; cuchillo.position = p; cuchillo.rotation = r; }
function reposo() { ponerCuchillo(REPOSO.p, REPOSO.r); }
let golpe = null;
function golpeAnimado(xCorte) {
  if (golpe || agarre.tomado) return;
  golpe = { t0: performance.now(), x: X0 + xCorte, hecho: false, xc: xCorte };
  const tick = setInterval(() => {
    if (!golpe) { clearInterval(tick); return; }   // lo tomaron con la mano
    const t = (performance.now() - golpe.t0) / 1000;
    // Baja en 0.09 s, corta, sube en 0.12 s.
    const y = t < 0.09 ? 0.12 - (t / 0.09) * 0.081 : Math.min(0.12, 0.039 + ((t - 0.09) / 0.12) * 0.081);
    ponerCuchillo({ x: golpe.x, y, z: 0.08 }, { x: 0, y: Math.PI / 2, z: 0 });
    if (!golpe.hecho && t >= 0.09) { golpe.hecho = true; cortar(golpe.xc); }
    if (t >= 0.25) { clearInterval(tick); golpe = null; setTimeout(() => { if (!golpe && !agarre.tomado) reposo(); }, 600); }
  }, 16);
}
Obj.boton(cuerpo, () => { if (largo > 0.02) golpeAnimado(largo - 0.009); });
for (const p of punta) Obj.boton(p, () => golpeAnimado(largo - 0.009));
Obj.boton(Obj.$("tabla"), () => { if (largo <= 0.02) nueva(); });
Obj.boton(Obj.$("agarrar"), () => { if (largo > 0.02) golpeAnimado(largo - 0.009); });
Obj.mensaje("nueva", nueva);

// En VR: el cuchillo en la mano.
...
let antesY = null;
const agarre = Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) {
    const c = cuchilloPos;
    return Math.hypot(p.x - c.x, p.y - c.y, p.z - c.z) < 0.12;
  },
  tomar(p, mano, e) { golpe = null; antesY = null; seguir(p, e); },
  mover(p, mano, e) { seguir(p, e); },
  soltar() { antesY = null; setTimeout(() => { if (!agarre.tomado) reposo(); }, 600); },
});
function seguir(p, e) {
  const f = norm(donde.dirLocal({ x: e.dx, y: e.dy, z: e.dz }));
  const q = { x: e.qx || 0, y: e.qy || 0, z: e.qz || 0, w: e.qw == null ? 1 : e.qw };
  const arribaM = Obj.rotarV(q, { x: 0, y: 1, z: 0 });
  let arriba = donde.dirLocal(arribaM);
  const d = arriba.x * f.x + arriba.y * f.y + arriba.z * f.z;
  arriba = norm({ x: arriba.x - d * f.x, y: arriba.y - d * f.y, z: arriba.z - d * f.z });
  if (!Number.isFinite(arriba.x) || Math.hypot(arriba.x, arriba.y, arriba.z) < 0.5) arriba = { x: 0, y: 1, z: 0 };
  // La mano toma el mango, 4.5 cm atrás del origen del cuchillo.
  const o = { x: p.x + f.x * 0.045, y: p.y + f.y * 0.045, z: p.z + f.z * 0.045 };
  ponerCuchillo(o, Obj.deEjes(f, arriba, cruz(f, arriba)));
  // El filo: de s = 0 a s = 0.2, 1.7 cm abajo del eje de la hoja.
  const filo = (s) => ({ x: o.x + f.x * s - arriba.x * 0.017, y: o.y + f.y * s - arriba.y * 0.017, z: o.z + f.z * s - arriba.z * 0.017 });
  const a = filo(0.02), b = filo(0.2);
  // ¿Cruza la línea de la verdura (z = 0)? ¿A qué altura y en qué x?
  if ((a.z > 0) === (b.z > 0)) { antesY = null; return; }
  const s = a.z / (a.z - b.z);
  const cruce = { x: a.x + (b.x - a.x) * s, y: a.y + (b.y - a.y) * s };
  const xv = cruce.x - X0;
  if (antesY !== null && antesY > EJE_Y && cruce.y <= EJE_Y && xv > 0.01 && xv < largo + (entera ? 0.03 : 0)) cortar(xv);
  antesY = cruce.y;
}

Obj.alCambiar((pp, a, cambios) => { if (cambios.includes("verdura")) nueva(); });
largo = tipo().largo;
dibujarVerdura();
reposo();
