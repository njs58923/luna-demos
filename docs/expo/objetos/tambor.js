...
;
// El tambor y el platillo: un golpe grave, un chasquido largo.
//
// El tambor es un seno que baja de afinación en los primeros milisegundos
// —eso es lo que hace que un parche suene a parche— más un poco de ruido para
// el golpe. El platillo es ruido con una caída lenta. Los dos se mueven al
// golpearlos: el parche se hunde, el platillo tiembla.
const P = Obj.props({ aro: "#C8102E", afinacion: 0 });

const sinte = Obj.sinte({ volumen: 0.9 });
const donde = Obj.marco();
const parche = Obj.$("parche");
const platillo = Obj.$("platillo");

for (let i = 0; i < 3; i++) {
  const a = (i / 3) * Math.PI * 2;
  Obj.crear("cylinder", { x: Math.sin(a) * 0.12, y: 0.22, z: Math.cos(a) * 0.12, rx: Math.cos(a) * 0.25, rz: -Math.sin(a) * 0.25, sx: 0.014, sy: 0.46, sz: 0.014, color: "#8A8A8A" }, Obj.$("patas_tambor"));
}

let semilla = 1;
const ruido = () => { semilla = (semilla * 1103515245 + 12345) & 0x7fffffff; return semilla / 0x3fffffff - 1; };

function sonar(pieza, fuerza) {
  const f = Math.max(0.2, Math.min(1, fuerza));
  if (pieza === "platillo") {
    sinte.sumar((t) => ruido() * 0.25 * f * Math.exp(-2.2 * t) * (0.6 + 0.4 * Math.sin(6.2832 * 5200 * t)), 2.5);
    temblor = performance.now() + 900;
  } else {
    const base = 90 * Math.pow(2, Math.max(-6, Math.min(6, Number(P.afinacion) || 0)) / 12);
    let fase = 0;
    sinte.sumar((t) => {
      const fr = base * (1 + 1.5 * Math.exp(-35 * t));
      fase += 6.2832 * fr / sinte.RATE;
      return (Math.sin(fase) * 0.8 + ruido() * 0.25 * Math.exp(-40 * t)) * f * 0.6 * Math.exp(-6 * t);
    }, 0.8);
    hundido = performance.now() + 140;
  }
  anim.iniciar();
  Obj.emitir("golpe", { pieza, fuerza: Math.round(f * 100) / 100 });
}

let hundido = 0, temblor = 0;
const anim = Obj.animador((t) => {
  const ahora = performance.now();
  const k = Math.max(0, (hundido - ahora) / 140);
  parche.position = { x: 0, y: 0.086 - 0.006 * k, z: 0 };
  const q = Math.max(0, (temblor - ahora) / 900);
  platillo.rotation = { x: 0.12 + 0.08 * q * Math.sin(t * 40), y: 0, z: 0.05 * q * Math.cos(t * 33) };
  return k > 0 || q > 0;
});

Obj.boton(Obj.$("toque_parche"), () => sonar("parche", 1));
Obj.boton(Obj.$("toque_platillo"), () => sonar("platillo", 1));
Obj.mensaje("golpe", (d) => sonar(d && d.pieza === "platillo" ? "platillo" : "parche", 1));

// En VR: pegar de arriba hacia abajo, más fuerte cuanto más rápido.
const manos = { left: null, right: null };
const zona = Obj.$("zona");
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    if (v < 0.4) return;
    const piezas = [
      { nombre: "parche", x: -0.12, z: 0, y: 0.61, r: 0.18 },
      { nombre: "platillo", x: 0.25, z: 0, y: 0.83, r: 0.2 },
    ];
    for (const p of piezas) {
      if (antes.y > p.y && l.y <= p.y + 0.01 && Math.hypot(l.x - p.x, l.z - p.z) < p.r) { sonar(p.nombre, v / 3); return; }
    }
  });
}

Obj.alCambiar(() => Obj.$("cuerpo").setAttribute("color", Obj.color(P.aro, "#C8102E")));
Obj.$("cuerpo").setAttribute("color", Obj.color(P.aro, "#C8102E"));
