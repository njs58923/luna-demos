...
;
// El lector de tarjeta: una pasada es un tramo de x con un tiempo.
//
// La ranura va de X0 a X1 a la altura de la tarjeta metida. Una pasada
// empieza cuando la tarjeta entra por la izquierda y termina cuando sale por
// la derecha: si en el medio se sale de la ranura o vuelve para atrás, se
// corta. Lo que se juzga es el tiempo total: entre `minimo` y `maximo`.
const P = Obj.props({ minimo: 0.5, maximo: 1.2, color: "#F2C200" });

const sinte = Obj.sinte({ volumen: 0.6 });
const donde = Obj.marco();
const tarjeta = Obj.$("tarjeta");
const X0 = -0.11, X1 = 0.11, Y_RANURA = 0.105, Z_RANURA = 0.052;
const BOLSILLO = { x: 0, y: -0.06, z: 0.042 };

let pos = { ...BOLSILLO };
let pasada = null;       // { t0, xMax }
let estado = "bolsillo"; // bolsillo | lista | pasando | mano
let ocupado = false;

// La tarjeta va siempre de frente (su plano es el de la placa): en la ranura
// entra de arriba, con la mitad de abajo metida en el lector.
function poner(p) {
  pos = { ...p };
  tarjeta.position = pos;
}
function mostrar(t, color) {
  Obj.$("texto").setAttribute("value", t);
  Obj.$("texto").setAttribute("color", color || "#7CFC8A");
}
function luz(c) { Obj.$("luz").setAttribute("color", c); }
const minimo = () => Math.max(0.1, Number(P.minimo) || 0.5);
const maximo = () => Math.max(minimo() + 0.1, Number(P.maximo) || 1.2);

function juzgar(segundos) {
  if (segundos < minimo()) return rechazar("rapido");
  if (segundos > maximo()) return rechazar("lento");
  mostrar("ACEPTADA", "#7CFC8A");
  luz("#30D158");
  for (let i = 0; i < 2; i++) sinte.nota(Obj.frecuencia(7 + i * 5), 0.25, { tipo: "campana", vol: 0.2, retraso: i * 0.12 });
  Obj.emitir("valor", { valor: 1 });
  Obj.emitir("aceptada", { segundos: +segundos.toFixed(2) });
  despues();
}
function rechazar(motivo) {
  mostrar({ rapido: "MUY RÁPIDO", lento: "MUY LENTO", cortada: "OTRA VEZ" }[motivo], "#FF5A5F");
  luz("#E63946");
  sinte.nota(140, 0.3, { tipo: "cuadrada", vol: 0.1, caida: 6 });
  Obj.emitir("valor", { valor: 0 });
  Obj.emitir("rechazada", { motivo });
  despues();
}
function despues() {
  pasada = null;
  ocupado = true;
  setTimeout(() => { ocupado = false; luz("#3A3F4B"); mostrar("PASE LA TARJETA"); if (estado !== "mano") { estado = "lista"; poner({ x: X0, y: Y_RANURA, z: Z_RANURA }); } }, 1500);
}

// Escritorio: tarjeta a la ranura; un clic la larga, otro la suelta al final.
let viaje = null;
const anim = Obj.animador(() => {
  if (!viaje) return false;
  const t = (performance.now() - viaje.t0) / 1000;
  // A la velocidad ideal: el medio entre el mínimo y el máximo.
  const ideal = (minimo() + maximo()) / 2;
  const x = X0 + ((X1 - X0) * t) / ideal;
  if (x >= X1 + 0.05) {
    // Se pasó sin que la suelten: fue muy lenta.
    viaje = null;
    estado = "lista";
    rechazar("lento");
    return false;
  }
  poner({ x: Math.min(X1 + 0.05, x), y: Y_RANURA, z: Z_RANURA });
  return true;
});
Obj.boton(Obj.$("toque_tarjeta"), () => {
  if (ocupado || estado !== "bolsillo") return;
  estado = "lista";
  poner({ x: X0, y: Y_RANURA, z: Z_RANURA });
  sinte.nota(900, 0.03, { tipo: "madera", vol: 0.3 });
});
Obj.boton(Obj.$("toque_ranura"), () => {
  if (ocupado) return;
  if (estado === "lista" && !viaje) { viaje = { t0: performance.now() }; estado = "pasando"; mostrar("..."); anim.iniciar(); return; }
  if (viaje) {
    // Soltarla: vale si está en el último tramo (el 20 % final).
    const t = (performance.now() - viaje.t0) / 1000;
    const ideal = (minimo() + maximo()) / 2;
    const frac = t / ideal;
    viaje = null;
    estado = "lista";
    if (frac < 0.8) rechazar("rapido");
    else juzgar(ideal * Math.min(1, frac));
  }
});

// VR: la tarjeta en la mano. La pasada se mide sola.
Obj.agarre({
  zona: Obj.$("zona"), donde, gatillo: true,
  cerca(p) { return Math.hypot(p.x - pos.x, p.y - pos.y, p.z - pos.z) < 0.06; },
  tomar() { estado = "mano"; viaje = null; pasada = null; },
  mover(p) {
    const enRanura = Math.abs(p.y - Y_RANURA) < 0.025 && Math.abs(p.z - Z_RANURA) < 0.04 && p.x > X0 - 0.03 && p.x < X1 + 0.04;
    poner(enRanura ? { x: p.x, y: Y_RANURA, z: Z_RANURA } : p);
    if (ocupado) return;
    const ahora = performance.now();
    if (enRanura) {
      if (!pasada && p.x < X0 + 0.02) pasada = { t0: ahora, xMax: p.x };
      else if (pasada) {
        if (p.x < pasada.xMax - 0.03) { rechazar("cortada"); return; }
        pasada.xMax = Math.max(pasada.xMax, p.x);
        if (p.x >= X1) juzgar((ahora - pasada.t0) / 1000);
      }
    } else if (pasada) {
      // Salió de la ranura antes de llegar al final.
      rechazar("cortada");
    }
  },
  soltar() { estado = "bolsillo"; pasada = null; poner(BOLSILLO); },
});

Obj.mensaje("reiniciar", () => { viaje = null; pasada = null; ocupado = false; estado = "bolsillo"; poner(BOLSILLO); luz("#3A3F4B"); mostrar("PASE LA TARJETA"); });
function pintar() { Obj.$("cuerpo_tarjeta").setAttribute("color", Obj.color(P.color, "#F2C200")); }
