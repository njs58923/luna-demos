...
;
// El techo: a dos aguas, a un agua, plano o mansarda. Con `paredes` > 0 se
// exhibe sobre una casita de ese alto (y el origen queda en el piso); en 0 el
// origen es el alero, para ponerlo arriba de paredes propias. Tocarlo pasa al
// tipo siguiente.
const P = Obj.props({ tipo: "dos_aguas", ancho: 2.4, fondo: 2, pendiente: 0.6, alero: 0.3, color: "", hastial: "", chimenea: true, paredes: 0 });
const ORDEN = ["dos_aguas", "una_agua", "plano", "mansarda"];
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let tipo = ORDEN.includes(P.tipo) ? P.tipo : "dos_aguas";

function dibujar() {
  const W = Obra.num(P.ancho, 2.4, 0.5, 60), D = Obra.num(P.fondo, 2, 0.5, 60);
  const alero = Obra.num(P.alero, 0.3, 0, 1.5);
  const Hp = Obra.num(P.paredes, 0, 0, 20);
  const nodos = [];
  if (Hp > 0) {
    // La casita: cuatro paredes de revoque, una puerta y una ventana pintadas.
    const hastial = Obra.color(P.hastial, "#E6D8C0");
    nodos.push(Obra.caja({ y: Hp / 2, sx: W, sy: Hp, sz: D, color: hastial }));
    nodos.push(Obra.caja({ x: -W * 0.22, y: Math.min(Hp * 0.8, 0.9) / 2, z: D / 2 + 0.005, sx: W * 0.16, sy: Math.min(Hp * 0.8, 0.9), sz: 0.01, color: "#6B4A30" }));
    nodos.push(Obra.caja({ x: W * 0.2, y: Hp * 0.55, z: D / 2 + 0.005, sx: W * 0.22, sy: Hp * 0.35, sz: 0.01, color: "#2A3440" }));
  }
  nodos.push(Obra.grupo({ y: Hp }, Obra.techo(Object.assign({}, P, { tipo, alero, chimenea: P.chimenea !== false }))));
  armar(nodos);
  const alto = Hp + (tipo === "plano" ? 2 : Math.max(0.6, (D / 2) * Obra.num(P.pendiente, 0.6, 0.1, 2) + 1));
  toque.position = { x: 0, y: alto / 2, z: 0 };
  toque.scale = { x: W + 2 * alero, y: alto, z: D + 2 * alero };
}

Obj.boton(toque, () => {
  tipo = Obra.siguiente(ORDEN, tipo);
  dibujar();
  Obj.emitir("cambio", { tipo });
});
Obj.alCambiar((p, antes, cambios) => {
  if (cambios.includes("tipo") && ORDEN.includes(p.tipo)) tipo = p.tipo;
  dibujar();
});
dibujar();
