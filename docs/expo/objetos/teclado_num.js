...
;
// El teclado numérico: doce teclas que se hunden y un texto.
//
// Cada tecla es un grupo que baja al apretarla. En VR cada mano aprieta la
// tecla que tiene debajo cuando entra más de la mitad del recorrido, y la
// suelta al salir (con histéresis, para que un temblor no la repita).
const P = Obj.props({ clave: "", largo: 8, ocultar: false, color: "#3A3F4B" });

const sinte = Obj.sinte({ volumen: 0.6 });
const teclasG = Obj.$("teclas");
const ETIQ = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "*", "0", "#"];
const PASO_X = 0.042, PASO_Y = 0.036, LADO = 0.034, REPOSO = 0.012, RECORRIDO = 0.006;
const Y0 = -0.03;

const clave = () => String(P.clave || "").replace(/[^0-9]/g, "").slice(0, 10);
const largo = () => clave().length || Math.max(1, Math.min(10, Math.round(Number(P.largo) || 8)));

const teclas = ETIQ.map((k, i) => {
  const col = i % 3, fila = Math.floor(i / 3);
  const x = (col - 1) * PASO_X, y = (1.5 - fila) * PASO_Y;
  const g = Obj.crear("group", { x, y, z: REPOSO }, teclasG);
  const tapa = Obj.crear("box", { sx: LADO, sy: LADO * 0.85, sz: 0.012, color: k === "#" ? "#2A9D8F" : k === "*" ? "#9D2A2A" : Obj.color(P.color, "#3A3F4B"), "border-radius": 0.004 }, g);
  Obj.crear("text", { z: 0.0065, value: k, size: 0.016, color: "#FFFFFF" }, g);
  const t = { k, x, y, g, tapa, abajo: 0 };
  Obj.boton(tapa, () => { hundir(t); tecla(k); }, { vr: false });
  // VR: cada tecla es un botón de las manos (_vr.js): se empuja con la punta
  // del mando, o grip o gatillo encima. Pegadas como están, casi sin margen.
  Obj.manos().boton({
    centro: { x, y: y + Y0, z: 0.02 + REPOSO + 0.006 }, ancho: LADO, alto: LADO * 0.85,
    recorrido: RECORRIDO, margen: 0.002,
    alApretar() { t.abajo = 1; t.g.position = { x: t.x, y: t.y, z: REPOSO - RECORRIDO }; tecla(k); },
    alSoltar() { t.abajo = 0; t.g.position = { x: t.x, y: t.y, z: REPOSO }; },
  });
  return t;
});
function hundir(t) {
  t.g.position = { x: t.x, y: t.y, z: REPOSO - RECORRIDO };
  setTimeout(() => { if (!t.abajo) t.g.position = { x: t.x, y: t.y, z: REPOSO }; }, 120);
}
...
// Oculto, con asteriscos: la fuente del motor no garantiza el punto medio.
const vista = () => (P.ocultar ? "*".repeat(escrito.length) : escrito) || "";

function tecla(k) {
  if (bloqueado) return;
  sinte.nota(k === "#" ? 1600 : k === "*" ? 700 : 1100 + ETIQ.indexOf(k) * 40, 0.05, { tipo: "cuadrada", vol: 0.08, caida: 20 });
  Obj.emitir("tecla", { k });
  if (k === "*") { escrito = ""; mostrar(vista()); return; }
  if (k === "#") { enviar(); return; }
  if (escrito.length >= largo()) return;
  escrito += k;
  mostrar(vista());
  // Con clave, al completarla se prueba sola.
  if (clave() && escrito.length === clave().length) setTimeout(enviar, 250);
}
function enviar() {
  if (bloqueado) return;
  const texto = escrito;
  const c = clave();
  const correcto = c ? texto === c : texto.length > 0;
  Obj.emitir("codigo", { texto, correcto });
  Obj.emitir("valor", { valor: correcto ? 1 : 0 });
  bloqueado = true;
  if (correcto) {
    mostrar(c ? "ABIERTO" : texto, "#7CFC8A");
    for (let i = 0; i < 3; i++) sinte.nota(Obj.frecuencia(3 + i * 4), 0.3, { tipo: "campana", vol: 0.18, retraso: i * 0.09 });
  } else {
    mostrar("ERROR", "#FF5A5F");
    sinte.nota(120, 0.4, { tipo: "cuadrada", vol: 0.12, caida: 3 });
  }
  setTimeout(() => { escrito = ""; bloqueado = false; mostrar(""); }, 1400);
}

Obj.mensaje("limpiar", () => { escrito = ""; mostrar(""); });
Obj.alCambiar(() => {
  escrito = ""; mostrar("");
  for (const t of teclas) if (t.k !== "#" && t.k !== "*") t.tapa.setAttribute("color", Obj.color(P.color, "#3A3F4B"));
});
