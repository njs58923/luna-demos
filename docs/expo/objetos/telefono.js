...
;
// El teléfono de disco: pulsos, tonos y una agenda de mundos.
//
// El disco está en el plano XZ de su grupo, que se inclina hacia quien mira:
// visto de frente, x es la derecha y -z es arriba. Cada agujero está a 30°
// del anterior; marcar un número es girar el disco en el sentido del reloj
// hasta que ese agujero llega al tope, y soltarlo: vuelve solo, despacio,
// dando un pulso cada 30° (uno para el 1, diez para el 0). Los tonos son los
// de la red argentina de siempre: 425 Hz para invitar a marcar, cortado para
// ocupado y de a ratos para la llamada.
const P = Obj.props({ agenda: {}, color: "#1C1C1C" });

const sinte = Obj.sinte({ volumen: 0.6 });
const disco = Obj.$("disco"), tubo = Obj.$("tubo");
const R_AGUJERO = 0.047;
const TOPE = (-60 * Math.PI) / 180;
const DIGITOS = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0];
const angDe = (d) => TOPE + ((d === 0 ? 10 : d) + 1) * (Math.PI / 6);   // en la pantalla, antihorario
const aLocal = (fi, r) => ({ x: Math.cos(fi) * r, z: -Math.sin(fi) * r });

// Números fijos debajo y agujeros en el disco.
for (const d of DIGITOS) {
  const p = aLocal(angDe(d), R_AGUJERO);
  Obj.crear("text", { x: p.x, y: 0.0045, z: p.z, rx: -Math.PI / 2, value: String(d), size: 0.012, color: "#1C1C1C" }, Obj.$("numeros"));
}
Obj.crear("cylinder", { sx: 0.12, sy: 0.003, sz: 0.12, color: "#C8CDD260", "material-alpha": "blend" }, disco);
for (const d of DIGITOS) {
  const p = aLocal(angDe(d), R_AGUJERO);
  // El agujero: un disquito translúcido, para ver el número de abajo.
  Obj.crear("cylinder", { x: p.x, y: 0.002, z: p.z, sx: 0.02, sy: 0.002, sz: 0.02, color: "#FFFFFF30", "material-alpha": "blend" }, disco);
  const blanco = Obj.crear("cylinder", { x: p.x, y: 0.004, z: p.z, sx: 0.022, sy: 0.006, sz: 0.022, color: "transparent", "material-alpha": "blend" }, disco);
  Obj.boton(blanco, () => marcar(d));
}

// ── Estado ─────────────────────────────────────────────────────────────────
let estado = "colgado";   // colgado | tono | marcando | llamando | ocupado | sonando | hablando
let numero = "";
let girando = null;       // { d, fase: "ida" | "vuelta", ang, meta }
let esperaLlamar = null;
let timbre = null;

function mostrarNumero() { Obj.$("numero").setAttribute("value", numero); }

// ── Tonos ──────────────────────────────────────────────────────────────────
function tono(tipo) {
  if (!tipo) { sinte.flujo(null); return; }
  const DOS_PI = Math.PI * 2;
  sinte.flujo((t) => {
    if (tipo === "invitar") return Math.sin(DOS_PI * 425 * t) * 0.12;
    if (tipo === "ocupado") return (t % 0.4 < 0.2 ? Math.sin(DOS_PI * 425 * t) : 0) * 0.12;
    if (tipo === "llamando") return (t % 3 < 1 ? Math.sin(DOS_PI * 425 * t) : 0) * 0.1;
    if (tipo === "timbre") {
      const on = t % 3 < 1.2;
      const golpe = Math.sin(DOS_PI * 20 * t) > 0 ? 1 : -1;
      return on ? (Math.sin(DOS_PI * 1050 * t) * 0.6 + Math.sin(DOS_PI * 1350 * t) * 0.4) * 0.18 * (0.6 + 0.4 * golpe) : 0;
    }
    return 0;
  });
}
...
// ── El tubo ────────────────────────────────────────────────────────────────
let alzado = 0, alzadoMeta = 0;
const cable = [];
for (let k = 0; k < 10; k++) cable.push(Obj.crear("cylinder", { sx: 0.007, sy: 0.02, sz: 0.007, color: "#1C1C1C" }, Obj.$("cable")));
function ponerTubo(t) {
  const sacude = estado === "sonando" ? Math.sin(t * 50) * 0.004 : 0;
  const pos = { x: sacude, y: 0.14 + alzado * 0.12, z: -0.065 + alzado * 0.08 };
  tubo.position = pos;
  tubo.rotation = { x: -alzado * 0.5, y: 0, z: alzado * 0.25 };
  // El cable: del costado del cuerpo a la punta izquierda del tubo, colgando.
  const a = { x: -0.11, y: 0.04, z: -0.03 };
  const b = { x: pos.x - 0.08, y: pos.y - 0.02, z: pos.z };
  let prev = a;
  for (let k = 1; k <= cable.length; k++) {
    const s = k / cable.length;
    const p = { x: a.x + (b.x - a.x) * s - 0.03 * Math.sin(Math.PI * s), y: a.y + (b.y - a.y) * s - 0.05 * Math.sin(Math.PI * s), z: a.z + (b.z - a.z) * s };
    const l = Obj.tender(cable[k - 1], prev, p);
    cable[k - 1].scale = { x: 0.007, y: l, z: 0.007 };
    prev = p;
  }
}

const anim = Obj.animador((t, dt) => {
  let seguir = estado === "sonando";
  alzado += (alzadoMeta - alzado) * Math.min(1, dt * 10);
  if (Math.abs(alzadoMeta - alzado) > 0.002) seguir = true; else alzado = alzadoMeta;
  ponerTubo(t);
  // El disco.
  if (girando) {
    seguir = true;
    if (girando.fase === "ida") {
      girando.ang = Math.min(girando.meta, girando.ang + dt * 4);
      if (girando.ang >= girando.meta) { girando.fase = "vuelta"; girando.pulsos = 0; }
    } else {
      // Vuelve a paso de regulador: 10 pulsos por segundo (30° cada uno).
      girando.ang = Math.max(0, girando.ang - dt * (Math.PI / 6) * 10);
      const pasados = Math.floor((girando.meta - girando.ang) / (Math.PI / 6) + 1e-6);
      // Soltado antes del tope (en VR): vuelve sin marcar nada.
      const debe = girando.nulo ? 0 : Math.min(pasados, girando.d === 0 ? 10 : girando.d);
      while (girando.pulsos < debe) { girando.pulsos++; sinte.nota(1800, 0.012, { tipo: "pulso", vol: 0.25 }); }
      if (girando.ang <= 0) { const g = girando; girando = null; if (!g.nulo) digito(g.d); }
    }
    disco.rotation = { x: 0, y: girando ? -girando.ang : 0, z: 0 };
  }
  return seguir;
});
...
function levantar() {
  ...
  sinte.nota(300, 0.05, { tipo: "madera", vol: 0.4 });
  anim.iniciar();
}
function colgar() {
  alzadoMeta = 0;
  estado = "colgado";
  girando = null;
  clearTimeout(esperaLlamar);
  tono(null);
  disco.rotation = { x: 0, y: 0, z: 0 };
  sinte.nota(250, 0.06, { tipo: "madera", vol: 0.5, retraso: 0.15 });
  anim.iniciar();
}
Obj.boton(Obj.$("toque_tubo"), () => (estado === "colgado" || estado === "sonando" ? levantar() : colgar()));

function marcar(d) {
  if (girando || !(estado === "tono" || estado === "marcando")) return;
  if (estado === "tono") { tono(null); estado = "marcando"; }
  clearTimeout(esperaLlamar);
  girando = { d, fase: "ida", ang: 0, meta: angDe(d) - TOPE, pulsos: 0 };
  anim.iniciar();
}
// ── En VR: el dedo en el agujero ───────────────────────────────────────────
// El disco vive en el grupo "frente" (inclinado rx hacia quien mira) y gira en
// su plano XZ. Con el grip o el gatillo en un agujero se lo lleva en el sentido
// del reloj hasta el tope; al soltar vuelve solo y marca, como con el clic.
// Soltado antes del tope, vuelve sin marcar.
const FRENTE = { y: 0.115, z: 0.04, rx: 0.35 }, DISCO_Y = 0.006;
const qFrente = Obj.qEje({ x: 1, y: 0, z: 0 }, FRENTE.rx);
const qInv = { x: -qFrente.x, y: -qFrente.y, z: -qFrente.z, w: qFrente.w };
/** Del objeto al plano del disco: r a la derecha, u arriba (el -z del disco), h hacia afuera. */
function enDisco(p) {
  const l = Obj.rotarV(qInv, { x: p.x, y: p.y - FRENTE.y, z: p.z - FRENTE.z });
  return { r: l.x, u: -l.z, h: l.y - DISCO_Y };
}
/** El ángulo en el disco, en el sentido del reloj desde arriba. */
const angEnDisco = (q) => Math.atan2(q.r, q.u);
let dedo = null;   // { d, meta, desde, ang }
let enDedo = false;
Obj.manos().agarrable({
  cerca(p) {
    if (enDedo || girando || !(estado === "tono" || estado === "marcando")) return false;
    const q = enDisco(p);
    if (q.h < -0.03 || q.h > 0.07) return false;
    let mejor = null, dm = 0.022;
    for (const d of DIGITOS) {
      const a = aLocal(angDe(d), R_AGUJERO);
      const dd = Math.hypot(q.r - a.x, q.u + a.z);
      if (dd < dm) { dm = dd; mejor = d; }
    }
    if (mejor === null) return false;
    dedo = { d: mejor, meta: angDe(mejor) - TOPE, desde: angEnDisco(q), ang: 0 };
    return dm;
  },
  tomar() {
    if (!dedo) return;
    ...
    if (estado === "tono") { tono(null); estado = "marcando"; }
    clearTimeout(esperaLlamar);
    sinte.nota(900, 0.02, { tipo: "madera", vol: 0.2 });
  },
  mover(p) {
    if (!dedo) return;
    const a = angEnDisco(enDisco(p));
    // Se sigue la vuelta: el disco no salta al pasar por abajo.
    const antes = dedo.ang;
    let ang = antes + Obj.difAngulo(a - dedo.desde, antes);
    ang = Math.max(0, Math.min(dedo.meta, ang));
    if (ang >= dedo.meta && antes < dedo.meta) sinte.nota(500, 0.03, { tipo: "madera", vol: 0.4 });
    dedo.ang = ang;
    disco.rotation = { x: 0, y: -ang, z: 0 };
  },
  soltar() {
    enDedo = false;
    if (!dedo) return;
    const llego = dedo.ang >= dedo.meta - 0.12;
    girando = { d: dedo.d, fase: "vuelta", ang: dedo.ang, meta: llego ? dedo.meta : dedo.ang, pulsos: 0, nulo: !llego };
    dedo = null;
    anim.iniciar();
  },
});
...
function agenda() {
  const a = P.agenda && typeof P.agenda === "object" ? P.agenda : {};
  const out = {};
  for (const [k, v] of Object.entries(a)) if (/^[0-9]{1,12}$/.test(k) && typeof v === "string") out[k] = v;
  return out;
}
function digito(d) {
  numero = (numero + d).slice(0, 12);
  mostrarNumero();
  Obj.emitir("marcado", { numero });
  const ag = agenda();
  // Si ya es un número de la agenda y ningún otro empieza igual, llama enseguida.
  const exacto = ag[numero] && !Object.keys(ag).some((k) => k !== numero && k.startsWith(numero));
  esperaLlamar = setTimeout(llamar, exacto ? 300 : 2500);
}
function llamar() {
  if (estado !== "marcando") return;
  const url = agenda()[numero] || "";
  Obj.emitir("llamada", { numero, url });
  if (!url) { estado = "ocupado"; tono("ocupado"); return; }
  estado = "llamando";
  tono("llamando");
  // Dos tonos de llamada y "atienden": al mundo.
  setTimeout(() => {
    if (estado !== "llamando") return;
    tono(null);
    let destino = "";
    try { destino = new URL(url, location.href).href; } catch (e) { destino = ""; }
    if (!destino) { estado = "ocupado"; tono("ocupado"); return; }
    try { hiperspace.world.navigate(destino); } catch (e) { console.warn("[telefono] no se pudo navegar:", (e && e.message) || e); }
  }, 5200);
}
...
function sonar() {
  ...
  tono("timbre");
  anim.iniciar();
  // Si nadie atiende en 15 s, deja de sonar.
  clearTimeout(timbre);
  timbre = setTimeout(() => { if (estado === "sonando") { estado = "colgado"; tono(null); } }, 15000);
}
Obj.mensaje("sonar", sonar);
Obj.mensaje("colgar", colgar);

function pintar() {
  const c = Obj.color(P.color, "#1C1C1C");
  Obj.$("cuerpo").setAttribute("color", c);
}
