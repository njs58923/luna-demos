...
;
// El televisor: canales dibujados con planos, sin videos.
//
// Cada canal arma su contenido en la pantalla al entrar y lo desarma al
// salir, y tiene su propio paso de animación. Todo lo de la pantalla va sin
// iluminación (material-unlit): una pantalla emite, no refleja. Apagado, no
// pide cuadros; al apagarlo, el punto blanco que se achica como en los de
// tubo.
const P = Obj.props({ canal: 1, mueble: "#6B4A30", prendido: true });

const W = 0.4, H = 0.3;
const pantalla = Obj.$("pantalla");
const CANALES = ["barras", "nieve", "reloj", "logo", "pecera"];
let canal = Math.max(1, Math.min(CANALES.length, Math.round(Number(P.canal) || 1)));
let prendido = P.prendido !== false;
let nodos = [];
let estado = {};
let osdHasta = 0;

const plano = (attrs) => { const el = Obj.crear("plane", Object.assign({ "material-unlit": "true" }, attrs), pantalla); nodos.push(el); return el; };
const texto = (attrs) => { const el = Obj.crear("text", Object.assign({ "material-unlit": "true" }, attrs), pantalla); nodos.push(el); return el; };

function limpiar() { for (const n of nodos) n.remove(); nodos = []; estado = {}; }

const ARMAR = {
  barras() {
    const C = ["#C0C0C0", "#C0C000", "#00C0C0", "#00C000", "#C000C0", "#C00000", "#0000C0"];
    C.forEach((c, i) => plano({ x: -W / 2 + (W / 7) * (i + 0.5), y: H * 0.1, z: 0.001, sx: W / 7 + 0.001, sy: H * 0.8, color: c }));
    ["#0000C0", "#131313", "#C000C0", "#131313", "#00C0C0", "#131313", "#C0C0C0"].forEach((c, i) =>
      plano({ x: -W / 2 + (W / 7) * (i + 0.5), y: -H * 0.4, z: 0.001, sx: W / 7 + 0.001, sy: H * 0.2, color: c }));
    estado.aviso = texto({ y: -H * 0.4, z: 0.002, value: "PRUEBA", size: 0.02, color: "#FFFFFF" });
  },
  nieve() {
    // Una grilla de celdas grises que cambian al azar.
    estado.celdas = [];
    const CX = 10, CY = 8;
    for (let j = 0; j < CY; j++) for (let i = 0; i < CX; i++) {
      estado.celdas.push(plano({ x: -W / 2 + (W / CX) * (i + 0.5), y: -H / 2 + (H / CY) * (j + 0.5), z: 0.001, sx: W / CX + 0.001, sy: H / CY + 0.001, color: "#808080" }));
    }
  },
  reloj() {
    plano({ z: 0.001, sx: W, sy: H, color: "#0B1D3A" });
    estado.hora = texto({ y: 0.02, z: 0.002, value: "", size: 0.09, color: "#FFD60A" });
    estado.fecha = texto({ y: -0.07, z: 0.002, value: "", size: 0.025, color: "#9DB4D8" });
  },
  logo() {
    plano({ z: 0.001, sx: W, sy: H, color: "#050505" });
    estado.logo = plano({ z: 0.002, sx: 0.1, sy: 0.05, "border-radius": 0.2, color: "#7B2FF7" });
    estado.logoTxt = texto({ z: 0.003, value: "LUNA", size: 0.025, color: "#FFFFFF" });
    estado.p = { x: 0, y: 0 };
    estado.v = { x: 0.07, y: 0.05 };
  },
  pecera() {
    plano({ z: 0.001, sx: W, sy: H, color: "#0E4C7A" });
    plano({ y: -H / 2 + 0.02, z: 0.0015, sx: W, sy: 0.04, color: "#D9C9A3" });
    estado.peces = [0, 1, 2].map((i) => ({
      el: plano({ z: 0.002 + i * 0.0005, sx: 0.05, sy: 0.025, "border-radius": 0.5, color: ["#FF7F11", "#FFD60A", "#4CC9F0"][i] }),
      x: (i - 1) * 0.1, y: (i - 1) * 0.05, v: 0.04 + i * 0.015, fase: i,
    }));
  },
};
...
const PASO = {
  barras() {},
  nieve() {
    for (const c of estado.celdas) {
      const g = Math.floor(Math.random() * 200 + 30).toString(16).padStart(2, "0");
      c.setAttribute("color", "#" + g + g + g);
    }
  },
  reloj() {
    const d = new Date();
    const dos = (n) => String(n).padStart(2, "0");
    const txt = dos(d.getHours()) + ":" + dos(d.getMinutes()) + (d.getSeconds() % 2 ? " " : ":") + dos(d.getSeconds());
    if (txt !== estado.ultima) { estado.ultima = txt; estado.hora.setAttribute("value", txt); }
    estado.fecha.setAttribute("value", d.getDate() + "/" + (d.getMonth() + 1) + "/" + d.getFullYear());
  },
  logo(dt) {
    const p = estado.p, v = estado.v;
    p.x += v.x * dt; p.y += v.y * dt;
    let pego = false;
    if (Math.abs(p.x) > W / 2 - 0.05) { v.x = -v.x; p.x = Math.sign(p.x) * (W / 2 - 0.05); pego = true; }
    if (Math.abs(p.y) > H / 2 - 0.025) { v.y = -v.y; p.y = Math.sign(p.y) * (H / 2 - 0.025); pego = true; }
    if (pego) estado.logo.setAttribute("color", ["#7B2FF7", "#FF3B30", "#30D158", "#0A84FF", "#FFD60A"][Math.floor(Math.random() * 5)]);
    estado.logo.position = { x: p.x, y: p.y, z: 0.002 };
    estado.logoTxt.position = { x: p.x, y: p.y, z: 0.003 };
  },
  pecera(dt, t) {
    for (const f of estado.peces) {
      f.x += f.v * dt;
      if (Math.abs(f.x) > W / 2 - 0.03) { f.v = -f.v; f.x = Math.sign(f.x) * (W / 2 - 0.03); }
      const y = f.y + 0.02 * Math.sin(t * 1.5 + f.fase);
      f.el.position = { x: f.x, y, z: f.el.position.z };
      f.el.rotation = { x: 0, y: f.v < 0 ? Math.PI : 0, z: 0 };
    }
  },
};
...
function entrar() {
  limpiar();
  if (!prendido) return;
  ARMAR[CANALES[canal - 1]]();
  osd = texto({ x: W / 2 - 0.04, y: H / 2 - 0.03, z: 0.004, value: String(canal), size: 0.04, color: "#30D158" });
  osdHasta = performance.now() + 1500;
  anim.iniciar();
}
let ultimoPaso = 0;
const anim = Obj.animador((t, dt) => {
  if (apagando) {
    const k = Math.min(1, (performance.now() - apagando) / 400);
    if (!estado.punto) { limpiar(); estado.punto = plano({ z: 0.003, sx: W, sy: 0.01, color: "#FFFFFF" }); }
    const s = 1 - k;
    estado.punto.scale = { x: Math.max(0.001, W * s), y: 0.01 * (1 - k * 0.5), z: 1 };
    if (k >= 1) { limpiar(); apagando = 0; return false; }
    return true;
  }
  if (!prendido) return false;
  if (osd && performance.now() > osdHasta) { osd.remove(); nodos = nodos.filter((n) => n !== osd); osd = null; }
  // La nieve y el reloj no necesitan sesenta cuadros por segundo.
  const nombre = CANALES[canal - 1];
  const cada = nombre === "nieve" ? 0.08 : nombre === "reloj" ? 0.25 : 0;
  if (t - ultimoPaso >= cada) { PASO[nombre](cada ? t - ultimoPaso : dt, t); ultimoPaso = t; }
  return nombre !== "barras" || !!osd;
});

function cambiar(n) {
  canal = ((n - 1 + CANALES.length) % CANALES.length) + 1;
  if (!prendido) prendido = true;
  entrar();
  Obj.emitir("canal", { numero: canal, nombre: CANALES[canal - 1], prendido });
}
function prender(v) {
  if (v === prendido) return;
  prendido = v;
  if (prendido) entrar(); else { apagando = performance.now(); anim.iniciar(); }
  Obj.emitir("canal", { numero: canal, nombre: CANALES[canal - 1], prendido });
}

Obj.boton(Obj.$("subir"), () => cambiar(canal + 1), { color: "#2A2A2A" });
Obj.boton(Obj.$("bajar"), () => cambiar(canal - 1), { color: "#2A2A2A" });
Obj.boton(Obj.$("encendido"), () => prender(!prendido), { color: "#C8102E" });
Obj.mensaje("canal", (d) => { if (d && Number.isInteger(d.numero)) cambiar(d.numero); });
Obj.mensaje("prender", () => prender(true));
Obj.mensaje("apagar", () => prender(false));
Obj.alCambiar(() => Obj.$("mueble").setAttribute("color", Obj.color(P.mueble, "#6B4A30")));
Obj.$("mueble").setAttribute("color", Obj.color(P.mueble, "#6B4A30"));
entrar();
