...
;
// El temporizador de cocina: minutos cargados, un dial que vuelve a cero y
// una campanita al final.
//
// El dial muestra lo que falta: una vuelta entera son 60 minutos, y el número
// que queda bajo la flecha es el que falta. Se mueve con un temporizador de
// 100 ms y no por cuadro: un dial que avanza un grado cada diez segundos no
// necesita sesenta dibujos por segundo.
const P = Obj.props({ minutos: 0, color: "#E63946" });

const dial = Obj.$("dial");
const lectura = Obj.$("lectura");
const largarTxt = Obj.$("largar_txt");
const sinte = Obj.sinte({ volumen: 0.7 });

// Los números del dial, de 5 en 5, parados sobre el borde. Miran hacia afuera:
// cada uno girado para que su +Z apunte desde el centro.
for (let m = 0; m < 60; m += 5) {
  const a = (m / 60) * Math.PI * 2;
  const r = 0.071;
  const g = Obj.crear("group", { x: Math.sin(a) * r, y: 0, z: Math.cos(a) * r, ry: a }, dial);
  Obj.crear("text", { z: 0.001, value: String(m), size: 0.011, color: "#1C1C1C" }, g);
}
for (let m = 0; m < 60; m++) {
  if (m % 5 === 0) continue;
  const a = (m / 60) * Math.PI * 2;
  Obj.crear("box", { x: Math.sin(a) * 0.066, y: 0.006, z: Math.cos(a) * 0.066, ry: a, sx: 0.0015, sy: 0.002, sz: 0.006, color: "#6A6258" }, dial);
}

let restante = Math.max(0, Math.min(60, Number(P.minutos) || 0)) * 60000;   // ms
let corriendo = false;
let ultimo = 0;
let fin = null;
...
function dibujar() {
  // La flecha está en +Z (a = 0). Girar el dial -a trae el minuto m al frente.
  const minutos = restante / 60000;
  dial.rotation = { x: 0, y: -(minutos / 60) * Math.PI * 2, z: 0 };
  // Redondeado hacia arriba, como cualquier temporizador: con 0,3 s dice 00:01.
  const s = Math.ceil(restante / 1000);
  lectura.setAttribute("value", dos(s / 60) + ":" + dos(s % 60));
  // Sin glifos raros: la fuente del motor puede no tener ▶ ni ❚.
  largarTxt.setAttribute("value", corriendo ? "II" : "GO");
}

function tic() {
  if (!corriendo) return;
  const ahora = performance.now();
  restante = Math.max(0, restante - (ahora - ultimo));
  ultimo = ahora;
  if (restante <= 0) {
    corriendo = false;
    clearInterval(reloj);
...
function cargar(min) {
  restante = Math.max(0, Math.min(60 * 60000, restante + min * 60000));
  dibujar();
}
function campana() {
  // Cuatro golpes de campanita, como el ring de un timer mecánico.
  for (let k = 0; k < 4; k++) sinte.nota(2093, 0.9, { tipo: "campana", vol: 0.3, caida: 5, retraso: k * 0.28 });
}
const clic = () => sinte.nota(1800, 0.02, { tipo: "pulso", vol: 0.15 });

Obj.boton(Obj.$("b_mas1"), () => { clic(); cargar(1); }, { color: "#F4F1EA" });
Obj.boton(Obj.$("b_mas5"), () => { clic(); cargar(5); }, { color: "#F4F1EA" });
Obj.boton(Obj.$("b_largar"), () => { clic(); corriendo ? pausar() : largar(); }, { color: "#30D158" });
Obj.boton(Obj.$("b_cero"), () => { clic(); pausar(); restante = 0; dibujar(); }, { color: "#1C1C1C" });
// Girar la perilla también carga: un minuto por toque.
Obj.boton(Obj.$("perilla"), () => { clic(); cargar(1); }, { color: "#F4F1EA" });

// En VR: el dial se toma y se gira con la muñeca (o la mano alrededor), como
// uno de verdad: en el sentido del reloj visto de arriba carga, al revés
// descarga. Una vuelta entera son 60 minutos; un clic por minuto.
Obj.manos().perilla({
  centro: { x: 0, y: 0.1, z: 0 }, eje: { x: 0, y: 1, z: 0 }, radio: 0.07,
  alGirar(d) {
    const antes = Math.floor(restante / 60000);
    restante = Math.max(0, Math.min(60 * 60000, restante + (d / (Math.PI * 2)) * 60 * 60000));
    if (Math.floor(restante / 60000) !== antes) clic();
    dibujar();
  },
});

Obj.mensaje("cargar", (d) => { if (d && Number.isFinite(d.minutos)) { pausar(); restante = 0; cargar(d.minutos); } });
Obj.mensaje("largar", largar);
Obj.mensaje("pausar", pausar);

function pintar() {
  const c = Obj.color(P.color, "#E63946");
  Obj.$("cuerpo").setAttribute("color", c);
  Obj.$("cuerpo_base").setAttribute("color", Obj.tono(c, -0.2));
}
