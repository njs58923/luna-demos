...
;
// El theremin: la nota sale de una distancia y el volumen de una altura.
//
// Nota: cuanto más cerca de la antena, más agudo; la distancia (de 3 cm a
// 60 cm para una mano, de 30 cm a 3 m para el visitante sin manos) se lleva
// a `octavas` octavas sobre `grave`, en escala exponencial como el oído. Si
// hay escala, la nota se pega al semitono o a la pentatónica más cercana.
// Volumen: la otra mano sobre el aro; apoyada, silencio. El sonido va por un
// flujo con portamento (la frecuencia se desliza hacia la pedida) y vibrato.
const P = Obj.props({ escala: "libre", octavas: 3, grave: 130.81 });

const sinte = Obj.sinte({ volumen: 0.8 });
const donde = Obj.marco();
const ANTENA = { x: 0.17, y0: 0.14, y1: 0.56 };
const ARO = { x: -0.26, y: 0.12, z: 0, r: 0.05 };
const NOMBRES = ["Do", "Do#", "Re", "Re#", "Mi", "Fa", "Fa#", "Sol", "Sol#", "La", "La#", "Si"];

// El aro, en segmentos.
for (let i = 0; i < 16; i++) {
  const a0 = (i / 16) * Math.PI * 2, a1 = ((i + 1) / 16) * Math.PI * 2;
  const s = Obj.crear("cylinder", { sx: 0.006, sy: 0.02, sz: 0.006, color: "#C0C0C0" }, Obj.$("aro"));
  const l = Obj.tender(s, { x: Math.cos(a0) * ARO.r - 0.05, y: 0, z: Math.sin(a0) * ARO.r }, { x: Math.cos(a1) * ARO.r - 0.05, y: 0, z: Math.sin(a1) * ARO.r });
  s.scale = { x: 0.006, y: l, z: 0.006 };
}

let prendido = false;
let fMeta = 220, vMeta = 0;       // lo que se pide
let fAhora = 220, vAhora = 0;     // lo que suena (se desliza)
const manos = { left: null, right: null };

function distAntena(p) {
  const y = Math.max(ANTENA.y0, Math.min(ANTENA.y1, p.y));
  return Math.hypot(p.x - ANTENA.x, p.y - y, p.z);
}
function pegar(f) {
  const grave = Math.max(40, Math.min(1000, Number(P.grave) || 130.81));
  const s = 12 * Math.log2(f / grave);
  if (P.escala === "cromatica") return grave * Math.pow(2, Math.round(s) / 12);
  if (P.escala === "pentatonica") {
    const PENTA = [0, 2, 4, 7, 9, 12];
    const oct = Math.floor(s / 12), r = s - oct * 12;
    const n = PENTA.reduce((a, b) => (Math.abs(b - r) < Math.abs(a - r) ? b : a));
    return grave * Math.pow(2, (oct * 12 + n) / 12);
  }
  return f;
}
function frecDe(k) {
  const grave = Math.max(40, Math.min(1000, Number(P.grave) || 130.81));
  const oct = Math.max(1, Math.min(4, Number(P.octavas) || 3));
  return pegar(grave * Math.pow(2, Math.max(0, Math.min(1, k)) * oct));
}

function voz() {
  let fase = 0;
  return (t) => {
    // Portamento y volumen suave, muestra a muestra.
    fAhora += (fMeta - fAhora) * 0.0012;
    vAhora += (vMeta - vAhora) * 0.0008;
    const vib = 1 + 0.006 * Math.sin(2 * Math.PI * 5.5 * t);
    fase += (2 * Math.PI * fAhora * vib) / 22050;
    return (Math.sin(fase) + 0.25 * Math.sin(2 * fase) + 0.1 * Math.sin(3 * fase)) * 0.16 * vAhora;
  };
}

let ultimoSemitono = null, ultimaLuz = "";
function paso() {
  if (!prendido) return;
  const ahora = performance.now();
  const vivas = ["left", "right"].map((m) => manos[m]).filter((m) => m && ahora - m.t < 300);
  // La mano más cerca de la antena da la nota; la otra, si está sobre el aro, el volumen.
  let k = null, v = 0.8;
  if (vivas.length) {
    const orden = vivas.slice().sort((a, b) => distAntena(a.p) - distAntena(b.p));
    const tono = orden[0];
    const d = distAntena(tono.p);
    if (d < 0.7) k = 1 - (Math.max(0.03, Math.min(0.6, d)) - 0.03) / 0.57;
    const otra = orden[1];
    if (otra && Math.hypot(otra.p.x - (ARO.x - 0.05), otra.p.z - ARO.z) < 0.2) {
      const h = otra.p.y - ARO.y;
      v = Math.max(0, Math.min(1, (h - 0.02) / 0.25));
    }
  }
  if (k === null) {
    const q = Obj.visitante(donde);
    if (q) {
      const d = Math.hypot(q.x - ANTENA.x, q.z);
      k = 1 - (Math.max(0.3, Math.min(3, d)) - 0.3) / 2.7;
    } else k = 0.4;
  }
  fMeta = frecDe(k);
  vMeta = v;
  // La marca sobre la antena y la luz.
  Obj.$("marca").position = { x: ANTENA.x, y: ANTENA.y0 + Math.max(0, Math.min(1, k)) * (ANTENA.y1 - ANTENA.y0), z: 0 };
  const luz = Obj.mezcla("#330000", "#FF3B30", vMeta);
  if (luz !== ultimaLuz) { ultimaLuz = luz; Obj.$("luz").setAttribute("color", luz); }
  const grave = Math.max(40, Math.min(1000, Number(P.grave) || 130.81));
  const semi = Math.round(12 * Math.log2(fMeta / 32.703));   // desde el Do 1
  if (semi !== ultimoSemitono) {
    ultimoSemitono = semi;
    const nombre = NOMBRES[((semi % 12) + 12) % 12] + (Math.floor(semi / 12) + 1);
    Obj.$("nota").setAttribute("value", P.escala === "libre" ? Math.round(fMeta) + " Hz" : nombre);
    Obj.emitir("nota", { frecuencia: +fMeta.toFixed(1), nombre, grave });
  }
}
setInterval(paso, 50);

function prender(si) {
  prendido = si == null ? !prendido : !!si;
  if (prendido) { vAhora = 0; sinte.flujo(voz()); }
  else { sinte.flujo(null); Obj.$("luz").setAttribute("color", "#330000"); ultimaLuz = ""; Obj.$("nota").setAttribute("value", ""); ultimoSemitono = null; }
  Obj.$("perilla").rotation = { x: Math.PI / 2, y: prendido ? -1.5 : 0, z: 0 };
  Obj.emitir("prendido", { si: prendido });
}
...
if (zona) {
  zona.addEventListener("posemove", (e) => {
    const m = e.hand === "left" ? "left" : "right";
    manos[m] = { p: donde.aLocal({ x: e.px, y: e.py, z: e.pz }), t: performance.now() };
  });
}
