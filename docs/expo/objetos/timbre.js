...
;
// El timbre: un botón que se hunde, se ilumina y suena.
//
// El "din-don" son dos campanas a una tercera mayor de distancia, la segunda
// un poco después y más larga. Mientras suena no se puede volver a tocar:
// el que toca mil veces seguidas no satura la mezcla.
const P = Obj.props({ sonido: "dindon", nombre: "", placa: "#C9A45C" });
...
function sonar() {
  ...
  let dur = 1.2;
  if (P.sonido === "campanilla") {
    for (let i = 0; i < 6; i++) sinte.nota(2093 * (i % 2 ? 1.12 : 1), 0.25, { tipo: "campana", vol: 0.18, caida: 9, retraso: i * 0.07 });
    dur = 0.8;
  } else if (P.sonido === "zumbador") {
    sinte.nota(220, 0.7, { tipo: "cuadrada", vol: 0.12, caida: 0.5 });
    dur = 0.8;
  } else {
    sinte.nota(659.25, 1.2, { tipo: "campana", vol: 0.3, caida: 2.2 });
    sinte.nota(523.25, 1.6, { tipo: "campana", vol: 0.3, caida: 1.8, retraso: 0.45 });
    dur = 1.6;
  }
  hasta = ahora + dur * 1000;
  veces++;
  Obj.emitir("timbre", { veces });
  // El botón se hunde y parpadea mientras dura el sonido.
  boton.position = { x: 0, y: -0.02, z: 0.016 };
  setTimeout(() => { boton.position = { x: 0, y: -0.02, z: 0.02 }; }, 150);
  const fin = hasta;
  const titila = setInterval(() => {
    const on = Math.floor(performance.now() / 150) % 2 === 0;
    boton.setAttribute("color", on ? "#FFFFFF" : "#FFB84D");
    if (performance.now() >= fin) { clearInterval(titila); boton.setAttribute("color", "#FFE7A0"); }
  }, 75);
}

Obj.boton(boton, sonar);
Obj.mensaje("tocar", sonar);
function pintar() {
  Obj.$("placa").setAttribute("color", Obj.color(P.placa, "#C9A45C"));
  Obj.$("nombre").setAttribute("value", String(P.nombre || "").slice(0, 14));
}
