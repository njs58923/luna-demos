...
;
// El toldo: la lona sale de la barra y los brazos se estiran con ella. El
// origen es la barra, contra la pared. Tocarlo lo extiende o lo recoge.
//
// Obra.toldo(op, k) arma el toldo entero para una extensión k; mientras se
// mueve se rearma unas veinte veces por segundo. Son treinta y pico de nodos,
// y así la geometría de los brazos sale de un solo lugar.
const P = Obj.props({ ancho: 2, salida: 1.3, color: "", color2: "", franjas: 8, caida: 0.35, faldon: true, clave: "" });
const memo = Obj.memoria("extendido");
const armar = Obra.montar(Obj.$("obra"), Obj.crear);
const toque = Obj.$("toque");
let extendido = memo.leer(true) !== false;
let k = extendido ? 1 : 0;
let ultimo = -1;

function dibujar() {
  const W = Obra.num(P.ancho, 2, 0.5, 20), S = Obra.num(P.salida, 1.3, 0.3, 5), C = Obra.num(P.caida, 0.35, 0, 2);
  armar(Obra.toldo(Object.assign({}, P, { faldon: P.faldon !== false }), Obj.suave(k)));
  // El blanco cubre lo que ocupa extendido: se lo toca igual estando recogido.
  toque.position = { x: 0, y: -C / 2 - 0.1, z: S / 2 };
  toque.scale = { x: W, y: C + 0.4, z: S + 0.2 };
}

const anim = Obj.animador((t, dt) => {
  const meta = extendido ? 1 : 0;
  k = meta > k ? Math.min(1, k + dt / 1.8) : Math.max(0, k - dt / 1.5);
  if (t - ultimo > 0.05 || k === meta) { ultimo = t; dibujar(); }
  return k !== meta;
});
function poner(v) {
  if (extendido === v) return;
  extendido = v;
  memo.guardar(extendido);
  anim.iniciar();
  Obj.emitir("cambio", { extendido });
}

Obj.boton(toque, () => poner(!extendido));
Obj.mensaje("extender", () => poner(true));
Obj.mensaje("recoger", () => poner(false));
