...
;
// La torta: velitas que se soplan una por una, y la canción al final.
//
// Cada vela es un cilindro con su llama (dos esferas sin iluminación que
// titilan) y su propio blanco táctil. Soplar una la apaga con un hilito de
// humo. Con todas apagadas suena "Cumpleaños feliz" (de dominio público)
// y al rato se prenden de nuevo, para el próximo deseo.
const P = Obj.props({ velas: 5, nombre: "", "baño": "#FFC8DD" });

const sinte = Obj.sinte({ volumen: 0.8 });
const donde = Obj.marco();
const grupo = Obj.$("velas");
const N = Math.max(1, Math.min(12, Math.round(Number(P.velas) || 5)));
const COLORES = ["#4CC9F0", "#F72585", "#FFD60A", "#80ED99", "#B5179E", "#FF9F1C"];

// Gotas de glaseado chorreando por el borde.
for (let i = 0; i < 14; i++) {
  const a = (i / 14) * Math.PI * 2;
  Obj.crear("cylinder", { x: Math.sin(a) * 0.152, y: -0.012 - (i % 3) * 0.006, z: Math.cos(a) * 0.152, sx: 0.03, sy: 0.03 + (i % 3) * 0.012, sz: 0.012, ry: a, color: "#FFC8DD" }, Obj.$("gotas"));
}

const velas = [];
for (let i = 0; i < N; i++) {
  const a = (i / N) * Math.PI * 2, r = N === 1 ? 0 : 0.09;
  const x = Math.sin(a) * r, z = Math.cos(a) * r;
  Obj.crear("cylinder", { x, y: 0.04, z, sx: 0.012, sy: 0.08, sz: 0.012, color: COLORES[i % COLORES.length] }, grupo);
  const llama = Obj.crear("group", { x, y: 0.095, z }, grupo);
  Obj.crear("sphere", { sx: 0.014, sy: 0.026, sz: 0.014, color: "#FF9F1C", "material-unlit": "true" }, llama);
  Obj.crear("sphere", { y: -0.003, sx: 0.007, sy: 0.013, sz: 0.007, color: "#FFF3B0", "material-unlit": "true" }, llama);
  const humo = Obj.crear("sphere", { x, y: 0.1, z, sx: 0.015, sy: 0.015, sz: 0.015, color: "#9A9A9A80", "material-alpha": "blend", visible: "false" }, grupo);
  const blanco = Obj.crear("box", { x, y: 0.07, z, sx: 0.05, sy: 0.12, sz: 0.05, color: "transparent", "material-alpha": "blend" }, grupo);
  const v = { x, z, llama, humo, prendida: true, humoT: 0 };
  Obj.boton(blanco, () => soplar(v));
  velas.push(v);
}

// Titileo de las llamas y humo de las recién apagadas, a 12 cuadros por segundo.
setInterval(() => {
  const t = performance.now() / 1000;
  for (const v of velas) {
    if (v.prendida) {
      const k = 1 + 0.15 * Math.sin(t * 9 + v.x * 50) + 0.1 * Math.sin(t * 15 + v.z * 40);
      v.llama.scale = { x: 1, y: k, z: 1 };
    } else if (v.humoT) {
      const k = (t - v.humoT) / 1.5;
      if (k >= 1) { v.humoT = 0; v.humo.setAttribute("visible", "false"); continue; }
      v.humo.setAttribute("visible", "inherit");
      v.humo.position = { x: v.x + 0.01 * Math.sin(k * 8), y: 0.1 + 0.12 * k, z: v.z };
      const s = 0.012 + 0.03 * k;
      v.humo.scale = { x: s, y: s, z: s };
    }
  }
}, 83);

// "Cumpleaños feliz", en notas: [semitonos desde el La 440, duración en tiempos].
const CANCION = [
  [-2, 0.75], [-2, 0.25], [0, 1], [-2, 1], [3, 1], [2, 2],
  [-2, 0.75], [-2, 0.25], [0, 1], [-2, 1], [5, 1], [3, 2],
  [-2, 0.75], [-2, 0.25], [10, 1], [7, 1], [3, 1], [2, 1], [0, 2],
  [8, 0.75], [8, 0.25], [7, 1], [3, 1], [5, 1], [3, 2],
];
function cantar() {
  let t = 0;
  const negra = 0.42;
  for (const [s, d] of CANCION) {
    sinte.nota(Obj.frecuencia(s), d * negra + 0.3, { tipo: "campana", vol: 0.22, caida: 2.5, retraso: t });
    t += d * negra;
  }
  return t;
}
...
let cantando = false;
function soplar(v) {
  if (!v.prendida || cantando) return;
  v.prendida = false;
  v.llama.setAttribute("visible", "false");
  v.humoT = performance.now() / 1000;
  sinte.nota(400, 0.15, { tipo: "ruido", vol: 0.12, caida: 18 });
  const quedan = velas.filter((x) => x.prendida).length;
  Obj.emitir("vela", { quedan });
  if (quedan === 0) {
    cantando = true;
    Obj.emitir("deseo", {});
    const dura = cantar();
    setTimeout(prenderTodas, (dura + 2.5) * 1000);
  }
}
function prenderTodas() {
  cantando = false;
  for (const v of velas) { v.prendida = true; v.llama.setAttribute("visible", "inherit"); }
}

// En VR: la mano que pasa rápido cerca de una llama la apaga (el soplido).
const manos = { left: null, right: null };
const zona = Obj.$("zona");
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    const rapidez = Math.hypot(l.x - antes.x, l.z - antes.z) / Math.max(0.001, (ahora - antes.t) / 1000);
    if (rapidez < 0.8) return;
    for (const v of velas) if (v.prendida && Math.hypot(l.x - v.x, l.y - 0.235, l.z - v.z) < 0.07) soplar(v);
  });
}

Obj.mensaje("prender", prenderTodas);
function pintar() {
  Obj.$("bano").setAttribute("color", Obj.color(P["baño"], "#FFC8DD"));
  Obj.$("nombre").setAttribute("value", P.nombre ? "¡Feliz cumple, " + String(P.nombre).slice(0, 14) + "!" : "¡Feliz cumple!");
}
