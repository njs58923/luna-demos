...
;
// La tostadora: palanca abajo, resistencias al rojo, y las tostadas saltan.
//
// Las tostadas cambian de color de a poco mientras están adentro, desde pan
// crudo hasta el punto elegido; al salir dan un salto con una parábola y un
// rebote. El resplandor de las resistencias es un plano sin iluminación que
// late despacio.
const P = Obj.props({ color: "#C8102E", segundos: 6, punto: 2 });

const sinte = Obj.sinte({ volumen: 0.7 });
const palanca = Obj.$("palanca");
const PAN = "#F2D7A6", PUNTOS = ["#F2D7A6", "#D9A45B", "#A8672E", "#4A2C18"];

const panes = [-0.025, 0.025].map((z) => {
  const g = Obj.crear("group", { z, y: 0.12 }, Obj.$("panes"));
  const miga = Obj.crear("box", { sx: 0.16, sy: 0.14, sz: 0.018, color: PAN, "border-radius": 0.01 }, g);
  const corteza = Obj.crear("box", { y: 0.068, sx: 0.165, sy: 0.012, sz: 0.02, color: "#C68A4A", "border-radius": 0.005 }, g);
  return { g, miga, corteza, z };
});
const brasas = [-0.05, 0, 0.05].map((z) => Obj.crear("plane", { z, rx: -Math.PI / 2, sx: 0.16, sy: 0.02, color: "#3A0A0A", "material-unlit": "true" }, Obj.$("brasas")));

let estado = "arriba";   // arriba | tostando | saltando
let inicio = 0, dorado = 0, saltoT = 0;
// El punto arranca en el de las props y lo cambia la perilla.
let elegido = Math.max(1, Math.min(3, Math.round(Number(P.punto) || 2)));
function punto() { return elegido; }
function segundos() { return Math.max(2, Math.min(60, Number(P.segundos) || 6)); }
function colorPan(k) {
  // De crudo al punto elegido.
  const hasta = punto();
  const i = Math.min(hasta, k * hasta);
  const a = Math.floor(i), f = i - a;
  return a >= hasta ? PUNTOS[hasta] : Obj.mezcla(PUNTOS[a], PUNTOS[a + 1], f);
}

const anim = Obj.animador((t) => {
  if (estado === "tostando") {
    const k = Math.min(1, (performance.now() - inicio) / (segundos() * 1000));
    dorado = k;
    const c = colorPan(k);
    for (const p of panes) p.miga.setAttribute("color", c);
    const late = 0.5 + 0.5 * Math.sin(t * 3);
    for (const b of brasas) b.setAttribute("color", Obj.mezcla("#7A1A0A", "#FF5A1F", late));
    if (k >= 1) saltar();
    return true;
  }
  if (estado === "saltando") {
    const k = Math.min(1, (performance.now() - saltoT) / 700);
    // Sube a 0.2 m y vuelve a apoyarse asomando; un rebote chico al final.
    const y = k < 0.7 ? 0.05 + 0.26 * Math.sin((k / 0.7) * Math.PI) : 0.05 + 0.03 * Math.sin(((k - 0.7) / 0.3) * Math.PI);
    for (const p of panes) p.g.position = { x: 0, y: 0.12 + y, z: p.z };
    if (k >= 1) { estado = "arriba"; return false; }
    return true;
  }
  return false;
});
...
function tostar() {
  ...
  dorado = 0;
  for (const p of panes) { p.g.position = { x: 0, y: 0.1, z: p.z }; p.miga.setAttribute("color", PAN); }
  palanca.position = { x: 0.16, y: 0.075, z: 0 };
  sinte.nota(900, 0.05, { tipo: "madera", vol: 0.25 });
  anim.iniciar();
}
function saltar() {
  estado = "saltando";
  saltoT = performance.now();
  palanca.position = { x: 0.16, y: 0.17, z: 0 };
  for (const b of brasas) b.setAttribute("color", "#3A0A0A");
  sinte.nota(1318.5, 1.2, { tipo: "campana", vol: 0.3, caida: 2.5 });
  sinte.nota(600, 0.06, { tipo: "madera", vol: 0.3 });
  Obj.emitir("lista", { punto: punto() });
}

Obj.boton(palanca, tostar, { color: "#2A2A2A" });
Obj.boton(Obj.$("perilla"), () => {
  elegido = (elegido % 3) + 1;
  Obj.$("perilla").rotation = { x: 0, y: 0, z: Math.PI / 2 + (punto() - 2) * 0.8 };
}, { color: "#2A2A2A" });
// En VR (las piezas de las manos, _vr.js): la palanca se baja con la mano;
// abajo del todo se traba y tuesta, soltada antes vuelve a subir. La perilla
// del punto se gira con la muñeca, de a muesca.
const ARRIBA = 0.17, ABAJO = 0.075;
let bajando = null;
Obj.manos().corredera({
  desde: { x: 0.16, y: ARRIBA, z: 0 }, hasta: { x: 0.16, y: ABAJO, z: 0 }, alcance: 0.06,
  t: () => (bajando !== null ? bajando : estado === "arriba" ? 0 : 1),
  alTomar() { if (estado === "arriba") bajando = 0; },
  alMover(t) {
    if (bajando === null) return;
    bajando = t;
    palanca.position = { x: 0.16, y: ARRIBA + (ABAJO - ARRIBA) * t, z: 0 };
    if (t > 0.95) { bajando = null; tostar(); }
  },
  alSoltar() {
    if (bajando === null) return;
    bajando = null;
    palanca.position = { x: 0.16, y: ARRIBA, z: 0 };
    sinte.nota(700, 0.03, { tipo: "madera", vol: 0.2 });
  },
});
let giroPunto = 0;
Obj.manos().perilla({
  // Está en el costado derecho: gira sobre X.
  centro: { x: 0.151, y: 0.05, z: 0 }, eje: { x: 1, y: 0, z: 0 }, arriba: { x: 0, y: 1, z: 0 }, radio: 0.02,
  alTomar() { giroPunto = 0; },
  alGirar(d) {
    giroPunto += d;
    while (Math.abs(giroPunto) >= 0.5) {
      const s = Math.sign(giroPunto);
      giroPunto -= s * 0.5;
      const nuevo = Math.max(1, Math.min(3, elegido + s));
      if (nuevo !== elegido) {
        elegido = nuevo;
        Obj.$("perilla").rotation = { x: 0, y: 0, z: Math.PI / 2 + (punto() - 2) * 0.8 };
        sinte.nota(1600, 0.015, { tipo: "pulso", vol: 0.2 });
      }
    }
  },
});

Obj.mensaje("tostar", tostar);
Obj.alCambiar(() => Obj.$("cuerpo").setAttribute("color", Obj.color(P.color, "#C8102E")));
Obj.$("cuerpo").setAttribute("color", Obj.color(P.color, "#C8102E"));
for (const p of panes) p.g.position = { x: 0, y: 0.17, z: p.z };
