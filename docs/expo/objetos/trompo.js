...
;
// El trompo: giro que se gasta, inclinación que crece, precesión.
//
// No es un trompo rígido integrado: es la forma de lo que se ve. El giro
// propio ω cae por rozamiento; la inclinación crece a medida que ω baja de un
// umbral (el trompo "duerme" derecho mientras gira rápido); la precesión —el
// eje dando vueltas— va más rápido cuanto más lento gira, como en uno de
// verdad (Ω ∝ 1/ω), y la punta describe un círculo que se agranda. Cuando la
// inclinación pasa de unos 70°, cae de costado y rueda un poco.
const P = Obj.props({ colores: ["#D62828", "#F77F00", "#FCBF49"] });

const sinte = Obj.sinte({ volumen: 0.6 });
const donde = Obj.marco();
const trompo = Obj.$("trompo"), giroG = Obj.$("giro"), piolin = Obj.$("piolin");

let omega = 0;          // vueltas propias, rad/s
let fase = 0;           // ángulo propio
let prec = 0;           // ángulo de precesión
let incl = 0;           // inclinación del eje, rad
let punta = { x: 0, z: 0 };
let estado = "quieto";  // quieto | baila | caido
let lanzadoEn = 0, caidoEn = 0;
let sonando = false;

function zumbido() {
  let f = 0;
  return (t) => {
    if (estado !== "baila") return 0;
    const frec = 60 + omega * 1.2;
    f += (2 * Math.PI * frec) / 22050;
    const v = Math.min(1, omega / 150);
    return (Math.sin(f) * 0.6 + 0.4 * Math.sin(2 * f + Math.sin(t * 7))) * 0.12 * v;
  };
}

function poner() {
  // El eje inclinado incl en la dirección prec; el giro propio, alrededor del eje.
  const qIncl = Obj.qMul(Obj.qEje(Obj.Y, prec), Obj.qEje(Obj.X, incl));
  // Muy inclinado apoya el costado, no la punta: se levanta lo que haga falta.
  const y = incl > 1 ? (0.045 * (incl - 1)) / 0.45 : 0;
  trompo.position = { x: punta.x, y, z: punta.z };
  trompo.rotation = Obj.euler(qIncl);
  giroG.rotation = { x: 0, y: fase, z: 0 };
}

const anim = Obj.animador((t, dt) => {
  if (estado === "baila") {
    // Rozamiento: un poco proporcional, un poco constante.
    omega -= (omega * 0.05 + 6) * dt;
    fase += omega * dt;
    const umbral = 110;
    const incMeta = omega > umbral ? 0.04 : 0.04 + (1 - omega / umbral) * 1.3;
    incl += (incMeta - incl) * Math.min(1, dt * 1.5);
    const Om = Math.min(14, 260 / Math.max(20, omega));
    prec += Om * dt;
    // La punta se pasea en círculo: radio que crece con la inclinación.
    const r = Math.min(0.12, 0.01 + incl * 0.08);
    punta = { x: Math.cos(prec * 0.35) * r * 0.6, z: Math.sin(prec * 0.35) * r };
    if (incl > 1.2 || omega <= 5) caer();
  } else if (estado === "caido") {
    // Rueda un poco sobre el costado hasta pararse.
    fase += omega * dt * 0.3;
    omega *= Math.exp(-dt * 3);
    incl += (1.45 - incl) * Math.min(1, dt * 8);
    if (performance.now() - caidoEn > 3500) { levantar(); return false; }
  }
  poner();
  return estado !== "quieto";
});

function lanzar(fuerza) {
  const f = Math.max(0.3, Math.min(1.5, Number(fuerza) || 1));
  estado = "baila";
  omega = 120 + 140 * f;
  incl = 0.03;
  prec = Math.random() * Math.PI * 2;
  lanzadoEn = performance.now();
  piolin.setAttribute("visible", "false");
  sinte.nota(300, 0.2, { tipo: "ruido", vol: 0.25, caida: 14 });
  if (!sonando) sonando = sinte.flujo(zumbido());
  Obj.emitir("lanzado", { vueltas_por_segundo: +(omega / (2 * Math.PI)).toFixed(1) });
  anim.iniciar();
}
function caer() {
  estado = "caido";
  caidoEn = performance.now();
  sinte.flujo(null); sonando = false;
  sinte.nota(180, 0.15, { tipo: "madera", vol: 0.5 });
  sinte.nota(140, 0.12, { tipo: "madera", vol: 0.35, retraso: 0.12 });
  Obj.emitir("cae", { segundos: +((caidoEn - lanzadoEn) / 1000).toFixed(1) });
}
function levantar() {
  estado = "quieto";
  omega = 0; incl = 0; punta = { x: 0, z: 0 };
  piolin.setAttribute("visible", "inherit");
  poner();
}

Obj.boton(Obj.$("toque"), () => { if (estado !== "baila") lanzar(1); });
Obj.boton(Obj.$("agarrar"), () => { if (estado !== "baila") lanzar(1); });
Obj.mensaje("lanzar", (d) => lanzar(d && d.fuerza));

// En VR: tomar la manija del piolín y tirar rápido.
let tiron = null;
Obj.agarre({
  zona: Obj.$("zona"), donde,
  cerca(p) { return estado !== "baila" && Math.hypot(p.x - 0.08, p.y - 0.035, p.z) < 0.08; },
  tomar(p) { tiron = { p, t: performance.now(), max: 0 }; },
  mover(p) {
    if (!tiron) return;
    const d = Math.hypot(p.x - tiron.p.x, p.y - tiron.p.y, p.z - tiron.p.z);
    piolin.position = { x: p.x, y: p.y - 0.012, z: p.z };
    if (d > 0.25) {
      const v = d / Math.max(0.05, (performance.now() - tiron.t) / 1000);
      tiron = null;
      piolin.position = { x: 0.08, y: 0.02, z: 0 };
      lanzar(v / 2.5);
    }
  },
  soltar() { tiron = null; piolin.position = { x: 0.08, y: 0.02, z: 0 }; },
});

function pintar() {
  const c = Array.isArray(P.colores) ? P.colores.map((x) => Obj.color(x, null)) : [];
  ["f0", "f1", "f2"].forEach((k, i) => {
    const col = c[i] || ["#D62828", "#F77F00", "#FCBF49"][i];
    for (const el of Obj.root.getElementsByClass(k)) el.setAttribute("color", col);
  });
}
