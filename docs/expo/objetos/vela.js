...
;
// La vela: una llama que titila, un soplido que la apaga, y cera que se gasta.
//
// El titileo es ruido de dos senos de períodos que no se repiten entre sí,
// aplicado a la escala de la llama doce veces por segundo: es lo que tarda el
// ojo en leerlo como "vivo", y no despierta al isolate en cada cuadro. El humo
// sí va por cuadros, pero sólo los dos segundos que dura.
const P = Obj.props({ cera: "#F4E1C1", plato: "#B08D57", encendida: true, consumo: 0, clave: "" });

const ALTO = 0.2;
const memo = Obj.memoria("vela");
const cuerpo = Obj.$("cuerpo");
const cera = Obj.$("cera");
const borde = Obj.$("cera_borde");
const mecha = Obj.$("mecha");
const llama = Obj.$("llama");
const afuera = Obj.$("llama_afuera");
const adentro = Obj.$("llama_adentro");
const humo = Obj.$("humo");

const guardado = memo.leer(null);
let encendida = guardado ? !!guardado.encendida : P.encendida !== false;
let queda = guardado && Number.isFinite(guardado.queda) ? Math.max(0.08, Math.min(1, guardado.queda)) : 1;

// La cera: el cilindro se acorta desde arriba y la mecha y la llama bajan con él.
function dibujarCera() {
  const h = ALTO * queda;
  cera.position = { x: 0, y: h / 2, z: 0 };
  cera.scale = { x: 0.05, y: h, z: 0.05 };
  borde.position = { x: 0, y: h - 0.001, z: 0 };
  mecha.position = { x: 0, y: h + 0.01, z: 0 };
  llama.position = { x: 0, y: h + 0.032, z: 0 };
  humo.position = { x: 0, y: h + 0.025, z: 0 };
}

function mostrarLlama() {
  llama.setAttribute("visible", encendida ? "inherit" : "false");
}

// ── Titileo y consumo ───────────────────────────────────────────────────────
let fase = Math.random() * 100;
let ultimoGuardado = 0;
setInterval(() => {
  if (!encendida) return;
  fase += 0.083;
  const n = 0.5 + 0.3 * Math.sin(fase * 7.3) + 0.2 * Math.sin(fase * 13.1 + 1.7);
  afuera.scale = { x: 0.02 + 0.004 * n, y: 0.042 + 0.014 * n, z: 0.02 + 0.004 * n };
  adentro.scale = { x: 0.011, y: 0.022 + 0.006 * n, z: 0.011 };
  // Un vaivén chico, como si hubiera aire.
  llama.rotation = { x: 0.06 * Math.sin(fase * 2.1), y: 0, z: 0.08 * Math.sin(fase * 1.7 + 0.4) };
  const minutos = Number(P.consumo) || 0;
  if (minutos > 0) {
    queda = Math.max(0.08, queda - 0.083 / (minutos * 60));
    dibujarCera();
    // Se guarda cada tanto, no en cada paso: localStorage es sincrónico.
    if (performance.now() - ultimoGuardado > 5000) { ultimoGuardado = performance.now(); guardar(); }
    if (queda <= 0.08) apagar();
  }
}, 83);
...
const bocanadas = [];
for (let i = 0; i < 6; i++) {
  bocanadas.push({ el: Obj.crear("sphere", { sx: 0.02, sy: 0.02, sz: 0.02, color: "#9A9A9A80", "material-alpha": "blend", visible: "false" }, humo), t0: 0 });
}
const animHumo = Obj.animador((t) => {
  let vivas = false;
  bocanadas.forEach((b, i) => {
    if (!b.t0) return;
    const k = (t - b.t0) / 1.8;
    if (k >= 1 || k < 0) { if (k >= 1) { b.t0 = 0; b.el.setAttribute("visible", "false"); } else vivas = true; return; }
    vivas = true;
    b.el.setAttribute("visible", "inherit");
    const s = 0.015 + 0.05 * k;
    b.el.position = { x: 0.02 * Math.sin(k * 6 + i), y: 0.18 * k, z: 0.01 * Math.cos(k * 5 + i) };
    b.el.scale = { x: s, y: s, z: s };
  });
  return vivas;
});
function soplido() {
  const t = performance.now() / 1000;
  bocanadas.forEach((b, i) => { b.t0 = t + i * 0.18; });
  animHumo.iniciar();
}

// ── Estado ──────────────────────────────────────────────────────────────────
function guardar() { memo.guardar({ encendida, queda: Math.round(queda * 1000) / 1000 }); }
function encender() {
  if (encendida || queda <= 0.08) return;
  encendida = true;
  mostrarLlama(); guardar();
  Obj.emitir("cambio", { encendida, queda });
}
function apagar() {
  if (!encendida) return;
  encendida = false;
  mostrarLlama(); guardar(); soplido();
  Obj.emitir("cambio", { encendida, queda });
}

Obj.boton(Obj.$("toque"), () => (encendida ? apagar() : encender()));
Obj.mensaje("encender", encender);
Obj.mensaje("apagar", apagar);
Obj.mensaje("nueva", () => { queda = 1; dibujarCera(); guardar(); });

function pintar() {
  const c = Obj.color(P.cera, "#F4E1C1");
  cera.setAttribute("color", c);
  borde.setAttribute("color", Obj.tono(c, -0.06));
  Obj.$("plato").setAttribute("color", Obj.color(P.plato, "#B08D57"));
}
