...
;
// La ventana: marco, hojas batientes y postigos que abren hasta quedar contra
// la pared. El origen es el centro del vano, sobre la pared. Al abrir van
// primero los postigos y después las hojas; al cerrar, al revés, como se hace
// de verdad.
//
// Colgada de una pared no tiene vano detrás: las hojas abren hacia afuera
// (hacia adentro se hundían en la pared) y el marco se apoya delante de ella,
// no a medio meter.
const P = Obj.props({ ancho: 1.1, alto: 1.2, hojas: 2, color: "", postigos: true, colorPostigos: "", alfeizar: true, clave: "" });
...
let k = abierta ? 1 : 0;
let hojas = [], postigos = [];
// Los postigos son decenas de tablillas: van en malla. Cada hoja y cada
// postigo es un grupo con id, con su malla adentro, para que se sigan
// abriendo.
const armar = Obra.montarConMallas(obra, Obj.crear);

function dibujar() {
  const v = Obra.ventana(Object.assign({}, P, { postigos: P.postigos !== false, alfeizar: P.alfeizar !== false }));
  const colgar = (h, id) => Obra.grupo({ id, x: h.bisagra.x, y: h.bisagra.y, z: h.bisagra.z }, h.nodos);
  // El marco mide 10 cm de fondo, centrado: se corre 5 para apoyarse en la pared.
  armar([Obra.grupo({ id: "cuerpo", y: -v.alto / 2, z: 0.05 }, [
    ...v.marco,
    ...v.hojas.map((h, i) => colgar(h, "hoja_" + i)),
    ...v.postigos.map((h, i) => colgar(h, "postigo_" + i)),
  ])]);
  hojas = v.hojas.map((h, i) => ({ g: Obj.$("hoja_" + i), dir: h.dir }));
  postigos = v.postigos.map((h, i) => ({ g: Obj.$("postigo_" + i), dir: h.dir }));
  toque.position = { x: 0, y: 0, z: 0.07 };
  toque.scale = { x: v.ancho + 0.1, y: v.alto + 0.1, z: 0.16 };
  pintar();
}
function pintar() {
  // Los postigos ocupan la primera mitad del recorrido; las hojas, la segunda.
  const kp = Obj.suave(Math.min(1, k * 2)), kh = Obj.suave(Math.max(0, k * 2 - 1));
  for (const h of hojas) h.g.rotation = { x: 0, y: -h.dir * 1.25 * kh, z: 0 };
  for (const p of postigos) p.g.rotation = { x: 0, y: -p.dir * 2.95 * kp, z: 0 };
}

const anim = Obj.animador((t, dt) => {
  const meta = abierta ? 1 : 0;
  k = meta > k ? Math.min(1, k + dt / 1.6) : Math.max(0, k - dt / 1.6);
  pintar();
  return k !== meta;
});
function poner(v) {
