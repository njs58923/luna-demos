...
;
// La ventana: marco, hojas batientes que abren hacia adentro y postigos que
// abren hacia afuera hasta quedar contra la pared. El origen es el centro del
// vano. Al abrir van primero los postigos y después las hojas; al cerrar, al
// revés, como se hace de verdad.
...
function dibujar() {
  ...
  const cuerpo = Obj.crear("group", { y: -v.alto / 2 }, obra);
  hechos.push(cuerpo);
  Obra.construir(v.marco, cuerpo, Obj.crear);
  const colgar = (h) => {
    const g = Obj.crear("group", { x: h.bisagra.x, y: h.bisagra.y, z: h.bisagra.z }, cuerpo);
    Obra.construir(h.nodos, g, Obj.crear);
    return { g, dir: h.dir };
  };
  hojas = v.hojas.map(colgar);
  postigos = v.postigos.map(colgar);
  toque.position = { x: 0, y: 0, z: 0.05 };
  toque.scale = { x: v.ancho + 0.1, y: v.alto + 0.1, z: 0.14 };
  pintar();
}
function pintar() {
