...
;
// El ventilador: una hélice que acelera y frena, y una cabeza que va y viene.
//
// La velocidad no salta: la hélice tiende a la velocidad elegida con una
// inercia, así al apagarlo sigue girando un rato. La rejilla son dos aros de
// dovelas y rayos. El soplido es ruido suave cuyo volumen sigue a la hélice.
const P = Obj.props({ color: "#E9ECEF", velocidad: 0, vaiven: false, clave: "" });

const memo = Obj.memoria("estado");
const sinte = Obj.sinte({ volumen: 0.6 });
const helice = Obj.$("helice");
const cabeza = Obj.$("cabeza");
const rejilla = Obj.$("rejilla");

for (let i = 0; i < 3; i++) {
  const g = Obj.crear("group", { rz: (i * Math.PI * 2) / 3 }, helice);
  Obj.crear("box", { y: 0.085, rx: 0.3, sx: 0.08, sy: 0.14, sz: 0.004, color: "#74C0FC", "border-radius": 0.03 }, g);
}
for (const [r, n] of [[0.21, 24], [0.12, 16]]) {
  for (let i = 0; i < n; i++) {
    const a = (i / n) * Math.PI * 2;
    Obj.crear("box", { x: Math.sin(a) * r, y: Math.cos(a) * r, z: 0.045, rz: -a, sx: (2 * Math.PI * r) / n + 0.004, sy: 0.006, sz: 0.006, color: "#ADB5BD" }, rejilla);
  }
}
for (let i = 0; i < 12; i++) {
  const a = (i / 12) * Math.PI * 2;
  Obj.crear("box", { x: Math.sin(a) * 0.105, y: Math.cos(a) * 0.105, z: 0.045, rz: -a, sx: 0.004, sy: 0.21, sz: 0.004, color: "#CED4DA" }, rejilla);
}

const g = memo.leer(null);
let velocidad = g && Number.isInteger(g.v) ? g.v : Math.max(0, Math.min(3, Math.round(Number(P.velocidad) || 0)));
let vaiven = g ? !!g.o : !!P.vaiven;
let giro = 0, omega = 0, fase = 0, angCabeza = 0;

// Los botones: 0, 1, 2, 3.
const botones = [0, 1, 2, 3].map((n) => {
  const x = -0.09 + n * 0.06;
  const b = Obj.crear("box", { x, sx: 0.045, sy: 0.02, sz: 0.035, color: "#495057" }, Obj.$("botones"));
  Obj.crear("text", { x, y: 0.011, rx: -Math.PI / 2, value: String(n), size: 0.02, color: "#FFFFFF" }, Obj.$("botones"));
  Obj.boton(b, () => poner(n, vaiven), { color: "#495057" });
  return b;
});
...
function soplo() {
  ...
  return () => {
    ...
    lp += ((s / 0x3fffffff - 1) - lp) * 0.12;
    return lp * 0.5 * Math.min(1, omega / 40);
  };
}
let sonando = false;

const anim = Obj.animador((t, dt) => {
  const meta = [0, 18, 30, 44][velocidad];
  omega += (meta - omega) * Math.min(1, dt * (meta > omega ? 1.2 : 0.5));
  giro += omega * dt;
  helice.rotation = { x: 0, y: 0, z: -giro };
  if (vaiven && velocidad > 0) fase += dt * 0.5;
  const objetivo = vaiven ? Math.sin(fase) * 0.8 : angCabeza;
  angCabeza += (objetivo - angCabeza) * Math.min(1, dt * 2);
  cabeza.rotation = { x: 0, y: angCabeza, z: 0 };
  // El soplido, sólo mientras gira algo.
  if (omega > 2 && !sonando) sonando = sinte.flujo(soplo());
  if (omega <= 2 && sonando) { sinte.flujo(null); sonando = false; }
  return omega > 0.05 || velocidad > 0;
});

function poner(v, o) {
  velocidad = v; vaiven = o;
  memo.guardar({ v: velocidad, o: vaiven });
  botones.forEach((b, i) => b.setAttribute("color", i === velocidad ? "#1C7ED6" : "#495057"));
  anim.iniciar();
  Obj.emitir("cambio", { velocidad, vaiven });
}
Obj.boton(Obj.$("perilla"), () => poner(velocidad, !vaiven), { color: "#495057" });
Obj.mensaje("velocidad", (d) => { if (d && Number.isInteger(d.velocidad)) poner(Math.max(0, Math.min(3, d.velocidad)), vaiven); });
Obj.mensaje("vaiven", () => poner(velocidad, !vaiven));
Obj.alCambiar(() => {
  Obj.$("base").setAttribute("color", Obj.color(P.color, "#E9ECEF"));
  for (const el of Obj.root.getElementsByClass("cuerpo")) el.setAttribute("color", Obj.color(P.color, "#E9ECEF"));
});
Obj.$("base").setAttribute("color", Obj.color(P.color, "#E9ECEF"));
botones.forEach((b, i) => b.setAttribute("color", i === velocidad ? "#1C7ED6" : "#495057"));
if (velocidad > 0) anim.iniciar();
