...
;
// El xilófono: ocho láminas que suenan al tocarlas.
//
// El sonido es sintetizado —un seno con un parcial alto que se apaga rápido,
// que es lo que da el golpe de madera—, así que no hay archivos que bajar.
//
// En VR, además del puntero, cada mano hace de palillo: si la mano cruza la
// cara de una lámina bajando rápido, suena, más fuerte cuanto más rápido baja.
// Las poses llegan en coordenadas de mundo y se pasan a las del objeto con
// Obj.marco(), así funciona aunque el objeto esté girado o escalado.
const P = Obj.props({ escala: "mayor", octava: 0, volumen: 0.8, madera: "#6B4A30" });
...
const COLORES = ["#E63946", "#F4A261", "#E9C46A", "#8AC926", "#2A9D8F", "#1982C4", "#6A4C93", "#D62AD0"];
const N = 8;
const ANCHO = 0.058, GAP = 0.012;
const X0 = -((N - 1) * (ANCHO + GAP)) / 2;
const Y_CARA = 0.075;

const sinte = Obj.sinte({ volumen: Math.max(0, Math.min(1, Number(P.volumen) || 0.8)) });
const marcoNodo = Obj.$("marco");
const grupoLaminas = Obj.$("laminas");
const donde = Obj.marco();

// El marco: dos largueros que se abren, como el trapecio de un xilófono real.
const maderas = [];
const oscuras = [];
function madera() { return Obj.color(P.madera, "#6B4A30"); }
for (const s of [-1, 1]) {
  const zA = s * 0.13, zB = s * 0.075;
  const dz = zB - zA, dx = (N - 1) * (ANCHO + GAP) + 0.08;
  maderas.push(Obj.crear("box", {
    y: 0.045, z: (zA + zB) / 2, ry: -Math.atan2(dz, dx),
    sx: Math.hypot(dx, dz), sy: 0.03, sz: 0.03, color: madera(),
  }, marcoNodo));
}
for (const x of [-0.29, 0.29]) {
  for (const s of [-1, 1]) {
    const z = s * (x < 0 ? 0.13 : 0.075);
    oscuras.push(Obj.crear("cylinder", { x, y: 0.015, z, sx: 0.025, sy: 0.03, sz: 0.025, color: Obj.tono(madera(), -0.3) }, marcoNodo));
  }
}
// Dos palillos apoyados adelante, de decoración.
for (const s of [-1, 1]) {
  const g = Obj.crear("group", { x: s * 0.07, y: 0.008, z: 0.18, ry: s * 0.25 }, marcoNodo);
  Obj.crear("cylinder", { rx: Math.PI / 2, sx: 0.008, sy: 0.24, sz: 0.008, color: "#C9A86A" }, g);
  Obj.crear("sphere", { z: -0.12, sx: 0.026, sy: 0.026, sz: 0.026, color: "#C8102E" }, g);
}

const laminas = [];
for (let i = 0; i < N; i++) {
  const largo = 0.3 - i * 0.017;
  const x = X0 + i * (ANCHO + GAP);
  const el = Obj.crear("box", {
    x, y: Y_CARA - 0.009, sx: ANCHO, sy: 0.018, sz: largo, "border-radius": 0.006, color: COLORES[i],
  }, grupoLaminas);
  // Los dos clavitos que la sostienen.
  for (const s of [-1, 1]) {
    Obj.crear("cylinder", { x, y: Y_CARA + 0.002, z: s * (largo / 2 - 0.03), sx: 0.008, sy: 0.006, sz: 0.008, color: "#C8CCD8" }, grupoLaminas);
  }
  const b = Obj.boton(el, () => golpe(i, 1), { color: COLORES[i] });
  laminas.push({ el, x, largo, b, hasta: 0 });
}

function frecuenciaDe(i) {
  const escala = ESCALAS[P.escala] || ESCALAS.mayor;
  const oct = Math.max(-1, Math.min(2, Math.round(Number(P.octava) || 0)));
  // Do5 queda 3 semitonos arriba del La4; más la octava pedida.
  return Obj.frecuencia(3 + escala[i] + 12 * oct);
}

// La lámina golpeada baja medio centímetro y se aclara, y vuelve.
const anim = Obj.animador(() => {
  let seguir = false;
  const ahora = performance.now();
  laminas.forEach((l, i) => {
    if (!l.hasta) return;
    const k = Math.max(0, (l.hasta - ahora) / 180);
    l.el.position = { x: l.x, y: Y_CARA - 0.009 - 0.005 * k, z: 0 };
    if (k <= 0) { l.hasta = 0; l.b.color = COLORES[i]; }
    else seguir = true;
  });
  return seguir;
});

function golpe(i, fuerza) {
  const l = laminas[i];
  if (!l) return;
  const f = frecuenciaDe(i);
  sinte.nota(f, 1.6, { vol: 0.28 * Math.max(0.25, Math.min(1, fuerza)), tipo: "campana" });
  l.hasta = performance.now() + 180;
  l.b.color = Obj.tono(COLORES[i], 0.45);
  anim.iniciar();
  Obj.emitir("nota", { indice: i, frecuencia: Math.round(f * 100) / 100 });
}

Obj.mensaje("tocar", (d) => { if (d && Number.isInteger(d.indice)) golpe(d.indice, 1); });
...
if (zona) {
  zona.addEventListener("posemove", (e) => {
    ...
    const ahora = performance.now();
    manos[mano] = { x: l.x, y: l.y, z: l.z, t: ahora };
    if (!antes || ahora - antes.t > 200) return;
    // Cruzó la cara de las láminas bajando, en este cuadro.
    if (!(antes.y > Y_CARA && l.y <= Y_CARA + 0.004)) return;
    const v = (antes.y - l.y) / Math.max(0.001, (ahora - antes.t) / 1000);
    if (v < 0.25) return;
    for (let i = 0; i < N; i++) {
      const la = laminas[i];
      if (Math.abs(l.x - la.x) < ANCHO / 2 + 0.01 && Math.abs(l.z) < la.largo / 2) {
        golpe(i, v / 2.5);
        break;
      }
    }
...
Obj.alCambiar(() => {
  for (const el of maderas) el.setAttribute("color", madera());
  for (const el of oscuras) el.setAttribute("color", Obj.tono(madera(), -0.3));
});
